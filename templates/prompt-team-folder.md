...
nothing gets committed, the task is never moved to `done/`, and the invocation is
recorded as a success that changed zero files.
...
- If a dispatch genuinely hangs and you must abandon it, go to "Failed" and say
  so in the task's `## Ralph failure` note. An honest failure re-enters triage; a
