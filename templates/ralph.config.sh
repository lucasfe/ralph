...
# ONE SYNTAX RULE, because this file is read twice and both readers now agree with bash.
# The loop SOURCES this file with `set -a`, and Ralph's own commands parse the same text to
# report what the loop will use. So every line here has to be an assignment A SHELL would
# make, and there is no space on either side of the `=`:
#
#   NAME=2h      NAME="2h"      NAME=""      # assignments
#   NAME = 2h    NAME=  2h                   # NOT assignments — bash runs a COMMAND
#
# A blank on the LEFT of the `=` makes the whole line a command called NAME. A blank on the
# RIGHT, followed by a WORD, makes `NAME=` an environment prefix scoped to that word: bash
# assigns nothing and runs the word instead. Either way the line has no effect, whatever was
# already exported in your shell stands, and — a config file that does nothing being the
# whole point of a config file — nothing warns you. If a value needs a space in it, quote
# it: NAME="a b".
#
# Two spellings that LOOK like the second one and really do assign, so they are safe:
# `NAME= ` with nothing after the blank (an assignment to empty), and `NAME= # a note`,
# where the `#` opens a comment rather than starting a word.
