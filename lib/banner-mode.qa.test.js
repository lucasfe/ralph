...
describe('QA resolveBannerMode — hostile values (#74)', () => {
  ...
  it('formats nothing: a printf token, a shell expansion and an escape survive verbatim', () => {
    ...
    // value reaches the warning, so a value with an ANSI sequence in it colours one line of
    // stderr. That is the same trade `resolveAgent` makes for a mistyped RALPH_AGENT (#559) —
    // echoing the value is the point of the message — and the containment that matters is
    // elsewhere: stdout stays clean, which is asserted through the real command in
    // lib/commands/start.banner-mode.qa.test.js.
