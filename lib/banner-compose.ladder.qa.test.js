...
describe('QA bannerLayout — the two rungs, at the column either side of each (#72)', () => {
  ...
  for (const [width, boxWidth, boxed, sprite] of BOUNDARIES) {
    it(`decides ${width} columns as boxWidth ${boxWidth}, boxed ${boxed}, sprite ${sprite}`, () => {
      expect(bannerLayout(width)).toEqual({ width, boxWidth, boxed, sprite })
    })
  }

  it('states its two rungs in the order the ladder needs them', () => {
    // The ORDER is the load-bearing part, not the numbers. The sprite sits ABOVE the box
    // rather than beside it, which makes the sprite the narrow element (26 cells, its own
    // width) and the box the wide one (a 60-column target). So the frame must be the first
    // thing to go and the sprite the last — and if these two constants ever crossed, a
    // 30-column terminal would draw a 30-column frame around rows under a sprite it could
    // not fit, which is the one arrangement the ladder exists to forbid.
  ...
  it('hands back a fresh decision each time, which a caller may keep or clobber', () => {
    ...
    first.boxed = true
    first.boxWidth = 999
    expect(bannerLayout(30)).toEqual({ width: 30, boxWidth: 30, boxed: false, sprite: true })
  })
  ...
  it('decides with Date, Math.random and process trip-wired', () => {
    ...
    // (update-check.js) defaults a `processEnv` to the real `process.env`.
