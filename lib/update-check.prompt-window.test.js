...
// defaulted to `npm` — the channel every run here reads AND asks, because no run passes a
// `latestSource` and `resolveUpdateDecision` defaults that parameter to NPM_VERSION_QUERY. (A
// caller that TRIES to name a channel and fails is the opposite case and reads nothing: see
// lib/update-check.cache-channel.test.js.) Without the stamp the throttled path serves nothing
// and the two windows this file is about could not be told apart. The spread is last so a test
// whose subject is the stamp can spell its own.
...
describe('recordPromptShown', () => {
  it('stamps last_prompted_at at `now` and preserves last_check_at and latest_version', () => {
    ...
    expect(written).toEqual({
      ...
      latest_version: '0.2.0',
      // #215: "preserves latest_version" now means the version AND the channel it is attributed
      // to — a stamp that kept the number but dropped the stamp would leave a version no copy
      // can read, so being prompted once would cost the notice for the rest of the week.
      latest_version_channel: 'npm',
    })
  ...
  it('creates the cache when there is none, stamping only the prompt field', () => {
    ...
    expect(readVersionCache({ fs: vol, home: HOME, processEnv: {} })).toEqual({
      ...
      last_prompted_at: iso(T0),
      latest_version: null,
      // #215: no version was resolved, so no channel is claimed — the pair is null together.
  ...
  it('writes ONLY the known fields — a hand-added declined_version is dropped', () => {
    const vol = seeded({
      last_check_at: iso(T0 - DAY),
      last_prompted_at: null,
      latest_version: '0.2.0',
    ...
    expect(Object.keys(raw).sort()).toEqual([
      ...
      'last_prompted_at',
      'latest_version',
      // #215: the list grew because the shape did. It is spelled out rather than loosened
      // precisely because the claim is a whitelist — `declined_version` is banned by there being
      // an exhaustive set of keys, so an inexhaustive one would prove nothing.
