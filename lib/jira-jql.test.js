...
// #129 — the two labels a ticket's SUCCESS path writes are both named here, in the module
// that composes the query that EXCLUDES them, and both are composed INTO that clause rather
// than retyped beside it. (#130 added a third, `failed`, which has its own section below for
// the same argument.) The failure this prevents is one bug with two spellings: a loop that stamps
...
describe('the labels Ralph writes are the labels the query excludes (#129)', () => {
  ...
  it('composes BOTH of them into the exclusion, from the constants and not by hand', () => {
    ...
    for (const label of [JIRA_IN_PROGRESS_LABEL, JIRA_DONE_LABEL, JIRA_FAILED_LABEL, 'do-not-ralph']) {
...
// #130 — the THIRD label Ralph writes. `failed` was already in the exclusion clause as a
// literal from #126, spelled for a sweep no code performed yet; #130 is the sweep, so the
// word stops being a placeholder and becomes a constant with two consumers. Same failure this
// prevents as the #129 pair above: a sweep that stamps `failed` and a query that excludes
// something else would hand the abandoned ticket straight back out, and the loop would spend
// a paid invocation per pass on work it has already given up on.
describe('the label the SWEEP writes is the label the query excludes (#130)', () => {
  it('names `failed` once, as a constant', () => {
    expect(JIRA_FAILED_LABEL).toBe('failed')
  })

  it('composes it into the exclusion, from the constant and not by hand', () => {
    // Positional, like the #129 assertion: a literal `failed` typed into the clause would
    // satisfy a `toContain` while leaving `JIRA_FAILED_LABEL` free to say something else.
    expect(JIRA_LABEL_EXCLUSION).toContain(
      `(${JIRA_IN_PROGRESS_LABEL}, ${JIRA_DONE_LABEL}, ${JIRA_FAILED_LABEL},`,
    )
  })

  it('so a SWEPT ticket drops out of the next composed query', () => {
    // The acceptance criterion "a failed ticket is excluded from the next queuePick",
    // asserted where it lives — there is no second Jira to run the query against, and the
    // label failTask writes is the label this clause refuses.
    const jql = composed('project = RALPH AND statusCategory != Done')
    expect(jql).toContain(`, ${JIRA_FAILED_LABEL},`)
