...
// #133: the JS-side reader of ralph.config.sh. A value init writes is only written
// correctly if this reads it back byte-identically, so the Jira knobs are asserted
// through it rather than through a substring match on the file.
...
describe('initCommand — task source selection (#565)', () => {
  ...
  it('the written config declares JIRA_JQL and documents whose half is whose (#126)', async () => {
    // The user writes ELIGIBILITY; Ralph writes the label exclusion and the ordering. A
    // user who does not know that writes their own `labels NOT IN` (harmless) or their own
    // ORDER BY (relocated, not refused) and cannot tell why the queue depth is what it is.
    // Asserted on the WRITTEN file, like the comment above it, so interpolation is covered.
    const { vol, run } = setup()
    await run()
    const cfg = vol.readFileSync(`${PROJECT}/ralph.config.sh`, 'utf8')
    // A live, empty knob — not a commented example: the loop sources this file, so an
    // assignment here is what stops a stray exported JIRA_JQL from deciding the queue.
    expect(cfg).toContain('JIRA_JQL=""')
    const comment = cfg.slice(cfg.indexOf('# Jira'), cfg.indexOf('JIRA_JQL=""'))
    expect(comment).toContain('TASK_SOURCE="jira"')
    expect(comment.toLowerCase()).toContain('eligib')
    // Ralph's half, spelled out — both clauses, because both change which tickets match.
    expect(comment).toContain('labels NOT IN (in-progress, done, failed, do-not-ralph)')
    expect(comment).toContain('labels IS EMPTY')
    expect(comment).toContain('ORDER BY created')
    // ...and that an empty value is "not configured" rather than "everything".
    expect(comment.toLowerCase()).toContain('empty means not configured')
    // Every line of the block is a comment line — an unquoted prose line in a file a
  ...
  it('the written config declares JIRA_DONE_STATUS and says a refusal is not fatal (#129)', async () => {
    // The one knob whose value Ralph cannot guess: status names are per-project workflow
    // ("Done", "Resolved", "Closed", "Ready for Release"), and a wrong one is not a typo
    // the loop can detect — Jira simply refuses the move. So the prose has to promise what
    // that refusal costs, or the first user to see the warning reads a working run as a
    // broken one and goes looking for a bug that is on their board.
    const { vol, run } = setup()
    await run()
    const cfg = vol.readFileSync(`${PROJECT}/ralph.config.sh`, 'utf8')
    // A live, empty knob — like JIRA_JQL above it, and empty for the same reason: there is
    // no status name that is right for every project, so shipping a guess would transition
    // tickets on somebody's board to a status they did not choose.
    expect(cfg).toContain('JIRA_DONE_STATUS=""')
    const comment = cfg.slice(
      cfg.indexOf('# Jira completion'),
      cfg.indexOf('JIRA_DONE_STATUS=""'),
    )
    expect(comment).not.toBe('')
    expect(comment).toContain('TASK_SOURCE="jira"')
    // The three facts a user needs: the value is a status NAME from their own workflow...
    expect(comment.toLowerCase()).toContain('workflow')
    // ...a refused transition costs a board move and never the run...
    expect(comment.toLowerCase()).toMatch(/refus/)
    expect(comment.toLowerCase()).toMatch(/never the run|not the run/)
    // ...and the `done` label goes on regardless, which is what drains the queue.
    expect(comment).toContain('done')
    expect(comment.toLowerCase()).toMatch(/empty|unset/)
    // Every line of the block is a comment line — an unquoted prose line in a file a
    // shell sources would be executed.
    for (const line of comment.split('\n').filter(Boolean)) {
      expect(line.startsWith('#'), line).toBe(true)
...
// ---------------------------------------------------------------------------
// #133: the three-way source picker, and the two Jira knobs a jira init fills.
//
// The prompt this replaces was a yes/no confirm — "Draw tasks from a local
// .ralph/tasks/ folder instead of GitHub? [y/N]" — which cannot express three
// options, so jira was reachable only by flag or by hand-editing the config. The
// picker is free text over `promptValue`, and the ANSWER is not validated here: it
// goes through the same `resolveSourceChoice` the flag does, so an unrecognized
// answer falls back to github rather than aborting an unattended run.
//
// The two knobs matter because a jira source with NO JQL is inert — an empty query
// counts nothing, selects nothing and runs no acli — so an init that picked jira and
// left JIRA_JQL empty would ship a config that cannot run until it is hand-edited.
// Only a jira init fills them; github and folder keep the empty values the template
// has always shipped, which is the no-regression half of this slice.
// ---------------------------------------------------------------------------
describe('initCommand — three-way source picker + Jira knobs (#133)', () => {
  const HOME = '/home/test'

  // The prompt strings, pinned as constants so a rename shows up as one failure
  // rather than as a scatter of them.
  const PICKER_Q = 'Draw tasks from github, folder or jira? [github]: '
  const DEFAULT_JQL =
    'assignee = currentUser() AND status NOT IN ("Done", "Closed", "Resolved", "Canceled")'
  const JQL_Q = `Jira eligibility query (JQL) [${DEFAULT_JQL}]: `
  const STATUS_Q = 'Jira status for a finished ticket [Done]: '

  // One harness for every case below. `answers` is a QUEUE consumed by promptValue in
  // the order init asks, and a question with no answer left resolves blank — which is
  // the "user pressed enter" case and has to be the default everywhere. The WhatsApp
  // gate (#5) is declined through `ask` so it never reaches promptValue, and home +
  // processEnv are injected so nothing looks at the real ~/.config/ralph.
  function harness({ answers = [], isTTY = true, ...overrides } = {}) {
    const vol = Volume.fromJSON({ [`${PROJECT}/.keep`]: '' }, '/')
    const stdout = makeStream()
    const stderr = makeStream()
    const questions = []
    const queue = [...answers]
    const promptValue = async (question) => {
  ...
  describe('the picker itself', () => {
    it('asks ONE question naming all three sources, with github as the shown default', async () => {
      const h = harness({ answers: [''] })
      await h.run()
      expect(h.questions).toEqual([PICKER_Q])
      // The three names a user has to be able to type, and the default a blank
      // answer takes — all in the one line they read.
      for (const name of ['github', 'folder', 'jira']) {
        expect(h.questions[0]).toContain(name)
      }
      expect(h.questions[0]).toContain('[github]')
    })

    it('a blank answer selects github and writes the zero-regression config', async () => {
      const h = harness({ answers: [''] })
      const result = await h.run()
      expect(result.source).toBe('github')
      expect(h.config()).toContain('TASK_SOURCE="github"')
      // The knobs stay at the pre-#133 literals — what a github init writes now that the
      // template carries placeholders rather than these bytes.
      expect(h.config()).toContain('JIRA_JQL=""')
      expect(h.config()).toContain('JIRA_DONE_STATUS=""')
      expect(h.vol.existsSync(`${PROJECT}/.ralph/tasks`)).toBe(false)
    })

    it('answering "folder" scaffolds both lanes and leaves the Jira knobs empty', async () => {
      const h = harness({ answers: ['folder'] })
      const result = await h.run()
      expect(result.source).toBe('folder')
      expect(h.config()).toContain('TASK_SOURCE="folder"')
      expect(h.config()).toContain('JIRA_JQL=""')
      expect(h.config()).toContain('JIRA_DONE_STATUS=""')
      for (const d of [
        '.ralph/tasks/afk/todo',
        ...
        expect(h.vol.existsSync(`${PROJECT}/${d}`), d).toBe(true)
      }
      // Folder mode asks nothing about Jira.
      expect(h.questions).toEqual([PICKER_Q])
    })

    it('answering "jira" writes TASK_SOURCE="jira" and scaffolds NO task tree', async () => {
      const h = harness({ answers: ['jira'] })
      const result = await h.run()
      expect(result.source).toBe('jira')
      expect(h.config()).toContain('TASK_SOURCE="jira"')
      expect(h.vol.existsSync(`${PROJECT}/.ralph/tasks`)).toBe(false)
      expect(h.vol.existsSync(`${PROJECT}/.ralph`)).toBe(false)
    })

    it('an unrecognized answer falls back to github rather than aborting', async () => {
      const h = harness({ answers: ['gitlab'] })
      const result = await h.run()
      expect(result.exitCode).toBe(0)
      expect(result.source).toBe('github')
      expect(h.config()).toContain('TASK_SOURCE="github"')
      // ...and it does not go on to ask the Jira questions.
      expect(h.questions).toEqual([PICKER_Q])
      expect(h.stderr.output()).toBe('')
    })

    it('normalizes case and surrounding whitespace at the picker', async () => {
      for (const [answer, expected] of [
        ...
        const result = await h.run()
        expect(result.source, answer).toBe(expected)
        expect(h.config(), answer).toContain(`TASK_SOURCE="${expected}"`)
      }
    })

    it('the --source flag beats the picker, which is never asked', async () => {
      const h = harness({ answers: ['folder'], source: 'jira' })
      const result = await h.run()
      expect(result.source).toBe('jira')
      // The picker question is absent; the first thing asked is the JQL.
      expect(h.questions).not.toContain(PICKER_Q)
      expect(h.questions[0]).toBe(JQL_Q)
    })

    it('with no TTY the picker is skipped entirely — nothing is asked, nothing blocks', async () => {
  ...
  describe('the JQL and status prompts', () => {
    it('asks for the JQL and the done status, in that order, showing both defaults', async () => {
      const h = harness({ answers: ['jira'] })
      await h.run()
      expect(h.questions).toEqual([PICKER_Q, JQL_Q, STATUS_Q])
      // The default is shown IN the question, because promptValue cannot prefill a
      // readline buffer — the bracketed value is how the user sees what enter takes.
      expect(h.questions[1]).toContain(DEFAULT_JQL)
      expect(h.questions[2]).toContain('[Done]')
    })

    it('a blank JQL answer writes the documented default VERBATIM', async () => {
      const h = harness({ answers: ['jira', '', ''] })
      const result = await h.run()
      expect(result.jiraJql).toBe(DEFAULT_JQL)
      // Byte-identical after a round trip through the file, which is the only
      // reading that matters — the value carries double quotes and parentheses.
      expect(parseConfigVar(h.config(), 'JIRA_JQL')).toBe(DEFAULT_JQL)
    })

    it('a blank status answer writes Done', async () => {
      const h = harness({ answers: ['jira', '', ''] })
      const result = await h.run()
      expect(result.jiraDoneStatus).toBe('Done')
      expect(h.config()).toContain('JIRA_DONE_STATUS="Done"')
      expect(parseConfigVar(h.config(), 'JIRA_DONE_STATUS')).toBe('Done')
    })

    it('a whitespace-only answer is a blank answer — both knobs take their default', async () => {
      const h = harness({ answers: ['jira', '   ', '  '] })
      const result = await h.run()
      expect(result.jiraJql).toBe(DEFAULT_JQL)
      expect(result.jiraDoneStatus).toBe('Done')
    ...
    it('a typed JQL and status are written and read back byte-identically', async () => {
      const jql = 'project = RALPH AND statusCategory != Done'
      const h = harness({ answers: ['jira', jql, 'Ready for Release'] })
      const result = await h.run()
      expect(result.jiraJql).toBe(jql)
      expect(result.jiraDoneStatus).toBe('Ready for Release')
      expect(parseConfigVar(h.config(), 'JIRA_JQL')).toBe(jql)
      expect(parseConfigVar(h.config(), 'JIRA_DONE_STATUS')).toBe('Ready for Release')
    })

    it('a typed JQL carrying quotes and parentheses round-trips byte-identically', async () => {
      // The shape the DEFAULT has, typed by hand, read back by the JS reader. What this
      // test does NOT prove is the quoting rule: measured, `parseConfigVar` closes a
      // quoted value at the LAST quote on the line, so it round-trips the naive
      // `JIRA_JQL="…"` line for this value too, and mutating quoteConfigValue to always
      // double-quote leaves every test in this file green. The rule is pinned where the
      // damage is — against a real `bash` — in lib/init.qa.test.js (24 of its rows fail
      // under that mutation). See quoteConfigValue in commands/init.js.
      const jql = 'project = RALPH AND status NOT IN ("Done", "Closed") AND labels IS EMPTY'
      const h = harness({ answers: ['jira', jql, ''] })
      await h.run()
      expect(parseConfigVar(h.config(), 'JIRA_JQL')).toBe(jql)
    })
  })

  describe('a jira init with no TTY', () => {
    // `ralph init --source jira` from a script is the whole reason the defaults exist
    // rather than a required prompt: it must write a config that RUNS, without asking.
    const nonInteractive = () => harness({ isTTY: false, source: 'jira' })

    it('writes the defaults silently, asking nothing', async () => {
      const h = nonInteractive()
      const result = await h.run()
      expect(h.questions).toEqual([])
      expect(result.jiraJql).toBe(DEFAULT_JQL)
      expect(result.jiraDoneStatus).toBe('Done')
    ...
    it('the written config is a working one — source, a non-empty JQL, and a status', async () => {
      const h = nonInteractive()
      await h.run()
      const cfg = h.config()
      expect(cfg).toContain('TASK_SOURCE="jira"')
      expect(parseConfigVar(cfg, 'JIRA_JQL')).toBe(DEFAULT_JQL)
      expect(parseConfigVar(cfg, 'JIRA_JQL')).not.toBe('')
      expect(parseConfigVar(cfg, 'JIRA_DONE_STATUS')).toBe('Done')
      expect(cfg).not.toContain('{{')
    })

    it('creates no .ralph/tasks/ — the local task tree is folder mode’s mechanism', async () => {
      const h = nonInteractive()
      await h.run()
      expect(h.vol.existsSync(`${PROJECT}/.ralph`)).toBe(false)
    })
  ...
  describe('the summary', () => {
    it('echoes the resolved source and, for jira, the JQL that was written', async () => {
      const h = harness({ answers: ['jira', '', ''] })
      await h.run()
      const out = h.stdout.output()
      expect(out).toContain('TASK_SOURCE:  jira')
      expect(out).toContain(`JIRA_JQL:     ${DEFAULT_JQL}`)
    })

    it('echoes a typed JQL rather than the default it replaced', async () => {
      const jql = 'project = RALPH'
      const h = harness({ answers: ['jira', jql, ''] })
      await h.run()
      expect(h.stdout.output()).toContain(`JIRA_JQL:     ${jql}`)
      expect(h.stdout.output()).not.toContain(DEFAULT_JQL)
    })

    it('draws no JIRA_JQL row for github or folder — there is no query to show', async () => {
      for (const answer of ['', 'folder']) {
        const h = harness({ answers: [answer] })
        await h.run()
        expect(h.stdout.output(), answer).toContain('TASK_SOURCE:')
        expect(h.stdout.output(), answer).not.toContain('JIRA_JQL:')
      }
    })
  })
})
  ...
  function harness(overrides = {}) {
    ...
    const run = () =>
      initCommand({
        ...
        ask: async () => false,
        // ...and answer every free-text prompt blank, for the same reason: since #133 the
        // source picker is built on `promptValue`, so a case below that flips isTTY on
        // would otherwise read the suite's stdin and hang. Blank means github.
        promptValue: async () => '',
        ...overrides,
      })
