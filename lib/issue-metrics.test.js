...
// ---------------------------------------------------------------------------
// #131: a jira event carries a `task_key` beside the number, and the aggregator
// must be indifferent to it — the tally and the per-run lists stay keyed on the
// NUMERIC field, which is what `ralph cycle`'s summary and exit code are built
// out of. The un-numbered case is the one worth pinning: an unrecognised key
// yields a null `issue_number`, which still has to COUNT even though there is
// no number to list.
// ---------------------------------------------------------------------------
describe('aggregateCycleCounts — jira events carrying task_key (#131)', () => {
  const line = (event) => 'RALPH_ISSUE_EVENT ' + JSON.stringify(event)

  it('tallies jira events into ok/failed and lists their derived numbers', () => {
    const text = [
      line({ issue_number: 123, task_key: 'FOO-123', verdict: 'pass', ts: 100 }),
      line({ issue_number: 124, task_key: 'FOO-124', verdict: 'fail', ts: 100 }),
    ].join('\n')
    const counts = aggregateCycleCounts(text, 0)
    expect(counts).toEqual({
      ok: 1,
  ...
  it('a keyed event with a NULL issue_number still counts, but lists no number', () => {
    // The permissive half of lib/jira-key.js reaching the aggregator: `FOO-BAR-1`
    // is a ticket Ralph will work and a key it cannot read a number out of, so the
    // run's outcome must still be counted — `ralph cycle`'s exit code depends on it
    // — while the OK/FAIL lists print numbers and have none to print.
    const text = [
      line({ issue_number: null, task_key: 'FOO-BAR-1', verdict: 'pass', ts: 100 }),
      line({ issue_number: null, task_key: 'WEIRD', verdict: 'fail', ts: 100 }),
    ].join('\n')
    const counts = aggregateCycleCounts(text, 0)
    expect(counts).toEqual({
      ok: 1,
  ...
  it('mixes jira, folder and github events in one log without noticing the difference', () => {
    const text = [
      line({ issue_number: 98, verdict: 'pass', ts: 100 }),
      line({ issue_number: 7, verdict: 'fail', ts: 100 }),
      line({ issue_number: 123, task_key: 'FOO-123', verdict: 'pass', ts: 100 }),
    ].join('\n')
    const counts = aggregateCycleCounts(text, 0)
    expect(counts.ok).toBe(2)
    expect(counts.failed).toBe(1)
    expect(counts.okIssues).toEqual([98, 123])
    expect(counts.failedIssues).toEqual([7])
  })
})
