...
//
// APPEND, NEVER OVERWRITE. Each digest is appended to `.ralph/digest.log` with an ISO
// stamp, the run id and the task in flight, so after a night the file reads as the
// whole night's narrative. `.ralph/` is already gitignored. The entry format is
...
// clock) arrives as a parameter. lib/commands/digest.js is the CLI shell over it.
...
import { formatClock, toJsonSnapshot } from './progress.js'
...
// The prompt template, and the history file. One file appended forever, next to the
// run record and the metrics the same directory already holds.
...
// the same width, so a history file skims as a column of entries.
const HEADING_WIDTH = 64

// Every narrative line in the history file is written at this indent. See
// formatHistoryEntry: it is what keeps an entry's own text out of the entry format.
const ENTRY_INDENT = '  '

// A one-line diagnostic is a promise about SHAPE, and the text it interpolates is
// somebody else's (an agent's stderr, an exception message), so it is collapsed and
...
// SEPARATOR cannot occur inside an entry: a leading newline, then timestamp, run id and
// in-flight task on the heading line (so the file is greppable by run and by task), then
// the narrative INDENTED line by line, then a blank line.
//
...
//   * `oneLine` ON THE HEADING, against the run record. The record is read verbatim off
//     disk and may be from a future version, hand-edited or truncated (see
//     lib/run-state.js), so a `run_id` holding a newline would split the heading and could
//     itself start a second `^── ` line — the very forgery the indent exists to prevent.
export function formatHistoryEntry({ at, runId, task, narrative } = {}) {
  const head = heading(oneLine(`${at || 'unknown'} · run ${runId || 'unknown'} · ${task || 'none'}`))
...
export async function runDigest({
  ...
  try {
    ...
    appendFile(path, formatHistoryEntry({ at: isoStamp(atMs), runId: record?.run_id, task, narrative }))
  } catch (e) {
...
// Exported because the CLI shell makes the same one-line promise about the same text
// and must keep it the same way — two flatteners would be two shapes, and the shape is
// the whole contract of a diagnostic that a reader greps or a launchd log collects.
export function oneLine(text) {
  const collapsed = String(text ?? '')
    .replace(/\s+/g, ' ')
    .trim()
  return collapsed.length > DIAGNOSTIC_MAX_CHARS
    ? `${collapsed.slice(0, DIAGNOSTIC_MAX_CHARS - 1)}…`
    : collapsed
}
