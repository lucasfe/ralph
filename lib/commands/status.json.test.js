...
// #222 — the document says WHERE, not just WHAT. Since #218 each task runs in its own git
// worktree, so a script driven off this document (the documented caller: a prompt line on a
// timer, a wrapper that wants the in-flight diff) has to be told which directory to look in —
// `git worktree list` and a guess at the branch name is not an interface. The leaf's shape is
// pinned in lib/progress.worktree.test.js; what this describe owns is the SHELL's half: that
// the record the command read is the record the leaf is published from, that the key survives
// the modes that null everything around it, and that the two surfaces agree.
describe('ralph status --json — the directory the run is working in (#222)', () => {
  const WORKTREE = '/repo/.ralph/worktrees/issue-31'
  // `runningRecord()` deliberately has no worktree — it is the pre-#222 record every
  // assertion above is written against — so the field is added here.
  const inWorktree = (worktree = WORKTREE) =>
    runningRecord({ current: { ...runningRecord().current, worktree } })

  // A record that names a worktree, read while the tmux session is gone: the run is over and
  // its record still says `running`, which is the shape `interrupted` is.
  const interruptedRecordDeps = () =>
    baseDeps({ json: true, exec: makeExec({ sessionAlive: false }), readRunState: () => inWorktree() })

  it('publishes the recorded worktree as the last key of tasks.current', async () => {
    const deps = baseDeps({ json: true, readRunState: () => inWorktree() })
    await statusCommand(deps)
    const doc = parseOnly(deps)
  ...
  it('says null, and never drops the key, for a record written before the field existed', async () => {
    // `jq .tasks.current.worktree` must answer on a run that was in flight when the user
    // upgraded, as it must on a task whose worktree was never created: `null` is this
    // document's word for unknown everywhere, and an absent key is a filter that works on
    // one machine and fails on the next.
    for (const readRunState of [() => runningRecord(), () => inWorktree(null), () => inWorktree('')]) {
      const deps = baseDeps({ json: true, readRunState })
      await statusCommand(deps)
      expect(parseOnly(deps).tasks.current.worktree).toBe(null)
      expect(deps.stdout.output()).toContain('"worktree":null')
    }
  })

  it('publishes the path a script can cd into — the RECORD’s text, not the row’s', async () => {
    // The shell hands `toJsonSnapshot` the record it read, which is what makes this leaf
    // verbatim: a path with a space in it is an ordinary macOS path, and one that has been
    // scrubbed or shortened for a terminal names no directory. `JSON.stringify` is the only
    // escaping a machine surface needs, and it keeps the document one line.
    const spaced = '/Users/someone/my repo/.ralph/worktrees/issue-31'
    const deps = baseDeps({ json: true, readRunState: () => inWorktree(spaced) })
    await statusCommand(deps)
    expect(parseOnly(deps).tasks.current.worktree).toBe(spaced)
    expect(deps.stdout.lines()).toHaveLength(1)
  })

  it('names no directory for a run that is over', async () => {
    // `endRun` keeps the last task on the record on purpose, so the path is still readable
    // after the run stopped — and the gate is the same one that nulls the predictions: a
    // dead run has no task in flight, so there is no `tasks.current` to hold a path.
    const deps = interruptedRecordDeps()
    await statusCommand(deps)
    expect(parseOnly(deps).mode).toBe('interrupted')
    expect(parseOnly(deps).tasks.current).toBe(null)
    expect(deps.stdout.output()).not.toContain('worktree')
  })

  it('reports the directory the human view prints, from the same read', async () => {
    // One snapshot, two surfaces (#58's second promise) applied to the new field: the row
    // and the leaf are the same recorded path, so a reader and a script can never be sent
    // to two different directories.
    const machine = baseDeps({ json: true, readRunState: () => inWorktree() })
    await statusCommand(machine)
    const human = baseDeps({ readRunState: () => inWorktree() })
    await statusCommand(human)
    expect(human.stdout.output()).toContain(`  worktree   ${parseOnly(machine).tasks.current.worktree}`)
  })
})
