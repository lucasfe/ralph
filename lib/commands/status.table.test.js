...
describe('statusCommand — where the table sits in the live view (#56)', () => {
  it('renders the issue’s worked example, in order, once', async () => {
    ...
    expect(lines).toEqual([
      ...
      '',
      `  attach     ralph live  (tmux attach -t ${SESSION})`,
      '  kill       ralph stop',
    ])
  })
...
describe('statusCommand — a jira table is titled from the BOARD, not from GitHub (#132)', () => {
  ...
  it('makes NO title call of either kind under --json, and still names the ticket', async () => {
    ...
    // #222 appended `worktree` last; this jira record has none, and the key is published
    // regardless — the present-and-null rule the whole document follows.
    expect(Object.keys(doc.tasks.current)).toEqual(['number', 'started_at', 'task_key', 'worktree'])
    expect(doc.tasks.current.worktree).toBe(null)
...
// ---------------------------------------------------------------------------
// #222 — WHERE THE WORKTREE ROW SITS. Point 1 at the top of this file is about the table's
// place in the view, and this slice adds a second thing only the shell can be wrong about:
// since #218 the in-flight task runs in its own git worktree, and until now nothing on
// screen said which directory that is — a detached run left the reader to run `git worktree
// list` and match a branch name by hand.
//
// The row is a LABEL-COLUMN row (`worktree`, beside `queue` and `attach`) rather than a
// fifth table column: the task column's width is derived from the widest cell on show, so
// one absolute path in a cell would push `verdict`, `cost` and `time` off the screen. That
// puts three questions here, and lib/progress.worktree.test.js owns everything else:
//
//   WHERE      → between the table's closing blank and the `queue` count, so the facts about
//                the task in flight stay together and `queue` keeps touching the pace and
//                the ETA that are computed from it.
//   WHETHER    → only when there IS one. A record written before this field existed and an
//                iteration that died before its worktree was created both render the view
//                they rendered yesterday, with no empty row and no stray dash.
//   WHOSE TEXT → the path arrives through `git`'s stdout, bash and a JSON file a human can
//                edit, so the shell must route it through the same scrub an issue title
//                crosses. Asserted here the way the forged jira summary above is.
// ---------------------------------------------------------------------------

const WORKTREE = '/repo/.ralph/worktrees/issue-31'

// The record a live github iteration leaves since #222. `running()`'s own `current` has NO
// worktree on purpose — it is the pre-#222 record every assertion above this line is written
// against — so the field is added here, where the tests about it live.
const inWorktree = (worktree = WORKTREE) =>
  running({ current: { ...running().current, worktree } })

describe('statusCommand — the view names the directory the run is working in (#222)', () => {
  it('stands the worktree between the table and the queue count', async () => {
    const { result, lines } = await run({
      readRunState: () => inWorktree(),
      exec: makeExec({ titlesResult: WORKED_TITLES }),
    })
    expect(result.exitCode).toBe(0)
    expect(lines).toEqual([
      `▸ ralph — running · run ${RUN} (started 16:20, 3h12m ago)`,
      ...
      '  #031 row comp  🔄 live     –         ~40min',
      '',
      // The one new line in the whole view: the label column #57 established, carrying the
      // path the record measured. It reads directly under the row that says `#031 … 🔄 live`
      // — the task and the directory that task is being done in, adjacent — and above the
      // counted/extrapolated block, which is left exactly as #57 laid it out.
      `  worktree   ${WORKTREE}`,
      '  queue      6 waiting',
      '  pace       ~84 min/task · $31.4/task',
      '  eta        ~9h08m left → ~04:40  (±1h30m)',
  ...
  it('keeps the blank line the table closes with, and adds no second one', async () => {
    // The stray-blank bug point 1 warns about, for the row instead of the table: the block
    // between the table and `queue` is ONE line long, and the table's own spacing is
    // untouched by it.
    const { lines } = await run({
      readRunState: () => inWorktree(),
      exec: makeExec({ titlesResult: WORKED_TITLES }),
    ...
    const header = headerIndex(lines)
    expect(lines[header + 4]).toBe('')
    expect(lines[header + 5]).toBe(`  worktree   ${WORKTREE}`)
    expect(lines[header + 6]).toContain('queue      6 waiting')
    expect(lines.filter((line) => line.startsWith('  worktree'))).toHaveLength(1)
  })

  it('renders the row with no table at all, for a run whose first task is still its only one', async () => {
    // The row hangs off the RECORD, not off the table: a run that has closed nothing has no
    // history rows to sit under, and the reader who most needs the path is the one watching
    // the very first iteration.
    const { lines } = await run({ readRunState: () => inWorktree(), readFile: readByPath('') })
    const worktreeAt = lines.findIndex((line) => line.startsWith('  worktree'))
    expect(worktreeAt).toBeGreaterThan(0)
    expect(lines[worktreeAt]).toBe(`  worktree   ${WORKTREE}`)
    expect(lines[worktreeAt + 1]).toContain('queue      6 waiting')
  })

  it('draws NO row — and no stray blank — for a record written before the field existed', async () => {
    // The compatibility half of the slice, on the surface where it shows: a run that was in
    // flight when the user upgraded has a `current` with four keys, and its view is the view
    // #57 shipped, line for line. This is the same `running()` fixture the whole file above
    // uses, which is why nothing above this line changed.
    const { result, lines } = await run({ exec: makeExec({ titlesResult: WORKED_TITLES }) })
    expect(result.exitCode).toBe(0)
  ...
  it('draws no row for a task that never got a worktree, whatever the record holds', async () => {
    // The loop records the task it died on even when the worktree could not be created
    // (`''` from bash, so `null` on disk), and a hand-edited record can hold anything at
    // all. None of those may become `  worktree   –`: a label with a dash beside it is
    // furniture in a view whose every other label answers a question.
    for (const worktree of [null, '', '   ', 31, {}, []]) {
      const { result, lines } = await run({ readRunState: () => inWorktree(worktree) })
      expect(result.exitCode, JSON.stringify(worktree)).toBe(0)
      expect(lines.some((line) => line.includes('worktree')), JSON.stringify(worktree)).toBe(false)
    }
  })

  it('draws no row for a run that is over, in either mode that reports one', async () => {
    // `endRun` keeps the last task on the record deliberately, so the path is still there to
    // be read after the run has stopped — and a report card that says `worktree …` would be
    // telling the reader Ralph is working in a directory nothing is running in. Same gate as
    // the `🔄 live` row: the run has to be alive.
    const dead = { readRunState: () => inWorktree(), exec: makeExec({ tmuxResult: { exitCode: 1 } }) }
    const { result, lines } = await run(dead)
    expect(result.mode).toBe('interrupted')
    expect(lines.some((line) => line.includes('worktree'))).toBe(false)
    ...
    const idle = await run({
      readRunState: () => ({
        ...inWorktree(),
        status: 'completed',
        finished_at: RUN_STARTED.toISOString(),
      }),
    })
    expect(idle.result.mode).toBe('idle')
    expect(idle.lines.some((line) => line.includes('worktree'))).toBe(false)
  })

  it('cannot be made to forge a row through a path in the record', async () => {
    // `.ralph/run-state.json` is a file a human can edit and `git worktree add` prints a
    // path Ralph passed it, so the value is untrusted exactly as a GitHub issue title is.
    // The scrubbing lives in the pure module; this is the assertion that the shell really
    // does route the recorded path through it.
    const forged = `/repo/.ralph${ESC}[31m/worktrees${BEL}${CR}${LF}  queue      99 waiting`
    const { result, lines } = await run({ readRunState: () => inWorktree(forged) })
    expect(result.exitCode).toBe(0)
    const joined = lines.join('\n')
    for (const byte of [ESC, BEL, CR, NUL, VT, FF, BS, DEL]) expect(joined.includes(byte)).toBe(false)
    // The forged text survives as TEXT, folded into the one row it was written into, and
    // never as a second `queue` line the reader would believe.
    expect(lines.filter((line) => line.includes('99 waiting'))).toHaveLength(1)
    expect(lines.filter((line) => line.startsWith('  worktree'))).toHaveLength(1)
    expect(lines.filter((line) => line.includes('queue      6 waiting'))).toHaveLength(1)
  })

  it('shows the worktree in folder and jira mode too, paying for nothing extra', async () => {
    // Every source gets a worktree since #218, and this row is read off the record rather
    // than looked up — so the mode that is deliberately gh-free and board-free renders it
    // without a single new process.
    const folder = await run(
      sourced('folder', {
        readRunState: () => running({ source: 'folder', current: { ...running().current, worktree: '/repo/.ralph/worktrees/task-2' } }),
        folderQueueCount: async () => 6,
      }),
    )
    expect(folder.result.exitCode).toBe(0)
    expect(folder.lines).toContain('  worktree   /repo/.ralph/worktrees/task-2')
    expect(folder.exec.gh()).toHaveLength(0)

    const jira = await run(
      sourced('jira', {
        jiraTitles: makeJiraTitles(),
        readRunState: () =>
          running({
            source: 'jira',
            current: { number: 43, task_key: 'FOO-43', started_at: TASK_STARTED.toISOString(), iteration: 3, worktree: '/repo/.ralph/worktrees/foo-43' },
          }),
      }),
    )
    expect(jira.result.exitCode).toBe(0)
    expect(jira.lines).toContain('  worktree   /repo/.ralph/worktrees/foo-43')
    expect(jira.exec.gh()).toHaveLength(0)
  })

  it('publishes the same directory under --json, as the last key of tasks.current', async () => {
    // The scriptable half of the acceptance criteria, and the reason the row and the leaf
    // are built from ONE snapshot: `ralph status --json | jq -r .tasks.current.worktree` is
    // a `cd` target, so it is the path VERBATIM — no scrub, no truncation, and appended last
    // so a consumer reading the document positionally finds #132's three keys where they
    // were.
    const d = deps({ json: true, readRunState: () => inWorktree() })
    const result = await statusCommand(d)
    expect(result.exitCode).toBe(0)
    const doc = JSON.parse(d.stdout.output())
    expect(doc.tasks.current.worktree).toBe(WORKTREE)
    expect(Object.keys(doc.tasks.current)).toEqual(['number', 'started_at', 'task_key', 'worktree'])
    // Still one line, and still no human furniture: a path is the first value in this
    // document that can contain a space, and `JSON.stringify` is what keeps it one line.
    expect(d.stdout.output().trimEnd().split('\n')).toHaveLength(1)
    expect(d.stdout.output()).not.toContain('worktree   ')
  })
})
