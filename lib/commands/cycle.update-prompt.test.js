...
//      launch its loop for the same reason (start.js:831-838); the cycle refuses to
  ...
  Volume.fromJSON(
    {
      [CACHE_PATH]: JSON.stringify({
        ...
        // #215: the version is now filed under the channel that resolved it, and a copy only
        // reads its OWN channel's number. Every run in this file classifies through
        // `npmGlobalLayout()`, so npm is the channel these deps ask — stamping it is what keeps
        // this seed a warm cache rather than an unattributable one, which is the subject of
        // lib/update-check.cache-channel.test.js and not of the prompt window.
...
describe('cycleCommand — the window is stamped before the answer is awaited (#52)', () => {
  ...
  it('stamps the window exactly once per cycle, and only the prompt field', async () => {
    ...
    expect(cacheOf(cacheFs)).toEqual({
      ...
      latest_version: '0.2.0',
      // #215: the check that fetched 0.2.0 files it under the channel it asked. Enumerated
      // rather than loosened because the claim is "and ONLY the prompt field" — the whole test
      // is which fields the stamp leaves alone.
      latest_version_channel: 'npm',
    })
  })
})
