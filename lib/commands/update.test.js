...
describe('updateCommand — Homebrew install (#198)', () => {
  it('spawns `brew upgrade ralph` and nothing else', async () => {
    ...
    // The whole spawn list, on exact argv rather than a substring: the registry
    // query, then one `brew upgrade`. No `npm root -g` (the marker decides above
    // it), no `npm install -g`, and no separate `brew update` — brew refreshes
    // its taps on its own auto-update cadence.
    expect(deps.exec.calls.map((c) => ({ cmd: c.cmd, args: c.args }))).toEqual([
      { cmd: 'npm', args: ['view', '@lucasfe/ralph', 'version'] },
