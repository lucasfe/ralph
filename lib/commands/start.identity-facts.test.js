...
describe('startCommand — which source, which repo (#69)', () => {
  ...
  it('names the repo in jira mode too — the row follows the rule, not the case (#125)', async () => {
    ...
    // exactly the repository it names (nothing selects a Jira ticket yet). The claim the row
    // makes is the one the test above states in the negative: FOLDER mode reads issues from no
    // repository, and everything else does. Asserting both arms together is what pins the gate
    // as the rule rather than as a list of blessed values a fourth one can fall off.
