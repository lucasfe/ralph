...
//
// The box's WORDING is lib/banner-compose.test.js's business and the RESOLUTION rules are
// lib/banner-model.test.js's. What is asserted here is everything that is this command's,
// and all of it is about where a fact comes from rather than about how it reads:
//
...
describe('startCommand — the new facts cost the run nothing (#69)', () => {
  ...
  it('writes no telemetry and changes no event shape — criterion 9, as a static read', async () => {
    ...
    expect(code).toMatch(/safeReadMetrics/)
  })
})
