...
describe('startCommand — which source, which repo (#69)', () => {
  ...
  it('names the repo in jira mode too — the row follows the rule, not the case (#125)', async () => {
    // #125 added a third TASK_SOURCE value, and the gate above this row used to read
    // `=== 'github'` — so a jira run lost the row while the loop went on reading issues from
    // exactly the repository it names. The claim the row makes is the one the test above
    // states in the negative: FOLDER mode reads issues from no repository, and everything
    // else does. Asserting both arms together is what pins the gate as the rule rather than
    // as a list of blessed values a fourth one can fall off.
    //
    // #127 MOVED WHOSE FACT THIS IS without moving the assertion: the loop now selects Jira
    // tickets and runs no `gh` command, but `ralph start` itself still gates on `gh auth`,
    // creates labels, sweeps orphans and counts GitHub issues under this source — so the
    // slug is still what this command's own calls will read. The row goes when those go.
    const { d } = await run({ config: 'TASK_SOURCE=jira\n' })
    expect(rowOf(d, 'source')).toBe('jira')
    expect(rowOf(d, 'repo')).toBe('lucasfe/ralph')

    const { d: folder } = await run({ config: 'TASK_SOURCE=folder\n' })
    expect(rowOf(folder, 'repo')).toBeUndefined()
  })
