...
import { queueCountResult as jiraQueueResult } from '../jira-queue.js'
...
export async function collectStatus({
  ...
  //
  // #127 left that third gate alone and it now costs a jira run one `gh` list it cannot use:
  // a Jira row is named by its KEY (lib/progress.js looks up no title for a task that has
  // one) and a jira run records no closed rows at all, so the map comes back and titles
  // nothing. Narrowing it to `=== 'github'` is part of the same follow-up as the gh gates in
  // `ralph start` and `ralph cycle`, and is deliberately not smuggled in here: this call is
  // a courtesy whose every failure is already `{}`, so the cost of leaving it is a spawn,
  // while the cost of moving one gate of four is a repo that half-believes gh is gone.
  //
