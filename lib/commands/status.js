...
//
// This no longer counts the way `ralph start` counts, and the divergence is #126's: that
// command still counts GITHUB issues under TASK_SOURCE=jira, because the loop it launches
// still works them. Two commands reporting different numbers for one repo is the honest
// answer while jira is half a loop — each names the queue its own next action will draw
// from — so do not "fix" one to agree with the other without moving ticket selection first.
//
