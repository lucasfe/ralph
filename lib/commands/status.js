...
// Reads only, like everything else here. The metrics TEXT is deliberately not in that
// list: it is read below and consumed below, and handing it back would invite a caller
// to build a second snapshot out of it — the one thing one gatherer exists to prevent.
//
...
export async function collectStatus({
  ...
  return { root, record, mode, session, tmuxAlive, queue, now: nowMs, progress, postMortem }
}
...
export async function statusCommand({
  ...
  // publishes as `null`.
  const { record, mode, session, queue, tmuxAlive, now: nowMs, progress, postMortem } =
