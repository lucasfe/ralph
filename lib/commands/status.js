...
// queue) around two pure functions — reconcileMode() and renderStatus() — so
// every judgement and every string is unit-testable without a clock, a
// filesystem or a subprocess. lib/run-state.js owns the record's fields; this
// file never writes one.
...
// Wall clock (`16:20`) in the reader's own timezone — this line answers "when did
// I start this?", which is a local-time question.
...
// PURE renderer: mode + record + queue + an injected `now` (epoch ms) in, lines
// out. No I/O, no Date.now — the hermetic suite pins the exact output.
//
...
export function renderStatus({ mode, record, session, queue, attachable = mode === 'running', now }) {
  if (mode === 'never-run') {
...
// successful read, so there is nothing to write to it.
export async function statusCommand({
  ...
  // The queue is only rendered by the live view, so idle/never-run spend nothing
  // — no `gh` call, no directory scan.
  const queue =
    mode === 'running' || mode === 'interrupted'
      ? await countQueue({ root, exec, exists, readFile, folderQueueCount, processEnv })
      : null

  const lines = renderStatus({ mode, record, session, queue, attachable: tmuxAlive, now: now() })
