...
// issues.jsonl and hands the text to the pure path.
//
...
// The pace/ETA/spend arithmetic AND the two time formatters the live view has used
...
export { formatClock, formatElapsed }
...
// PURE renderer: mode + record + queue + the raw metrics text + an injected `now`
...
//
// `metricsText` is issues.jsonl verbatim (#57); it defaults to empty, which
// renders pace/ETA/spend as `unknown` — the honest reading when there is no
...
export function renderStatus({
  ...
  const lines = [
    ...
    ...renderProgress(buildProgress({ metricsText, record, queue, now })),
    '',
  ]
...
// Same guard cycle.js puts around the same file: a missing, unreadable or
// half-written issues.jsonl degrades to '' — which renders pace/ETA/spend as
// `unknown` — because a read-only view must never abort over its own metrics.
