...
// how long it has been going, which task is in flight and for how long, which
// directory that task is being done in (#222), how deep the queue is right now, and
// how to reach the run — the session's attach/kill
...
// #126: the Jira queue depth. `jira-queue.js` imports no process spawner (its `exec` is the
// one this command already injects) and reaches only the pure composer, which imports nothing
// at all — so this adds no third-party edge to this command's import graph, which QA #76 pins
// as an exact set in status.identity-box.qa.test.js.
//
// THE RESULT-SHAPED probe, not the number-shaped one: a view has to be able to say it does not
// know. See defaultJiraQueueCount at the foot of this file.
// ...and #132's batch title lookup, which is the same story exactly: one `acli` call for every
// key the table is about to draw, through the injected spawner, returning `{}` on every failure.
import { queueCountResult as jiraQueueResult, titlesFor } from '../jira-queue.js'
...
// #139: the same query `ralph start` and the loop use — imported now rather than retyped, so
// "6 waiting" here means the same six items the loop would pick up next BY CONSTRUCTION and
// not by three copies of one string happening to agree. Imports nothing itself, so this adds
// no edge to the graph status.identity-box.qa.test.js pins.
...
export function renderStatus({
  ...
  const lines = [
    ...
    // WHERE the in-flight task is being done (#222) — between the table and `queue`, so
    // everything this view knows about the current task sits together and `queue` still
    // touches the pace and ETA computed from it. A row, not a fifth table column: the path is
    // long and unbounded while the table derives its widths from its cells, and the record
    // names only the task IN FLIGHT — so such a column would be empty on every completed
    // row and wide because of one. Like the table it can be NOTHING — a spread of zero
    // lines rather than a `worktree   unknown` — because an iteration that died before its
    // worktree existed, and a record written before #222, have nothing to say here rather
    // than a missing value.
    ...renderWorktreeRow(progress),
  ...
  if (attachable) {
    // #169: `ralph live` (./live.js) named first, with the tmux command it replaces kept
    // parenthetically beside it — ONE row, not two. Every two-column row in this view puts
    // its value at column 14, so a continuation line under `attach` would read as a new
    // key in the queue/pace/eta/spend/attach/kill block. The parenthetical keeps the
    // session name, which is why a reader with several loops reads this row at all.
    lines.push(`  attach     ralph live  (tmux attach -t ${session})`, '  kill       ralph stop')
  } else {
...
export async function collectStatus({
  ...
  // ralph.config.sh, read ONCE and asked FOUR times (#63, #76, #126): the task source
  // decides how the queue is counted, JIRA_JQL is the query that counts it under
  // TASK_SOURCE=jira, the digest interval decides when a narration counts as late, and
  // RALPH_BANNER decides whether the identity box prints. Reading it four times would let a
  // config rewritten in between answer the questions differently — see parse-config-var.js,
  // whose grammar takes text for this reason. All four are asked HERE, on this side of the
  // seam, and only the answers are handed back; see the return below for why the text itself
  ...
  // JIRA_JQL out of the SAME text (#126) — never a second read of the file, for the reason
  // the comment above gives. Config-only, with no `processEnv` fallback beside it unlike
  // TASK_SOURCE: an eligibility query is a property of the repo, the assignment is always
  // present in the config (init writes it on every path — a working default for jira since
  // #133, empty otherwise), and `set -a` puts whatever it holds, empty included, in the
  // environment of every child the loop spawns — so an env fallback would read as
  // "unconfigured" in one process and "configured" in the next.
  const jiraJql = parseConfigVar(configText, 'JIRA_JQL')
  const queue = measured
    ? await countQueue({ root, source, exec, folderQueueCount, jiraQueueCount, jiraJql })
    : null
  // Read at the RUN's root, where the loop appended it — same anchor as the record
  // and the lock.
  //
  // READ BEFORE THE TITLES, and since #132 that ordering is load-bearing rather than
  // incidental: the jira title lookup asks about the keys the table is about to draw, and those
  // keys are in this text. A file read, not a spawn, so nothing about the gh call order below
  // moved with it.
  const metricsText = measured ? safeReadText(readFile, metricsPath(root)) : ''
  // TASK TITLES for the task table (#56, #132), behind a FOURTH gate — tighter than
  // `measured`, tighter than the digest's, and both reasons are independent:
  //
  ...
  //                          keeps the document free of prose AND keeps `--json` the
  //                          cheap surface a prompt or notifier can poll. #132 added
  //                          `task_key` to that document and did NOT add a title: a key is
  //                          an identity a machine can resolve for itself, a summary is
  //                          prose, and this gate is what keeps the cheap surface cheap.
  //
  // THE THIRD GATE BECAME A SWITCH (#132), which is the follow-up the note here used to
  // promise. It said narrowing `source !== 'folder'` to `=== 'github'` was "part of the same
  // follow-up as the gh gates in `ralph start` and `ralph cycle`" — this is that slice for this
  // call site, and it does more than narrow, because a jira row now HAS somewhere to get a
  // title from. Each source asks whoever knows: github asks `gh`, jira asks the board through
  // jira-queue.js's `titlesFor`, and folder asks nobody, because a folder task's title lives
  // inside its own file and that mode is deliberately gh-free — it is the mode for repos with
  // no GitHub at all. A jira run therefore no longer pays for a `gh` list it cannot use.
  //
  // A COURTESY, never a fact: any failure resolves to `{}` and every row renders as its own
  // name, which is exactly what folder mode does on purpose. Nothing is logged, because there
  // is no stderr in this deps bag to log to — see the note below.
  const titles =
    mode === 'running' && !json
      ? await readTaskTitles({ source, root, exec, metricsText, record, jiraTitles })
      : {}
...
export async function statusCommand({
  ...
  // folderQueueCount, jiraQueueCount, peekLock, now — is forwarded to `collectStatus`
...
// Live queue depth. The task source comes from ralph.config.sh, and each source counts its
// own way: folder walks the local .ralph/tasks tree and spawns nothing, jira runs JIRA_JQL
// through acli (#126), github asks `gh issue list`. Returns null — rendered as `unknown` —
...
//
// This no longer counts the way `ralph start` counts, and since #127 that divergence is a
// DEBT rather than a defensible split: that command still counts GITHUB issues under
// TASK_SOURCE=jira, while the loop it launches selects and claims JIRA tickets. So this
// number is the one about the queue that drains, `ralph start`'s is about a board the run
// will not touch, and the fix belongs there — moving it, with the gh gates around it, is
// the follow-up (README: "What is still GitHub's"). Do not make this one agree with that
// one instead.
//
...
async function countQueue({ root, source, exec, folderQueueCount, jiraQueueCount, jiraJql }) {
  if (source === 'folder') {
    try {
  ...
  // #126: jira counts through acli instead of gh. Same degradation as the folder arm — a
  // count that cannot be taken is `null`, rendered as `unknown`, which is this file's
  // posture and deliberately NOT `ralph cycle`'s: a view says when it does not know, while a
  // scheduler with no provable work has nothing to do and reads the same failure as 0.
  if (source === 'jira') {
    try {
      return finiteOrNull(await jiraQueueCount({ jql: jiraJql, exec, cwd: root }))
    } catch {
      return null
    }
  }
  ...
  const queue = await exec(
    'gh',
    ['issue', 'list', '--search', ISSUE_SEARCH_QUERY, '--limit', '100', '--json', 'number', '-q', '. | length'],
...
// WHICHEVER LOOKUP THE TASK SOURCE KNOWS ABOUT (#132) — the switch behind the title gate, and
// the whole of what this slice added to the read plan. THREE SOURCES, THREE ANSWERS: github asks
// `gh` (unchanged, above), jira asks the board, folder asks nobody.
//
// THE TWO MAPS ARE KEYED DIFFERENTLY AND THAT IS THE DESIGN, not an accident to paper over: a
// github row is named by its issue number and a jira row by its ticket key, and lib/progress.js
// looks a row's title up by the same string that NAMES the row — which is what let that module
// drop the older defence of withholding the titles map from a keyed row entirely.
//
// THE SPACES ARE DISJOINT BECAUSE OF WHAT IS WRITTEN, not because they could not meet. This
// function's github map is keyed by `titles[number]`, i.e. by the string '123', and nothing
// validates a `task_key` against the Jira grammar on the way in, so a digits-only key WOULD
// read a GitHub issue's title out of it. Nothing writes one — the key comes off Atlassian's
// answer, and Jira keys are `PROJECT-123` — and the full argument, including who the writers
// are, lives on the row itself in lib/progress.js's `buildTaskRows`. Do not read this paragraph
// as a type argument; it is a claim about writers, and it is the one that holds.
//
// EVERY FAILURE IS `{}`, including a `jiraTitles` seam that throws. `titlesFor` promises never
// to throw, but it is INJECTED here (see `defaultJiraTitles`) and a stub, a wrapper or a
// replacement is not bound by that promise — the same argument `readIssueTitles` makes about
// `reject: false` one function up, and the same finding review made there. A title is a
// courtesy; a read-only view must not go down over one.
async function readTaskTitles({ source, root, exec, metricsText, record, jiraTitles }) {
  if (source === 'github') return readIssueTitles({ root, exec })
  // Neither folder mode nor a source this file has never heard of gets a lookup: the answer to
  // "what is this task called" for a folder task is inside the task's own file.
  if (source !== 'jira') return {}
  // ONLY THE ROWS THE TABLE IS ABOUT TO NAME, and lib/progress.js decides which those are —
  // the cap, the ordering, the run scoping and the scrub are its policy, so `taskKeysFor` is
  // the same rule the renderer draws with rather than a second copy of it here. No key means
  // no question: a jira run whose telemetry predates #131 records no keys, and this then
  // spawns nothing at all.
  const keys = taskKeysFor({ metricsText, record })
  if (keys.length === 0) return {}
  try {
    const titles = await jiraTitles({ keys, exec })
    // A seam that answered with something other than a map is no map: the pure module tolerates
    // one (it has to — this is a subprocess's output), but handing it a string would be this
    // side passing a shape along rather than reading it.
...
// Default jira title resolver (#132), handed the spawner this command already injects — the
// same arrangement as `defaultJiraQueueCount` below and for the same reason: NO TEST IN THIS
// SUITE CAN REACH A REAL `acli`, because the only spawner it can reach is the one the deps bag
// carried in. ONE call for every key, however many rows the table has; see `titlesFor`.
async function defaultJiraTitles({ keys, exec }) {
  return titlesFor(keys, { exec })
}
...
// Default jira-queue counter (#126), handed the spawner this command already injects, so no
// test in this suite can reach a real `acli`.
//
// WHERE `unknown` COMES FROM for jira, and why this reads the DISCRIMINATED result rather
// than the plain count: `queueCount` answers 0 for every failure it can have — no acli on
// PATH, a logged-out session, a flag it does not know, an unconfigured JIRA_JQL, output
// nobody can parse — and by that contract 0 is ALSO a real, empty queue. A scheduler wants
// exactly that conflation; a view must not have it, because `0 waiting` is a claim about the
// Jira board and reads as "almost done" when the truth is that nobody could reach it. So the
// probe is asked for its provenance, `ok:false` becomes the `null` this file renders as
// `unknown`, and a count acli genuinely reports as 0 still reads as 0.
async function defaultJiraQueueCount({ jql, exec }) {
  const result = await jiraQueueResult(jql, { exec })
