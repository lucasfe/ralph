...
//
// Returns `{root, record, mode, session, tmuxAlive, queue, now, progress}`. Reads only,
// like everything else here. The metrics TEXT is deliberately not in that list: it is
// read below and consumed below, and handing it back would invite a caller to build a
// second snapshot out of it — the one thing one gatherer exists to prevent.
export async function collectStatus({
  ...
  const nowMs = now()
  const progress = buildProgress({ metricsText, record: live ? record : null, queue, now: nowMs })

  return { root, record, mode, session, tmuxAlive, queue, now: nowMs, progress }
}
...
export async function statusCommand({
  ...
  const { record, mode, session, queue, tmuxAlive, now: nowMs, progress } = await collect(gather)
