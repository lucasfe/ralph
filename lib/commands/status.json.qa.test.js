...
describe('ralph status --json — one clock, one snapshot, no drift (#58 QA)', () => {
  ...
  it('does not double-count a single read, a single probe or a single count', async () => {
    ...
    expect(jsonDeps.exec.calls.map((c) => c.key)).toEqual(humanDeps.exec.calls.map((c) => c.key))
...
describe('ralph status --json — #58’s document, unchanged by #59 (#59 QA)', () => {
  ...
  it('builds exactly one post-mortem snapshot, from the one clock reading', () => {
    ...
    // shell must build it rather than let `renderStatus`'s default do it, which would
    // build a card with no metrics behind it.
    const SOURCE = readFileSync(new URL('./status.js', import.meta.url), 'utf8')
    const CODE = SOURCE.split('\n')
      .map((line) => (line.trimStart().startsWith('//') ? '' : line.replace(/\s\/\/.*$/, '')))
      .join('\n')
    const start = CODE.indexOf('export async function statusCommand(')
    const body = CODE.slice(start, CODE.indexOf('\n}\n', start))
    expect(body.match(/buildPostMortem\(/g), 'one post-mortem snapshot').toHaveLength(1)
    expect(body.match(/\bnow\(\)/g), 'one clock reading').toHaveLength(1)
    // Rendering is the module's job, not the shell's: a `renderPostMortem(` in the
    // shell would be a second card assembled beside the one `renderStatus` returns.
    expect(body).not.toMatch(/renderPostMortem\(/)
  })
})
