...
describe('ralph status --json — finish_at is null only when there is no ETA (#58 QA round 2)', () => {
  ...
  it('types a hostile started_at out of run-state.json into a jq instant or nothing', async () => {
    ...
    for (const [started_at, expected] of cases) {
      ...
      // `task_key` (#132) and `worktree` (#222) are null throughout because these records name
      // their task by number and predate the worktree field; they are listed so a hostile
      // `started_at` is shown not to disturb the leaves beside it.
      expect(doc.tasks.current, String(started_at)).toEqual({
        number: 31,
        started_at: expected,
        task_key: null,
        worktree: null,
  ...
  it('publishes the same key paths, with the same types, in all four modes', () => {
    ...
    const expected = {
      running: [
        ...
        // #132's leaf. Null in every mode this file drives, because these fixtures are github
        // runs — and that is the half of the contract worth pinning here: a key was ADDED to
        // the document, and a consumer of a run that has no Jira ticket still reads the key
        // rather than losing it, which is what makes `jq .tasks.current.task_key` safe to write.
        'tasks.current.task_key: null',
        // #222's leaf, appended after it and null for the same reason: these fixtures'
        // records were written before the field existed, and `jq .tasks.current.worktree`
        // still reads a key rather than nothing.
        'tasks.current.worktree: null',
        'pace.basis: string',
    ...
    const nulledForADeadRun = {
      // A dead run has no task in flight, so every leaf under `tasks.current` collapses
      // into the one null key that replaces them.
      'tasks.current.number: number': 'tasks.current: null',
      'tasks.current.started_at: string': null,
      'tasks.current.task_key: null': null,
      'tasks.current.worktree: null': null,
