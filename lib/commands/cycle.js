...
export async function cycleCommand({
  ...
  stdin = process.stdin,
  // #52: TTY-gated, derived from the RESOLVED `stdin` above exactly as ./start.js:122
  ...
  // #200: how this copy was installed — the channel the check asks and the command
  // the notice names. No default here so an undefined value reaches runUpdateGate's
  // OWN default (`classifyInstall`), keeping one source of truth for it.
  classify,
  ...
  try {
    ...
    // most one version query and one question a week, out of the same budget
    ...
    //
    // Unwrapped for the reasons at ./start.js:772-777, plus one that is only true
    ...
    if (updateGate.installed) {
      ...
      // an update (./start.js:795-802) because THIS process holds pre-update module
      ...
      // as ./start.js:803-813, and the gate is `installed` NEVER `to` — updateCommand
...
async function runPreflight({
  ...
  //            --state all --label in-progress` (orphan-cleanup.js:39) and, per orphan
  //            found, `gh issue edit N --remove-label in-progress`
