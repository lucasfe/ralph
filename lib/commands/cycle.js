...
export async function cycleCommand({
  ...
  try {
    ...
    // .ralph/tasks, jira runs the configured JQL through acli (#126). Why a jira run still
    // needs an authenticated `gh` despite counting elsewhere is recorded once, at
    // `runPreflight`; why an unprovable count reads as 0 here and as `unknown` in `ralph
    // status` is recorded once, in jira-queue.js.
    const queueCount =
      source === 'folder'
...
async function runPreflight({ exec, exists, readFile, root, claudeCredentialsPath, processEnv = {}, source = 'github' }) {
  ...
  // lands its work as a GitHub PR. So `jira` — registered by #125 and resolving no
  // ticket yet — IS a github run for now, and needs github's preflight.
  //
  // #126 KEPT THIS AS IT IS, ON PURPOSE. That slice moved the jira QUEUE COUNT off
  // `gh issue list` and onto acli, so a jira run no longer reads GitHub to decide
  // whether there is work — but it still branches, pushes and opens a PR there, and
  // nothing yet resolves a Jira ticket into a task. Requiring gh here is therefore
  // still the honest gate, and it is a DECISION rather than an oversight: the slice
  // that lands ticket selection is the one that gets to revisit it. Loosening it
  // early would move the failure from the door to the middle of a run.
  //
