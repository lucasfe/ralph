...
export async function cycleCommand({
  ...
  stdin = process.stdin,
  // #52: TTY-gated, derived from the RESOLVED `stdin` above exactly as ./start.js:134
  ...
  try {
    ...
    //
    // Unwrapped for the reasons at ./start.js:808-813, plus one that is only true
    ...
    if (updateGate.installed) {
      ...
      // an update (./start.js:831-838) because THIS process holds pre-update module
      ...
      // as ./start.js:839-849, and the gate is `installed` NEVER `to` — updateCommand
