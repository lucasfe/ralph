...
export async function cycleCommand({
  ...
  stdin = process.stdin,
  // #52: TTY-gated, derived from the RESOLVED `stdin` above exactly as ./start.js:113
  ...
  try {
    ...
    //
    // Unwrapped for the reasons at ./start.js:659-664, plus one that is only true
    ...
    if (updateGate.installed) {
      ...
      // an update (./start.js:681-688) because THIS process holds pre-update module
      ...
      // as ./start.js:689-700, and the gate is `installed` NEVER `to` — updateCommand
