...
// #215: the cached version is filed under the channel that resolved it, and a run only reads its
// OWN channel's number. Every run in this file resolves through the npm query (its `exec` stub
// answers `npm view`, and no run is placed in a Cellar), so the stamp is spread in FIRST as a
// default: a seed that named no channel would be a number this command may not read, which turns
// every "the window was already closed" case below into a vacuous pass. A test that wants an
// unattributed file still says so by passing the field explicitly.
...
describe('QA #26 the stamp is written BEFORE the answer is awaited', () => {
  ...
  it('keeps the window burned when ask REJECTS (the Ctrl-C at the prompt)', async () => {
    ...
    expect(cacheOf(cacheFs)).toEqual({
      ...
      // #215: the check that produced 0.2.0 files it under the channel it asked. Spelled out
      // rather than loosened, because these shapes are this file's way of saying WHICH fields a
      // path touches — and the attribution is a field the prompt window must never move.
...
describe('QA #26 the two windows crossing, through the real decision and stamp', () => {
  it('fresh check + ancient prompt: no query, one question, check stamp untouched', async () => {
    ...
    expect(cacheOf(cacheFs)).toEqual({
      ...
      last_prompted_at: iso(T0),
      latest_version: '0.2.0',
      // #215: carried through from the seed — a throttled run neither re-asks the channel nor
      // re-attributes what it reads.
      latest_version_channel: 'npm',
    })
  })
  ...
  it('ancient check + fresh prompt: one query, no question, prompt stamp preserved to the byte', async () => {
    ...
    expect(JSON.parse(rawOf(cacheFs))).toEqual({
      ...
      last_prompted_at: promptedAt,
      latest_version: '0.2.0',
      // #215: the refreshed number is re-filed under the channel that answered THIS query, which
      // is the npm one the exec stub served.
      latest_version_channel: 'npm',
    })
  })
  ...
  it('both windows open: the stamp is built on the refreshed cache, not a stale snapshot', async () => {
    ...
    expect(JSON.parse(writes[1].data)).toEqual({
      ...
      last_prompted_at: iso(T0),
      latest_version: '0.2.0',
      // #215: and the stamp re-reads the attribution along with the version — the second write
      // is the one that could have dropped it, which is why the field is enumerated here too.
      latest_version_channel: 'npm',
    })
  })
  ...
  it('an offline run stamps the prompt it showed and keeps the cached version', async () => {
    ...
    expect(cacheOf(cacheFs)).toEqual({
      ...
      last_prompted_at: iso(T0),
      latest_version: '0.2.0',
      // #215: the offline query resolved nothing, so the SEEDED pair survives — including its
      // channel, which is what keeps the version readable at all on the next run.
      latest_version_channel: 'npm',
    })
  })
...
describe('QA #26 back-to-back runs over one shared cache', () => {
  ...
  it('a #24-era cache with no last_prompted_at key at all prompts, then gains the field', async () => {
    ...
    //
    // #215 MOVED THE CHECK WINDOW IN THIS FIXTURE, from a day ago to a month, and the reason is
    // that the old seed had stopped being able to observe the claim. A #24-era file also names no
    // CHANNEL, and an unattributed version is served to nobody — so inside the check window that
    // file now produces no notice and no question, whatever `last_prompted_at` says, and an
    // `asks: 1` would have been impossible to reach for a reason that has nothing to do with the
    // prompt window. With the check window open, the run resolves 0.2.0 from its own npm query
    // and the MISSING STAMP is once again the only thing deciding whether it asks — which is
    // what this test is about. (That a legacy file is served to no channel is pinned where it
    // belongs, in lib/update-check.cache-channel.test.js and doctor.cached-channel.test.js.)
    ...
    expect(Object.keys(JSON.parse(rawOf(cacheFs))).sort()).toEqual([
      ...
      'last_prompted_at',
      'latest_version',
      // #215: a fourth key, and the upgrade path's claim is unchanged — a file written by an
      // older Ralph gains the fields it lacks rather than being rejected.
  ...
  for (const [label, raw] of hostile) {
    it(`${label} → one question, and the file is repaired`, async () => {
      ...
      expect(cacheOf(cacheFs)).toEqual({
        ...
        last_prompted_at: iso(T0),
        latest_version: '0.2.0',
        // #215: a repaired file is repaired into the CURRENT shape, attribution included — the
        // hostile inputs above are all unreadable, so the number here is this run's own query's.
        latest_version_channel: 'npm',
      })
    })
  }
