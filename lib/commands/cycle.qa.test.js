...
describe('QA cycle — the acli spawn a jira run actually makes (#126)', () => {
  ...
  it('never asks gh for a count and never touches the folder seam in jira mode', async () => {
    ...
    // falls back to the repo path), `gh issue list --state all --label claude-working`, and
    // then per orphan `gh issue edit N --remove-label claude-working` — the last a WRITE to
