...
describe('QA cycle — the acli spawn a jira run actually makes (#126)', () => {
  ...
  it('never asks gh for a count and never touches the folder seam in jira mode', async () => {
    ...
    // ...and `gh auth status` still ran. #126 moved the COUNT to Jira and #127 the ticket
    // SELECTION, and neither moved this gate — pinned as the shape of that debt (the
    // argument is at cycle.js's `runPreflight`), not as behaviour worth keeping.
