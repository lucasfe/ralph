...
export async function startCommand({
  ...
  const configText = safeReadConfig(resolve(cwd, 'ralph.config.sh'), { exists, readFile })
  ...
  // #62 owns the digest itself — where it runs and what it prints. This line claims
  // only what THIS command can see: the interval the config asks for, and that the
  // digest keeps the loop company. Naming a tmux window here would be a promise
  // about a layout #60 cannot check, and a wrong one the day #62 chooses another.
  // Until #62 lands, no repo configures the variable and the line never prints.
  if (digest) out(`   Digest:         every ${digest} — runs alongside the loop`)
...
// ralph.config.sh as TEXT, under readConfigSource's own never-throws guard: a
// missing or unreadable config leaves every setting at its default rather than
// aborting a launch. Read here rather than through readConfigSource — which reads
// AND parses one variable — because this command needs two of them out of one read.
function safeReadConfig(path, { exists, readFile }) {
  try {
...
// The digest's interval, '' when the digest is off (#60). Zeroing a knob is how a
// shell config turns it off, and an interval of zero is not an interval — so ANY
// spelling of zero reads as off, not just a bare `0`. `RALPH_DIGEST_INTERVAL=0m` is
// the likelier way to write it, since the live value carries a unit. A leading-number
// parse is the whole test, because the value is an interval and the unit follows
// the number.
function digestInterval(configText) {
  const raw = parseConfigVar(configText, 'RALPH_DIGEST_INTERVAL')
