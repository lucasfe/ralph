...
export async function startCommand({
  ...
  // line of Ralph's own diagnostics.
  ...
  // model, context window, task source and repo are more entries in this one object —
  // read on this impure side, from the config text and the cache, and handed to a module
  // that still touches nothing. #70's `whatsNew` is the second fact to arrive this way,
  // and the pattern it followed is the one to copy: a total helper at the bottom of this
  // file, an injected reader, and no way for the read to cost the run. #74 removed the one
  // obstacle that note warned about: `configText` is now resolved at the top of this
  // function, so #69's facts are a `parseConfigVar` away rather than a reordering away.
  //
  ...
  //
  // ONE read of the file, THREE values out of it (#60, #74): the source, the digest
  // interval the hint block below needs, and the banner mode — which is why the read
  // itself now happens at the top of this function, above the picture it decides.
  // Reading the file twice would let a config rewritten mid-preflight answer the three
  // questions differently, and the parse is over text either way — see
  // parse-config-var.js for the shared grammar.
