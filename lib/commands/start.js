...
import { RALPH_HOME, defaultRalphBinary, templatePath } from '../paths.js'
...
import { cachedVersionFor, readVersionCache } from '../version-cache.js'
// #215: which channel's published versions this copy is compared against. The cache is one
// global file and a machine can carry two copies of Ralph from two channels, so the box's
// `update` row reads the number ITS OWN channel resolved and never the other copy's — the same
// rule, out of the same two functions, that `ralph doctor`'s `cached` row reads.
//
// THE PURE HALF of install recognition (`node:path` and nothing else) rather than
// lib/install-target.js, which imports a process runner at module scope: this line runs before
// the first preflight line of every `ralph start`, so it may not cost a spawn. The `classify`
// seam this command already has is the update GATE's — a promise resolved several hundred lines
// below, long after the box is on screen. #201's WORDING function is deliberately not imported:
// this command grows no channel row, only a channel-correct comparison.
import { versionChannelFor } from '../install-markers.js'
...
export async function startCommand({
  ...
  // #200: how this copy was installed, which decides the channel the update check
  // asks and the command its notice names. No default here so an undefined value
  // reaches runUpdateGate's OWN default (`classifyInstall`) — one source of truth, the
  // same convention as `cacheFs` below, and the one lib/update-gate.js:52-55 states.
  classify,
  ...
  // round trip to `ralph start`'s first paint. The version check is #24's, it happens
  ...
  // #215: where this copy of Ralph is installed — the one input that decides WHICH CHANNEL's
  // version the box may report. Defaulted here, unlike doctor's `ralphHome`, and the difference
  // is doctor's import graph rather than a difference of opinion: this file already imports
  // lib/paths.js, so the honest answer is available without a new dependency and a
  // `ralph start` never has to be told where it lives. Injectable for #41's reason — the
  // default is impure, and a test that wants a Cellar or a store layout cannot express it as
  // anything but this path. No filesystem seam beside it: `versionChannelFor` takes no probe
  // (its own comment argues why), so this costs the startup path not one syscall.
  ...
  const boxLines = banner.box
    ? composeBanner({
        facts: {
          ...
          // #215: the cached number for THIS COPY'S CHANNEL. Two copies of Ralph installed
          // from two channels share one cache file, and the tap and the registry hold
          // different versions on purpose (#196) — so a row offering the other copy's number
          // would send this user to their own channel for a release it does not have.
          //
          // Still NOT a `channel` fact: this box grows no #201 row, which is doctor's. The
          // channel is an argument to a cache read, spelled `versionChannel` so that reading
          // this file cannot confuse the two (lib/commands/doctor.install-channel.qa.test.js
          // sweeps this source for exactly that confusion).
          latestVersion: cachedLatestVersion({
            readCache,
            fs: cacheFs,
            processEnv,
            home,
            versionChannel: versionChannelFor({ ralphHome }),
          }),
          cwd,
  ...
  if (hasCommand('tmux')) {
    ...
    if (has.exitCode === 0) {
      ...
      // the session above was derived from this command's own cwd (:720) and `ralph live`
  ...
  // its own cwd (:720) and `ralph live` derives it from the git toplevel
...
// have been through it. #215 moved that check inside `cachedVersionFor`, which applies the
// same rule to both fields and returns a normalized string or null — so an injected reader's
// hand-built cache is still type-checked, by one function shared with `ralph doctor` instead of
// by a second copy of the test here.
//
...
//
// #215: `versionChannel` is which channel's number this box may report — derived from this copy's
// own install path by the caller. A number the other copy's channel resolved reads as no number
// at all, which draws no row: `ralph start`'s box only ever speaks up about an update the user can
// actually install with the command this same box would name. Spelled the same as in doctor's
// twin, and not `channel`, for the reason that helper's comment gives.
function cachedLatestVersion({ readCache, fs, processEnv, home, versionChannel }) {
  if (isUpdateCheckDisabled(processEnv)) return null
  try {
    return cachedVersionFor(readCache({ fs, processEnv, home }), versionChannel)
  } catch {
    return null
  }
}
