...
export async function startCommand({
  ...
  if (updateDecision?.isNewer) {
    ...
    //
    // Gated on `isNewer && isTTY`, deliberately NOT on updateDecision.shouldPrompt:
    // that field is hardcoded false until its last_prompted_at throttle lands in
    // #26. RALPH_NO_UPDATE_CHECK needs no handling here — resolveUpdateDecision
    // already returns isNewer:false for it, so the opt-out suppresses the prompt
    // together with the check.
    const accepted = isTTY && (await ask('Update now? [y/N]: ', { input: stdin, output: stdout }))
    if (accepted) {
