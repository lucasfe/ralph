...
export async function startCommand({
  ...
  for (const line of renderStaticBanner({ isTTY: stdoutIsTTY, color, width: columns })) out(line)
  ...
  // file, an injected reader, and no way for the read to cost the run. Note what that
  // costs #69: `source` and `digest` below are parsed AFTER this point, so a slice that
  // wants them in the box must move the config read above it, not the box below them.
  //
