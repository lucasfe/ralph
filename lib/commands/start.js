...
export async function startCommand({
  ...
  const configSourceRaw = readConfigSource(resolve(cwd, 'ralph.config.sh'), { exists, readFile })
  ...
  // 2.5. #24: weekly, cache-backed update notice. It sits HERE, and not where
  // the old step-8.5 block did, for two reasons: 8.5 ran after the empty-queue
  // early return, so a user with an empty queue never saw the notice; and
  // running before the gh label/orphan/queue calls means #25's accept path
  // won't discard work already done.
  //
  ...
  //
  // resolveUpdateDecision never throws and prints nothing itself, so a failed or
  // throttled check is silent.
  const updateDecision = await update({
    currentVersion,
    exec,
    now,
    processEnv,
  ...
  if (updateDecision?.isNewer) {
    out(
      pc.yellow(
        `New version available: ${updateDecision.latestVersion} (run npm i -g @lucasfe/ralph to update)`,
      ),
    )

    // #25: on an interactive terminal the notice becomes a question. The notice
    // above is printed FIRST and unchanged on every isNewer run, TTY or not —
    // it is #24's tested contract (one line, naming the manual upgrade command)
    // and it stays the useful answer for a user who declines. The question is an
    // addition after it, never a replacement.
    //
    // #26: the gate is `shouldPrompt && isTTY`. What shouldPrompt means — the
    // prompt window, why it is independent of #24's network one, and why
    // RALPH_NO_UPDATE_CHECK needs no handling at this site — is documented once,
    // on resolveUpdateDecision in ../update-check.js, which owns the rule. What
    // matters HERE: only the QUESTION is throttled. The notice above deliberately
    // is not, and keeps printing on every isNewer run.
    //
    // The stamp lands here, and BEFORE the answer is awaited, because this line is
    ...
    let accepted = false
    if (updateDecision.shouldPrompt && isTTY) {
      recordPrompt({ now, processEnv, home, fs: cacheFs })
      accepted = await ask('Update now? [y/N]: ', { input: stdin, output: stdout })
    }
    if (accepted) {
      ...
      if (installed.ok) {
        ...
        out(
          pc.green(
            `✅ Updated to ${installed.to ?? updateDecision.latestVersion} — run \`ralph start\` again.`,
          ),
        )
        return { exitCode: 0, started: false }
      }
      // Not updated — a failed install, or nothing to update here (an npx run,
      // a linked dev checkout). Either way the loop still launches on the
      // current version: an update is never worth losing a run over.
      // runUpdate wrote its own diagnostics, so this adds one neutral line and
      // falls through.
...
// #25: run the update and answer the one question step 2.5 needs — "is a NEW
// version installed now?". Gated on `updated`, NEVER on `to`: updateCommand's
// `to` names the version that is out there, so it is set even when the install
// failed or was refused (npx run, linked dev checkout), and gating on it would
// announce an update that never happened. Every throw is swallowed for the same
// reason the non-zero exit is tolerated — updateCommand has already written its
// own diagnostics, and an update is never worth losing a run over.
