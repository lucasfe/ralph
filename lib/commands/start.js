...
export async function startCommand({
  ...
  //
  // PRESENT OR ABSENT, not truthy or falsy, and the difference is a real run. A `||` here read the
  // environment whenever the file's value was empty — but a config that assigns the knob an empty
  // value MASKS the environment in the shell that sources it, so `RALPH_AGENT=""` meant the loop
  // would resolve claude with nothing to report while this line warned about the environment's
  // `codx`: a diagnostic about text no run will read, which is the exact trade the paragraph above
  // rules out. It cost the box the same way, a row reading `codex` over a loop running claude,
  // which is #69's confusion restored. That `||` is gone; `configAssignsVar` is the presence
  // question it could not ask, and lib/parse-config-var.js carries the measurement that settles it.
  const configuredAgent = configAssignsVar(configText, 'RALPH_AGENT')
    ? parseConfigVar(configText, 'RALPH_AGENT')
    : null
  const { agent, warning: agentWarning } = resolveAgent({
    // `??`, so a present-but-blank config value keeps the environment out. An absent one is the
    // only case that falls through, which is the only case bash falls through on.
    RALPH_AGENT: configuredAgent ?? processEnv.RALPH_AGENT,
  })
  ...
  //
  // #122: ...AND THE SHAPE TWO OF THE BOX'S KNOBS SHARE IS STATED ONCE, HERE, instead of being
  // spelled out as a `parseConfigVar(configText, NAME) || processEnv[NAME]` pair at both sites.
  // Read it as that shape and no more, not as the loop's rule restated in code: `||` is not
  // `set -a`. The paragraph above holds for a config value that is present and NON-EMPTY, and
  // diverges for a blank one, which is what the divergence note below is about.
  //
  // What the name buys is not length — it is that a knob which DEPARTS from the shape is visible
  // BY NOT CALLING IT. Three of the box's knobs depart, each argued where it is read:
  //
  //   RALPH_BANNER   environment over config, the deliberate inversion — `resolveBannerMode`
  //                  above, and lib/banner-mode.js.
  //   RALPH_AGENT    present or absent (#118): `configAssignsVar` and `??`, never `||`, because a
  //                  config that assigns the knob an EMPTY value MASKS the environment in the
  //                  shell that sources it — see `configuredAgent` above.
  //   GH_REPO        the same presence shape for the same reason (#120) — `bannerRepoSlug`, at
  //                  the bottom of this file.
  //
  // TASK_SOURCE is not a fourth: same `||`, same divergence, and it keeps a line of its own
  // because what differs is the READER — `parseConfigSource` knows the file's own spellings of
  // that knob, which is not a `parseConfigVar` question. See its site below.
  //
  // AND THIS IS NOT AN INVENTORY OF THE FUNCTION'S CONFIG READS. Three more belong to the digest
  // rather than to the box and are not counted above: `digestInterval(configText)` at the
  // preflight, and `RALPH_AGENT` and `RALPH_DIGEST_MODEL` in the digest window's launch near the
  // bottom — those two CONFIG-ONLY, with no environment fallback at all, which is a third
  // precedence again. Whether they should share one is a wider question than this closure's.
  //
  // WHERE THE `||` DIVERGES FROM THE LOOP: a config file can ASSIGN a knob a BLANK value and
  // `set -a` exports the blank rather than skipping it — measured against a real bash with an
  // inherited value in place, five of the six spellings a file can blank with (`=""`, `=''`, a
  // bare `=`, unquoted trailing spaces, and an `export` of any of them) leave the loop holding an
  // empty string, and `||` reaches past all five to the environment. So a repo that blanks
  // RALPH_CODEX_MODEL while the invoking shell exports one gets a box naming a model
  // `buildAgentInvocation` will not pass; a blanked RALPH_CONTEXT_WINDOW gets `200k tokens` over
  // a run whose first event records the window it was actually given.
  //
  // PINNED, NOT ENDORSED: lib/commands/start.precedence.qa.test.js drives a blanked file through
  // this command and asserts TODAY'S answer, each test saying what its expectation becomes when
  // the answer changes. The fix is `configAssignsVar` and `??`, the shape #118 and #120 already
  // landed, and it is #149's rather than #122's: moving knobs onto a different precedence is a
  // behaviour change and gets its own review instead of riding along inside a refactor.
  //
  // (The sketch above writes the knob as an UNQUOTED `NAME` in brackets on purpose. #41's
  // ambient-surface sweep is a regex over these sources and does not skip comments, so a dotted
  // SCREAMING_CASE placeholder in prose reads to it as a real environment read: spelling the
  // sketch the other way round put a variable nothing neutralizes onto the suite's env surface
  // and reddened the idempotence spec.)
  const configOverEnv = (name) => parseConfigVar(configText, name) || processEnv[name]
  ...
  //
  // AND IT DOES NOT CALL `configOverEnv` — deliberately, on the reader rather than the precedence:
  // see the TASK_SOURCE line in the note above, and the divergence paragraph under it, which is
  // about this `||` too.
  ...
  const identity = resolveBannerModel({
    metricsText,
    agent,
    // Both knobs the same shape — `parseConfigVar` of the file, then `||` to the environment —
    // spelled by the closure above rather than twice more here (#122). The blank-value divergence
    // that `||` carries is recorded at the closure; it is not restated at each site.
    configuredModel: configOverEnv('RALPH_CODEX_MODEL'),
    ...
    // had.
    configuredWindow: configOverEnv('RALPH_CONTEXT_WINDOW'),
  })
  ...
  //
  // ON THE `RALPH_AGENT` SHAPE, present-or-absent, and deliberately NOT the `||` the two knobs
  // above use: the note at `resolveAgent` further up carries that argument in full and #118
  // measured it. A config that assigns the knob an EMPTY value MASKS the environment in the
  // shell that sources it, so `GH_REPO=""` means the loop's `gh` sees an empty variable, reads
  // it as unset, and resolves its base repository from origin — and `resolveBannerRepo` already
  // treats a blank `ghRepo` as unset and falls through to origin's url. So handing the blank
  // straight through names exactly the repository the loop will honour, where a `||` would reach
  // past it into the environment and name one no `gh` call in the run is about to touch.
  //
  ...
  // no repository — and a knob that has since grown a third value (`jira`, resolving
  // no ticket yet, so still reading GitHub issues through the loop) would otherwise
  ...
  const repo =
    banner.box && source !== 'folder'
      ? bannerRepoSlug({ readFile, cwd, configText, processEnv })
      : null
...
// there, next to the `??` that implements it; what belongs here is only that the CALLER decides
// this, so the grammar below still sees one string and knows nothing about where it came from.
//
...
function bannerRepoSlug({ readFile, cwd, configText, processEnv }) {
  try {
    // The file's answer when it HAS one, the empty string included — `configAssignsVar` is the
    // presence question `parseConfigVar`'s '' cannot ask, and lib/parse-config-var.js carries the
    // bash transcript that settles which of the two answers matters.
    const configured = configAssignsVar(configText, 'GH_REPO')
      ? parseConfigVar(configText, 'GH_REPO')
      : null
    return resolveBannerRepo({
      // `??`, so a present-but-blank config value keeps the environment out. An absent assignment
      // is the only case that falls through, which is the only case bash falls through on.
      ghRepo: configured ?? processEnv?.GH_REPO,
