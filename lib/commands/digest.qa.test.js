...
describe('QA: ralph digest — what the --help text has to tell a reader (#61)', () => {
  ...
  it('declares only the loop options, so there is no third way to run it', async () => {
    // #61's AC#1 was "runs once" and that is still what plain `ralph digest` does;
    // #62 added exactly one alternative — a timer — as --loop plus the --interval it
    // needs. Anything else appearing here would be a mode with no acceptance criteria
    // of its own.
    const { stdout } = await cli('digest', '--help')
    ...
    const declared = options
      ...
      .filter(Boolean)
      .map((m) => m[1])
    expect(declared).toEqual(['--loop', '--interval', '-h'])
  })
