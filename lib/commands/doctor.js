...
// #215: `cachedVersionFor` is the read rule for the channel-keyed cache — one function, so the
// box's `cached` row and `ralph start`'s banner cannot come to disagree about whose number they
// are allowed to report. It reaches nothing new: version-cache.js is already on this graph.
...
// out at install-markers.js:374). The wording lives over there, on the same table row as the
// marker that earned it — this file decides only that the row is drawn and with which seams.
//
// #215: `versionChannelFor` is the same path matching asked a narrower question — WHICH
// CHANNEL'S published versions this copy should be compared against, off the same table rows and
// the same `ralphHome`, so the `cached` row and the `channel` row cannot disagree about what this
// install is. No new import and NO NEW SYSCALL: that function takes no filesystem probe at all,
// which is what keeps this command's counted syscall budget exactly where #201 left it.
import { describeInstallChannel, versionChannelFor } from '../install-markers.js'
  ...
  if (banner.box) {
    for (const line of composeBanner({
      facts: {
        ...
        // lose a verdict.
        //
        // #215: ...FOR THIS COPY'S CHANNEL, which is the fix rather than a refinement. The
        // cache is one global file and a machine can carry two copies of Ralph installed from
        // two channels; before this, whichever copy last ran the weekly check owned the row,
        // so a Homebrew copy reported `cached 0.23.0` — npm's last accepted version, a number
        // the tap cannot produce. Derived from the same `ralphHome` the `channel` row below is
        // derived from, so the two rows are one answer about one install — and it costs this
        // command NOTHING it was not already paying: `versionChannelFor` takes no probe of its
        // own (its own comment argues why), so the syscall count two suites pin here is the
        // same count as before this line existed.
        cachedLatest: cachedLatestVersion({
          readCache,
          fs: cacheFs,
          processEnv: env,
          home,
...
// "unknown" on its own.
//
// #215: and it is the last-known latest version FOR ONE CHANNEL — `versionChannel`, derived from
// this copy's own install path by the caller. The cache is global across repos on purpose (#24)
// and was global across CHANNELS by accident, which is the bug this parameter closes: a number
// resolved by the npm copy on a machine that also has a Homebrew one is not an answer about the
// Homebrew one, and reporting it under a `channel Homebrew` row — as this command did — is the
// most misleading line a version diagnostic can print. A number this channel cannot claim reads
// as "nobody has checked yet", which is a state the row has drawn since #75 and which the
// `channel` row directly under it explains.
//
// Named `versionChannel` rather than `channel`, here and in `ralph start`'s twin, so the two
// helpers stay spelled identically: over there the bare word is claimed by #201's ROW, which
// that command deliberately does not draw, and a source sweep in
// lib/commands/doctor.install-channel.qa.test.js reads it as one.
function cachedLatestVersion({ readCache, fs, processEnv, home, versionChannel }) {
  let latest
  try {
    // The call AND the property reads both sit inside the guard, so each field is
    // read exactly once, by `cachedVersionFor`. The load-bearing reason is the TypeError
    ...
    // bound by no such guarantee — reading a property twice out here would let a
    // stub answer isValidSemver and .trim() differently.
    latest = cachedVersionFor(readCache({ fs, processEnv, home }), versionChannel)
  } catch {
