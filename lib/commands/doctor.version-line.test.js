...
//
// #215: which now includes the channel that resolved the number, because doctor reads the
// version FOR a channel and an unattributed number belongs to none. These runs pass no
// `ralphHome`, which doctor reads as the npm channel, so `npm` is what keeps this cache warm.
// Nothing in this file is about the key — it is about the four states of the `cached` row — and
// lib/commands/doctor.cached-channel.test.js owns the key itself.
