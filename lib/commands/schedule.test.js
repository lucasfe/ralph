...
const baseDeps = (overrides = {}) => {
  ...
  return {
    ...
    // Default to a global config that supplies no creds so the notification
    // warning is deterministic and never reads the real machine's ~/.config.
    loadEnv: () => ({}),
    ...overrides,
  }
}
...
describe('scheduleInstallCommand — pre-flight', () => {
  ...
  it('warns but proceeds when no creds are in .env.local, the environment, or the global config', async () => {
    let installCalled = false
    const deps = baseDeps({
    ...
    expect(deps.stdout.output()).toMatch(/notifications will be skipped/i)
  })

  // #4: the "notifications will be skipped" warning must account for global
  // creds — it is misleading to warn when ~/.config/ralph/.env supplies them.
  it('does NOT warn when the global config supplies creds even though .env.local is missing', async () => {
    ...
    const deps = baseDeps({
      ...
      installAgent: async () => ({ plistPath: PLIST_PATH, label: LABEL, loadResult: { exitCode: 0 } }),
    })
    const result = await scheduleInstallCommand(deps)
    expect(result.exitCode).toBe(0)
    expect(deps.stdout.output()).not.toMatch(/notifications will be skipped/i)
  })

  // #4 QA: partial creds are still useless — one key without the other cannot
  // send a notification, so the warning must still fire.
  it('warns when only one of CALLMEBOT_KEY / WHATSAPP_PHONE resolves', async () => {
    const deps = baseDeps({
      exists: (p) => !p.endsWith('.env.local') && !p.endsWith('.plist'),
      processEnv: { CALLMEBOT_KEY: 'pk' },
      loadEnv: () => ({}),
      installAgent: async () => ({ plistPath: PLIST_PATH, label: LABEL, loadResult: { exitCode: 0 } }),
    })
    const result = await scheduleInstallCommand(deps)
  ...
  // #4 QA: repo .env.local creds also silence the warning (highest precedence).
  it('does NOT warn when .env.local supplies the creds', async () => {
    const deps = baseDeps({
      exists: (p) => !p.endsWith('.plist'), // .env.local present, no plist yet
      processEnv: {},
      loadEnv: (path) =>
        path.endsWith('.env.local') ? { CALLMEBOT_KEY: 'rk', WHATSAPP_PHONE: '+repo' } : {},
      installAgent: async () => ({ plistPath: PLIST_PATH, label: LABEL, loadResult: { exitCode: 0 } }),
    })
    const result = await scheduleInstallCommand(deps)
    expect(result.exitCode).toBe(0)
  ...
  // #4: creds present only in the environment must also silence the warning.
  it('does NOT warn when CALLMEBOT_KEY/WHATSAPP_PHONE are set in the environment', async () => {
    const deps = baseDeps({
      exists: (p) => !p.endsWith('.env.local') && !p.endsWith('.plist'),
...
// #51: `ralph cycle` runs the update gate inside its lock, so the documented
// off-switch (RALPH_NO_UPDATE_CHECK) now has to be reachable from launchd — which
// reads no shell profile and sees only the plist's EnvironmentVariables dict.
// These two tests came from QA's #51 sweep (cycle.update-notice.qa.test.js) and
// live here because the behavior they pin is the install's, not the cycle's.
describe('scheduleInstallCommand — plist environment (#51)', () => {
  function trackInstalls() {
    const calls = []
    const installAgent = async (args) => {
      calls.push(args)
  ...
  it('a scheduled cycle can be silenced: the install forwards RALPH_NO_UPDATE_CHECK into the plist', async () => {
    const installAgent = trackInstalls()
    const deps = baseDeps({
      exists: (p) => !p.endsWith('.plist'),
      processEnv: { PATH: '/usr/local/bin:/usr/bin', RALPH_NO_UPDATE_CHECK: '1' },
      installAgent,
    })
    await scheduleInstallCommand(deps)
    const cycleCall = installAgent.calls.find((c) => c.kind === 'cycle')
    expect(cycleCall).toBeDefined()
    expect(cycleCall.environment.PATH).toBe('/usr/local/bin:/usr/bin')
    expect(cycleCall.environment.RALPH_NO_UPDATE_CHECK).toBe('1')
    // Both agents share baseEnv, and that is intended: the heartbeat is another
    // unattended launchd run, so it gets the same answer.
    const heartbeatCall = installAgent.calls.find((c) => c.kind === 'heartbeat')
    expect(heartbeatCall.environment.RALPH_NO_UPDATE_CHECK).toBe('1')
  })

  it('an install with the opt-out unset stays exactly as it is today', async () => {
    // The other half of the fix's contract: nothing extra in the plist for the
    // overwhelmingly common case, so the change cannot leak an empty variable
    // that isUpdateCheckDisabled would read as "keep checking" anyway.
    const installAgent = trackInstalls()
    const deps = baseDeps({
      exists: (p) => !p.endsWith('.plist'),
    ...
    await scheduleInstallCommand(deps)
    const cycleCall = installAgent.calls.find((c) => c.kind === 'cycle')
    expect(Object.keys(cycleCall.environment)).toEqual(['PATH'])
  })

  it('an empty RALPH_NO_UPDATE_CHECK is not forwarded either', async () => {
    // '' means "keep checking" to isUpdateCheckDisabled, so writing the key would
    // claim an opt-out the runtime does not honor.
    const installAgent = trackInstalls()
    const deps = baseDeps({
      exists: (p) => !p.endsWith('.plist'),
      processEnv: { PATH: '/usr/local/bin:/usr/bin', RALPH_NO_UPDATE_CHECK: '' },
      installAgent,
    })
    await scheduleInstallCommand(deps)
    const cycleCall = installAgent.calls.find((c) => c.kind === 'cycle')
...
describe('scheduleHeartbeatCommand', () => {
  ...
  // #3: creds resolve repo .env.local → processEnv → global config file.
  describe('global config credential resolution (#3)', () => {
    const GLOBAL_PATH = globalConfigPath({ processEnv: {}, home: HOME })

    it('picks up CALLMEBOT_KEY/WHATSAPP_PHONE from the global file when repo+processEnv lack them', async () => {
      const deps = heartbeatBaseDeps({
        processEnv: { RALPH_DAILY_SUMMARY_TIME: '09:00' },
        loadEnv: (path) =>
          path === GLOBAL_PATH ? { CALLMEBOT_KEY: 'gk', WHATSAPP_PHONE: '+global' } : {},
      })
      const result = await scheduleHeartbeatCommand(deps)
      expect(result.exitCode).toBe(0)
      expect(deps.sendWa.calls).toHaveLength(1)
      expect(deps.sendWa.calls[0].apiKey).toBe('gk')
      expect(deps.sendWa.calls[0].phone).toBe('+global')
    })

    it('repo .env.local overrides processEnv overrides global', async () => {
      const deps = heartbeatBaseDeps({
        processEnv: { CALLMEBOT_KEY: 'pk', WHATSAPP_PHONE: '+proc', RALPH_DAILY_SUMMARY_TIME: '09:00' },
        loadEnv: (path) =>
          path === GLOBAL_PATH
      ...
      const result = await scheduleHeartbeatCommand(deps)
      expect(result.exitCode).toBe(0)
      expect(deps.sendWa.calls[0].apiKey).toBe('rk')
      expect(deps.sendWa.calls[0].phone).toBe('+repo')
    })

    it('processEnv overrides global when repo lacks the key', async () => {
      const deps = heartbeatBaseDeps({
        processEnv: { CALLMEBOT_KEY: 'pk', WHATSAPP_PHONE: '+proc', RALPH_DAILY_SUMMARY_TIME: '09:00' },
        loadEnv: (path) =>
          path === GLOBAL_PATH ? { CALLMEBOT_KEY: 'gk', WHATSAPP_PHONE: '+global' } : {},
      ...
      const result = await scheduleHeartbeatCommand(deps)
      expect(result.exitCode).toBe(0)
      expect(deps.sendWa.calls[0].apiKey).toBe('pk')
      expect(deps.sendWa.calls[0].phone).toBe('+proc')
    })
  })
