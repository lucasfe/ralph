...
// #215: the cached number is keyed by the channel that resolved it, and a reader is only served
// its own channel's answer. No run in this file passes `ralphHome`, so doctor places itself with
// the npm fallback (see `installChannel`'s note in doctor.js on why a caller that named no path
// is read as npm rather than as nobody) — which makes `npm` the stamp that turns this seed into a
// warm cache. Without it every verdict below reads "no update check cached yet", which is a true
// answer to a different question than the one this file asks.
