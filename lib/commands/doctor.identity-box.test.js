...
describe('doctor identity box (#75) — RALPH_BANNER', () => {
  ...
  it('keeps the box for a value it does not recognize, and says nothing about it', async () => {
    // A typo'd knob costs a picture at worst, never a diagnostic — and doctor does not
    // print the warning `ralph start` prints, because reaching `oneLine` would give it a
    // transitive execa dependency through lib/digest.js. See doctor.js.
