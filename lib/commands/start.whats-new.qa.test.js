...
describe('QA startCommand — a reader that misbehaves costs the section, and nothing else', () => {
  ...
  it('adds only lines — one per bullet, plus the pointer — and moves nothing else', async () => {
    ...
    // the rows a reader looks for stay at the same place on the screen from run to run. #69's
    // agent and source rows are facts, so they are above it too — and the pin is the whole list
    // rather than a search for `new`, which is what makes that ordering claim real.
    expect(labelsOf(d)).toEqual(['agent', 'cwd', 'source', 'new', '', 'more'])
    const withHint = await run({
      // #215: the hint row only appears for a version filed under the channel this copy asks.
      // These runs pass no `ralphHome`, so the box places itself with the npm fallback (see
      // `installChannel`'s note in doctor.js for the same argument on the same seam) — the stamp
      // is what makes 9.9.9 THIS copy's answer and the row appear at all.
      readCache: () => ({
        ...EMPTY_VERSION_CACHE,
    ...
    expect(labelsOf(withHint.d)).toEqual(['update', 'agent', 'cwd', 'source', 'new', '', 'more'])
  })
})
...
describe('QA startCommand — the width the terminal reported, and nothing else', () => {
  it('holds the box to the stream’s columns, in every shape a stream reports them', async () => {
    ...
    for (const columns of [200, 80, 60, 44, 30, 26, 12, 5, 1, 0, undefined, Number.NaN, '80', 44.7]) {
      ...
      // thing written (the sprite draws nothing), and below 44 columns — or at one or two,
      // where even the corner would be clipped to an ellipsis — there is no `╭` left to
      // search for at all. A frame-hunting helper would return an empty list at exactly
      // those widths and every assertion under it would pass by checking nothing.
      //
      // Which is also why the HEIGHT is computed: under 44 columns #72 drops the frame,
      // and the bottom rule is part of it, so a bare banner is one line shorter (title,
      // cwd, section) than a boxed one (top rule, cwd, section, bottom rule). `ceiling` is
      // already the usable width capped at 60, and capping cannot cross 44, so it answers
      // the rung too.
      const rows = (ceiling >= 44 ? 3 : 2) + SAMPLE_SECTION_ROWS
      const box = d.stdout.lines().slice(0, rows)
      expect(box, why).toHaveLength(rows)
