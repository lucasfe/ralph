...
describe('startCommand — folder task source (#565)', () => {
  // exists() true only for ralph.config.sh so readConfigText reads it; other
...
describe('startCommand — launch projection and hints in the startup box (#60)', () => {
  ...
  // Six lines since #169: `Watch live:` names `ralph live` and the tmux command it
  // shortcuts sits on the continuation line under it. See ./start.live-hint.test.js for
  // the pair's own assertions.
  const EXISTING_TAIL = [
