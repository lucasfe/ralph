...
//   - the global cache: accepting must not re-stamp it and must leave
//     last_prompted_at null — that field belongs to #26;
...
describe('QA #25 the global cache is untouched by the prompt', () => {
  it('accepting writes exactly what #24 wrote — and leaves last_prompted_at NULL (#26 owns it)', async () => {
  ...
  it('a throttled accept leaves the cache file byte-identical (no re-stamp)', async () => {
  ...
  it('a pre-existing last_prompted_at is preserved, never cleared, by an accept', async () => {
    // Forward-compat with #26: this slice must neither write nor wipe the field.
...
describe('QA #25 the gate is isNewer && isTTY — not shouldPrompt, not the queue', () => {
  ...
  it('prompts when isNewer is true even though shouldPrompt is false (#26 owns that throttle)', async () => {
    const d = deps({
...
describe('QA #25 throttled, cache-served runs still prompt', () => {
  ...
  it('two runs sharing one home: the second is throttled but still asks', async () => {
    const cacheFs = new Volume()
  ...
  it('PINNED (#26): declining does NOT stop the very next run from asking again', async () => {
    // There is no last_prompted_at throttle yet, so a user who declines is asked
    // on every `ralph start` until they update. That is #26's job; this test is
    // the tripwire that will need updating when it lands.
    const cacheFs = new Volume()
    const runs = []
