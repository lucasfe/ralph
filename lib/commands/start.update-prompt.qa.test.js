...
// #215: a cached version is only served to the channel that resolved it, so a warm cache names
// one. Every run in this file resolves the npm query (its exec stub answers `npm view`, and no run
// is placed in a Cellar) — without the stamp the runs below would be throttled onto a number they
// may not read, and "prompts from the CACHED version" would be unreachable.
...
describe('QA #25 prompt ordering — one timeline, every side effect', () => {
  ...
  it('on DECLINE the question precedes the .env.local read, gh auth, labels, orphans, queue and launch', async () => {
    ...
    for (const marker of [
      ...
      'exec:gh auth status',
      'exec:gh label create in-progress',
      'exec:gh issue list --state open --label in-progress',
...
describe('QA #25 the accept path has no side effects', () => {
  ...
  it('never sweeps orphans, so an interrupted previous run is not reported twice', async () => {
    const d = deps({}, { orphan: '  #7 stuck\n  #8 also stuck' })
    await startCommand(d)
    expect(d.stdout.output()).not.toContain('in-progress')
...
describe('QA #25 the global cache around the prompt', () => {
  ...
  it('accepting writes #24’s fields plus #26’s prompt stamp, and nothing else', async () => {
    ...
    expect(cache).toEqual({
      ...
      // #215: plus the channel that answered the query — enumerated, because "and nothing else"
      // is the claim, and a field that appeared without being argued for is what it guards.
      latest_version_channel: 'npm',
    })
