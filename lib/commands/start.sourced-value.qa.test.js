// QA augmentation for #149 — `sourcedValue` (lib/commands/start.js:263) attacked as a MATRIX
...
// (RALPH_CODEX_MODEL); the other four call sites — RALPH_AGENT (:333), TASK_SOURCE (:463),
// RALPH_CONTEXT_WINDOW (:497) and GH_REPO (:1187, through `bannerRepoSlug`) — are each pinned on a
...
//      its sweep says what is left: an `export` prefix and a subshell tail.
//
// Two divergences are pinned rather than fixed. In the last block, three commands still read
// TASK_SOURCE on a `||` (cycle.js:190, status.js:379, doctor.js:160 — all byte-identical to the
...
// where bash makes the assignment in a subshell.
//
...
describe('QA #149 — the three commands still on a `||`, and what the difference costs', () => {
  ...
  it('PINNED DIVERGENCE: `ralph doctor` reports green where `ralph start` will abort', async () => {
    ...
    // of #149's scope and names status.js:379 and cycle.js:190 as the rest of the follow-up; what
...
describe('QA #149 review — `envPrefixedNothing`, at the boundary it draws', () => {
  ...
  it('never refuses a line bash assigns, and never leaves a blank one claiming presence', async () => {
    ...
    for (const [label, line] of FAMILY) {
      ...
      // ...and wherever the shell DID assign, the pair lands on the shell's own answer.
      if (held[N] !== INHERITED) expect(resolved, why).toBe(held[N])
    }
  ...
  it('a `#` where the blank goes no longer masks a value bash keeps either', async () => {
    ...
    // more spellings is the sweep in the test above and the 3,744-row one in parse-config-var.js:
