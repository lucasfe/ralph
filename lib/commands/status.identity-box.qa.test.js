...
describe('QA #76 — the read plan, op by op', () => {
  ...
  it('issues one queue probe per invocation, box on or off', async () => {
    for (const processEnv of [{}, { RALPH_BANNER: 'off' }]) {
      ...
      expect(exec.of('gh'), JSON.stringify(processEnv)).toHaveLength(1)
      expect(exec.of('git'), JSON.stringify(processEnv)).toHaveLength(1)
      expect(exec.of('tmux'), JSON.stringify(processEnv)).toHaveLength(1)
    }
  })
...
describe('QA #76 — no sprite, structurally rather than by discipline', () => {
  ...
  it('reached the three modules #76 added (guards against a vacuous pass)', () => {
    ...
    // this command's graph — banner-compose.js pulls update-check.js and the version cache in
    // behind it, which is why `node:os` appears in the bare set below.
