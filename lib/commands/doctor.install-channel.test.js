...
describe('doctor `channel` row — no install path, no row (#201)', () => {
  ...
  it('draws nothing for a blank or unusable install path either', async () => {
    ...
    // function UP for `cwd` — `configPathFor` is at doctor.js:518 and `installChannel` at :537,
    // which is the direction that function's own note gives at :524. A number, or a NON-EMPTY
    ...
    // lib/install-markers.qa.test.js:712-718, next to a one-element array, which DOES resolve).
