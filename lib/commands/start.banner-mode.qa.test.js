...
//
// #169 EDITED THE LITERAL, deliberately and once: the two hint blocks now name `ralph live`
// (../commands/live.js) above the tmux command they shortcut. What this constant is for is
// unaffected — it is the BANNER's zero-cost claim, that `off` prints the launch and nothing
// around it — so the bytes are re-spelled and every subtraction below still measures the
// picture. The rows' own text belongs to ./start.live-hint.test.js.
