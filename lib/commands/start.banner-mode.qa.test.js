...
const PREFIX = {
  full: `${SPLASH_BLOCK}${BOX_BLOCK}`,
  static: `${STATIC_BLOCK}${BOX_BLOCK}`,
  off: '',
}
 ...
 */
function expectMode(d, mode, where) {
  const output = d.stdout.output()
  expect(output.startsWith(PREFIX[mode]), `${where}: prefix`).toBe(true)
...
const GRAMMAR = [
  ...
  // LOOSER THAN BASH, and pinned so it stays deliberate: `VAR = value` is a command
  // invocation to a shell, not an assignment. This parser accepts it, so a user who typed a
  // space gets the banner they asked for rather than a silent default — but the bash loop
  // that SOURCES the same file would not, so the two disagree about this line and only the
  // JS half honours it.
  ['spaces around the equals', 'RALPH_BANNER = off', 'off', false],
  ...
  // A byte-order mark, which is what an editor can leave on the first line of a file.
  ['a BOM on the assignment line', '\uFEFFRALPH_BANNER=off', 'off', false],
  ...
  // NOT BASH, and the row most likely to bite a real user: blanking the knob further down the
  // file does NOT clear an earlier value, because an empty assignment does not match the
  // parser's `VAR=(something)` at all and therefore is not "the last assignment". Bash would
  // export the empty string and this resolver would read that as no opinion — `full`. So a
  // file that ends in `RALPH_BANNER=` still resolves to the earlier `off`.
  ['a blanking reassignment', 'RALPH_BANNER=off\nRALPH_BANNER=', 'off', false],
...
describe('QA startCommand — RALPH_BANNER=static writes #67’s banner and nothing else (#74)', () => {
  it('writes exactly the bytes renderStaticBanner produces, at every width that holds a sprite', async () => {
    ...
    for (const columns of [26, 43, 60, 200]) {
...
describe('QA startCommand — the config read that moved to the top (#74)', () => {
  ...
  it('is the first thing that happens, and still the only thing above the banner', async () => {
    ...
    // ...then an unbroken run of stdout writes: the whole banner, with no exec, no stat, no
    // second read and nothing on stderr threaded through it.
    const firstEffect = kinds.findIndex((kind, index) => index >= 2 && kind !== 'write')
    expect(kinds.slice(2, firstEffect)).toEqual(new Array(firstEffect - 2).fill('write'))
...
describe('QA startCommand — the warning a typo earns, contained (#74)', () => {
  ...
  it('lets no escape sequence out of the value reach stdout', async () => {
    ...
    // a `| tee` and a CI transcript keep, and it must stay plain. The warning itself echoes the
    // value as written — escapes included, which is the same trade `resolveAgent` makes for a
    // mistyped RALPH_AGENT (#559), since showing the user what they typed is the point of the
    // message — so the assertion is one-sided on purpose: nothing on stdout, complaint on
    // stderr, and the run unaffected.
