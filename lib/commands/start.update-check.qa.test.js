...
describe('QA start step 2.5 on the paths the old step 8.5 never reached (#24)', () => {
  ...
  it('has already checked (and stamped) when the run aborts on gh auth', async () => {
    ...
    expect(readVersionCache({ fs: cacheFs, home: HOME, processEnv: {} })).toEqual({
      ...
      // #215: the number is filed under the channel that answered — npm, the query this run's
      // exec stub served. Enumerated with the rest because this assertion is about exactly which
      // fields step 2.5 leaves behind.
...
describe('QA the cache never disturbs the global .env during start (#24)', () => {
  ...
  it('never writes a credential value into the cache file', async () => {
    ...
    expect(Object.keys(JSON.parse(raw)).sort()).toEqual([
      ...
      'last_prompted_at',
      'latest_version',
      // #215: a fourth key, and the claim this list makes is unchanged — the cache file holds
      // cache fields and no credential ever leaks into it.
    ...
    const cacheFs = Volume.fromJSON(
      {
        [CACHE_PATH]: JSON.stringify({
          ...
          // #215: stamped with the channel this run would ask, so the cache really COULD have
          // served the notice. Without it the silence below would be the attribution's doing and
          // the opt-out would never be exercised at all.
          latest_version_channel: 'npm',
        }),
      },
      '/',
...
describe('QA start stays silent when there is nothing to announce (#24)', () => {
  ...
  it('says nothing when currentVersion is not a comparable semver, but still stamps the window', async () => {
    ...
    expect(readVersionCache({ fs: cacheFs, home: HOME, processEnv: {} })).toEqual({
      ...
      last_prompted_at: null,
      latest_version: '0.2.0',
      // #215: an uncomparable `currentVersion` changes nothing about the attribution — the
      // channel that answered is a fact about the QUERY, not about this copy's version.
      latest_version_channel: 'npm',
    })
  })
    ...
    const cacheFs = Volume.fromJSON(
      {
        [CACHE_PATH]: JSON.stringify({
          ...
          last_prompted_at: null,
          latest_version: '0.1.0',
          // #215: stamped, so what silences the notice is the VERSION COMPARISON — an
          // unattributed number would be withheld before it was ever compared.
          latest_version_channel: 'npm',
        }),
      },
      '/',
  ...
  it('a stale cached version survives one failed refresh so the notice is not lost', async () => {
    const cacheFs = Volume.fromJSON(
      {
        [CACHE_PATH]: JSON.stringify({
          ...
          last_prompted_at: null,
          latest_version: '0.2.0',
          // #215: the stale number is only a pending notice for the channel that RESOLVED it, so
          // the seed names one. #24's promise — a flaky night must not lose a notice — is a
          // promise about this channel's own answer, and never a licence to serve another's.
          latest_version_channel: 'npm',
        }),
      },
      '/',
    ...
    expect(readVersionCache({ fs: cacheFs, home: HOME, processEnv: {} })).toEqual({
      ...
      last_prompted_at: null,
      latest_version: '0.2.0',
      // #215: a failed query overwrites neither the version nor its channel — the surviving pair
      // is the seed's, which is what keeps it readable on the next run.
      latest_version_channel: 'npm',
    })
  })
})
