...
describe('QA #120 start — GH_REPO as a line in the file the loop sources', () => {
  ...
  it('names the file’s repo for the two spellings bash assigns nothing at all for', async () => {
    // THE ONE PLACE THIS ROW CAN NOW OUTRUN THE LOOP, pinned rather than fixed — and pinned
    // here because #120 is what made it reachable on this knob. Both shapes below are the
    // shared grammar's, not this call site's: lib/parse-config-var.qa.test.js measures the
    // first against a real bash and pins it as "LOOSER than bash", and both were already true
    // of RALPH_AGENT when #118 gave it this same present-or-absent shape. What changed is the
    // consequence, because this row's contract is stricter than that knob's: naming a
    // repository the loop is NOT about to read is the one answer lib/git-remote-slug.js says
    // it may never give.
    //
    // Measured, so the disagreement is a record and not a guess:
    //
    ...
    //   []
    //
    // So the loop reads `ambient/repo` for the first and origin for the second, while the box
    // reads `committed/repo` for both: `configAssignsVar` says yes to a spelling bash never
    // assigned, and the bare `=` of the second matches no VALUE, so `parseConfigVar`'s
    // last-one-wins leaves the live line above it standing. Whitespace around the `=` is also
    // the one of the two that #120 made WORSE rather than differently wrong — before it, that
    // line was invisible here and the row read the environment, which is what the loop reads.
    for (const assignment of [
      `GH_REPO = committed/repo${LF}`,
      `GH_REPO=committed/repo${LF}GH_REPO=${LF}`,
    ]) {
      const { d, result } = await run({
        config: committed(assignment),
        processEnv: { ...AMBIENT },
      ...
      expect(rowOf(d, 'repo'), assignment).toBe('committed/repo')
    }
  })
  ...
  const NOT_A_SLUG = [
    ...
    ['a space in the name', 'GH_REPO=committed/re po', undefined],
