...
// #215: a cached version is only served to the channel that resolved it, so a seed with no
// channel is a seed no copy will read. No run in this file passes `ralphHome`, so doctor places
// itself with the npm fallback (doctor.js's `installChannel` note argues why a caller that named
// no path reads as npm) — `npm` is therefore the stamp that keeps this a WARM cache, which is
// what every verdict, width and escape assertion below is driven by.
const warmCache = (latest) =>
  Volume.fromJSON(
    {
      [CACHE_PATH]: JSON.stringify({ latest_version: latest, latest_version_channel: 'npm' }),
    },
    '/',
  )
...
describe('QA #75 — RALPH_BANNER, driven from both sources at once', () => {
  ...
  for (const value of OFF_SPELLINGS) {
    ...
    it(`silences the box for RALPH_BANNER=${JSON.stringify(value)} in ralph.config.sh`, async () => {
      ...
      // AND SO IS THE BLANK BETWEEN THE `=` AND THE WORD (#149 review). The expectation is an
      // EXPLICIT TABLE of what a real bash does with each of these ten lines, not a computation —
      // deriving it from `configAssignsVar`/`parseConfigVar` would build the oracle out of the two
      // functions the command under test calls, and the row could then only fail for a wiring break,
      // never for a wrong grammar. This file also injects every seam precisely so nothing in it
      // reaches a shell (see 3. in the header), so the shell ran ONCE, here, and its answers are
      // written down. GNU bash 5.3.15 (aarch64-apple-darwin25.4.0), sourced the way
      // templates/ralph.sh does it, with `RALPH_BANNER=ambient` already exported:
      //
      //   $ printf 'RALPH_BANNER=%s\n' "$v" > b.sh
      //   $ RALPH_BANNER=ambient bash -c 'set -a; . ./b.sh; set +a; printf "[%s]" "${RALPH_BANNER-UNSET}"'
      //
      //   'off' 'OFF' 'Off' 'oFF'  -> [off] [OFF] [Off] [oFF]   assigned, so the box goes
      //   'off '                   -> [off]                     padding AFTER the value is dropped
      //   'off\n'                  -> [off]                     line 1 is the whole assignment
      //   ' off' '  off  ' '\toff' -> [ambient]  + `off: command not found`
      //                                                         a blank after the `=` makes this
      //                                                         bash's environment-PREFIX syntax:
      //                                                         `off` is the command, and the file
      //                                                         never sets the knob at all
      //   '\r\noff\r\n'            -> []        + `$'off\r': command not found`
      //                                                         line 1 is `RALPH_BANNER=\r`, an
      //                                                         assignment of nothing under this
      //                                                         repo's CR line-ending policy
      //
      // So six of the ten silence the box and four leave it standing, for two different reasons —
      // where a plain trim of the raw text would have said `off` for all ten.
      const ASSIGNED_OFF = new Set(['off', 'OFF', 'Off', 'oFF', 'off ', 'off\n'])
      const silenced = ASSIGNED_OFF.has(value)
      expect(boxLines(out).length === 0, JSON.stringify(value)).toBe(silenced)
      expect(result.exitCode).toBe(0)
    })
  }
  ...
  for (const value of NOT_OFF_SPELLINGS) {
    it(`keeps the box for RALPH_BANNER=${JSON.stringify(value)}, silently`, async () => {
      ...
      // warning. Not for want of a safe way to word it — #108 put `oneLine` in a module that
      // imports nothing, and doctor now reaches that module for the RALPH_AGENT warning it DOES
      // print — but because a cosmetic knob's typo is not a diagnostic. `ralph start` says it.
  ...
  const CONFIG_GRAMMAR = [
    ...
    // The row stays as the assertion that this command moved WITH the grammar rather than
    // keeping a reading of its own — which is the whole promise of the table.
...
describe('QA #75 — the box is first, even on the path that returns early', () => {
  ...
  it('closes the gap (#108): a hostile RALPH_AGENT forges no line under the box', async () => {
    // THE GAP THIS TEST USED TO DOCUMENT, now closed. `resolveAgent` interpolated the RAW
    // RALPH_AGENT into its warning (#559) and doctor prints that warning verbatim, so a value
    // containing a newline produced an extra stdout line. #75 did not introduce that, but it
    // changed what the extra line could be mistaken FOR: there are frame glyphs in doctor's
    // output now, so a forged line reading `│ cwd     /elsewhere` sat directly under a real
    // `╰────╯` and read as part of the box in a pasted report — which is precisely what
    // doctor's output is for.
    //
    // #108 fixed it in resolveAgent's own wording rather than at this call site, so `ralph init`
    // inherited the fix and a third caller will too. It is kept as a test rather than deleted,
    // and kept HERE, because this file is where the forgery was found and where the shape of it
    // is understood: one `out()` call, one terminal line, and a row set of exactly four.
    const PLACEHOLDER = String.fromCharCode(0xfffd)
    const hostile = `x\n│ cwd     /elsewhere`
    const { out, raw, chunks, result } = await runDoctor({ env: { RALPH_AGENT: hostile } })

    // The fix, in the same three shapes the gap was pinned in: no write emits more than one
    // line, no line is shaped like a row that nobody drew, and a reader counting rows sees the
    // FOUR facts doctor has — the second `cwd` in the old list was the forgery.
    const forged = chunks.filter((c) => (c.match(/\n/g) ?? []).length > 1)
    expect(forged).toHaveLength(0)
    expect(out.split('\n').filter((l) => l.startsWith('│ cwd     /elsewhere'))).toEqual([])
    expect(labelsOf(out)).toEqual(['os', 'agent', 'cached', 'cwd'])

    // ...and the warning still ECHOES what was typed, which is the whole point of it: the
    // newline is REPLACED by a visible placeholder rather than stripped, so the user can see
    // that what they set contains something unprintable rather than being told they set a value
    // they did not (lib/banner-rows.js's argument for the same placeholder).
    const warningLine = out.split('\n').find((l) => l.includes('unrecognized'))
    expect(warningLine).toBe(`  ! RALPH_AGENT='x${PLACEHOLDER}│ cwd     /elsewhere' unrecognized; falling back to 'claude'. Valid: claude, codex.`)

    // The BOX itself was never poisoned, and that is the part #75 owns: the row it drew names
    // the agent that was RESOLVED, the real rows are unchanged, and the warning is still below
    // the closing rule rather than between two real rows.
    expect(rowValue(out, 'agent')).toBe('claude')
    expect(rowValue(out, 'cwd')).toBe(CWD)
    expect(boxLines(out).filter((l) => l.startsWith('╰'))).toHaveLength(1)
    const lines = out.split('\n')
    expect(lines.indexOf(warningLine)).toBeGreaterThan(lines.findIndex((l) => l.startsWith('╰')))
  ...
  it('lets no terminal instruction out of RALPH_AGENT either (#108)', async () => {
    // The other half of the same hole, and the one that was never pinned: the warning echoed
    // the value byte for byte, so an ESC in RALPH_AGENT reached the stream of the command whose
    // output people paste. Doctor's own escape claims (`emits no cursor control, no C1
    // introducer and no carriage return, in any mode`) are all driven through RALPH_BANNER,
    // which doctor never echoes — so none of them was watching this door.
    //
    // Swept as a CLASS: a clear-screen, a window-title sequence, a bare CR that redraws over
    // the frame, a C1 introducer that needs no ESC at all, and a NUL that truncates the line
    // for some terminals. Every one is built from its code point (#107).
    ...
    for (const [label, value] of Object.entries({
      ...
      expect(raw, label).not.toContain(CR)
      expect(raw, label).not.toContain(NUL)
      expect(raw, label).not.toContain(C1_CSI)
      expect(raw, label).not.toContain(BEL)
      // Still four rows, still one line per write, still the dependency check's exit code —
      // and the warning is still there, because a scrubbed value is not a silenced one.
