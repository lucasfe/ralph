...
describe('ralph live — step 4: no live session is a benign no-op (#167)', () => {
  ...
  it('offers `ralph status` BEFORE `ralph start`, because only one of them can start a duplicate', async () => {
    ...
    // `ralph start` in a subdirectory runs under `sessionNameFor(cwd)` (start.js:684, spent
    // at :996) and is invisible here, but NOT to `ralph status`: templates/ralph.sh:57
    ...
    // cwd-derived name (start.js:708), so a `ralph start` at the root over a
  ...
  it('names the REPO ROOT as the place to run `ralph start`, not the cwd it was typed in', async () => {
    ...
    // the cwd it is handed (start.js:684) while this line's session name is the hash of the
