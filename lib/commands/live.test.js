...
describe('ralph live — step 4: no live session is a benign no-op (#167)', () => {
  ...
  it('offers `ralph status` BEFORE `ralph start`, because only one of them can start a duplicate', async () => {
    ...
    // `ralph start` in a subdirectory runs under `sessionNameFor(cwd)` (start.js:720, spent
    // at :1032) and is invisible here, but NOT to `ralph status`: templates/ralph.sh:57
    // anchors `PROJECT_ROOT` on the git toplevel wherever it was launched from and records
    // the run there (ralph.sh:395-398), and status.js:310,317 reads `record?.session ||
    ...
    // cwd-derived name (start.js:744), so a `ralph start` at the root over a
  ...
  it('names the REPO ROOT as the place to run `ralph start`, not the cwd it was typed in', async () => {
    ...
    // the cwd it is handed (start.js:720) while this line's session name is the hash of the
