...
//    giving lib/install-markers.js's `probe` the whole question as a thunk (:449-455). See
...
describe('QA #201 the installFs seam must be TOTAL at the command level', () => {
  ...
  // (lib/install-markers.js:449-455) and `linkSignal` asks it
  // `probe(() => fs.lstatSync(home).isSymbolicLink())` at :404, so the stat, the property read
  // and the call are all inside the one try. That argument is written out at :392-403 and
  // :434-448. These cases STAY because the guard boundary is re-narrowable by a single edit —
...
describe('QA #201 no other box moved, and the frame still holds', () => {
  it('leaves `ralph start`s and `ralph status`s fact bags without a channel', () => {
    ...
    //
    // #215 SPLIT THIS ASSERTION, because it was three claims wearing one name and only two of
    // them were ever about the row. `ralph start` now derives WHICH CHANNEL'S published versions
    // its box may compare against (`versionChannelFor`, out of lib/install-markers.js), so the
    // blanket ban on naming that module was refuted by a legitimate second use of it: a copy
    // that cannot place itself cannot tell whose "latest version" is in the shared cache, which
    // is the whole of #215. The ban was a PROXY for "start grew no channel row" and it was never
    // the same statement — the row is a WORDING, and the two claims that really carry it are the
    // ones kept below: the wording function is not imported, and no `channel:` fact reaches the
    // composer. Nothing about the import graph changed either: install-markers.js imports
    // `node:path` and nothing else (pinned in lib/install-markers.qa.test.js), so this is not
    // the spawner half of install recognition sneaking into the startup path.
    ...
    // ...and the module ban stands for `ralph status`, which has no version comparison to make
    // and therefore no reason to place itself at all.
    expect(codeWithoutComments(new URL('./status.js', import.meta.url))).not.toMatch(
      /install-markers/,
    )
  })
