...
//
// #215 keyed the number by the channel that resolved it, so "as writeVersionCache would leave
// it" now includes the stamp — and a cache without one is a cache no channel may read. Every
// run in this file leaves `ralphHome` unset, which doctor reads as the npm channel (the fallback
// `classifyInstall` makes for a path no marker claims; the argument is in doctor.js), so `npm`
// is what makes these caches WARM. It is not a new subject: this file is about the wording of
// the verdict for a version doctor already has, and a version doctor cannot attribute is a
// version it does not have. lib/commands/doctor.cached-channel.test.js owns the key itself.
...
describe('QA #27 doctor version line — every shape a cache FILE can take', () => {
  ...
  it('trims a whitespace-padded latest_version instead of rejecting it', async () => {
    // #215: the raw literals from here down carry the channel stamp, because the number is now
    // read FOR a channel and this file's runs are the npm one (see warmCache above). None of
    // these tests is about the stamp — each is about what doctor does with a version it has —
    // so the stamp is the cheapest way to keep them asking their own question.
    const { out } = await runDoctor({
      cacheFs: rawCache('{"latest_version":"  0.18.0\\n","latest_version_channel":"npm"}'),
    })
    expect(identity(out)).toEqual(BEHIND('0.17.0', '0.18.0'))
  })
  ...
  it('takes the LAST duplicated latest_version key (pins JSON.parse semantics)', async () => {
    const { out } = await runDoctor({
      cacheFs: rawCache(
        '{"latest_version":"9.9.9","latest_version":"0.16.0","latest_version_channel":"npm"}',
      ),
    })
    expect(identity(out)).toEqual(CURRENT('0.17.0', '0.16.0'))
  })
  ...
  it('ignores unknown keys and cannot be prototype-polluted by a hand-edited cache', async () => {
    const raw =
      '{"latest_version":"0.18.0","latest_version_channel":"npm","CALLMEBOT_KEY":"leaked",' +
  ...
  it('survives a bloated cache file and still finds the real field', async () => {
    ...
    junk.latest_version_channel = 'npm'
  ...
  for (const [label, extra] of stamps) {
    it(`still reports the cached latest when last_check_at is ${label}`, async () => {
      const cacheFs = rawCache(
        JSON.stringify({ latest_version: '0.18.0', latest_version_channel: 'npm', ...extra }),
      )
...
describe('QA #27 doctor version line — hostile readCache seam', () => {
  ...
  it('reads an inherited latest_version — a prototype read is still a read', async () => {
    // #215: the stamp is inherited alongside the version, which is the point — the read is two
    // plain property reads, so both resolve up the chain by the same rule.
    const proto = { latest_version: '9.9.9', latest_version_channel: 'npm' }
  ...
  it('reads latest_version EXACTLY once, so a type-changing getter cannot become a TypeError', async () => {
    ...
    //
    // #215 put a second field in front of it (`cachedVersionFor` checks the channel before it
    // touches the version), which does not weaken the probe: the count below is of THIS field's
    // reads, and a channel that fails to match returns before the version is read at all — so
    // the assertion still fails for two reads and still fails for none.
    let reads = 0
    const cache = {
      latest_version_channel: 'npm',
      get latest_version() {
...
describe('QA #27 doctor version line — hostile home and env (the documented TypeError path)', () => {
  ...
  it('an env bag hostile on EVERY get fails in the agent resolver, not in the #27 cache read', async () => {
    // Documents where the remaining gap is: resolveAgent (doctor.js:162) and
    // resolveSource (:208) read the bag before the version line is rendered, and neither is
...
describe('QA #27 doctor version line — hostile cache fs', () => {
  ...
  it('accepts a Buffer from readFileSync (the real fs contract)', async () => {
    const cacheFs = {
      readFileSync: () =>
        Buffer.from(
          JSON.stringify({ latest_version: '0.18.0', latest_version_channel: 'npm' }),
          'utf8',
        ),
    }
    const { out } = await runDoctor({ cacheFs })
