...
//
// #69's facts are handed in because `ralph start` hands them in, and they are the same for every
// run in this file: the source is folder (see `cfg` below), and this bag's `readFile` answers ''
// for every path but the config, so there is no metrics log for a model to have come from and no
// `.git/config` for a repo slug. Composed rather than spelled out — which bytes those rows are
// is lib/banner-compose.test.js's business, and this file is about a mode knob.
const BOX_FACTS = {
  version: VERSION,
  latestVersion: null,
  cwd: REPO,
...
const boxLines = (width) =>
  composeBanner({ facts: BOX_FACTS, width, capabilities: { color: false } })
const BOX = boxLines()
...
describe('the shipped ralph.config.sh template documents the knob (#74, criterion 7)', () => {
  ...
  it('produces the run a repo with no banner line at all produces', async () => {
    ...
    // about behaviour.
    //
    // BOTH placeholders are rendered, because a user's file has both: `{{RALPH_AGENT}}` is
    // substituted by the same `ralph init` pass, and since #118 an unrendered one reaches
    // `resolveAgent` as an unrecognised value and earns the line of stderr this run asserts is
    // empty. Which is exactly the trap the RALPH_BANNER assertion above guards against for its
    // own knob — the difference is that this one IS interpolated, so the fixture is what has to
    // interpolate it.
    const template = deps({
      isTTY: true,
      config: TEMPLATE.replace('{{TASK_SOURCE}}', 'folder').replace('{{RALPH_AGENT}}', 'claude'),
    })
