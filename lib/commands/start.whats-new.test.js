...
//
// #69's facts come along because the command passes them, and every run here produces the same
// three: the config says folder, and this bag's `readFile` answers '' for every other path — so
// no metrics log to have learned a model from (the agent row promises nothing) and no
// `.git/config` to read a slug out of (no repo row). What this file is about is the section
// UNDER them.
