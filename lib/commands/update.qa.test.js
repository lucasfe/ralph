...
describe('updateCommand — the version short-circuits on a Homebrew path (#198 QA)', () => {
  it('reinstalls through brew under --force when already current', async () => {
    ...
    // The whole spawn list: the registry query, then ONE `brew upgrade`. --force
    // repairs a broken install, so it re-runs the manager's command — but it must
    // not run it twice, and must still not probe npm.
    expect(d.exec.calls.map((c) => c.key)).toEqual([VIEW_KEY, BREW_KEY])
    const out = d.stdout.output()
  ...
  it('never reaches brew when already current without --force', async () => {
    // The short-circuit sits ABOVE the classification, so a brew install that is
    // already current must not spawn `brew upgrade` at all — an upgrade of a
    // formula with nothing to do is still a tap refresh and a lock the user did
    // not ask for.
  ...
  it('asks the registry exactly once, whatever brew then does', async () => {
    // One question to npm per run, on every brew outcome — the query is not
    // retried per attempt, and the classification never asks a second time.
    const cases = [
    ...
    for (const [, d] of cases) {
      await updateCommand(d)
      expect(d.exec.calls.filter((c) => c.key === VIEW_KEY)).toHaveLength(1)
...
describe('updateCommand — what exit 0 from brew does and does not prove (#198 QA)', () => {
  it('reports a successful update from the exit code alone (characterized)', async () => {
    ...
    // names a FORMULA instead, and the formula's version is bumped separately from
    // the npm publish — scripts/generate-homebrew-formula.js renders it, and its
    // own header records that the release job to run it does not exist yet (#196).
    // So the formula can be behind what `npm view` reports, and brew then exits 0
    // having done nothing ("Warning: ralph 0.15.6 already installed") while Ralph
    // still prints "Updated Ralph 0.15.6 → 0.16.0", because the exit code is the
    // only signal it has. Pinned, not endorsed: closing it needs a second version
    // read after the upgrade, which is a change of its own.
    //
    // A stale TAP reaches the same output by a second route, and is the one case
    // the single-spawn argv cannot cover. `brew upgrade` normally refreshes its
    // taps itself — `upgrade` is in auto-update.sh's AUTO_UPDATE_COMMANDS, on a
    // 24-hour default cadence (HOMEBREW_AUTO_UPDATE_SECS) — which is why Ralph
    // runs no separate `brew update`. But `auto-update()` returns immediately when
    // HOMEBREW_NO_AUTO_UPDATE is set, so a user who exports it gets `brew upgrade
    // ralph` against whatever the tap last knew: exit 0, nothing done, and the
    // same "Updated" line. Also pinned rather than fixed, because the fix is the
    // same second version read.
    const d = brewSaying({
      exitCode: 0,
