...
// #215: the box only reports a cached version to the channel that RESOLVED it, so a seam that
// answers a bare `latest_version` answers a number no copy may read. Every run in this file is
// placed at REPO (see `ralphHome` in `deps` below), a path no install marker claims, so the box
// asks the npm channel — this helper is what makes a seeded version this run's own answer rather
// than another channel's. There is deliberately no default stamp inside the command: inventing a
// channel for a file that names none is precisely the #215 bug.
const npmLatest = (latest_version) => ({ latest_version, latest_version_channel: 'npm' })
...
//
// #69's five facts are handed in because the command hands them in, and defaulted to what every
// bag in this file produces: `readFile` here answers '' for everything but ralph.config.sh, so
// there is no metrics log to have learned a model from and no `.git/config` to read a slug out
// of — an agent row that promises nothing, no context row, no repo row. `source` is a parameter
// because the config is, and the row follows it.
const boxFor = ({
  version = VERSION,
  latestVersion = null,
  cwd = REPO,
  ...
  composeBanner({
    facts: { version, latestVersion, cwd, agent, model, provenance, contextWindow, source, repo },
    width,
    capabilities: { color },
  })
...
// The files the banner reads before it draws a byte, by the tail of their paths — see
// `firstEffect` below for why all three are excused there.
...
function makeTimeline() {
  ...
  return {
    ...
    // The first event that is neither a stdout write nor one of the reads the banner is made
    // of, counted with those reads DROPPED. #74 moved the config read above the banner, because
    // the file is where RALPH_BANNER is written and the mode has to be known before anything is
    // drawn — and #69 added the metrics log the model row comes from and, in github mode,
    // `.git/config`. A read that runs no shell, writes nothing and prints nothing is not
    // something that happened TO the user. So "nothing happened before the banner" is still the
    // claim; this is the index it is made at, and dropping the reads rather than counting them
    // is what keeps that index the banner's HEIGHT instead of its height plus its inputs.
    firstEffect: () =>
      events
        .filter(
          (event) =>
...
const deps = ({
  ...
  return {
    ...
    sleep: async () => {},
    signals: null,
    // #215: where this copy is installed, which is the only input to WHICH CHANNEL's cached
    // version the box may report. Pinned to the run's own repo path — a path no install marker
    // claims, so the box asks the npm channel — rather than left to the impure `RALPH_HOME`
    // default, for #41's reason: the layout a developer's checkout happens to sit in would
    // otherwise decide whether the hint rows in this file appear at all.
    ralphHome: REPO,
    ...overrides,
  }
}
...
describe('startCommand — the sprite banner (#67)', () => {
  ...
  it('writes it above every side effect, below only the config read it now needs (#74)', async () => {
    ...
    // command DOES to the machine still happens after all of them. #69 added two more reads
    // of exactly that kind, which is why `firstEffect` drops them rather than counting them.
    expect(d.timeline.events[0]).toEqual({
      kind: 'readFile',
      detail: `${REPO}/ralph.config.sh`,
    })
    expect(d.timeline.firstEffect()).toBe(SPLASH.length + BOX.length)
...
describe('startCommand — the identity box (#68)', () => {
  ...
  it('writes the box under the sprite, above every preflight side effect', async () => {
    ...
    // run. The count is the banner's height alone: #74's inert config read and #69's inert
    // metrics read are dropped rather than counted; see the sprite's ordering test for why
    // neither is something happening.
    expect(d.timeline.firstEffect()).toBe(SPLASH.length + BOX.length)
  })
  ...
  it('names a newer cached version and points at `ralph update`', async () => {
    const d = deps({ readCache: () => ({ ...EMPTY_VERSION_CACHE, ...npmLatest('9.9.9') }) })
    await startCommand(d)
  ...
  it('reads a real cache file through the injected fs, with no seam override', async () => {
    ...
    const cacheFs = Volume.fromJSON({ [path]: JSON.stringify(npmLatest('7.0.0')) }, '/')
  ...
  it('asks no network for the hint — the cache is the only source', async () => {
    ...
    const d = deps({ readCache: () => npmLatest('9.9.9') })
    await startCommand(d)
  ...
  it('paints the hint on a colour-capable TTY, and nothing else in the box', async () => {
    const tty = deps({ isTTY: true, readCache: () => npmLatest('9.9.9') })
    await startCommand(tty)
