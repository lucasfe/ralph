...
describe('startCommand — the identity box (#68)', () => {
  ...
  it('degrades with the terminal, and decides nothing itself (#72)', async () => {
    ...
    expect(tiny.stdout.output()).not.toContain('[38;2;')
    ...
    // 30 columns: the sprite fits, the box does not.
    const narrow = deps({ isTTY: true, columns: 30 })
    await startCommand(narrow)
    const narrowBox = boxFor({ width: 30, color: true })
    expect(narrow.stdout.lines().slice(0, BANNER.length)).toEqual(BANNER)
    expect(
      narrow.stdout.lines().slice(BANNER.length, BANNER.length + narrowBox.length),
    ).toEqual(narrowBox)
    ...
    expect(wide.stdout.lines().slice(0, BANNER.length)).toEqual(BANNER)
    expect(wide.stdout.lines().slice(BANNER.length, BANNER.length + BOX.length)).toEqual(BOX)
  })
