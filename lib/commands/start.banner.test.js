...
describe('startCommand — the sprite banner (#67)', () => {
  ...
  it('writes it before the config read and before the tmux uniqueness check', async () => {
    ...
    // The first thing that is not a stdout write is the whole banner in: the splash's
    // eleven writes and the box under them, then the preflight the command always did.
    const banner = SPLASH.length + BOX.length
...
describe('startCommand — the identity box (#68)', () => {
  ...
  it('writes the box under the sprite, above every preflight side effect', async () => {
    ...
    // on a successful run.
    expect(d.timeline.firstOther()).toBe(SPLASH.length + BOX.length)
  })
