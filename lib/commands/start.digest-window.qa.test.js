...
const deps = ({
  ...
  return {
    ...
    // #68: the identity banner reads the global update-check cache for its update hint.
    // Injected, like every other seam here, so no run in this file reads a real
    // ~/.config/ralph — and so the byte-identity assertions below cannot grow a row
    // because of what a developer's own cache happens to hold (#41).
    readCache: () => ({ latest_version: null }),
    // #70: same again for the box's what's-new section, which reads the changelog inside
    // the installed package. This file asserts whole-stdout equality against a hardcoded
    // box, so the real reader would make every one of those assertions a function of the
    // last release (#41). The digest window is what is under test here; the banner's
...
// #68's identity banner, which now sits above every line this command prints — on a
// pipe as well as on a terminal, because it is text rather than the TTY-gated sprite.
// Spelled out for the same reason the launch box below is: byte identity is a claim
// about these exact characters. At the 60-column default (these streams report no
// `columns`), with no update row — deps injects an empty cache.
//
// #69 added two of these rows, and both are constant for every run in this file. The
// AGENT row is the no-history sentence because `readFile` here answers '' for every path
// but the config, so there is no metrics log to have learned a model from — which is the
// state a launch in a fresh checkout is in, and the one worth pinning under a digest
// suite. The SOURCE row is whatever the config said, which is why it is a parameter: the
// four runs below with no readable config fall back to github. There is no REPO row in
// either case — the same '' answers `.git/config`, so no slug is cheaply knowable.
const boxRow = (label, value) => `│ ${label.padEnd(8)}${value.padEnd(48)} │`
const NO_HISTORY_AGENT = 'claude — model resolves at first run'
const identity = (source) => [
  `╭─ ralph ${VERSION} ${'─'.repeat(44)}╮`,
  boxRow('agent', NO_HISTORY_AGENT),
  boxRow('cwd', REPO),
  boxRow('source', source),
  `╰${'─'.repeat(58)}╯`,
]
const IDENTITY = identity('folder')

// The launch summary a repo with no digest gets, byte for byte. Hardcoded rather than
// captured, because "unchanged from today" is a claim about these exact characters.
const STARTUP_WITHOUT_DIGEST = [
  ...
  // #169: the row's value is the shortcut, the tmux command its continuation line.
...
// ...and the whole stdout stream of such a run: banner, then summary.
const BOX_WITHOUT_DIGEST = [...IDENTITY, ...STARTUP_WITHOUT_DIGEST]
// The same stream for a run whose config could not be read at all, where the source falls
// back to github. Only that one row differs, which is the whole point of spelling it.
const BOX_WITHOUT_CONFIG = [...identity('github'), ...STARTUP_WITHOUT_DIGEST]
...
// The digest line goes third IN THE SUMMARY — after `Progress:` — which is why this
// splices the summary rather than the whole stream: the banner's height must not be an
// index anything here has to know.
const boxWithDigest = (interval, opened = true) => [
  ...IDENTITY,
  ...STARTUP_WITHOUT_DIGEST.slice(0, 3),
  digestLine(interval, opened),
  ...STARTUP_WITHOUT_DIGEST.slice(3),
]
...
describe('QA: no interval configured — byte-identical to a pre-#62 run (#62)', () => {
  ...
  it.each([
    ...
    // digest's default is off. These runs fall back to the github source, which since #69
    // the box says out loud — so it is the same box but for that one row.
    const d = deps({ config: '', ...overrides })
    const result = await startCommand(d)
    expect(result.started).toBe(true)
    expect(d.stdout.lines()).toEqual(BOX_WITHOUT_CONFIG)
    expect(d.stderr.output()).toBe('')
    expect(windowCalls(d)).toHaveLength(0)
  })
...
// The file as a USER has it, which means EVERY placeholder `ralph init` substitutes rendered —
// both of them. `{{RALPH_AGENT}}` matters since #118: an unrendered one reaches `resolveAgent`
// as a value, is not a recognised agent, and now earns a line of stderr, so a fixture that
// leaves it in place is asserting against a config no `ralph init` ever wrote. `claude` is what
// init writes for the default choice.
const rendered = () => TEMPLATE.replace('{{TASK_SOURCE}}', 'folder').replace('{{RALPH_AGENT}}', 'claude')
...
describe('QA: a padded interval is the same interval everywhere it is read (#62 fix)', () => {
  it.each([
    ...
    ['unquoted, padded after the value only', 'RALPH_DIGEST_INTERVAL=2h  ', '2h'],
    ['a tab each side, quoted', 'RALPH_DIGEST_INTERVAL="\t1d\t"', '1d'],
    ['a trailing comment after the value', 'RALPH_DIGEST_INTERVAL=60  # hourly', '60'],
    ['no padding at all — the control row', 'RALPH_DIGEST_INTERVAL="90s"', '90s'],
    // TWO ROWS LEFT THIS TABLE IN #149's REVIEW — `RALPH_DIGEST_INTERVAL=  2h  ` and
    // `RALPH_DIGEST_INTERVAL= 60  # hourly`, both with a blank BETWEEN the `=` and the value. A
    // blank there is not padding: it makes the `=` an environment prefix to the word after it, so
    // bash runs `2h` as a command and assigns nothing, and both readers here now agree with it.
    // The two spellings are pinned at their new answer — no window, no Digest line, no warning —
    // in "and so does a blank AFTER the `=`" at the foot of this file. The rows above keep the
    // coverage that is still real: padding INSIDE quotes, and padding after an unquoted value.
...
// ---------------------------------------------------------------------------
// QA #147 — what the tightened assignment grammar cost THIS knob.
//
// #147 stopped `lib/parse-config-var.js` from reading `VAR = value` as an assignment,
// on the measured ground that bash runs such a line as a command and assigns nothing,
// so the JS layer was resolving a value no run would ever hold. That argument is
// exactly right for a knob BOTH halves read — TASK_SOURCE, RALPH_AGENT, GH_REPO,
// JIRA_JQL — where the fix takes an answer away from the JS reader and hands it the
// one the loop already had.
//
// RALPH_DIGEST_INTERVAL is not such a knob. `templates/ralph.sh` never mentions it
// (nor RALPH_BANNER); `ralph start` is its ONLY reader, and what it does with the
// value is open a second tmux window. So for this knob the tightening is not two
// readers agreeing — it is the only reader changing its mind, and a repo that spelled
// the line with a space around the `=` gets a launch that is missing a window it used
// to get. Silently: an empty interval is "digest off", which is the documented default
// and warns about nothing.
//
// That is a defensible trade (one grammar beats one knob's convenience), and it is the
// trade parse-config-var.qa.test.js's #147 block already states rather than one this
// block catches it out on. That block splits the seven knobs into two bullets: for the
// ones bash also reads the fix "takes a value away from the JS reader and leaves it
// holding the nothing the loop already held", and for RALPH_BANNER,
// RALPH_DIGEST_INTERVAL and RALPH_DIGEST_MODEL it "CHANGES BEHAVIOUR OUTRIGHT … A repo
// whose config said `RALPH_DIGEST_INTERVAL = 30m` used to get a digest window and now
// gets none, silently" — and it names THIS test as where that is pinned instead of
// arguing it there. So this block is the MEASUREMENT behind that second bullet, at the
// command, which is what makes the claim checkable rather than asserted.
// ---------------------------------------------------------------------------

// AND #149's REVIEW WIDENED IT, which is why the second test in this block reads the opposite way
// from the way it was written. #147 refused a blank BEFORE the `=`; the #149 review refused the
// whole family bash refuses — a line whose assignment is followed by a command word — because the
// narrow version of the same rule masked an environment value on the knobs that DO have two
// readers. `RALPH_DIGEST_INTERVAL=  2h  ` is inside that family, so this knob's silent cost now
// covers one spelling more. The argument, the bash transcript and the sweep behind it are in
// lib/parse-config-var.js above `endOfWord`.
describe('QA #147/#149: a spaced interval assignment now opens no window at all', () => {
  it('opens no digest window, prints no Digest line, and warns about nothing', async () => {
    // The whole stream, byte for byte, against the one every repo with no digest gets —
    // because the claim is not "the interval was refused" (that path warns and says NOT
    // running) but "there was never an interval here", which is indistinguishable from a
    // config that left the knob empty. A user who wrote the space gets no hint that the
    // line they edited did nothing.
    const d = deps({ config: 'TASK_SOURCE=folder\nRALPH_DIGEST_INTERVAL = 30m\n' })
    const result = await startCommand(d)
    expect(result.started).toBe(true)
    expect(windowCalls(d)).toHaveLength(0)
  ...
  it('and so does a blank AFTER the `=`, which #149 brought under the same rule', async () => {
    // THIS TEST USED TO PIN THE OPPOSITE — "still opens the window for the padded spellings the
    // slice deliberately kept" — and the flip is the one behaviour change #149's review cost
    // outside `ralph start`'s identity box. `RALPH_DIGEST_INTERVAL=  2h  ` was the unquoted row of
    // the padded-interval table above, and the reason the grammar left whitespace after the `=`
    // alone at all. bash never assigned it: it runs `2h` as a command with an empty
    // RALPH_DIGEST_INTERVAL in THAT command's environment and the sourcing shell keeps what it
    // held. Two review rounds then found the same defect one spelling over — first `X= ""`, then
    // `X=#c off`, both of them lines whose PRESENT-and-blank verdict masked an environment value —
    // so the grammar now models bash's word rule rather than carving spellings out of it, and this
    // knob pays the same silent price as the spelling above: no window and no warning.
    //
    // Nothing this repo SHIPS wrote it that way. templates/ralph.config.sh writes
    // `RALPH_DIGEST_INTERVAL=""`, and no line anywhere under templates/ has an unquoted blank
    // after the `=` followed by a word — so what this change cost was this pin, deliberately,
    // rather than a rendered configuration.
    for (const line of ['RALPH_DIGEST_INTERVAL=  2h  ', 'RALPH_DIGEST_INTERVAL= 2h']) {
      const d = deps({ config: `TASK_SOURCE=folder\n${line}\n` })
      const result = await startCommand(d)
      expect(result.started, line).toBe(true)
      expect(windowCalls(d), line).toHaveLength(0)
      expect(d.stdout.lines(), line).toEqual(BOX_WITHOUT_DIGEST)
      expect(d.stderr.output(), line).toBe('')
    }
    // The INDENT is a different rule and is still tolerated, which is what keeps this about WORDS
    // rather than about whitespace: bash skips blanks before the name and assigns.
    const indented = deps({ config: 'TASK_SOURCE=folder\n  RALPH_DIGEST_INTERVAL=2h\n' })
    await startCommand(indented)
    expect(windowCalls(indented)).toHaveLength(1)
    expect(indented.stdout.lines()).toEqual(boxWithDigest('2h'))
  })
})
