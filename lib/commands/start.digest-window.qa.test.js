...
describe('QA #147: a spaced interval assignment now opens no window at all', () => {
  ...
  it('still opens the window for the padded spellings the slice deliberately kept', async () => {
    // The other half of the scope decision, asserted here so the row above cannot be read
    // as "spaces near the `=` disable the digest". Whitespace AFTER the `=` was left alone
    // precisely so these keep working, and this is the knob the argument was made about —
    // `RALPH_DIGEST_INTERVAL=  2h  ` is a row in the padded-interval table above. bash
    // assigns nothing for that line either (it reads `2h` as a command with an empty
    // `RALPH_DIGEST_INTERVAL` in its environment), which costs nothing here for the same
    // reason the row above costs something: no bash reader of this knob exists to disagree.
    for (const line of [
      ...
      const d = deps({ config: `TASK_SOURCE=folder\n${line}\n` })
      await startCommand(d)
      expect(windowCalls(d), line).toHaveLength(1)
      expect(d.stdout.lines(), line).toEqual(boxWithDigest('2h'))
    }
  })
})
