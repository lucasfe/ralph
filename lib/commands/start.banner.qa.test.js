...
describe('QA startCommand banner — every exit path, piped and on a TTY (#67)', () => {
  ...
  it('puts the frame above the error on every aborting path', async () => {
    ...
    for (const name of aborts) {
      ...
      // Written before the guard that aborted, not merely present: the first event that is
      // not a stdout write is event 14 on every one of these — the splash's eleven writes
      // and #68's three box lines, then the preflight. Counted in WRITES rather than lines
      // since #73, because a frame is now one write of seventeen rows.
      expect(d.timeline.firstOther(), name).toBe(SPLASH.length + BOX.length)
    }
  })
...
describe('QA startCommand identity box — the first line of every piped run (#68)', () => {
  for (const [name, options] of Object.entries(PATHS)) {
    it(`opens the ${name} path with the box, above every other side effect`, async () => {
      ...
      // the box is lines 0..2 of stdout, and the first event that is not a stdout write is
      // event 3. Both halves matter — the slice proves it is there and in one piece,
      // `firstOther` proves nothing happened before it, and the 3 proves the suppressed
      // splash cost this run no write of its own (not a hide, not a lone reset).
