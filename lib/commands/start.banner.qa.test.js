...
function makeTimeline() {
  ...
  return {
    ...
    // moved that read above the banner — the file is where RALPH_BANNER is written, so the
    // mode has to be known before anything is drawn — and it runs no shell, writes nothing
    // and prints nothing, so excusing it keeps "nothing HAPPENED before the banner" the
    // claim these twelve paths make rather than weakening it.
    firstEffect: () =>
      events.findIndex(
        (event) =>
          event.kind !== 'write' &&
...
describe('QA startCommand banner — every exit path, piped and on a TTY (#67)', () => {
  ...
  it('puts the frame above the error on every aborting path', async () => {
    ...
    for (const name of aborts) {
      ...
      // Written before the guard that aborted, not merely present: the first thing this
      // command DOES is event 15 on every one of these — #74's inert config read, then the
      // splash's eleven writes and #68's three box lines, then the preflight. Counted in
      // WRITES rather than lines since #73, because a frame is now one write of seventeen
      // rows.
      expect(d.timeline.firstEffect(), name).toBe(1 + SPLASH.length + BOX.length)
    }
  })
...
describe('QA startCommand identity box — the first line of every piped run (#68)', () => {
  for (const [name, options] of Object.entries(PATHS)) {
    it(`opens the ${name} path with the box, above every other side effect`, async () => {
      ...
      // the box is lines 0..2 of stdout, and the first thing this command DOES is event 4 —
      // #74's inert config read plus those three lines. Both halves matter — the slice
      // proves it is there and in one piece, `firstEffect` proves nothing happened before
      // it, and the count proves the suppressed splash cost this run no write of its own
      // (not a hide, not a lone reset).
      ...
      expect(d.timeline.firstEffect(), name).toBe(1 + BOX.length)
...
describe('QA startCommand identity box — the width comes from the stream (#68, #72)', () => {
  ...
  it('prints the same facts bare, at every width too narrow to hold a frame', async () => {
    ...
    // SAME two facts in the same order, unadorned, each clipped to the columns the stream
    ...
    for (const [columns, expected] of [
      [43, ['ralph 1.2.3', 'cwd     /repo']],
      [30, ['ralph 1.2.3', 'cwd     /repo']],
      [26, ['ralph 1.2.3', 'cwd     /repo']],
      [12, ['ralph 1.2.3', 'cwd     /re…']],
      [5, ['ralp…', 'cwd …']],
      [1, ['…', '…']],
    ]) {
      const d = deps()
  ...
  it('lets an explicit columns option beat what the stream reports', async () => {
    ...
    expect(bare.stdout.lines().slice(SPLASH_LINES, SPLASH_LINES + 2)).toEqual([
      'ralph 1.2.3',
      'cwd     /repo',
    ])
  })
...
describe('QA startCommand banner — the sprite rung, through the real command (#72)', () => {
  ...
  const bannerHeight = (columns, boxHeight = BOX.length) => {
    ...
    return (layout.sprite ? SPLASH_LINES : 0) + (layout.boxed ? boxHeight : boxHeight - 1)
  }
  ...
  it('draws the sprite at twenty-six columns and not at twenty-five', async () => {
    ...
    expect(fits.stdout.lines().slice(SPLASH_LINES, SPLASH_LINES + 2)).toEqual([
      'ralph 1.2.3',
      'cwd     /repo',
    ])
    ...
    // would leave behind: line 0 of the run is the title.
    expect(tooNarrow.stdout.lines().slice(0, 2)).toEqual(['ralph 1.2.3', 'cwd     /repo'])
  })
  ...
  it('never lets the sprite spill past the columns the stream reported', async () => {
    ...
    for (const columns of [26, 27, 43, 60, 200]) {
  ...
  it('asks the stream it was handed and never the terminal it is running in', async () => {
    ...
    await startCommand(d)
    expect(settledFrame(d)).toEqual(BANNER)
    expect(d.stdout.lines().slice(SPLASH_LINES, SPLASH_LINES + 2)).toEqual([
      'ralph 1.2.3',
      'cwd     /repo',
    ])
