...
// #215: a cached version is only reported to the channel that RESOLVED it, so a seed with no
// channel is a number no copy may read. Every run here is placed at REPO (see `ralphHome` in the
// bag below), a path no install marker claims, so the box asks the npm channel. The stamp is on
// the REJECTED values too, not just the accepted ones: an unstamped `latest_version: 'banana'`
// would produce no hint because nobody may read it, and this file's claim is that the SEMVER
// check rejected it.
...
function deps({
  ...
  return {
    ...
    // default, for the same #41 reason as `readCache` above: the layout a developer's checkout
    // happens to sit in must not decide whether a hint row appears in these byte comparisons.
    ralphHome: REPO,
    ...overrides,
  }
}
...
describe('QA startCommand identity box — the cache is read, and nothing else (#68)', () => {
  ...
  it('reads the real cache file through the injected fs when no seam is given', async () => {
    ...
    const cacheFs = Volume.fromJSON({ [CACHE_PATH]: JSON.stringify(npmLatest('9.9.9')) }, '/')
  ...
  it('honors an XDG_CONFIG_HOME the run was given rather than the home dir', async () => {
    ...
    const cacheFs = Volume.fromJSON({ [path]: JSON.stringify(npmLatest('9.9.9')) }, '/')
...
describe('QA startCommand identity box — the opt-out gates the hint and the read (#68)', () => {
  ...
  it('reads the opt-out from a non-string and an inherited value, like the check does', async () => {
    ...
    for (const processEnv of [
      ...
      const d = deps({ processEnv, readCache: () => npmLatest('9.9.9') })
      await startCommand(d)
...
describe('QA startCommand identity box — the readCache seam, abused (#68)', () => {
  ...
  const RETURNS = [
    ...
    ['undefined', () => undefined],
    ['an empty object', () => ({})],
    ['a numeric latest_version', () => npmLatest(42)],
    ['a null latest_version', () => npmLatest(null)],
    ['a blank latest_version', () => npmLatest('   ')],
    ['a non-semver latest_version', () => npmLatest('banana')],
    ['an array', () => []],
    ['a string', () => 'nope'],
    ['a number', () => 7],
    ['a promise', () => Promise.resolve(npmLatest('9.9.9'))],
    ['a throwing latest_version getter', () => ({ latest_version_channel: 'npm', get latest_version() { throw new Error('boom') } })],
    ['a throwing read', () => { throw new TypeError('The "path" argument must be of type string.') }],
    ['a null prototype cache', () => Object.assign(Object.create(null), npmLatest(null))],
  ]
  ...
  it('does not await a promised cache into a hint, and does not warn about it', async () => {
    ...
    const d = deps({ readCache: async () => npmLatest('9.9.9') })
    await startCommand(d)
    expect(boxOf(d)).toEqual(BOX)
...
describe('QA startCommand identity box — the width comes from the stream (#68, #72)', () => {
  ...
  it('never lets a narrow terminal spill the box past the columns it reported', async () => {
    ...
    for (const columns of [59, 44, 27, 26, 12, 5, 2, 1]) {
      const d = deps({ columns, readCache: () => npmLatest('9.9.9') })
      await startCommand(d)
