...
describe('QA startCommand identity box — the width comes from the stream (#68, #72)', () => {
  ...
  it('lets an explicit columns option beat what the stream reports', async () => {
    ...
    expect(bare.stdout.lines().slice(BANNER.length, BANNER.length + 2)).toEqual([
      'ralph 1.2.3',
      'cwd     /repo',
    ])
  })
...
describe('QA startCommand banner — the sprite rung, through the real command (#72)', () => {
  ...
  const bannerHeight = (columns, boxHeight = BOX.length) => {
    ...
    return (layout.sprite ? BANNER.length : 0) + (layout.boxed ? boxHeight : boxHeight - 1)
  }
  ...
  it('draws the sprite at twenty-six columns and not at twenty-five', async () => {
    ...
    expect(fits.stdout.lines().slice(0, BANNER.length)).toEqual(BANNER)
    expect(fits.stdout.lines().slice(BANNER.length, BANNER.length + 2)).toEqual([
      'ralph 1.2.3',
      'cwd     /repo',
    ])
  ...
  it('never lets the sprite spill past the columns the stream reported', async () => {
    ...
    for (const columns of [26, 27, 43, 60, 200]) {
      ...
      await startCommand(d)
      const sprite = d.stdout.lines().slice(0, BANNER.length)
      expect(sprite, String(columns)).toEqual(BANNER)
  ...
  it('still draws the sprite for every column count a stream can lie with', async () => {
    ...
    for (const columns of [undefined, 0, -1, -80, Number.NaN, '80', null, {}, 0.5, Infinity]) {
      ...
      expect(d.stdout.lines().slice(0, BANNER.length), label).toEqual(BANNER)
      expect(d.stdout.lines().slice(BANNER.length, BANNER.length + BOX.length), label).toEqual(BOX)
    }
  })
  ...
  it('loses not one line of the run to a narrow terminal', async () => {
    ...
    // one thing the width is allowed to change: seventeen rows of sprite at 26 and above, and
    // a bottom rule only where the frame was drawn.
  ...
  it('asks the stream it was handed and never the terminal it is running in', async () => {
    ...
    await startCommand(d)
    expect(d.stdout.lines().slice(0, BANNER.length)).toEqual(BANNER)
    expect(d.stdout.lines().slice(BANNER.length, BANNER.length + 2)).toEqual([
      'ralph 1.2.3',
      'cwd     /repo',
    ])
