...
// defaulted to `npm`: `cycleCommand` reaches the check through `runUpdateGate`, whose classify
// seam is left at its default here and answers a plain npm layout for this file's fake home. An
// unstamped number is readable by no channel, which would turn every "the notice is served from
// the cache" assertion below into a test of silence. The spread is last so a test can override.
...
describe('QA cycle #51 — a hostile global cache never costs a drain', () => {
  ...
  for (const [label, raw] of corrupt) {
    it(`drains and still notices the new version with ${label}`, async () => {
      ...
      expect(drainEvidence(d)).toEqual(DRAINED)
      // The unusable file is replaced by the well-formed shape — which since #215 includes the
      // channel the run just asked, since a repaired file whose number nobody can read would be
      // repaired in name only.
  ...
  for (const [label, currentVersion, expectNotice] of [
    ...
    it(`${expectNotice ? 'notices' : 'says nothing'} when currentVersion is ${label}`, async () => {
      ...
      expect(cacheOf(cacheFs)).toEqual({
        ...
        last_prompted_at: null,
        latest_version: '0.2.0',
        // #215: the write files the number under the channel that answered — the same npm one on
        // every run here, whatever the running version is. Which is the point of the assertion:
        // nothing about the caller's `currentVersion` reaches the attribution.
        latest_version_channel: 'npm',
      })
    })
  }
