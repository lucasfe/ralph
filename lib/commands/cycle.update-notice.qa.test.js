...
//   - NO QUESTION, EVER: an ambient process.stdin that reports isTTY and explodes
//     on any stream access proves this call site pins `isTTY: false` instead of
//     deriving it, and that no readline is ever constructed.
...
// A stdin that LOOKS interactive, records every property read, and detonates on any
// stream method a readline would need. Reading `.isTTY` is recorded (not fatal) so
// the assertion can say "this call site never even asked".
...
describe('QA cycle #51 — a hostile decision seam never costs the drain', () => {
  ...
  it('prints the notice and drains when the decision claims isNewer with truthy non-booleans', async () => {
    ...
    expect(drainEvidence(d)).toEqual(DRAINED)
    // shouldPrompt was true and still nothing was asked: `isTTY: false` is pinned.
...
describe('QA cycle #51 — the weekly windows across a real schedule', () => {
  ...
  it('leaves the window the next interactive `ralph start` needs, so the human is still asked', async () => {
    // The whole reason this slice pins isTTY:false rather than deriving it: the
    // shared prompt window belongs to the ASKING, and a launchd cycle never asks.
    const cacheFs = new Volume()
...
describe('QA cycle #51 — the cycle can never host a question', () => {
  it('never reads the ambient process.stdin, not even its isTTY flag', async () => {
    const { proxy, reads } = trapStdin()
    ...
    // The gate DOES resolve `stdin = process.stdin` (that is a read of the process
    // object, not of the stream), but this call site pins isTTY:false, so not one
    // property of the stream is ever touched.
    expect(reads).toEqual([])
  })

  it('never constructs a readline even when the decision demands a prompt on a TTY', async () => {
    const { proxy, reads } = trapStdin()
    await withProcessStdin(proxy, async () => {
      const d = deps({
        update: async () => ({ latestVersion: '0.2.0', isNewer: true, shouldPrompt: true }),
  ...
  it('never blocks: a cycle whose prompt would hang still settles', async () => {
    // If the gate ever reached `confirm` over a stream that never ends a line, the
    // real command would hang forever under launchd. This proves it settles.
    const { proxy } = trapStdin()
    await withProcessStdin(proxy, async () => {
      const d = deps({
        update: async () => ({ latestVersion: '0.2.0', isNewer: true, shouldPrompt: true }),
      })
