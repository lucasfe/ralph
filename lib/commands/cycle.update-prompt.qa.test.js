...
describe('QA cycle #52 — the stop cannot starve the schedule', () => {
  ...
  it('six ticks after an accepted install produce one stop and five drains', async () => {
    ...
    expect(cacheOf(cacheFs)).toEqual({
      ...
      // #215: the first tick's check filed 0.2.0 under the channel it asked (npm, through
      // `npmGlobalLayout()`), and the five throttled ticks after it neither re-ask nor
      // re-attribute. Enumerated because the point of the assertion is that six ticks leave
      // exactly ONE pair of stamps behind.
