...
describe('QA live — the session it derives is the session the other commands use (#167)', () => {
  ...
  // `ralph stop`, hermetic: nothing to kill, so the name is read off the probe it spends
  ...
  it('documents the divergence: from a SUBDIRECTORY only `ralph live` is anchored on the repo', async () => {
    // `ralph live` resolves the git toplevel (../repo-session.js) while `ralph start`
    // (start.js:679) and `ralph stop` (stop.js:22) hash the cwd they were handed, so the
    // three names agree only when the cwd IS the toplevel. Consequences, both real:
    //
    ...
    //   * `ralph stop` in `lib/` cannot kill a session `ralph start` opened at the root.
    //
    // #168 is where `ralph stop` moves onto this module, and it is what actually closes this
    // — the ordering above only makes the first move a safe one. Pinned here so the state of
    // the repo is written down rather than inferred: this is what the shared module has NOT
    // fixed yet, and the pair of assertions flips the moment either sibling migrates.
    const [started, stopped, attached] = [
    ...
    expect(stopped).toBe(sessionNameFor(SUBDIR))
