...
// first-word shortcut this keeps a multi-word label — the view already has one, `in
// flight` — whole.
...
// The FACTS the summary enumerates, which is everything above the blank separator
// that precedes the advice. What follows that blank is advice — `attach`, `kill`,
// `restart`, `logs` — and a one-line summary of what the command SHOWS has no
// business listing it.
//
// lastIndexOf since #56: the task table is fenced by blank lines of its own, so the
// FIRST blank in the view is now the top of the table rather than the top of the
// advice block. Taking the first would silently narrow this derivation to a single
// line — the failure mode this file's third premise exists to prevent.
//
// The table's ROWS are dropped and its header kept: a row's label column is a task
// NUMBER (`#029`), which no summary could sensibly name, while `task` is the name of
// the surface and so is exactly the kind of thing the summary must mention.
...
describe('ralph status --help — the summary and the live view agree (#64)', () => {
  it('names every labeled fact line of the live view', async () => {
    ...
    // typed list, so a fourth line — counted or derived — turns this red until the
    // summary is widened. Whole label, not its first word: `in flight` has to be named
    // as `in flight`.
    //
  ...
  it('promises nothing the live view does not print', async () => {
    ...
    // in flight` for a line labelled `in flight`. That is the honest weakening — an
    // item mixing a real line with an invented one could still pass — and it still
    // catches an item with no line behind it at all.
    //
...
describe('ralph status --help — the derivation the assertions rest on (#64)', () => {
  it('keeps each derived label inside its first word, so the shortcut asserts all of it', () => {
    ...
    // word of a longer one. The live view already has a two-word label (`in flight`),
    // so this is not hypothetical: were a derived line ever labelled `per task`, the
    // canary below would compare `per` against the real name and never notice.
