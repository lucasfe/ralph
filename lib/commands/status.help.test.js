...
// The FACTS the summary enumerates, which is the block above the blank separator.
// What follows the blank is advice — `attach`, `kill`, `restart`, `logs` — and a
// one-line summary of what the command SHOWS has no business listing it.
const factLinesOf = (lines) => {
  const blank = lines.indexOf('')
  return (blank === -1 ? lines : lines.slice(0, blank)).filter((line) => line.startsWith('  '))
}
