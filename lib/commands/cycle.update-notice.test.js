...
describe('cycleCommand — an unattended notice never becomes a question (#51/#52)', () => {
  ...
  it('leaves the shared prompt window untouched, so the next interactive run still asks', async () => {
    ...
    expect(cacheOf(cacheFs)).toEqual({
      ...
      // #215: the check window's write now names the channel that answered it. It is enumerated
      // here rather than matched loosely because the claim is about which fields a cycle TOUCHES
      // — `last_prompted_at` staying null is the whole test — so the shape has to be exact.
