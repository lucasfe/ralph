...
// the one path that can run unattended for weeks while staying stale. It is also
// the one path with no human at the other end, which is why this slice is
// NOTICE-ONLY: the gate is called with `isTTY: false`, so #25's question is off and
// nothing here can construct a readline or block a drain.
//
...
// #25's question, which this slice must never ask.
...
// Swaps the AMBIENT process.stdin, which is what the gate's own `isTTY` default
// derives from. Used to prove this call site does not consult it.
...
describe('cycleCommand — the notice never becomes a question (#51)', () => {
  it('never prompts and never constructs a readline, even under an interactive terminal', async () => {
    // The gate's `ask` default is the REAL confirm and its `isTTY` default derives
    // from the ambient process.stdin. If this call site ever stopped pinning
    // isTTY:false, createInterface would be handed this fake stdin and throw here —
    // which is the point: a launchd drain must fail loudly in a test rather than
