...
//
//   1. THE BOX IS A TESTED OUTPUT SURFACE, and the tail a user's muscle memory and
//      every screenshot in the README are built on is `EXISTING_TAIL` below. So the
//      assertions are on the WHOLE box — the `✅` line verbatim, the new lines at the
//      position they claim, and every line of that tail in its original relative
//      order — rather than on the new lines alone. Driven through BOTH sources,
//      because the box is source-independent and folder mode is the only one the
//      dev's file exercises.
...
//
// #169 made the `Watch live:` value `ralph live` and moved the tmux command onto a
// continuation line under it, so the tail is SIX lines now. The rest is untouched, and
// what this constant is for is unchanged: the labels, their order and the session name
// every tmux line below is about. The pair's own text is pinned in
// ./start.live-hint.test.js.
const EXISTING_TAIL = [
...
function baseDeps({
  ...
  return {
    ...
    // ...and so is #68's cache read, for the banner's update hint. The injected `home`
    // and empty `processEnv` below already keep it away from a developer's real
    // ~/.config/ralph, but without this it is still a readFileSync against the real
    // filesystem — a syscall whose answer depends on whether a machine happens to have
    // a /home/me. Seams all the way down is the rule here (#41), so the read is a stub
    // like every other one in this bag.
...
describe('QA start box — the pre-existing tail keeps its text and order (#60)', () => {
  ...
  it('produces a byte-identical box in FOLDER mode', async () => {
    // The queue's provenance is not the box's business: same history, same depth, same
    // box line for line, whether the count came from `gh` or from .ralph/tasks. Asserted
    // as an equality between the two boxes rather than against a count, so #169's sixth
    // tail line — and the next one the tail grows — costs no edit here.
  ...
  it('keeps the tmux lines contiguous, in order, and last', async () => {
    // Pinned positionally as well as by value: the projection and the hint are
    // INSERTED above them, so nothing may be interleaved into the tail. Sliced by the
    // constant's own length rather than a literal, so #169's sixth line — and the next
    // one the tail grows — is counted in one place.
    const deps = baseDeps()
    await startCommand(deps)
    const lines = box(deps)
    expect(lines.slice(-EXISTING_TAIL.length)).toEqual(EXISTING_TAIL)
...
describe('QA start box — the projection is best-effort I/O (#60)', () => {
  ...
  it('still reads it once on the empty-queue early return, and projects nothing', async () => {
    // WAS "never reads the metrics file on this path", and #69 changed the premise rather
    // than the guarantee. The box's model row is resolved from this same text, and the box is
    // printed above every preflight line — so the read is now hoisted to the top of the run
    // and happens wherever the run ends. What is worth asserting is what still costs
    // something: ONE read (two would let the box and the hint disagree about a log the loop
    // appended to mid-run) and NO projection block on a path that accepted no queue.
    //
    // Nothing is lost by the move: this is the never-throws read every consumer of that file
    // shares, so an early return pays for one absent-file read and no line of output.
    for (const [label, overrides] of [
      ...
      expect(metricsReads(deps), label).toEqual([METRICS_PATH])
  ...
  it('reads it once on a run that aborts, and projects nothing', async () => {
    // Same change of premise as above, for the aborting paths: the identity box prints above
    // every guard, so its read happens before any of them can abort. The projection is what
    // must not appear — a forecast under a failure is a forecast of a run that is not
    // happening — and the read must still be exactly one.
    const aborts = {
...
describe('QA start box — whether the digest hint fires at all (#60)', () => {
  ...
  it('never fires for a repo running the SHIPPED ralph.config.sh', async () => {
    ...
    // BOTH placeholders, because `ralph init` substitutes both and this test's whole claim is
    // "the file a real user has". Leaving `{{RALPH_AGENT}}` in place made the box's claude row
    // come from `resolveAgent`'s fallback for an unrecognised value — and, since #118, earned a
    // spurious `⚠️  RALPH_AGENT='{{RALPH_AGENT}}' unrecognized` on stderr. `claude` is what init
    // writes for the default choice.
    const config = template
      .replace('{{TASK_SOURCE}}', 'github')
      .replace('{{RALPH_AGENT}}', 'claude')
    const deps = baseDeps({ config })
    await startCommand(deps)
    expect(box(deps)).toEqual([STARTED, ...PROJECTION, PROGRESS, ...EXISTING_TAIL])
  })
...
describe('QA start box — an unreadable ralph.config.sh changes nothing (#60)', () => {
  ...
  const broken = {
    ...
    // Three NULs as `\u0000` escapes, not as bytes (#107): a committed raw NUL makes
    // `file` call this suite `data`, which takes the whole file out of grep, rg and git
    // grep. What the config seam is handed is byte-identical.
