...
// a single `gh label create <in-progress label>` prefix in start.update-prompt.qa.test.js.
...
// module now says.
//
// #140 IS THE ONE EDIT THIS PIN HAS TAKEN, and it moved exactly two of its strings: the
// first two labels' NAMES, which dropped their `claude-` prefix. The colours, the descriptions,
// the flag order, the call order, the call count and the `{ reject: false }` are all still the
// pre-#139 bytes — which is the point, because a rename is supposed to change the name and
// nothing else. Updated by hand here rather than sourced from MANAGED_LABELS, for the reason in
// the paragraph above: this file's whole value is that it can disagree with the module.
//
...
// The three command lines as start.js spelled them before #139, with the two names #140
// renamed — name, then `--color`, then `--description`, one `await` each, in this order.
// Literals on purpose: see the header.
const EXPECTED_CREATE_ARGV = [
  ['label', 'create', 'in-progress', '--color', 'FFA500', '--description', 'Ralph loop in progress'],
  ['label', 'create', 'failed', '--color', 'B60205', '--description', 'Ralph loop tried and gave up'],
  [
    'label',
    'create',
    'pending-merge',
