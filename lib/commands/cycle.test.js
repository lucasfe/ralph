...
// ralph.sh normalizes any non-folder value to github, the queue still comes from
// `gh issue list`, and the work still lands as a GitHub PR — so a jira run IS a
// github run and needs github's preflight. The gate is spelled `!== 'folder'`
// rather than `=== 'github'` for exactly this reason: folder mode is the one that
// opted out of gh, and a new name must not opt out by accident.
//
...
describe('cycleCommand — the jira source still takes the github path (#125)', () => {
  ...
  it('runs gh auth status and counts the queue through gh issue list', async () => {
    const deps = jiraDeps({
      exec: makeExec({
        ...baseHandlers(),
        'gh issue list --search state:open -label:claude-working -label:claude-failed -label:do-not-ralph -label:pending-merge --limit 100 --json number -q . | length': {
