...
// start`'s launch box (start.js:1123) or `ralph status`'s advice block (status.js:252).
// No arguments and no flags: the repo decides which session, and there is only one.
//
// #169 CLOSED THE OTHER HALF OF THAT, in TWO SHAPES: `ralph start`'s launch box and its
// session-already-exists abort now print `ralph live` as the row's value with the tmux
// command on a continuation line under it (start.js:1123, start.js:754), while `ralph
// status` keeps that command parenthetically on the same row (status.js:252) — a bare second
// line in its key/value block would read as another key. Neither shape drops the raw
// command: `ralph start` names the session after its own cwd (start.js:720) and this one
// derives it from the git toplevel, so a subdirectory launch answers to the raw one (step 4).
//
...
// `stdio: 'inherit'` for the attach itself, the way ./cycle.js:683-690 spawns the bash
...
// `ralph start`'s ❌-on-stderr/hint-on-stdout split (start.js:746-755). One of the
...
export async function liveCommand({
  ...
  // destructuring trick ./start.js:134 and ./cycle.js:94 use, so injecting a stream is
  ...
  // (start.js:134) and escape sequences on `stdout` alone (start.js:169). `tmux attach`
  ...
  if (!hasCommand('tmux')) {
    ...
    // (doctor.js:413), and `ralph doctor` is where the install command for this platform
  ...
  if (alive && (processEnv.TMUX ?? '').trim() !== '') {
    ...
    // The same detach wording `ralph start`'s launch box prints (start.js:1124), so a
  ...
  if (!alive) {
    ...
    // hypothetical:
    //
    //   * `ralph start` launches under `sessionNameFor(cwd)` (start.js:720, spent at :1032),
    ...
    //     (ralph.sh:395-398), and status.js:310,317 reads `record?.session ||
    ...
    //     its own cwd-derived name (start.js:744), which by construction misses the loop in
