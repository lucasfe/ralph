...
// start`'s launch box (start.js:1087) or `ralph status`'s advice block (status.js:252).
...
// command on a continuation line under it (start.js:1087, start.js:718), while `ralph
...
// command: `ralph start` names the session after its own cwd (start.js:684) and this one
...
// `ralph start`'s ❌-on-stderr/hint-on-stdout split (start.js:710-719). One of the
...
export async function liveCommand({
  ...
  // (start.js:122) and escape sequences on `stdout` alone (start.js:157). `tmux attach`
  ...
  if (alive && (processEnv.TMUX ?? '').trim() !== '') {
    ...
    // The same detach wording `ralph start`'s launch box prints (start.js:1088), so a
  ...
  if (!alive) {
    ...
    // hypothetical:
    //
    //   * `ralph start` launches under `sessionNameFor(cwd)` (start.js:684, spent at :996),
    ...
    //     its own cwd-derived name (start.js:708), which by construction misses the loop in
