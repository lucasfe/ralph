...
// start`'s launch box (start.js:1081) or `ralph status`'s advice block (status.js:252).
...
// command on a continuation line under it (start.js:1081, start.js:713), while `ralph
...
// command: `ralph start` names the session after its own cwd (start.js:679) and this one
...
// `ralph start`'s ❌-on-stderr/hint-on-stdout split (start.js:705-714). One of the
...
export async function liveCommand({
  ...
  if (alive && (processEnv.TMUX ?? '').trim() !== '') {
    ...
    // The same detach wording `ralph start`'s launch box prints (start.js:1082), so a
  ...
  if (!alive) {
    ...
    // hypothetical:
    //
    //   * `ralph start` launches under `sessionNameFor(cwd)` (start.js:679, spent at :990),
