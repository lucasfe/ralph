...
describe('startCommand — no interval, no window (#62)', () => {
  ...
  it('leaves the launch box free of every digest line, as a run before #62 was', async () => {
    // AC#5: every repo on earth has this config today, so this path is the one #62 must
    // not add a character to. WAS "byte-identical to a run before #62", and #169 changed
    // the box under it — `Watch live:` now names `ralph live` with the tmux command on the
    // continuation line — so what this pins is the box as it stands MINUS a digest line.
    // The claim it is here for is the same one: no `Digest:` row, nothing else moved.
