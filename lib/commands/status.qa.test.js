...
describe('statusCommand — jira mode reads GitHub for titles and NEVER for the count (#126 QA)', () => {
  it('makes no --search call, and exactly one --state titles call while running', async () => {
    // The gate on the titles lookup is `source !== 'folder'`, and that is deliberate rather
    // than an oversight: only the COUNT moved to Jira. A jira run still branches, pushes and
    // opens a PR on GitHub and still works GitHub issues, so the task table's row labels
    // still come from `gh`. Folder mode is the one source that has no GitHub at all.
    const d = jiraConfig()
    await statusCommand(d)
