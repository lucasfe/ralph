...
describe('statusCommand — jira mode reads GitHub for titles and NEVER for the count (#126 QA)', () => {
  it('makes no --search call, and exactly one --state titles call while running', async () => {
    // The gate on the titles lookup is `source !== 'folder'`: folder mode is the one source
    // that has no GitHub at all. #127 landed Jira ticket selection and did not narrow it, so
    // this pins what a jira `ralph status` costs today — one `gh` list whose titles nothing
    // uses, since a Jira row is named by its key and a jira run records no closed rows. The
    // argument for leaving it (and the follow-up that removes it, with the gh gates in
    // `ralph start` and `ralph cycle`) is at the gate in status.js.
    const d = jiraConfig()
    await statusCommand(d)
    expect(queueCountsOf(d)).toBe(0)
    expect(d.exec.of('gh').filter((c) => c.args.includes('--state'))).toHaveLength(1)
...
describe('statusCommand — where JIRA_JQL comes from, and where it does not (#126 QA)', () => {
  ...
  it('IGNORES a JIRA_JQL in the environment — the query is a property of the repo', async () => {
    // Deliberately unlike TASK_SOURCE beside it. The template ships `JIRA_JQL=""` and the
    // loop sources that file with `set -a`, so every child of a run has the empty value
    // exported: an env fallback would read as "unconfigured" in this process and
    // "configured" in the next one. Nothing is spawned, and the row says unknown.
  ...
  const grammar = {
    ...
    'the empty value the template ships': [`TASK_SOURCE="jira"${LF_}JIRA_JQL=""${LF_}`, null],
