...
describe('statusCommand — a hostile tmux probe still yields a mode (#55 QA)', () => {
  ...
  it('only ever probes tmux once, and makes at most one gh call per purpose', async () => {
    ...
    // TWO gh calls since #56, and the budget is stated per PURPOSE rather than as a
    // total: one COUNT of the queue, one read of the task table's TITLES. Counting
    // them together would let a future third call hide behind a loosened total.
    const gh = d.exec.of('gh')
    expect(gh.filter((c) => c.args.includes('--search'))).toHaveLength(1)
    expect(gh.filter((c) => c.args.includes('--state'))).toHaveLength(1)
...
describe('statusCommand — folder mode makes ZERO gh calls (#55 QA)', () => {
  ...
  it('lets ralph.config.sh win over the environment (config github + env folder => gh)', async () => {
    ...
    await statusCommand(d)
    // The COUNT call specifically (#56 added a second gh call for the table's
    // titles): what this test is about is that the queue was counted through gh and
    // not through the folder scan.
    expect(d.exec.of('gh').filter((c) => c.args.includes('--search'))).toHaveLength(1)
...
describe('statusCommand — a wrecked record still renders and exits 0 (#55 QA)', () => {
  ...
  it('says "nothing in flight" rather than inventing a task when current is falsy but present', async () => {
    for (const current of [null, undefined, 0, '', false]) {
      ...
      await statusCommand(d)
      const out = d.stdout.output()
      const because = `current=${JSON.stringify(current)}`
      // #56 moved this reading onto the progress line, which also has to keep the
      // absent task OUT of the numerator: 0 done of the 6 waiting, not 0 of 7.
      expect(out, because).toContain('  progress   0/6 done · nothing in flight  [────────] 0%')
      // And no table over a run with no row to draw.
      expect(out, because).not.toContain('verdict')
    }
  })

  it('degrades the task row to "#?" with an unknown elapsed instead of dropping it', async () => {
    const d = deps({ readRunState: () => running({ current: 'issue 31' }) })
    await statusCommand(d)
    const out = d.stdout.output()
    // A `current` that is not the shape run-state writes still COUNTS as a task in
    // flight — a run mid-task must not read as a run between tasks — and every field
    // it could not supply says so rather than reading as zero (#56).
    expect(out).toContain('  progress   0/7 done · #? in flight (unknown)  [────────] 0%')
...
describe('statusCommand — a hostile metrics read still renders (#57 QA)', () => {
  ...
  it('keeps the three lines between the queue and the attach block, in that order', async () => {
    ...
    expect(lines.map((l) => l.trim().split(/\s{2,}/)[0])).toEqual([
      ...
      'progress',
      // The task table (#56) sits between the progress line and the queue, fenced by
      // blanks — the run's history, above the facts that are true right now.
      '',
      'task',
      '#029',
      '#030',
...
describe('reconcileMode / renderStatus — pure edges (#55 QA)', () => {
  ...
  // Thirteen and twelve since #56 replaced the `in flight` line with a progress line
  // and a task table: heading, progress, blank, table header, one row for the task in
  // flight, blank, queue, pace, eta, spend, blank, then the two-line tmux pair (or the
  // single restart hint when there is no session left to attach to).
  it('renders exactly thirteen lines in the live view and twelve in the interrupted one', () => {
    ...
    expect(interrupted.length).toBe(12)
