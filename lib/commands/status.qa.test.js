...
describe('reconcileMode / renderStatus — pure edges (#55 QA)', () => {
  ...
  it('renders thirteen lines in the live view and a card, with no tmux advice, when interrupted', () => {
    ...
    expect(interrupted).not.toContain(`  attach     ralph live  (tmux attach -t ${SESSION})`)
  ...
  it('never emits an attach hint for a session that is gone', () => {
    ...
    // #169 gave the attach row a second spelling of itself — `ralph live` ahead of the
    // tmux command — so "no attach hint" is now two tokens rather than one. `tmux` above
    // would catch today's row on its parenthetical alone, which is exactly the kind of
    // incidental coverage that stops holding the day the parenthetical moves.
...
describe('statusCommand — a hostile digest.log cannot change the view (#63 QA)', () => {
  ...
  it('keeps the attach pair reachable however much prose a model wrote', async () => {
    ...
    for (const [label, narrative] of Object.entries(narratives)) {
      ...
      expect(lines.slice(-2), label).toEqual([`  attach     ralph live  (tmux attach -t ${SESSION})`, '  kill       ralph stop'])
  ...
  it('keeps the attach pair reachable however many tasks the run has closed', async () => {
    ...
    expect(result.exitCode).toBe(0)
    const lines = d.stdout.lines()
    expect(lines.slice(-2)).toEqual([`  attach     ralph live  (tmux attach -t ${SESSION})`, '  kill       ralph stop'])
