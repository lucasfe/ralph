...
import { defaultRalphBinary } from '../paths.js'
import { InvalidDurationError, parseDuration } from '../duration.js'
import {
...
// #62: the GRAMMAR moved to ../duration.js so `30m` means 1800 seconds here, in
// ralph.config.sh, and in `ralph digest --loop` alike. This function is now a
// wrapper, and deliberately a total one: same accepted formats, same 4h default,
// same ScheduleAbort with the same exit code and the same message bytes, so a
// scheduled repo cannot tell the extraction happened.
//
// The DEFAULT stays here because it is launchd policy, not grammar: a plist's
// StartInterval has to be some number of seconds, so `ralph schedule install` with
// no --interval has to pick one. The digest has no such obligation and must not
// inherit an answer of 4h from a shared parser it merely borrows.
export function parseInterval(input) {
  if (input == null) return DEFAULT_INTERVAL_SECONDS
  try {
    return parseDuration(input)
  } catch (e) {
    // Re-thrown as this command's own abort, message untouched — the message text is
    // what a user reads and what schedule.test.js pins. Anything that is NOT a
    // duration complaint is a bug, not a bad interval, so it travels as-is rather
    // than being relabelled "invalid interval".
    if (e instanceof InvalidDurationError) throw new ScheduleAbort(e.message, 1)
    throw e
  }
}
...
export async function scheduleInstallCommand({
  ...
  if (platform !== 'mac') {
    err(`❌ ralph schedule only supports macOS (detected: ${platform}).`)
  ...
  // #4: warn only when notification creds resolve nowhere. Missing .env.local
  // is not enough — creds may live in the environment or the global config
  // (~/.config/ralph/.env), which the shell loop and heartbeat both honor.
  const repoEnv = loadEnvIfExists(exists, loadEnv, resolve(root, '.env.local'))
  const resolveCred = createCredentialResolver({ repoEnv, processEnv, home, loadEnv })
  if (!resolveCred('CALLMEBOT_KEY') || !resolveCred('WHATSAPP_PHONE')) {
    out(
      'ℹ️  No WhatsApp/healthcheck creds found in .env.local, the environment, or ~/.config/ralph/.env — notifications will be skipped at runtime.',
    )
  }
  ...
  // #51: the plist's EnvironmentVariables dict is the only environment a launchd
  // run ever sees — no login shell, so no .zshrc, no .zprofile, and no `export`
  // you typed in a terminal an hour ago. PATH has always been forwarded for
  // exactly that reason; #51 is what made RALPH_NO_UPDATE_CHECK matter to
  // `ralph cycle` too (it now runs the update gate inside its lock), so the
  // documented off-switch has to travel the same road or it cannot reach the six
  // scheduled cycles a day it exists to silence. Same snapshot semantics as
  // PATH: this is captured now, at install time, so changing your mind means
  // re-running `ralph schedule install --force`. Only forwarded when actually
  // set to a non-empty value — isUpdateCheckDisabled() reads '' as "keep
  // checking" anyway, so an empty key would be noise, and an install on a
  // machine that never set it writes the same plist it wrote before this slice.
  if (processEnv.RALPH_NO_UPDATE_CHECK) {
    baseEnv.RALPH_NO_UPDATE_CHECK = processEnv.RALPH_NO_UPDATE_CHECK
  }
...
export async function scheduleHeartbeatCommand({
  ...
  const heartbeatTimeStr = resolveCred('RALPH_DAILY_SUMMARY_TIME') ?? DEFAULT_HEARTBEAT_TIME
