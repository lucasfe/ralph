...
// already local: GH_REPO if anything set it, otherwise origin's url out of `.git/config`. Both
// are read by the caller — see `bannerRepoSlug` in lib/commands/start.js — and arrive here as
// strings. WHERE GH_REPO came from is the caller's business too and has never been visible here:
// since #120 it is `ralph.config.sh` over the process environment, matching the loop, which
// sources that file with `set -a`. The grammar below is unchanged by that and would be unchanged
// by the next such decision, which is the point of the seam.
//
 ...
 *
 * @param {object} [input]
 * @param {string} [input.ghRepo] GH_REPO, as the caller resolved it — `ralph.config.sh` over
 *   the process environment, the same way round as the loop. gh's own spelling is
...
export function resolveBannerRepo(input) {
  ...
  // variable reads to gh too, which is why the caller may hand a `GH_REPO=""` from
  // `ralph.config.sh` straight through and get the origin row the loop will honour (#120).
