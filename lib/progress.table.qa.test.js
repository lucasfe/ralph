...
import { buildProgress, renderProgressLine, renderTaskTable, toJsonSnapshot } from './progress.js'
...
describe('toJsonSnapshot — the machine contract #127 did NOT change (#127 QA)', () => {
  it('keeps tasks.current at exactly {number, started_at}', () => {
    // Pinned deliberately: `--json` is what a script reads, and the key is a rendering
    // concern. Adding `task_key` here would be a schema change for every consumer, so it must
    // be a decision somebody makes on purpose rather than a field that leaks in.
    const snapshot = keyed('FOO-9')
    const document = toJsonSnapshot(snapshot, {
      mode: 'run',
      record: { run_id: RUN, current: { ...IN_FLIGHT, task_key: 'FOO-9' } },
    })
    expect(Object.keys(document.tasks.current)).toEqual(['number', 'started_at'])
    expect(document.tasks.current.number).toBe(31)
    // The key is absent, and so is any spelling of it.
    expect(JSON.stringify(document)).not.toContain('FOO-9')
  })
})
...
describe('a hybrid log names each row from its OWN source (#131 QA)', () => {
  ...
  it('titles the github row from the map and refuses to title the ticket from it', () => {
    // The whole collision in one assertion: the same `123` is looked up for one row and
    // withheld from the other, because `titles` is `gh issue list --json number,title` and
    // knows nothing about a board.
  ...
  it('still uses the title the EVENT carries on a keyed row', () => {
    // Only the injected map is withheld from a keyed row. Nothing writes `event.title` today,
    // so this is the forward-compatible half: a slice that starts recording the Jira summary
    // needs no change in buildTaskRows.
    const snapshot = buildProgress({
