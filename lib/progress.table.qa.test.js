...
describe('toJsonSnapshot — the machine contract, changed ON PURPOSE by #132 (#132 QA)', () => {
  ...
  it('publishes tasks.current as exactly {number, started_at, task_key, worktree}', () => {
    const document = projected('FOO-9')
    // `worktree` (#222) was appended after the key; this fixture records none, so it is
    // present-and-null here for the same reason the key is on a github run.
    expect(Object.keys(document.tasks.current)).toEqual([
      'number',
      'started_at',
      'task_key',
      'worktree',
  ...
  it('leaves a GitHub run’s document identical to the one #58 published', () => {
    ...
    expect(document.tasks.current).toEqual({
      ...
      task_key: null,
      // ...and #222's leaf reads the same way for a record that predates it, which is the
      // only reason a field appended to a published document is safe to append.
      worktree: null,
    })
    expect(JSON.stringify(document)).toContain('"task_key":null')
    expect(JSON.stringify(document)).toContain('"worktree":null')
  })
})
