...
// defaulted to `npm` — every run in this file leaves `latestSource` unset, and that parameter
// defaults to NPM_VERSION_QUERY, so npm is both the channel these runs ask and the one that
// would have written what the seeds pretend was written. (Leaving it unset is NOT the same as a
// channel lookup that failed, which reads nothing — lib/update-check.cache-channel.test.js pins
// the pair apart.) Without the stamp the throttled path serves nothing and #24's "reuses the
// cached version" contract could not be stated. The spread is last so a test can spell its own.
...
describe('resolveUpdateDecision — 7-day network throttle', () => {
  it('queries npm and stamps last_check_at when the cache is empty', async () => {
    ...
    expect(result.updatedCache).toEqual({
      ...
      // #215: the query the run just spawned names its own channel, so the number is filed under
      // it. This is the write half of the fix — no reader can pick a version up unless the writer
      // said who answered it.
...
describe('resolveUpdateDecision — contracts', () => {
  ...
  // #26: shouldPrompt is isNewer AND an open last_prompted_at window, on the
  // network path and the throttled cache path alike. The window itself — the
  // skew rule, the two windows in isolation and in combination, and the
  // recordPromptShown stamp — is owned by update-check.prompt-window.test.js;
  // this pins only that the field is live on both paths and not hardcoded.
  it('prompts on both the network and the cache path when nothing has been prompted', async () => {
    ...
    expect(cached.shouldPrompt).toBe(true)
  })

  it('stops prompting inside the prompt window, on both paths', async () => {
    const window = (lastCheckAt) =>
      resolveUpdateDecision(
        base({
          exec: makeExec(),
          fs: seeded({
            last_check_at: lastCheckAt,
            last_prompted_at: new Date(T0 - 2 * DAY).toISOString(),
            latest_version: '0.2.0',
          }),
        }),
      )
    const network = await window(new Date(T0 - 30 * DAY).toISOString())
    expect(network.source).toBe('network')
    expect(network.isNewer).toBe(true)
    expect(network.shouldPrompt).toBe(false)
    const cached = await window(new Date(T0 - DAY).toISOString())
    expect(cached.source).toBe('cache')
