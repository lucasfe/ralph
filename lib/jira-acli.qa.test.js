...
//
//   THE SEVEN BUILDERS AS A SET, AND WHICH OF THEM ARE WRITES. This file's own header says
...
//   it." That is only true of a write some verb already makes: an EIGHTH builder — added
...
describe('jira-acli — the seven builders as a SET, and which of them are writes (#129 QA)', () => {
  it('exports exactly seven argv builders — the count its own header claims', () => {
    // MEASURED against the module rather than transcribed from the prose: the header says
    // "seven argv builders above" and "there are seven invocations and four of them are
    // WRITES", and both numbers are now assertions. An eighth builder fails this test, which
    // is the point — it is the moment somebody has to decide whether it is a read or a write
    // and whether the sweep below covers it.
  ...
  it('four of the seven are WRITES, and every one of them carries --yes LAST', () => {
  ...
  it('names the ticket with --key on every per-ticket builder, and never with a bare argument', () => {
    // The two searches take a query; the other five take a KEY, and all five spell it the
