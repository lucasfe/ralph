...
describe('renderSplashFrames — every frame, or nothing (#73)', () => {
  const splash = (options) => renderSplashFrames(options)
  const enabled = { isTTY: true, color: true }

  it('renders every frame in the asset, each with its own delay', () => {
    // The animation's INPUT, not the animation: rendered lines and a number of
    // milliseconds, so lib/sprite-player.js can be handed a data structure and this
    // module can stay a pure function of the capabilities.
    const rendered = splash(enabled)
    expect(rendered).toHaveLength(frames.length)
    for (const [index, frame] of rendered.entries()) {
      expect(frame.lines, `frame ${index}`).toHaveLength(17)
      for (const line of frame.lines) expect(cellCount(line), `frame ${index}`).toBe(26)
      expect(frame.delayMs, `frame ${index}`).toBeGreaterThan(0)
    }
  })

  it('starts at the static frame, so the splash begins and ends on the poster frame', () => {
    // The ORDER is the decision: the sequence is rotated to begin at
    // STATIC_FRAME_INDEX rather than at the asset's index 0, so that "what the
    // animation settles on" and "what an unanimated banner shows" are the same
    // sentence rather than two facts that happen to agree today.
    expect(splash(enabled)[0].lines).toEqual(renderStaticBanner(enabled))
    expect(splash(enabled).map((frame) => frame.delayMs)).toEqual(
      frames.map((_, index) => frames[(STATIC_FRAME_INDEX + index) % frames.length].delayMs),
    )
    for (const [index, frame] of splash(enabled).entries()) {
      const source = frames[(STATIC_FRAME_INDEX + index) % frames.length]
      expect(frame.lines, `frame ${index}`).toEqual(renderSprite({ palette, rows: source.rows }))
    }
  })

  it('renders frames that differ — the animation has something to animate', () => {
    // Cheap, and it has caught a generator bug before: two identical frames would make
    // the splash a one-second pause with no motion in it.
    const [first, second] = splash(enabled)
    expect(second).toBeDefined()
    expect(second.lines).not.toEqual(first.lines)
  })

  it('answers the same three gates the static banner answers, with the same silence', () => {
    // ONE gate for both entry points, because two copies of it are two answers the day
    // one of them moves: a run where the sprite was suppressed but the splash was not
    // would write five frames of escapes into a piped log.
    for (const gate of [
      { isTTY: false, color: true },
      { isTTY: true, color: false },
      { isTTY: false, color: false },
      ...
      expect(splash(gate), JSON.stringify(gate) ?? 'nothing').toEqual([])
    }
    for (const width of [25, 20, 12, 1]) {
      expect(splash({ ...enabled, width }), String(width)).toEqual([])
    }
    for (const width of [26, 27, 44, 60, 200]) {
      expect(splash({ ...enabled, width }), String(width)).toHaveLength(frames.length)
    }
    for (const width of [undefined, null, 0, Number.NaN, '80', {}]) {
      expect(splash({ ...enabled, width }), JSON.stringify(width) ?? String(width)).toHaveLength(
        frames.length,
      )
    }
  })
  ...
  it('emits no escape sequence at all in any suppressed case', () => {
    for (const gate of [
      { isTTY: false, color: true },
      { isTTY: true, color: false },
      { isTTY: true, color: true, width: 10 },
    ]) {
      const written = splash(gate)
  ...
  it('hands out fresh arrays, never the asset’s own rows', () => {
    // The player writes what it is given and the caller may keep it; neither should be
    // able to mutate the generated module out from under the next run.
    const first = splash(enabled)
    const second = splash(enabled)
    expect(first).not.toBe(second)
    expect(first[0].lines).not.toBe(second[0].lines)
    expect(first[0].lines).not.toBe(frames[STATIC_FRAME_INDEX].rows)
...
describe('sprite-banner — purity', () => {
  it('reads no clock, no environment and no filesystem', () => {
    ...
    // same argument banner-rows.js makes for importing update-check.js: the width at
