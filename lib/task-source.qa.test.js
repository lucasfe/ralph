...
import { resolveSource, worksThroughGitHub, VALID_SOURCES, DEFAULT_SOURCE } from './task-source.js'
...
// ===========================================================================
// QA augmentation for #131 — `worksThroughGitHub`, the ALLOWLIST OF ONE.
// ===========================================================================
//
// #131 replaced two `source !== 'folder'` gates in lib/capture-issue-event.js with this
// predicate, so it decides which iterations read a verdict off issue labels and which pay
// for a `gh pr` subprocess. MEASURED: it has no direct test of its own anywhere else in the
// suite (`grep -rn worksThroughGitHub lib/ test/` finds the definition, its two call sites in
// lib/capture-issue-event.js — the verdict source and the PR-diff fetch — and this file),
// which is what this block is for.
//
// THE COMPOSITION IS THE RISK, not the one-line body. The predicate takes a RESOLVED source,
// so its answer for an unrecognised TASK_SOURCE depends on `resolveSource` having already
// turned it into 'github' — and if the two are ever read in the other order, a typo stops
// looking like github mode. Both properties are pinned below.
describe('worksThroughGitHub — the allowlist of one (#131 QA)', () => {
  it('answers true for github and false for the other two members', () => {
    expect(worksThroughGitHub('github')).toBe(true)
    expect(worksThroughGitHub('folder')).toBe(false)
    expect(worksThroughGitHub('jira')).toBe(false)
  })

  it('is true for exactly one member of VALID_SOURCES', () => {
    // The property the "allowlist of one" name claims, asserted over the registry rather
    // than over three literals: a fourth source added to that list gets the non-github
    // answer by default, and this test is what says so on the day it appears.
    expect(VALID_SOURCES.filter((source) => worksThroughGitHub(source))).toEqual(['github'])
  })

  it('composed with resolveSource, every unrecognised value keeps github behaviour', () => {
    for (const value of ['gitlab', 'jirah', 'FOLDERS', '', '   ', undefined, null, 123, {}]) {
      expect(worksThroughGitHub(resolveSource({ TASK_SOURCE: value })), String(value)).toBe(true)
    }
  })

  it('composed with resolveSource, every spelling of the two non-github names opts OUT', () => {
    for (const value of ['folder', 'FOLDER', ' Folder\n', 'jira', 'JIRA', ' Jira\t']) {
      expect(worksThroughGitHub(resolveSource({ TASK_SOURCE: value })), value).toBe(false)
    }
  })

  it('demands a RESOLVED source: an unnormalized spelling of github answers FALSE', () => {
    // Not a defect — the docblock says it takes `resolveSource`'s output, and both
    // production call sites pass exactly that — but it is the trap a third call site would
    // fall into, since the wrong answer here is the SAFE-looking one (no `gh` call) and
    // costs a github run its verdict rather than crashing.
    for (const raw of ['GitHub', ' github', 'github ', 'GITHUB']) {
      expect(worksThroughGitHub(raw), raw).toBe(false)
    }
  })

  it('never throws for a non-string, and answers false', () => {
    for (const value of [undefined, null, 0, 1, true, {}, [], Symbol('github')]) {
      expect(() => worksThroughGitHub(value)).not.toThrow()
      expect(worksThroughGitHub(value)).toBe(false)
    }
  })
})
