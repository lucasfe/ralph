...
//     mode. That is lib/banner-mode.js (#74). What this module offers it is `cycles`: a
//     count of 1 is already, byte for byte, the unanimated banner #67 shipped — no hide, no
//     restore, no cursor movement at all — so the mode resolver adds nothing here but a
//     number, and `static` is that number and no other change to this file.
...
// nobody wants; the fallback answers it with the banner. NO SHIPPED CALLER REACHES IT: #74
// was the one expected to, and its resolver answers with a MODE rather than a count — `ralph
// start` spends that as a `cycles` of 1 or the default — so a typo in RALPH_BANNER can no
// more ask for a million frames than it can ask for two. The ceiling guards the programmatic
// caller that assembles its own count, which is the reason it stays. See SPLASH_MAX_FRAMES.
//
...
// the beat the settled frame holds before whatever its caller writes next: since #161 that is
// the identity box on a terminal too narrow to fit the box beside the picture, and the run's
// own first line on one wide enough that the box came up inside the frames), and four moves
 ...
 *
 * Exported for the specs, and for a caller that assembles its own frame list and wants to
 * check it against the same number rather than discovering the limit by watching a count
 * silently become five. NOTHING IN THE CLI REACHES IT TODAY, and that is worth saying plainly
 * rather than leaving as an implication: `ralph start` plays either the rendered splash or no
 * frames at all, and #74's mode resolver returns a MODE — `full`, `static` or `off` — never a
 * count, so the ceiling is unreachable from any invocation. It guards the programmatic caller
 * and the future one, not the command.
 *
...
export async function playSplash({
  ...
  // and what #74's `static` mode is built out of.
