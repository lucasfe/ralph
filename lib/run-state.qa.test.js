...
describe('the record’s shape is this module’s property (#127 QA)', () => {
  it('writes exactly five fields inside `current`, in one order', () => {
    ...
    //
    // UPDATED by #222, which appended `worktree` — the directory the in-flight task is
    // being done in. Appended rather than inserted, and named here rather than left to the
    // tests that are about it, because this is the assertion that says what the record IS.
    const v = vol()
    beginTask(ROOT, { number: '', iteration: '1', taskKey: 'FOO-9' }, v)
    expect(Object.keys(onDisk(v).current)).toEqual([
      'number',
      'task_key',
      'started_at',
      'iteration',
