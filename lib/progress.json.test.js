...
describe('toJsonSnapshot — the schema is the contract (#58)', () => {
  it('emits the mode discriminator, the run id and the five sections, and nothing else', () => {
    const doc = project()
  ...
  it('pins the keys of every section', () => {
    ...
    expect(Object.keys(doc.tasks.current)).toEqual(['number', 'started_at'])
...
describe('toJsonSnapshot — one shape for all four modes (#58)', () => {
  const KEYS = ['mode', 'run_id', 'progress', 'tasks', 'pace', 'eta', 'spend']
