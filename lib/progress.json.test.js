...
describe('toJsonSnapshot — the schema is the contract (#58)', () => {
  ...
  it('pins the keys of every section', () => {
    ...
    // `task_key` (#132) and then `worktree` (#222) are appended for the same reason
    // `digest` was: the document grows at the end, so a consumer reading it positionally
    // is undisturbed.
  ...
  it('projects the issue’s worked example, section by section', () => {
    ...
    expect(doc.tasks.current).toEqual({
      ...
      task_key: null,
      // Same rule for the worktree (#222): the example's record predates the field, and
      // a record that never carried it publishes null rather than dropping the key.
      worktree: null,
    })
