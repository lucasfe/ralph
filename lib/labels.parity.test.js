...
// names, and 11 of those mentions are label-shaped (`-label:` or a `--…-label` flag) — plus one
// verbatim copy of the whole search query, so a rename that skipped it would leave the
// documented remediation instructions doing nothing. CONTRIBUTING.md is listed for
// `do-not-ralph`, which it spells 5 times: the Jira lane's restriction, plus the convention
// section that names this module as the only JavaScript allowed to spell a label. That section
// deliberately refers to the OTHER names by their export identifiers (`IN_PROGRESS_LABEL`,
// `FAILED_LABEL`, `PENDING_MERGE_LABEL`) rather than by their words — a habit worth keeping for
// its own sake, though since #140 the negative half no longer enforces it for the two renamed
// names, because CONTRIBUTING.md now writes `in-progress` and `failed` about the folder and
// Jira lanes for reasons that have nothing to do with a GitHub label.
...
// ---------------------------------------------------------------------------
//
// The acceptance criterion of #140: no source file, template or test outside the legacy
// mapping and the CHANGELOG may still spell either retired name. Asked here rather than in the
...
describe('the rename reached every tracked file (#140)', () => {
  ...
  it('no tracked file outside the mapping, its spec and the CHANGELOG spells a retired name', () => {
    expect(
  ...
  it('and every allowlisted file really does still carry one', () => {
    ...
    // only legitimate while the file needs it, so a `lib/labels.js` that lost its mapping — or a
    // CHANGELOG.md whose history somebody "tidied" — goes red here.
