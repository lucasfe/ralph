...
//     subtype,             // 'success' | error subtype | null
//     num_turns,           // integer
...
// the token sum), pinned by tests migrated from issue-event.test.js. The Codex
...
function parseCodex(rawStream) {
  ...
  let subtype = null
  if (failed) subtype = 'error'
  else if (completedTurns > 0) subtype = 'success'
