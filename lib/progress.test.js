...
// A run that is over is not a run with no history — it is a run with a complete one.
// `runAlive: false` is the input that separates the two, and #59's review is where it
// came from: `ralph status --json` on an interrupted repo was naming the task the dead
// run was "working on" and predicting when the queue would empty, beside a terminal
// report card saying the run was over.
describe('buildProgress — a dead run measures, and predicts nothing (#59 review)', () => {
  it('keeps every measurement and nulls every prediction', () => {
    const dead = worked({ runAlive: false })
    // MEASURED — the two tasks really ran, at that pace, for that money.
    expect(dead.completed).toBe(2)
    expect(dead.remaining).toBe(6)
    expect(dead.paceMs).toBe(84 * MIN)
    expect(dead.paceBasis).toBe('last3-in-run')
    expect(dead.samples).toBe(2)
    expect(dead.spendUsd).toBeCloseTo(62.85, 5)
    expect(dead.costPerTaskUsd).toBeCloseTo(31.425, 5)
    // INVENTED — nothing is in flight and there is no future to extrapolate.
    expect(dead.inFlight).toBe(0)
    expect(dead.etaMs).toBe(null)
    expect(dead.finishAt).toBe(null)
    expect(dead.spreadMs).toBe(null)
    expect(dead.projectedUsd).toBe(null)
  })

  it('counts the record’s surviving `current` as no task in flight', () => {
    // run-state's endRun and a hard kill both leave `current` on the record — it is the
    // last task the run worked on, not a task running now. This is the field that made
    // the two surfaces disagree.
    expect(worked().inFlight).toBe(1)
    expect(worked({ runAlive: false }).inFlight).toBe(0)
    // ...and the denominator stays a fact, one smaller.
    expect(worked().total).toBe(9)
    expect(worked({ runAlive: false }).total).toBe(8)
  })

  it('defaults to a live run, so a caller that never asked is unaffected', () => {
    // Every caller before #59 only ever asked about a run in flight, so the default is
    // the reading they were already getting — this file's other 40 tests included.
    expect(buildProgress({ metricsText: WORKED_EXAMPLE, record: inFlightRecord(), queue: 6, now: NOW })).toEqual(
      worked({ runAlive: true }),
    )
  })

  it('renders the pace and the spend it measured, with no eta line to fill', () => {
    expect(renderProgress(worked({ runAlive: false }))).toEqual([
      '  pace       ~84 min/task · $31.4/task',
      '  eta        unknown',
      '  spend      $62.85 so far',
    ])
...
describe('formatClock — no caller can be handed a NaN clock (#59 review)', () => {
  it('reads a finite instant beyond the calendar as unspellable, not as NaN:NaN', () => {
    // `Date` holds ±8.64e15 ms; past that `new Date(ms)` is an Invalid Date and every
    // getter answers NaN. These all pass a finiteness check, which is why the guard is
    // the CALENDAR's — shared with the document's own instants and with the report card.
    for (const ms of [8.7e15, -8.7e15, 1e300, -1e300, Number.MAX_VALUE, 253402300800000]) {
      expect(formatClock(ms), String(ms)).toBe('--:--')
    }
  })

  it('still formats the instants either side of the boundary', () => {
    // The guard is a calendar check, not a magnitude the formatter refuses: the last
    // instant of year 9999 is a wall-clock reading like any other.
    expect(formatClock(253402300799999)).toMatch(/^\d\d:\d\d$/)
    expect(formatClock(NOW)).toBe('19:32')
    expect(formatClock(0)).toMatch(/^\d\d:\d\d$/)
  })

  it('keeps saying --:-- for the absences it always did', () => {
    for (const ms of [null, undefined, NaN, Infinity, -Infinity, '19:32', {}]) {
      expect(formatClock(ms), JSON.stringify(ms) ?? 'undefined').toBe('--:--')
    }
  })
})
