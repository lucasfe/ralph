...
describe('the exclusion after `done` joined it (#129 QA)', () => {
  it('names exactly four labels, and the THREE labels Ralph WRITES come from the constants', () => {
    ...
    //
    // `failed` BECAME THE THIRD CONSTANT IN #130, and until then it was spelled here as a
    // transcribed literal — which is precisely the drift the paragraph above forbids, left in
    // place only because the word named a sweep no code performed yet. Now that the sweep
    // writes `JIRA_FAILED_LABEL`, the literal is gone from this file: a rename that reached the
    // constant and not the clause reddens this row rather than stranding a swept ticket in the
    // queue forever, which is the same failure `done` has and one paid invocation per pass.
    ...
    // two spellings would not. `do-not-ralph` stays a literal on purpose — a HUMAN writes that
    // one, so no constant of Ralph's can be the thing it has to agree with.
    ...
    expect(occurrences(JIRA_LABEL_EXCLUSION, JIRA_FAILED_LABEL)).toBe(1)
  })
