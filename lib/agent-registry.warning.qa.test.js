...
describe('QA #108 — the structure that makes "one place, at the source" true', () => {
  ...
  it('names the whole set of modules that touch the warning: the resolver and its two printers', () => {
    // The set criterion 3 is about, pinned as a SET rather than as a property of each member —
    // that the two printers print it safely is what the doctor and init specs drive end to end,
    // and what this sweep adds is that there is no THIRD printer they missed. Every other caller
    // destructures `{ agent }` only, so the warning reaches a terminal from exactly two places —
    // `ralph doctor` and `ralph init` — and those two are the ones #108 names; the resolver
    // itself is in the list because it is where the word `warning` is written. A fourth entry
    // appearing here is not a failure in itself; it is a demand that whoever added it also added
    // a line-count test for their printer.
    const consumers = FILES.filter((path) => /resolveAgent\(/.test(codeWithoutComments(path)))
      .filter((path) => /\bwarning\b/.test(codeWithoutComments(path)))
      .map((path) => path.slice(LIB.length))
      .sort()
    expect(consumers).toEqual(['agent-registry.js', 'commands/doctor.js', 'commands/init.js'])
  })
