...
// in-progress, and a sweep scoped to open issues can never see it — which is
...
const LIST_KEY =
  'gh issue list --state all --label in-progress --limit 100 --json number,title,updatedAt'
...
describe('cleanupOrphans', () => {
  ...
  it('removes the in-progress label from each orphan and returns the cleared numbers', async () => {
    const exec = makeExec({
      'gh issue edit 12 --remove-label in-progress': { exitCode: 0, stdout: '', stderr: '' },
      'gh issue edit 34 --remove-label in-progress': { exitCode: 0, stdout: '', stderr: '' },
    })
    ...
    expect(exec.calls.map((c) => c.key)).toEqual([
      'gh issue edit 12 --remove-label in-progress',
      'gh issue edit 34 --remove-label in-progress',
    ])
  })
...
// in-progress survives on a successfully resolved (CLOSED) issue. A sweep
// scoped to `--state open` cannot see those, so the backlog can only grow.
// ---------------------------------------------------------------------------
describe('findOrphans — CLOSED issues holding in-progress (#40)', () => {
  it('lists issues in ANY state, so a CLOSED issue holding the label is visible', async () => {
    ...
    expect(ghCall.args).toContain('in-progress')
  })
  ...
  it('returns a CLOSED orphan and cleanupOrphans clears it (backlog repaired, not just prevented)', async () => {
    ...
    expect(exec.calls.map((c) => c.key)).toEqual([
      LIST_KEY,
      'gh issue edit 22 --remove-label in-progress',
      'gh issue edit 27 --remove-label in-progress',
    ])
  })
...
describe('QA: findOrphans — adversarial payloads (#40)', () => {
  it('returns a MIXED open+closed page verbatim and in page order (state filtering is gh\'s job, not ours)', async () => {
    ...
    // in-progress` is by definition an orphan. Pin that no client-side state
...
describe('QA: cleanupOrphans — adversarial (#40)', () => {
  it('partial success on a CLOSED backlog: the failing number is skipped, the rest still clear', async () => {
    ...
    const exec = makeExec({
      ...
      'gh issue edit 7 --remove-label in-progress': {
        exitCode: 1,
        stdout: '',
    ...
    expect(exec.calls.map((c) => c.key)).toEqual([
      LIST_KEY,
      'gh issue edit 91 --remove-label in-progress',
      'gh issue edit 7 --remove-label in-progress',
      'gh issue edit 55 --remove-label in-progress',
    ])
    expect(log.messages.join('\n')).toMatch(/403|#7/)
  })
  ...
  it('never calls gh for malformed orphan entries', async () => {
    ...
    expect(exec.calls.map((c) => c.key)).toEqual(['gh issue edit 12 --remove-label in-progress'])
  })
