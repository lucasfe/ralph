...
describe('both roots are substituted as literal text, never re-scanned (#221 QA)', () => {
  it.each([
    ...
    // The fallback only has to be a string no render can contain, which is why it is a NUL
    // — but SPELLED, not typed. Written as a literal it put a raw U+0000 in this file, git
    // then classified the file as binary, and #107's control-byte sweep went red on main
    // (it reads `git ls-files`, so an untracked file is invisible to it and a local run of
    // the whole suite passes right up until the commit lands).
    expect(out).not.toContain(process.env.HOME ?? `${String.fromCharCode(0)}never`)
  })
