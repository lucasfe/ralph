    ...
    it(`draws the row list verbatim, in order, at every width — ${name}`, () => {
      ...
      for (const width of WIDTHS) {
        ...
        expect(bannerLayout(width), String(width)).toEqual({
          ...
          boxed,
          sprite: limit >= 26,
          // #161's rung, in the same spirit as the two above it: 26 cells of sprite and two
          // of air taken off the terminal, and what is left is a box's width if it is still
          // 44. Nothing on this side of the seam changes with it — the rows are the rows —
          // which is exactly why it is stated here rather than left out of the comparison.
          beside: limit - 26 - 2 >= 44,
          besideWidth: Math.min(Math.max(0, limit - 26 - 2), BANNER_WIDTH),
        })
