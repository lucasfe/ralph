...
//   lastTaskKey    — that task's Jira key (#127) when the run recorded one, or null
...
export function buildPostMortem({ metricsText, record, queue, now } = {}) {
  ...
  return {
    ...
    // The KEY beside the number, because for a Jira run the number is derived from the
    // key (`FOO-123` → 123) and derived numbers are not names: `#123` is a task nobody
    // can look up, and in a repo that also has GitHub issues it reads as issue #123.
    // Both fields are carried rather than one resolved string, so the snapshot stays a
    // record of what was WRITTEN and the choice between them stays in the renderer.
    // Scrubbed by the same boundary the live view uses (progress.js taskKeyOf): this
    // string came off acli's stdout through bash and a file.
    lastTaskKey: taskKeyOf(record?.current),
  ...
  return [
    ...
    ...(interrupted
      ? [
          ...
          // THE KEY WINS OVER THE NUMBER (#127), and it wins even when there is a number
          // to print: under `TASK_SOURCE=jira` the number was DERIVED from the key, so
          // the two never disagree about which ticket this was — one of them is just the
          // only one a reader can act on. `unknown` is now reserved for a record that
          // named the task neither way, which is what it always meant.
          row('last task', [
            lastTaskKey ??
