...
describe('toJsonSnapshot — the document names the ticket in flight (#132)', () => {
  ...
  it('publishes `task_key` beside the number, as the last key of tasks.current', () => {
    // LAST, because a document grows at the end: a consumer reading positionally (and jq
    // users do) sees the two fields it always saw, then the new one.
  ...
  it('says `null` for every task that has no key — every github and folder run', () => {
    ...
    expect(Object.keys(github.tasks.current)).toEqual(['number', 'started_at', 'task_key'])
