...
describe('the in-flight task is named by its Jira key when it has one (#127)', () => {
  ...
  it('carries the key on the in-flight row, and on no closed row this run recorded', () => {
    // The closed rows here are the worked example's GITHUB iterations, which have no key to
    // carry. A closed row CAN be keyed since #131 — when the event itself recorded one; that
    // is the describe below, and this fixture is what makes the two readable apart.
...
// ---------------------------------------------------------------------------
// #131 — A JIRA ITERATION NOW RECORDS AN EVENT, so a CLOSED row can be a ticket too. The
// event lib/capture-issue-event.js appends under TASK_SOURCE=jira carries `task_key` beside
// the number derived from it — the same field name lib/run-state.js writes for the task in
// flight, which is why one `taskKeyOf` reads both. Before #131 the jira arm appended
// nothing, so every closed row this file measures came from a run whose tasks had no key.
//
// The two properties are the ones the in-flight row already owed a reader, asked of a row
// that has FINISHED: the ticket NAMES it, and the numeric titles map does not TITLE it.
// ---------------------------------------------------------------------------
describe('a closed row is named by the Jira key its event recorded (#131)', () => {
  const KEY = 'FOO-123'

  // Two recorded jira iterations, shaped as lib/issue-event.js builds them: the key, and
  // the number it was derived from (`FOO-123` → 123).
  const JIRA_RUN = jsonl(
    event({ number: 123, task_key: KEY, minutes: 97, cost: 34.1, ts: 1 }),
    event({ number: 124, task_key: 'FOO-124', minutes: 71, cost: 28.75, ts: 2, verdict: 'fail' }),
  )
  const closed = (overrides = {}) => worked({ metricsText: JIRA_RUN, ...overrides })

  it('carries the key on each closed row, beside the derived number', () => {
    expect(closed().tasks.slice(0, -1)).toMatchObject([
      { number: 123, key: KEY, inFlight: false },
      { number: 124, key: 'FOO-124', inFlight: false },
    ])
  })

  it('names the ticket in the task cell instead of the derived number', () => {
    const lines = renderTaskTable(closed())
    expect(lines[1]).toContain(KEY)
    expect(lines[2]).toContain('FOO-124')
    // `#123` is the number Ralph derived: it names nothing on the board, and in a repo that
    // also has GitHub issues it names somebody else's issue.
    expect(lines.join('\n')).not.toContain('#123')
    expect(aligned(lines), lines.join('\n')).toBe(true)
  })

  it('does NOT title a keyed closed row from the numeric titles map', () => {
    // The same collision the in-flight row is guarded against, now reachable from a
    // recorded row: `titles` comes from `gh issue list --json number,title`.
    const titles = { 123: 'a different repository’s issue' }
    expect(closed({ titles }).tasks[0].title).toBe(null)
    expect(renderTaskTable(closed({ titles })).join('\n')).not.toContain('different repository')
  })

  it('scrubs a recorded key exactly as it scrubs the in-flight one', () => {
    // The event is a file this view merely reads, and the key inside it was printed by acli
    // and passed through bash — so the trust boundary is the record's, byte for byte.
    const hostile = `FOO${ESC}[31m-9${BEL}${LF}  #999 pass`
    const lines = renderTaskTable(
      worked({ metricsText: jsonl(event({ number: 9, task_key: hostile, minutes: 5, cost: 1, ts: 1 })) }),
    )
    expect(lines.join('\n')).not.toContain(ESC)
    expect(lines.join('\n')).not.toContain(BEL)
    // Header, the one closed row, the in-flight row: a forged row cannot arrive through a
    // recorded key either.
    expect(lines).toHaveLength(3)
  })

  it('leaves a github run’s rows exactly as #56 rendered them', () => {
    // Zero regression, stated where it can fail: an event with no `task_key` is every event
    // written before #131, and the whole table is compared rather than the field alone.
    expect(renderTaskTable(worked())).toEqual(TABLE)
    for (const row of worked().tasks) expect(row.key).toBe(null)
  })
})

// ---------------------------------------------------------------------------
// #132 — A TICKET HAS A SUMMARY, and until now this view had nowhere to get one. #127 and
// #131 gave a jira row its NAME (`FOO-123`) and deliberately withheld the injected titles
// map from it, because that map was `gh issue list --json number,title` keyed by GitHub
// issue number and the derived 123 would have printed a stranger's issue title beside the
// ticket. The withholding was the only defence available while the map had one shape.
//
// SO THE RULE CHANGED RATHER THAN GAINING AN EXCEPTION: a row's title is looked up by the
// same thing that NAMES the row. A github row is named `#123` and looks itself up by 123; a
// jira row is named `FOO-123` and looks itself up by `FOO-123`. The old collision is then
// avoided by what any writer actually writes rather than by a null — no `task_key` is ever
// digits alone, because it comes off a board whose keys are `PROJECT-123` — and ONE map can
// carry both kinds of row, which is what makes a hybrid log render correctly with no branch.
//
// NOT "IMPOSSIBLE", and this file must not claim it is: a review measured `task_key: '123'`
// against a github map and got GitHub issue 123's title on a row belonging to task 999. The
// spaces CAN meet — `readIssueTitles` keys by the string '123' and `taskKeyOf` deliberately
// refuses to validate the grammar (its own comment says why) — so the bound is a claim about
// writers, stated in full in lib/progress.js's `buildTaskRows`. It is not a security claim
// either way: a log that can write that key can already write `event.title`.
//
// The shell decides which lookup source to build (lib/commands/status.js: `gh` for github,
// jira-queue.js's `titlesFor` for jira, neither for folder). This file owns the rule.
// ---------------------------------------------------------------------------
describe('a keyed row is titled by its KEY, not by its derived number (#132)', () => {
  const KEY = 'FOO-123'

  // Two closed jira iterations and a third in flight — the shape #131's telemetry makes.
  const JIRA_RUN = jsonl(
    event({ number: 123, task_key: KEY, minutes: 97, cost: 34.1, ts: 1 }),
    event({ number: 124, task_key: 'FOO-124', minutes: 71, cost: 28.75, ts: 2 }),
  )
  const jiraRecord = (current = {}) =>
    inFlightRecord({
      current: { number: 125, task_key: 'FOO-125', started_at: TASK_STARTED.toISOString(), iteration: 1, ...current },
    })
  const jira = (overrides = {}) =>
    worked({ metricsText: JIRA_RUN, record: jiraRecord(), titles: undefined, ...overrides })

  const SUMMARIES = {
    'FOO-123': 'the sidebar ticket',
    'FOO-124': 'the persistence ticket',
    'FOO-125': 'the row component ticket',
  }

  it('titles every row of a jira table from a key-keyed map', () => {
    expect(jira({ titles: SUMMARIES }).tasks.map((row) => [row.key, row.title])).toEqual([
      ['FOO-123', 'the sidebar ticket'],
      ['FOO-124', 'the persistence ticket'],
      ['FOO-125', 'the row component ticket'],
    ])
  ...
  it('draws key and summary in the task cell, on both surfaces, with the grid aligned', () => {
    const snapshot = jira({ titles: SUMMARIES })
    const lines = renderTaskTable(snapshot)
    expect(lines[1]).toContain('FOO-123 the sidebar ticket')
    expect(lines[3]).toContain('FOO-125 the row component ticket')
    expect(aligned(lines), lines.join('\n')).toBe(true)
    // The progress line names the ticket and never the derived number.
    expect(renderProgressLine(snapshot)).toContain('FOO-125 in flight')
    expect(renderProgressLine(snapshot)).not.toContain('#125')
  })

  it('reads a `Map` as well as a plain object, for a key exactly as for a number', () => {
    const titles = new Map([['FOO-123', 'the sidebar ticket']])
    expect(jira({ titles }).tasks[0].title).toBe('the sidebar ticket')
  })

  it('is ONE rule, not two paths: one map titles a github row and a ticket in the same table', () => {
    // The hybrid log — a repo that worked GitHub issues and then switched TASK_SOURCE — with
    // the derived-number collision live: issue #123 and ticket FOO-123 in one file, one map.
    // Each row finds its own entry, and neither can reach the other's.
    const hybrid = jsonl(
      event({ number: 123, minutes: 40, cost: 2, ts: 1 }),
      event({ number: 123, task_key: KEY, minutes: 50, cost: 3, ts: 2 }),
    )
    const titles = { 123: 'the GitHub issue', 'FOO-123': 'the Jira ticket' }
    const rows = worked({ metricsText: hybrid, record: jiraRecord(), titles }).tasks
    expect(rows[0]).toMatchObject({ number: 123, key: null, title: 'the GitHub issue' })
    expect(rows[1]).toMatchObject({ number: 123, key: KEY, title: 'the Jira ticket' })
    const lines = renderTaskTable(worked({ metricsText: hybrid, record: jiraRecord(), titles }))
    expect(lines[1]).toContain('#123 the GitHub issue')
    expect(lines[2]).toContain('FOO-123 the Jira ticket')
    expect(lines[2]).not.toContain('the GitHub issue')
    expect(aligned(lines), lines.join('\n')).toBe(true)
  })

  it('still refuses to title a NUMBERED row from a key-keyed map', () => {
    // The collision from the other side, which is the half a `null` never covered: a jira
    // lookup source is in play for the whole table, and a github row in that same table must
    // not borrow a ticket's summary either.
    const github = worked({ titles: { 'FOO-123': 'a Jira ticket’s summary' } })
    for (const row of github.tasks) expect(row.title).toBe(null)
    expect(renderTaskTable(github).join('\n')).not.toContain('a Jira ticket')
  })

  it('looks a row up by the key it DISPLAYS, so a scrubbed key and its title agree', () => {
    // `taskKeyOf` bounds and scrubs the key before it names the row, and the same value is
    // the lookup handle — one string, so a truncated or scrubbed key cannot name one ticket
    // and be titled from another. A map keyed by the RAW text therefore resolves nothing,
    // which is a missing courtesy and not a mislabelled row.
    const raw = 'FOO-1' + ESC + '[31m'
    const snapshot = worked({
      metricsText: jsonl(event({ number: 1, task_key: raw, minutes: 5, cost: 1, ts: 1 })),
      record: jiraRecord(),
      titles: { [raw]: 'found by raw bytes', 'FOO-1': 'found by the clean key' },
    })
    expect(snapshot.tasks[0].title).toBe('found by the clean key')
    expect(renderTaskTable(snapshot).join('\n')).not.toContain(ESC)
  })

  it('scrubs a summary exactly as it scrubs a GitHub title — acli printed it', () => {
    const hostile = `sum${ESC}[31mmary${BEL}${LF}  #999  pass`
    const lines = renderTaskTable(jira({ titles: { 'FOO-123': hostile } }))
    expect(lines.join('\n')).not.toContain(ESC)
    expect(lines.join('\n')).not.toContain(BEL)
    // Header plus three rows: a forged row cannot arrive through a summary either.
    expect(lines).toHaveLength(4)
    expect(lines.filter((line) => line.includes('#999'))).toHaveLength(1)
  })

  it('holds the grid for keys of very different lengths in one table', () => {
    // The shipped, pinned trade (progress.table.qa.test.js — 'lets a key past the ceiling
    // overflow its OWN line, and no other'), asserted for a JIRA table: `AB-1` is four
    // columns, `LONGPROJ-98765` is fourteen, and fourteen is past NUMBER_COLUMN_MAX. The long
    // row goes ragged; every other row stays exactly where it was, and the identity is never
    // shortened to fit.
    const mixed = jsonl(
      event({ number: 1, task_key: 'AB-1', minutes: 10, cost: 1, ts: 1 }),
      event({ number: 98765, task_key: 'LONGPROJ-98765', minutes: 20, cost: 2, ts: 2 }),
    )
    const lines = renderTaskTable(
      worked({ metricsText: mixed, record: jiraRecord({ task_key: 'FOO-3', number: 3 }), titles: undefined }),
    )
    expect(lines[1]).toContain('AB-1')
    expect(lines[2]).toContain('LONGPROJ-98765')
    expect(aligned(lines.filter((_, i) => i !== 2)), lines.join('\n')).toBe(true)
    expect(lines).toHaveLength(4)
  })

  it('leaves a github run’s two surfaces exactly as #56 rendered them', () => {
    // The zero-regression assertion, restated for the rule change: the numeric path is
    // untouched, titles map and all.
    expect(renderTaskTable(worked())).toEqual(TABLE)
    expect(renderProgressLine(worked())).toBe(
      '  progress   2/9 done · #031 in flight (40min)  [██──────] 22%',
    )
  ...
  it('pads a numeric row exactly as it always did, 1e21 included', () => {
    // `padTaskNumber` is untouched by #132 and stated here because the rule change went
    // through the LOOKUP rather than through the name: a corrupt `issue_number` must still
    // print its digits rather than `#1e+21`.
    expect(padTaskNumber(31)).toBe('031')
    expect(padTaskNumber(1234567)).toBe('1234567')
    expect(padTaskNumber(1e21)).toBe('1000000000000000000000')
    expect(padTaskNumber(Infinity)).toBe('?')
    expect(padTaskNumber('31')).toBe('?')
    const lines = renderTaskTable(
      worked({ metricsText: jsonl(event({ number: 1e21, minutes: 5, cost: 1, ts: 1 })), record: inFlightRecord() }),
    )
    expect(lines[1]).toContain('#1000000000000000000000')
    expect(lines[1]).not.toContain('1e+21')
  })

  it('looks a row up by the number it PRINTS, not by the raw field it came from', () => {
    // A REVIEW MEASURED THIS AND IT WAS WRONG. `#132` widened the lookup handle from a number
    // to "whatever names the row", and the first cut passed the RAW `issue_number` while the
    // row's own `number` field went through `finiteOrNull`. With `issue_number: "123"` the two
    // then disagreed: the cell rendered `#?` — the row saying it could not read its own number,
    // which is what `padTaskNumber('31')` above returns — and the title resolved out of
    // `titles[123]` anyway, so the row was titled `GitHub issue 123 title` by a task it had
    // just declined to name. Measured before the fix, both arms: the closed row came out
    // `{"number":null,"key":null,"title":"GitHub issue 123 title"}` and drew as
    // `#? GitHub issue 123 title`. Reachable from `.ralph/metrics/issues.jsonl` and from a
    // hand-edited `.ralph/run-state.json`, both of which this repo only ever reads.
    //
    // Both arms now hoist that `finiteOrNull(...)` and hand the SAME local to the field and to
    // the lookup, so the cell and the title can no longer name different tasks.
    const closed = worked({
      metricsText: jsonl(event({ number: '123', minutes: 1, cost: 1, ts: 1 })),
      titles: { 123: 'GitHub issue 123 title' },
    })
    expect(closed.tasks[0].number).toBe(null)
    expect(closed.tasks[0].title).toBe(null)
    expect(renderTaskTable(closed)[1]).toContain('#?')
    expect(renderTaskTable(closed).join('\n')).not.toContain('GitHub issue 123 title')

    // ...and the in-flight arm, whose number comes from the run record rather than an event.
    const live = worked({
      metricsText: '',
      record: inFlightRecord({
        current: { number: '123', started_at: TASK_STARTED.toISOString(), iteration: 1 },
      }),
      titles: { 123: 'GitHub issue 123 title' },
    })
    expect(live.tasks.at(-1).number).toBe(null)
    expect(live.tasks.at(-1).title).toBe(null)
    expect(renderTaskTable(live)[1]).toContain('#?')
    expect(renderTaskTable(live).join('\n')).not.toContain('GitHub issue 123 title')

    // A READABLE number still resolves, which is the whole point of the handle: this narrows
    // the widening, it does not undo it.
    expect(worked().tasks[0].title).toBe('sidebar')
    expect(worked().tasks.at(-1).title).toBe('row comp')
  })
})

// The keys a title lookup should ASK about — the rows the table is going to name, and no
// more. Exported from this module rather than derived in the shell because "which rows the
// table draws" is this module's policy: the cap, the in-flight-last ordering and the
// scrubbing all already live here, and a second copy in lib/commands/status.js would drift
// the moment the cap moved.
describe('taskKeysFor — the keys the table is about to draw (#132)', () => {
  const keyed = (n, ts) => event({ number: n, task_key: `FOO-${n}`, minutes: 10, cost: 1, ts })

  it('lists the closed rows’ keys in file order, with the in-flight one last', () => {
    expect(
      taskKeysFor({
        metricsText: jsonl(keyed(1, 1), keyed(2, 2)),
        record: inFlightRecord({
          current: { number: 3, task_key: 'FOO-3', started_at: TASK_STARTED.toISOString() },
        }),
      }),
    ).toEqual(['FOO-1', 'FOO-2', 'FOO-3'])
  })

  it('asks about nothing at all for a github or folder run', () => {
    expect(taskKeysFor({ metricsText: WORKED_EXAMPLE, record: inFlightRecord() })).toEqual([])
    expect(taskKeysFor({})).toEqual([])
    expect(taskKeysFor()).toEqual([])
  })

  it('never asks about more keys than the table has rows', () => {
    // TABLE_CAP closed rows plus the one in flight — nine, whatever the run has done. This is
    // the bound on what one `acli` call is asked for, so a night's run cannot turn a courtesy
    // into a megabyte of JQL.
    const many = jsonl(...Array.from({ length: 40 }, (_, i) => keyed(i + 1, i + 1)))
    const keys = taskKeysFor({
      metricsText: many,
      record: inFlightRecord({
        current: { number: 99, task_key: 'FOO-99', started_at: TASK_STARTED.toISOString() },
      }),
    })
    expect(keys).toHaveLength(TABLE_CAP + 1)
    // The LAST closed rows, matching what the renderer draws — a live view answers "what just
    // happened", and the elided rows are the old ones.
    expect(keys).toEqual([...Array.from({ length: TABLE_CAP }, (_, i) => `FOO-${33 + i}`), 'FOO-99'])
  })

  it('asks once about a key two rows share, and skips the run that is not this one', () => {
    const text = jsonl(keyed(1, 1), keyed(1, 2), { ...keyed(2, 3), run_id: OTHER_RUN })
    expect(taskKeysFor({ metricsText: text, record: inFlightRecord({ current: null }) })).toEqual(['FOO-1'])
  })

  it('asks about the SCRUBBED key, which is the one the row will be named by', () => {
    // The lookup handle and the drawn name are one string — see the sibling test above. So the
    // key that goes out to acli is the scrubbed one, and a key that scrubs away to nothing is
    // not asked about at all.
    const text = jsonl(
      event({ number: 1, task_key: `FOO-1${BEL}`, minutes: 5, cost: 1, ts: 1 }),
      event({ number: 2, task_key: ESC + '[31m', minutes: 5, cost: 1, ts: 2 }),
      event({ number: 3, task_key: 42, minutes: 5, cost: 1, ts: 3 }),
    )
    expect(taskKeysFor({ metricsText: text, record: inFlightRecord({ current: null }) })).toEqual(['FOO-1'])
  })

  it('never throws on the untrusted text and record it is handed', () => {
    for (const metricsText of [undefined, null, 42, '', 'not jsonl', 'RALPH_ISSUE_EVENT {']) {
      expect(() => taskKeysFor({ metricsText, record: inFlightRecord() }), describeInput(metricsText)).not.toThrow()
    }
    for (const record of [undefined, null, 42, 'run', { current: 'FOO-1' }, { current: { task_key: {} } }]) {
      expect(() => taskKeysFor({ metricsText: WORKED_EXAMPLE, record }), describeInput(record)).not.toThrow()
    }
  })
})

// The `--json` document names the ticket too (#132). Its key set is a published contract, so
// this is the one place in the slice where a key is ADDED to it rather than a value moved —
// a decision made on purpose, which is exactly what the #127 pin that held it out asked for.
describe('toJsonSnapshot — the document names the ticket in flight (#132)', () => {
  const KEY = 'FOO-123'
  const jiraRecord = {
    run_id: RUN,
    current: { number: 123, task_key: KEY, started_at: TASK_STARTED.toISOString(), iteration: 1 },
  }
  const doc = (record = jiraRecord) =>
    toJsonSnapshot(worked({ record: inFlightRecord({ current: record.current }) }), {
      mode: 'running',
      record,
    })

  it('publishes `task_key` beside the number, after the two keys #58 published', () => {
    // APPENDED, because a document grows at the end: a consumer reading positionally (and jq
    // users do) sees the two fields it always saw, then the new one. `worktree` (#222) later
    // took the last place the same way, which is why this no longer says "last".
    expect(Object.keys(doc().tasks.current)).toEqual([
      'number',
      'started_at',
      'task_key',
      'worktree',
    ])
    expect(doc().tasks.current.task_key).toBe(KEY)
    expect(doc().tasks.current.number).toBe(123)
  })

  it('says `null` for every task that has no key — every github and folder run', () => {
    // The key is PRESENT and null rather than absent, which is this document's rule
    // everywhere else: `JSON.stringify` drops an `undefined` leaf, so a key that comes and
    // goes is a `jq` filter that works on one repo and fails on another.
    const github = toJsonSnapshot(worked(), { mode: 'running', record: inFlightRecord() })
    expect(Object.keys(github.tasks.current)).toEqual([
      'number',
      'started_at',
      'task_key',
      'worktree',
    ])
    expect(github.tasks.current.task_key).toBe(null)
    for (const task_key of ['', '   ', 42, {}, [], true, null, undefined]) {
      const d = doc({ ...jiraRecord, current: { ...jiraRecord.current, task_key } })
      expect(d.tasks.current.task_key, describeInput(task_key)).toBe(null)
    }
  })

  it('carries no `tasks.current` at all when nothing is in flight', () => {
    const idle = toJsonSnapshot(worked({ record: null, metricsText: '', queue: null }), { mode: 'idle' })
    expect(idle.tasks.current).toBe(null)
  })

  it('publishes the key the RECORD holds, verbatim, exactly as it publishes `run_id`', () => {
    // A machine surface, so the identity is not truncated to a terminal column: `JSON.stringify`
    // escapes what a document must escape, and the scrubbing belongs to the two renderers. The
    // same argument `run_id` already carries (lib/commands/status.json.qa.test.js — 'keeps a
    // hostile run id from breaking the one-line contract').
    const hostile = `FOO-1${ESC}[31m` + LF + 'not-a-document'
    const d = doc({ ...jiraRecord, current: { ...jiraRecord.current, task_key: hostile } })
    expect(d.tasks.current.task_key).toBe(hostile)
    expect(JSON.stringify(d)).not.toContain(LF)
  })
})
