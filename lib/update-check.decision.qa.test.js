...
// defaulted to `npm` — no run in this file passes a `latestSource`, and omitting it takes the
// parameter default, which is the npm query itself (lib/update-check.js:251), so these runs
// NAME npm as both writer and reader of what the seeds pretend was written. A named channel,
// not a fallback: a run that tries to name its channel and fails reads nothing
// (`readingChannelOf`). Without it the throttled path serves nothing, and the spawn-count contract this file
// exists to pin would be measured against decisions that carry no version at all. The spread is
// last so a test whose subject is the stamp can spell its own.
...
describe('QA throttle boundary — the spawn count is the contract (#24)', () => {
  ...
  it('concurrent runs cannot corrupt the cache, and the window is intact afterwards', async () => {
    ...
    expect(readVersionCache({ fs: v, home: HOME, processEnv: {} })).toEqual({
      ...
      last_prompted_at: null,
      latest_version: '0.2.0',
      // #215: three interleaved writers all resolved the same channel, so whichever one landed
      // last, the number on disk is attributed — a torn file that had lost the stamp would leave
      // every copy reading nothing until the next window.
...
describe('QA semver decision matrix (#24)', () => {
  ...
  // #26: shouldPrompt tracks isNewer exactly while the prompt window is open (an
  // empty cache has never prompted), and is never set for a version that is not
  // an upgrade — "nothing to offer" outranks "the window says you may ask".
  it('sets shouldPrompt only where isNewer, whatever the versions say', async () => {
    for (const [current, latest, expected] of [
      ...
      expect(result.isNewer).toBe(expected)
      expect(result.shouldPrompt).toBe(expected)
    }
  })
...
describe('QA the never-throws claim holds for hostile injection too (#24)', () => {
  // resolveUpdateDecision documents itself as never throwing, and that claim has to
  // hold on its own: its call site is runUpdateGate (lib/update-gate.js, #50), whose
  // guard around this call is a belt-and-braces second line of defence and not a
  // licence to throw — anything that escapes both would abort `ralph start` with a
  // raw stack trace over what is only advice. The claim is
  ...
  it('a cache read that throws is swallowed and treated as an empty cache', async () => {
    ...
    expect(result.updatedCache).toEqual({
      ...
      last_prompted_at: null,
      latest_version: '0.2.0',
      // #215: the channel is resolved from the DESCRIPTOR, not from the cache, so a read that
      // blew up cannot cost the attribution of the number this run just fetched.
      latest_version_channel: 'npm',
    })
  })
