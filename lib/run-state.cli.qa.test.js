...
describe('run-state.js CLI — bash string arguments (#55 QA)', () => {
  ...
  it('begin-task carries the worktree in the 5th argument, independently of the key (#222)', () => {
    // #222 added a second optional trailing argument, and the loop's ONE call site passes
    // both for all three sources — so every combination of "has a key" and "has a
    // worktree" arrives here, each absent one spelled `''` by bash.
    cli('begin-task', root, '31', '1', '', '/repo/.ralph/worktrees/issue-31')
    expect(recordOf().current).toMatchObject({
      number: 31,
      task_key: null,
      worktree: '/repo/.ralph/worktrees/issue-31',
    })
    cli('begin-task', root, '', '2', 'FOO-9', '')
    expect(recordOf().current).toMatchObject({ task_key: 'FOO-9', worktree: null })
    // ...and a FOUR-argument call — the shape this CLI took before #222 — still records a
    // task. Not a claim about version skew: the loop script is run in place from the install
    // and resolves this module relative to itself, so no caller of this CLI is a version
    // behind it AT LAUNCH — an update that lands after one can still pair a script bash has
    // already opened with a newer sidecar, which is why the short call is measured here rather
    // than assumed away. It is the arity contract of a positional CLI with optional trailing
    // arguments: an argument nobody passed reads exactly like the `''` bash sends for a value
    // it does not have, and neither costs the caller its iteration.
    cli('begin-task', root, '31', '3')
    expect(recordOf().current).toMatchObject({ number: 31, iteration: 3, worktree: null })
  })
