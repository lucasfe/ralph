...
describe('initCommand — the Jira knobs, measured against a real bash (#133 QA)', () => {
  ...
  describe('bytes the LINE survives: the two readers, on characters nobody listed', () => {
    ...
    // block below rather than guessed at, because reachability is the whole of the
    // severity here.

    it('U+2028 / U+2029: both readers now agree, off a line bash reads whole', async () => {
      // WAS THE WORST OF THE FOUR, and the only one a real prompt can deliver (readline
      // hands both back intact — measured below — so a PASTED JQL carries one). What QA
      ...
      // user typed — while `ralph cycle` (lib/commands/cycle.js reads JIRA_JQL through
      ...
      // that were split at LF and so cannot contain one — measured there against bash.
    ...
    it('a carriage return: agreed too, by the same one-character fix', async () => {
      ...
      // class had to leave alone and did: the lazy group hands a trailing \r to `\s*$`.
  ...
  describe('reachability, measured at the REAL prompt rather than argued', () => {
    ...
    it.each([
      ...
      // SURVIVES. So the U+2028 divergence above is reachable by PASTE — copying a JQL
      // out of a rendered page can carry one — and is the only shape in the divergence
      // block that is.
      ['U+2028 in the middle', `a${LS}b`, `a${LS}b`],
      ['U+2029 in the middle', `a${PS}b`, `a${PS}b`],
