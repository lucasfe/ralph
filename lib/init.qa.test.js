  ...
  describe('why the rule exists — the naive shape, measured', () => {
    ...
    it.each([
      ...
      [
        ...
        // AND THE READER NOW SAYS NOTHING TOO — this column used to hold
        // `summary ~ "Ready for Release"`, the string a `""`-spliced value reads as. bash gets
        // `JIRA_JQL=summary ~ Ready` as one assignment word and then `for` as a COMMAND word, so
        // the assignment is an environment prefix and dies with it; the #149 review taught both
        // readers that rule, so the naive line is now no assignment on either side. The reason
        // for `quoteConfigValue` is unchanged and is the row above: there, bash really does
        // assign, and it assigns something the JS reader does not agree with.
        '',
      ],
      ...
      // THE HARM, and which row still carries it. Where bash ASSIGNS, the loop's shell and
      // `ralph cycle` / `ralph status` work from two different queries off one line, and bash
      // reports nothing wrong about it (it exits 0) — that is the row `quoteConfigValue` exists
      // for. Where bash assigns NOTHING, the readers now agree that nothing was assigned, which
      // is asserted as the agreement rather than left as a passing `not.toBe` against a sentinel.
      if (fromBash === null) {
        expect(configAssignsVar(`${naiveLine}${LF}`, 'JIRA_JQL')).toBe(false)
      } else {
