...
// ...and #132's `titles` verb, which is reachable only as a PROCESS: `runCli` is not
// exported, so the verb, its output shape and its exit code are asserted by spawning node
// against a fake `acli` this file writes into a temp directory on PATH.
...
// ---------------------------------------------------------------------------
// #132 — THE SUMMARY BESIDE THE KEY. `ralph status` names a jira row `FOO-123` (#127, #131)
// and has nothing to say about it: `readIssueTitles` in lib/commands/status.js asks GitHub,
// which knows no tickets. So this module gains the jira analog, and the thing being asserted
// here is mostly about COST and about TRUST rather than about text:
//
//   ONE CALL FOR N KEYS. The table draws at most nine rows, and a summary is a courtesy —
//   nine `acli` processes per `ralph status` on a view people drive off a prompt timer is not
//   a courtesy, it is a tax. So the keys go into one `key IN (…)` query.
//
//   THE KEYS ARE UNTRUSTED. They arrive from `.ralph/run-state.json` and from
//   `.ralph/metrics/issues.jsonl`, both of which are files this repo merely reads (lib/run-
//   state.qa.test.js measures a megabyte of hostile text going into the first). A key is
//   about to be spelled INSIDE a JQL string, which is the one place in Ralph where a
//   stranger's text becomes query syntax — so the grammar is the gate, and everything it
//   cannot read is dropped rather than quoted and hoped for.
//
//   AND EVERY FAILURE IS `{}`. Same posture as the `gh` lookup it mirrors: a title nobody
//   could resolve leaves a table of keys, which is exactly what a pre-#132 jira run rendered.
// ---------------------------------------------------------------------------
describe('titlesFor — N keys, ONE acli call (#132)', () => {
  it('resolves both keys from a single search, and names the argv it used', async () => {
    const exec = ok(searchJson(item('FOO-1', 'first'), item('FOO-2', 'second')))
    expect(await titlesFor(['FOO-1', 'FOO-2'], { exec })).toEqual({
      'FOO-1': 'first',
      'FOO-2': 'second',
    })
    // ONE process for two keys — the property the whole function exists for.
    expect(exec).toHaveBeenCalledTimes(1)
    const [bin, argv, options] = exec.mock.calls[0]
    expect(bin).toBe('acli')
    expect(argv).toEqual([
  ...
  it('asks nine keys in one call, not nine calls', async () => {
    // Nine is the table's height: MAX_TABLE_ROWS closed rows plus the one in flight
    // (lib/progress.js). The count is the assertion; the argv is pinned above.
    const keys = Array.from({ length: 9 }, (_, i) => `FOO-${i + 1}`)
    const exec = ok(searchJson(...keys.map((key) => item(key, `summary ${key}`))))
    const titles = await titlesFor(keys, { exec })
    expect(exec).toHaveBeenCalledTimes(1)
    expect(Object.keys(titles)).toEqual(keys)
    expect(argvsFor(exec, 'search')).toHaveLength(1)
  })

  it('asks for only the fields acli’s `search` allows', async () => {
    // The same allowlist `queuePick` is held to, restated for the second reader of it:
    // `--fields` on `search` accepts only issuetype, key, assignee, priority, status,
    // summary, reporter and labels, and a field outside it is a search acli refuses —
    // which here would be every summary silently missing.
    const exec = ok(searchJson(item('FOO-1', 'first')))
    await titlesFor(['FOO-1'], { exec })
    const argv = exec.mock.calls[0][1]
    const fields = argv[argv.indexOf('--fields') + 1].split(',')
    const ALLOWED = ['issuetype', 'key', 'assignee', 'priority', 'status', 'summary', 'reporter', 'labels']
    for (const field of fields) expect(ALLOWED, field).toContain(field)
  ...
  it('asks about the keys ONLY — no eligibility clause, no exclusion, no ordering', async () => {
    // This is the one query in Ralph that `composeJiraJql` does not build, and deliberately:
    // it is not asking what is ELIGIBLE, it is asking what a named ticket is called. Most of
    // the tickets it asks about carry `done` — the table's closed rows are finished work —
    // and the exclusion the composer adds would filter every one of them out.
    const exec = ok(searchJson(item('FOO-1', 'first')))
    await titlesFor(['FOO-1'], { exec })
    const jql = exec.mock.calls[0][1][exec.mock.calls[0][1].indexOf('--jql') + 1]
    expect(jql).not.toContain('labels NOT IN')
    expect(jql).not.toContain('ORDER BY')
    expect(jql).not.toContain(JQL)
  })

  it('reads a summary out of every envelope shape acli might print', async () => {
    // The same tolerance `queuePick` has, applied to the LIST rather than to the first item:
    // a bare array, a `{issues: […]}` page, a `{workItems: […]}` one, a `{results: […]}` one,
    // and a single bare object.
    const shapes = {
      'a bare array': searchJson(item('FOO-1', 'first')),
      'an issues page': JSON.stringify({ issues: [item('FOO-1', 'first')] }),
      'a workItems page': JSON.stringify({ workItems: [item('FOO-1', 'first')] }),
      'a results page': JSON.stringify({ results: [item('FOO-1', 'first')] }),
      'one bare object': JSON.stringify(item('FOO-1', 'first')),
      'a flat summary': searchJson({ key: 'FOO-1', summary: 'first' }),
    }
    for (const [what, stdout] of Object.entries(shapes)) {
      expect(await titlesFor(['FOO-1'], { exec: ok(stdout) }), what).toEqual({ 'FOO-1': 'first' })
    }
  })

  it('keeps the FIRST summary when acli names one key twice', async () => {
    // `readIssueTitles` in status.js makes the same promise for GitHub numbers, and for the
    // same reason: a later entry rewriting an earlier one makes the map depend on paging.
    const exec = ok(searchJson(item('FOO-1', 'first'), item('FOO-1', 'second')))
    expect(await titlesFor(['FOO-1'], { exec })).toEqual({ 'FOO-1': 'first' })
  })

  it('drops an entry whose key or summary is not a string', async () => {
    // Untrusted SHAPE, filtered on the way in — an object `summary` would reach the renderer
    // as `[object Object]`, and a missing one as the word "undefined".
    const exec = ok(
      JSON.stringify([
        { key: 'FOO-1', fields: { summary: { text: 'nested' } } },
        { key: 42, fields: { summary: 'a number for a key' } },
        { key: 'FOO-2' },
        { key: 'FOO-3', fields: { summary: 'kept' } },
      ]),
    )
    expect(await titlesFor(['FOO-1', 'FOO-2', 'FOO-3'], { exec })).toEqual({ 'FOO-3': 'kept' })
  })

  it('answers about the key the CALLER asked with, not the spelling Jira answers in', async () => {
    // The caller looks a row up by the string it holds (lib/progress.js resolves a row's
    // title by the very text that NAMES the row), so a map keyed by Jira's own uppercase
    // spelling would miss every lowercase key a record happened to hold.
    const exec = ok(searchJson(item('FOO-1', 'first')))
    expect(await titlesFor(['foo-1'], { exec })).toEqual({ 'foo-1': 'first' })
  })

  it('never asks about a key the grammar cannot read, and spawns nothing when none survives', async () => {
    // THE INJECTION GATE. A key is interpolated into a JQL string, so the strict grammar —
    // not `usableJiraKey`, which passes any non-empty string through — is what decides what
    // may be spelled there. Each of these would otherwise become query syntax.
    const hostile = [
      'FOO-1") OR key IN ("BAR-2',
      'FOO-1 ORDER BY created DESC',
      'FOO-1, BAR-2',
      'project = SECRET',
      'FOO-1)',
      '',
    ...
    const exec = ok(searchJson(item('FOO-1', 'first')))
    expect(await titlesFor(hostile, { exec })).toEqual({})
    expect(exec).not.toHaveBeenCalled()

    // ...and mixed in with a real key, the survivor is the only thing asked about.
    const mixed = ok(searchJson(item('FOO-1', 'first')))
    expect(await titlesFor([...hostile, 'FOO-1'], { exec: mixed })).toEqual({ 'FOO-1': 'first' })
    const jql = mixed.mock.calls[0][1][mixed.mock.calls[0][1].indexOf('--jql') + 1]
    expect(jql).toBe('key IN (FOO-1)')
  })

  it('spawns nothing for no keys at all, or for something that is not a list', async () => {
    for (const keys of [[], undefined, null, 'FOO-1', 42, { 0: 'FOO-1' }]) {
      const exec = ok(searchJson(item()))
      expect(await titlesFor(keys, { exec }), JSON.stringify(keys) ?? String(keys)).toEqual({})
      expect(exec, JSON.stringify(keys) ?? String(keys)).not.toHaveBeenCalled()
    }
  })

  it('bounds how many keys one query may ask about', async () => {
    // The caller caps its own list at the table's height, but this function is public and the
    // record it is fed is untrusted: ten thousand keys must not become a megabyte of JQL
    // handed to a process. Still ONE call, and a bounded one.
    const keys = Array.from({ length: 10_000 }, (_, i) => `FOO-${i + 1}`)
    const exec = ok(searchJson(item('FOO-1', 'first')))
    expect(await titlesFor(keys, { exec })).toEqual({ 'FOO-1': 'first' })
    expect(exec).toHaveBeenCalledTimes(1)
    const argv = exec.mock.calls[0][1]
    const jql = argv[argv.indexOf('--jql') + 1]
    expect(jql.length).toBeLessThan(1000)
    expect(jql.split(',').length).toBeLessThanOrEqual(32)
    // The limit and the query agree about how many were asked for.
    expect(Number(argv[argv.indexOf('--limit') + 1])).toBe(jql.split(',').length)
  })

  it('degrades to an empty map for every way acli can fail to answer', async () => {
    const failures = {
      'a non-zero exit': vi.fn(async () => ({ exitCode: 1, stdout: searchJson(item('FOO-1', 'x')), stderr: '' })),
      'a logged-out session': vi.fn(async () => ({ exitCode: 2, stdout: '', stderr: 'You are not logged in.' })),
      'no acli on PATH': vi.fn(async () => {
        throw Object.assign(new Error('spawn acli ENOENT'), { code: 'ENOENT' })
      }),
      'no exitCode at all': vi.fn(async () => ({ stdout: searchJson(item('FOO-1', 'x')) })),
      'nothing at all': vi.fn(async () => undefined),
      'empty output': ok(''),
      'prose instead of JSON': ok('No work items found\n'),
      'JSON that is not a list of items': ok('"FOO-1"'),
      'JSON null': ok('null'),
      'a number': ok('42'),
      'an unreadable stdout': vi.fn(async () => ({
        exitCode: 0,
        get stdout() {
          throw new Error('stream destroyed')
        },
      })),
      'no exec at all': undefined,
    }
    for (const [what, exec] of Object.entries(failures)) {
      await expect(titlesFor(['FOO-1'], { exec }), what).resolves.toEqual({})
    }
    // ...and with no deps bag whatsoever, which is the shape a caller that forgot the seam has.
    await expect(titlesFor(['FOO-1'])).resolves.toEqual({})
  })
})

// The `titles` CLI verb, as a PROCESS. `runCli` is module-private, so this is the only way to
// reach the verb, its output shape and its exit code. Hermetic by construction: PATH holds one
// temp directory and the only `acli` in it is a shell script this test wrote, so the real
// Atlassian CLI cannot be reached even if the implementation regressed.
describe('jira-queue.js titles — the verb a template can shell out to (#132)', () => {
  const LF = String.fromCharCode(10)
  const CLI = join(REPO_ROOT, 'lib', 'jira-queue.js')
  // A POSIX shim; Windows has no /bin/sh to give it a shebang.
  const runnable = process.platform !== 'win32'

  function cli(args, { stdout: acliStdout = '[]', exit = '0' } = {}) {
    const dir = mkdtempSync(join(tmpdir(), 'ralph-jira-titles-'))
    const shim = join(dir, 'acli')
    writeFileSync(
      shim,
      ['#!/bin/sh', 'printf "%s" "$ACLI_STDOUT"', 'exit "$ACLI_EXIT"'].join(LF) + LF,
    )
    ...
    try {
      const res = spawnSync(process.execPath, [CLI, ...args], {
        ...
        encoding: 'utf8',
        timeout: 20000,
        env: { PATH: dir, ACLI_STDOUT: acliStdout, ACLI_EXIT: exit },
      })
  ...
  it.skipIf(!runnable)('prints one `<key>\\t<summary>` line per resolved key', () => {
    // `pick`'s idiom, one line per ticket: bash cuts at the tab, so a summary containing
    // spaces can never be confused with a key.
    const r = cli(['titles', 'FOO-1 FOO-2'], {
      stdout: searchJson(item('FOO-1', 'first'), item('FOO-2', 'second summary')),
    })
    expect(r.status).toBe(0)
    expect(r.stdout).toBe('FOO-1\tfirst' + LF + 'FOO-2\tsecond summary' + LF)
    expect(r.stderr).toBe('')
  })

  it.skipIf(!runnable)('takes the keys as several arguments, or comma separated', () => {
    const stdout = searchJson(item('FOO-1', 'first'), item('FOO-2', 'second'))
    for (const args of [['titles', 'FOO-1', 'FOO-2'], ['titles', 'FOO-1,FOO-2']]) {
      const r = cli(args, { stdout })
      expect(r.status, JSON.stringify(args)).toBe(0)
      expect(r.stdout, JSON.stringify(args)).toBe('FOO-1\tfirst' + LF + 'FOO-2\tsecond' + LF)
    }
  })

  it.skipIf(!runnable)('prints nothing and still exits 0 when nothing resolves', () => {
    // A title nobody could look up is not a failure — the table shows keys, which is what a
    // pre-#132 jira run showed. A non-zero exit here would invite a `|| true` at the call
    // site that hid the lines as well.
    for (const opts of [{ stdout: '[]' }, { stdout: 'not json' }, { stdout: '[]', exit: '1' }]) {
      const r = cli(['titles', 'FOO-1'], opts)
      expect(r.status, JSON.stringify(opts)).toBe(0)
      expect(r.stdout, JSON.stringify(opts)).toBe('')
    }
  })

  it.skipIf(!runnable)('exits 2 with a usage line naming the verb when no key is given', () => {
    const r = cli(['titles'])
    expect(r.status).toBe(2)
    expect(r.stdout).toBe('')
    expect(r.stderr).toContain('titles')
  })
})
