...
// ---------------------------------------------------------------------------
//
// From templates/ralph.sh's remove_issue_worktree header, of the module called the way the
// loop calls it: it "exits non-zero only after git AND its own filesystem sweep both failed
// to take the tree apart", which is the contract its warning rests on — non-zero ⇒ the tree
// is still on disk. That sentence is the whole seam between the two files, and nothing
// exercised the state it describes.
...
// ---------------------------------------------------------------------------
//
// The one thing the doubles above cannot check, and the only thing templates/ralph.sh's
// warning depends on: that a sweep the real filesystem refuses reaches the shell as a
// NON-ZERO EXIT with the tree still on disk. Both permission shapes are exercised — a
// sealed directory INSIDE the tree, and a sealed `.ralph/worktrees` parent — because the
// sweep meets them at different points; neither pins node's wording, for the reason
// measured in the block above the two tests.
...
describe('worktree.js remove — the CLI contract on a jammed sweep (#220 QA)', () => {
  ...
  // WHAT THESE TWO ASSERT IS THE CONTRACT, NOT NODE'S SENTENCE — and this file is the
  // reason we know the difference. The first version of both tests pinned the errno
  // message verbatim, and CI went red on exactly them: MEASURED locally (macOS, git 2.50.1
  // Apple Git-155 / node v20.20.2) the module reports `EACCES: permission denied, unlink
  // '<wt>/undeletable/x'`, while the GitHub Actions Linux runner reported `EACCES,
  // Permission denied: <wt> '<wt>'` for the same fixture — different capitalisation,
  // different separator placement, NO syscall name at all, and the worktree directory
  // rather than the inner file. So `unlink`-versus-`rmdir` is a macOS observation, not a
  // property of the module, and neither test may turn on it.
  //
  // Every assertion below holds for both observed strings: the exit status and the empty
  // stdout come from the CLI, git's refusal line is git's own text (stable), and the
  // module's line is matched only on its own framing (`worktree.js: remove failed (`), the
  // errno CLASS (`EACCES`, which is what POSIX gives for a delete blocked by a directory's
  // mode either way), and the worktree path — which appears in the local string as the
  // prefix of the inner file's path and in the CI string outright.

  it.runIf(NOT_ROOT)('exits 1 and keeps the tree when a directory INSIDE it is sealed', () => {
    // A file inside a mode-0500 directory cannot be deleted without a chmod first, so the
    // module's recursive sweep cannot finish this tree off however it is spelled.
    ...
    // Two lines, and only two: the module writes one warning, the CLI writes one line, and
    // nothing else on this path writes to stderr at all.
    const lines = res.stderr.trimEnd().split('\n')
    expect(lines).toHaveLength(2)
    expect(lines[0]).toBe(
      `⚠️  worktree: git worktree remove declined ${wt} (fatal: '${wt}' is not a working tree) — deleting the directory`,
    )
    expect(lines[1]).toMatch(/^worktree\.js: remove failed \(EACCES\b/)
    expect(lines[1]).toContain(wt)
    // The warning is not a false alarm: the tree really did outlive both attempts.
    expect(existsSync(wt)).toBe(true)
  })

  it.runIf(NOT_ROOT)('exits 1 and keeps the tree when the worktrees PARENT is sealed', () => {
    // A different jam through the same contract: everything inside the tree is deletable
    // here, and what cannot go is the tree's own directory entry, since removing an entry
    // needs write permission on the directory holding it. Worth its own test because it is
    // the shape a human protecting a tree by locking `.ralph/worktrees` would produce, and
    // because the sweep reaches it at a different point — but the assertions are the same
    // contract, for the portability reason in the block above.
