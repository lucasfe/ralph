...
//                     with the in-flight one last: {number, title, verdict, costUsd,
//                     durationMs, inFlight}. The rows ARE the numerator: their count
...
// a map (a plain object or a `Map`) from task number to issue title. It exists because
...
// or half-filled, every row simply renders as its number, which is exactly what folder
// mode does on purpose. A title is CONTEXT; the number is the fact.
...
function buildTaskRows({ runEvents, current, titles, now }) {
  const rows = runEvents.map((event) => ({
    ...
    // by, so a CLOSED row has none — the field is on every row for one shape rather than
    // two, and null is what the renderers fall back from.
    key: null,
  ...
  if (current) {
    ...
    rows.push({
      ...
      key,
      // A KEYED ROW IS NOT SHOWN THE INJECTED MAP — hence the `null` in its place —
      // and that is the same collision seen from the titles side. `titles` is built from
      // `gh issue list --json number,title` and keyed by GITHUB issue number
      // (lib/commands/status.js fetches it for every source but folder), so looking up a
      // derived 123 would print GitHub issue #123's title beside FOO-123 as if it were
      // the ticket's summary. The record's OWN title still counts for both: nothing
      // writes one today, and a slice that starts recording the Jira summary needs no
      // change here.
      title: resolveTitle(key === null ? titles : null, current.number, current.title),
...
// The in-flight task's Jira key (#127), or null for a run that has none — every github and
// folder run, and every run recorded before #127 existed.
//
...
// but when both exist the injected one is what the shell just read from GitHub, and the
// recorded one is however the issue was titled when it was closed.
function resolveTitle(titles, number, eventTitle) {
  const injected = lookupTitle(titles, number)
  const raw =
...
// this out of `gh issue list --json number,title` output, which is a subprocess's
// stdout — so "not a map at all" is a shape this has to answer, not an assertion it may
// make. Anything unrecognised reads as no title, which renders the row as its number.
//
...
// access coerces the number itself.
function lookupTitle(titles, number) {
  ...
  // `getOwnPropertyDescriptor` trap, and `titles[number]` fires an own getter. A throw
  ...
  try {
    if (typeof titles.get === 'function') {
      return titles.get(number) ?? titles.get(String(number))
    }
    if (typeof number !== 'number' || !Number.isFinite(number)) return undefined
    // `hasOwn`, so a task numbered like a prototype key cannot inherit `[object Object]`
    // as its title.
    return Object.hasOwn(titles, number) ? titles[number] : undefined
  } catch {
    return undefined
  }
}
