...
function buildTaskRows({ runEvents, current, titles, now }) {
  const rows = runEvents.map((event) => {
    ...
    return {
      ...
      key,
      // AND A KEYED ROW IS NOT SHOWN THE INJECTED MAP, for the reason written out on the
      // in-flight row below: `titles` is keyed by GITHUB issue number, so the derived 123
      // would print GitHub issue #123's title beside FOO-123 as the ticket's summary.
      title: resolveTitle(key === null ? titles : null, event.issue_number, event.title),
