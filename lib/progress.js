...
// past which the number is a corrupt row rather than a repo's issue and gets to
// overflow its OWN line instead of shifting every other row right.
...
export function renderProgressLine(snapshot) {
  ...
  const flight = current
    ? `#${padTaskNumber(current.number)} in flight (${elapsed == null ? 'unknown' : taskMinutes(elapsed)})`
    : 'nothing in flight'
...
// ---------------------------------------------------------------------------
// TITLES — the one untrusted string in the view.
// ---------------------------------------------------------------------------
...
const numberText = (task) => `#${padTaskNumber(task?.number)}`
