...
//
// ONE MORE RENDERER OVER THAT SAME SNAPSHOT (#222): `renderWorktreeRow` writes the
// `  worktree   /repo/.ralph/worktrees/issue-31` line, or NOTHING — an empty array is
// the answer it gives for every mode that is not a live run, and it is the reason the
// row can be added without a `worktree   –` appearing in views that have no path to
// name. Here rather than in the shell for the reason the four above are: the path it
// draws was already scrubbed for a terminal by `buildProgress`, so the shell that
// prints it decides nothing about what a terminal obeys.
//
...
//
// AND IT OWNS THE SANITIZING OF THE UNTRUSTED STRINGS IN THE VIEW. There are three:
// an issue TITLE (#56), the in-flight task's Jira KEY (#127) and, since #222, its
// WORKTREE PATH. Every other input here is a number written by Ralph's own loop; those
// three are prose and identities somebody else wrote, arriving over a pipe into a
// terminal. All three are cleaned here, in the module where they are unit tested, by
// ONE scrubber that deliberately SUPERSEDES the one
...
// follow-up and not #56. See `scrubForTerminal` — and `cleanTitle` for the WIDTH cap
// #222 split away from it, since a title is shortened to fit a column and a path is not.
...
// The same bound, for the OTHER untrusted string this view draws: the in-flight task's
// worktree path (#222). Larger than a title's and with no column cap beside it, because a
// path is an IDENTITY rather than prose — 24 columns of `/Users/someone/repos/ral…` names
// no directory a reader could cd into, while 24 columns of an issue title is still a
// useful hint. This limit is only the "bound the work first" rule of the scrub below: a
// megabyte hand-written into `.ralph/run-state.json` cannot be walked five times and then
// drawn.
//
// IT IS NOT A CLAIM THAT A FILESYSTEM CANNOT HOLD A LONGER PATH — one can, and a deep tree
// walked down with relative `mkdir` calls produces one easily (measured by hand while
// writing lib/progress.worktree.qa.test.js, whose header records the numbers; no test
// re-measures it). What `PATH_MAX` bounds is the pathname a SYSCALL will accept: `getconf
// PATH_MAX /` reports 1024 on this machine, and past the limit an absolute path stops
// resolving at all. So a path a run can actually `cd` into — which is the only kind git
// could have printed for the loop to record — is far under this bound, and what the bound
// defends against is a value somebody typed into the record.
...
//   worktree        — the directory the in-flight task is being done in (#222), scrubbed
//                     for a terminal, or null when there is no task in flight and when the
//                     one in flight has no worktree
//
...
export function buildProgress({
  ...
  return {
    ...
    // ...and WHERE that task is being done (#222): the worktree the loop recorded, scrubbed
    // for a terminal, or null. Behind the SAME `inFlight` gate as the rows for the same
    // reason — `record.current` survives the end of a run, and a report card that named a
    // directory would be telling the reader Ralph is working in a tree nothing is running
    // in. A field rather than a row of `tasks`: it is one fact about the run, and the table
    // derives its widths from the cells it is given, so an absolute path in one would push
    // `verdict`, `cost` and `time` off the screen.
    worktree: inFlight ? worktreeOf(record?.current) : null,
  }
}
...
// THE WORKTREE ROW (#222) — `  worktree   /repo/.ralph/worktrees/issue-31`, or NOTHING.
//
// AN ARRAY, and a one-line one at most, because "nothing" is the answer it gives most
// often: a record written before #222, an iteration that never got a worktree, and every
// mode that is not a live run. Returning `[]` is the bargain `renderTaskTable` already
// strikes with its header — the shell spreads it, so a view with no worktree is the view it
// rendered before this row existed, with no `  worktree   –` and no blank line where a row
// nearly went. A label with a dash beside it would be furniture in a view whose every other
// label answers a question.
//
// A LABEL-COLUMN ROW RATHER THAN A FIFTH TABLE COLUMN. The table derives its task column
// from the widest cell on show, so one absolute path in a cell would push `verdict`, `cost`
// and `time` off a standard terminal for every row — and a path cut to fit would name no
// directory. Here it costs one line and overflows only its own.
//
// GUARDS ITS INPUT, like both renderers beside it: it is public, a caller can hand it
// anything, and a read-only view must not be the thing that throws — nor the thing that
// prints `[object Object]` where a directory belongs. The scrubbing is NOT repeated here;
// `buildProgress` did it (see `worktreeOf`), which is what keeps one policy in one place.
export function renderWorktreeRow(snapshot) {
...
export function toJsonSnapshot(snapshot, { mode, record, digest } = {}) {
  ...
  return {
    ...
    //
    // AND WHAT THE BOARD CALLS IT (#132), for a jira run, AND WHERE IT IS BEING DONE (#222).
    // This object GROWS AT THE END and only at the end — `task_key` third, `worktree` fourth
    // and currently last — so a consumer reading positionally sees the fields it always saw,
    // then the new one. Both are PRESENT AND NULL when they do not apply (no key outside jira,
    // no worktree on an iteration that failed before one was created) rather than absent, which
    // is this document's rule everywhere: `JSON.stringify` drops an `undefined` leaf, so a key
    // that comes and goes is a `jq` filter that works on one repo and fails on another.
    //
    // #127 DELIBERATELY LEFT THE KEY OUT and pinned the two-key shape, on the grounds that
    // adding to a published contract "must be a decision somebody makes on purpose". #132 was
    // that decision: the number here is DERIVED from the key and is not unique across projects
    // (`AAA-7` and `BBB-7` both yield 7), so a machine reading this document had no way to
    // name the ticket the run is working on. The pin moved with the decision, and moves again
    // here: without `worktree` a script watching a detached run could name the task but not
    // find the checkout it is happening in, which since #218 is not the project root.
    tasks: {
      current: inFlight
        ? {
            number: finiteOrNull(record?.current?.number),
            started_at: isoUtcSecondsOrNull(Date.parse(record?.current?.started_at)),
            task_key: recordedTextOrNull(record?.current?.task_key),
            worktree: recordedTextOrNull(record?.current?.worktree),
          }
        : null,
    },
...
// A string the RECORD holds, for the `--json` document alone — a string or null, never the
// empty string and never a number, so a consumer can test the field rather than the shape of
// the field. Two leaves are published this way: the Jira key (#132) and the in-flight task's
// worktree path (#222). Both are IDENTITIES somebody resolves — a ticket on a board, a
// directory to `cd` into — which is why they share one rule.
//
// NOT `taskKeyOf`/`worktreeOf`, WHICH SCRUB, and that difference is the same one `runIdOrNull`
// already makes: this is a MACHINE surface, and `JSON.stringify` escapes everything a document
// has to escape, so a value carrying an ANSI escape stays a one-line document with the bytes
// intact. Truncating or scrubbing an identity for a consumer that is not a terminal would
// publish a key that names no ticket, or a path that names no directory — the terminal-safety
// scrub belongs to the two renderers, which is where it is. (Measured for `run_id` in
// lib/commands/status.json.qa.test.js: 'keeps a hostile run id from breaking the one-line
// contract'.)
//
// BLANK IS NOTHING, decided on the TRIMMED text and then published VERBATIM: `'   '` is not an
// identity, while `' FOO-1 '` is a key somebody hand-edited whitespace into — and a path may
// legitimately BEGIN or END in a space, since a directory can be named that. This document's
...
// identity would publish a value that resolves to nothing, which is worse for the only consumer
// these fields have (a machine), and the size is bounded by the file the caller already chose to
// read. The TERMINAL surfaces are bounded — `cleanTitle`, `worktreeOf` and the table's overflow
// rule — because there the cost is somebody else's column, not a wrong answer.
//
...
// wrote numeric run ids), while this one must never coerce, because a number is neither a Jira
// key nor a path, and `123` published as `"123"` would name a ticket that does not exist. Two
// one-liners with two rules beats one with a flag.
const recordedTextOrNull = (value) =>
...
// ---------------------------------------------------------------------------
// THE STRINGS THIS VIEW DOES NOT TRUST. There are three, and all three go through
// `scrubForTerminal` below: the issue TITLE, (#127) the in-flight task's Jira KEY,
// and (#222) the in-flight task's WORKTREE PATH. A title comes from a `gh` call the
// shell made or from a recorded event, a key from acli and a path from git, both by
// way of `.ralph/run-state.json` — nothing in any of them was written by this repo,
// and all three end up drawn to a terminal. Only the first two are also shortened to
// a column budget; `worktreeOf` explains why a path is not.
// ---------------------------------------------------------------------------
...
// The in-flight task's WORKTREE (#222), terminal-ready — the directory the task is being
// done in, or null for a task that has none (an iteration whose worktree could not be
// created, and every run recorded before this field existed).
//
// THE THIRD STRING THIS VIEW DOES NOT TRUST, and it arrives the same way the key does: git
// printed it, lib/worktree.js echoed it, bash passed it on and `.ralph/run-state.json` — a
// file a human can hand-edit — is where this module reads it from. So the four rules above
// apply unchanged, including the one that matters most for a path: a `\n` in it would
// otherwise forge a second label-column row (`  queue      99 waiting`) that a reader has
// no way to tell from the real one.
//
// AND IT IS NOT TRUNCATED, which is the one place this parts company with `cleanTitle`: a
// title survives being shortened and a directory does not, because the only thing a reader
// does with this value is type it after `cd`. `row` truncates no value either — the
// `attach` row already prints a session name of whatever length the repo's path produced —
// so a deep worktree simply makes a long line, which is the honest failure.
function worktreeOf(current) {
  const raw = current?.worktree
  return typeof raw === 'string' ? scrubForTerminal(raw, RAW_PATH_LIMIT) : null
}
...
//      length a stranger chose, so the caller passes the `limit` its own value has a
//      reason for — 1000 characters for a title, far more than 24 columns can show;
//      `RAW_PATH_LIMIT` for a path, which no `cd`-able directory reaches.
...
//   4. AN EMPTY RESULT IS NOTHING AT ALL, not `''`, which would silently widen a column
//      and print a trailing space. A title of null renders the number alone and a worktree
//      of null renders no row — both honest about having no value to show.
//
...
//
// THE FOUR POINTS ARE `scrubForTerminal`; the WIDTH is `cleanTitle`'s alone. They came
// apart in #222, which added a third untrusted string — the in-flight task's worktree path
// — that needs every one of the four and must NOT be shortened (see `worktreeOf`). Split
// rather than copied, because the four rules are one policy about what a terminal obeys: a
// second hand-written chain of `.replace` calls is how a sequence added to one pass and not
// the other becomes a bug nobody is looking for.
function scrubForTerminal(raw, limit) {
  const bounded = raw.length > limit ? raw.slice(0, limit) : raw
...
function cleanTitle(raw) {
  const printable = scrubForTerminal(raw, RAW_TITLE_LIMIT)
  return printable === null ? null : truncateToWidth(printable, TITLE_WIDTH)
}
