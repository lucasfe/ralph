...
describe('checkDeps — source-gated acli for the jira source (#125)', () => {
  // #125 adds a SECOND source-gated dep through the mechanism gh already uses,
  // so these are gh's tests read the other way round: what jira pulls in, and
  // what the two existing sources must NOT have gained.
  it('source=jira INCLUDES acli, as critical', () => {
    const acli = checkDeps({ hasCommand: allPresent, source: 'jira' }).find(
      (r) => r.name === 'acli',
    )
    expect(acli).toBeDefined()
    expect(acli.critical).toBe(true)
  })

  it('source=jira EXCLUDES gh entirely', () => {
    // A Jira repo may have no GitHub at all — the same argument folder mode makes.
    const got = names(checkDeps({ hasCommand: allPresent, source: 'jira' }))
    expect(got).not.toContain('gh')
  })

  it('source=jira still includes the shared deps and the selected agent CLI', () => {
    const got = names(checkDeps({ hasCommand: allPresent, source: 'jira', agent: 'codex' }))
    for (const dep of ['git', 'tmux', 'node', 'npm', 'jq', 'curl']) expect(got).toContain(dep)
    expect(got).toContain('codex')
    expect(got).not.toContain('claude')
  })

  it('source=github is UNCHANGED — gh in, acli out', () => {
    for (const args of [{ hasCommand: allPresent }, { hasCommand: allPresent, source: 'github' }]) {
      const got = names(checkDeps(args))
      expect(got).toContain('gh')
      expect(got).not.toContain('acli')
    }
  })

  it('source=folder is UNCHANGED — neither gh nor acli', () => {
    const got = names(checkDeps({ hasCommand: allPresent, source: 'folder' }))
    expect(got).not.toContain('gh')
    expect(got).not.toContain('acli')
  })

  it('EXACTLY ONE source-gated dep appears in any result set', () => {
    const gated = (source) =>
      names(checkDeps({ hasCommand: allPresent, source })).filter(
        (n) => n === 'gh' || n === 'acli',
      )
    expect(gated('github')).toEqual(['gh'])
    expect(gated('jira')).toEqual(['acli'])
    expect(gated('folder')).toEqual([])
  })

  it('present flag reflects hasCommand for acli', () => {
    const acli = checkDeps({ hasCommand: (cmd) => cmd !== 'acli', source: 'jira' }).find(
      (r) => r.name === 'acli',
    )
    expect(acli.present).toBe(false)
  })

  it('REQUIRED_DEPS marks acli with source=jira and a hint per platform', () => {
    expect(REQUIRED_DEPS.acli.source).toBe('jira')
    expect(REQUIRED_DEPS.acli.critical).toBe(true)
    expect(REQUIRED_DEPS.acli.install.mac).toBe('brew tap atlassian/acli && brew install acli')
    // WSL is Linux here, exactly as every other entry treats the pair.
    expect(REQUIRED_DEPS.acli.install.linux).toBe(REQUIRED_DEPS.acli.install.wsl)
    expect(REQUIRED_DEPS.acli.install.linux).toContain('https://acli.atlassian.com/linux/latest/')
  })
})
