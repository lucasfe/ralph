...
// consumer that built `'claude' + '-working'`, read a name off a config default, or kept a
...
// `claude-working`, its call comes out carrying the REAL name while the module says
...
//
// The fake names are deliberately NOT plausible label names: `qa-in-progress` can only be
// here by having travelled through the module, and a real name in this file's output can
// only be a literal somewhere in lib/.
...
const FAKE = vi.hoisted(() => {
  const IN_PROGRESS_LABEL = 'qa-in-progress'
...
const REAL_NAMES = ['claude-working', 'claude-failed', 'pending-merge', 'do-not-ralph']
...
describe('QA #139 — the seam under substitution: `ralph start`', () => {
  ...
  it('creates the SUBSTITUTED labels — name, colour and description all follow the module', async () => {
    ...
    expect(creates.map((c) => c.args)).toEqual([
      ['label', 'create', 'qa-in-progress', '--color', '111111', '--description', 'QA in progress'],
  ...
  it('sweeps for orphans under the SUBSTITUTED in-progress label, and says so in the notice', async () => {
    ...
    expect(list.args[list.args.indexOf('--label') + 1]).toBe('qa-in-progress')
    ...
    expect(out).toContain("Issues with the 'qa-in-progress' label")
    expect(out).toContain('gh issue edit <n> --remove-label qa-in-progress')
  })
  ...
  it('selects work with the SUBSTITUTED query', async () => {
    ...
    expect(searchOf(deps.exec)).toBe(
      'state:open -label:qa-in-progress -label:qa-gave-up -label:qa-hands-off -label:qa-awaiting-rollforward',
    )
  })
...
describe('QA #139 — the seam under substitution: the orphan sweep', () => {
  it('lists and clears under the SUBSTITUTED label, in both of its argv', async () => {
    ...
    expect(edit.args).toEqual(['issue', 'edit', '7', '--remove-label', 'qa-in-progress'])
...
describe('QA #139 — the seam under substitution: the outcome precedence', () => {
  ...
  it('and no longer recognises the real ones — proof it is not reading a second copy', async () => {
    ...
    // `claude-failed` beside the import, this would still say `fail` and the rename would
    // half-land: metrics keyed to a word the loop no longer stamps.
    expect(verdictOf(['claude-failed'], 'OPEN')).toBe('unknown')
