...
// #223 — the label that marks an OPEN issue as done-but-not-yet-closed. Taken from
// lib/labels.js rather than spelled here, so the sweep's "is this issue done?" verdict
// cannot drift from the one word the rest of the package (and templates/ralph.sh's own
// classification) agrees on. lib/labels.js is the ONLY JavaScript allowed to spell a
// Ralph label as a literal (#139), which is what this import keeps true.
import { PENDING_MERGE_LABEL } from './labels.js'
...
// Real issue-state query (#223), the one seam the loop-start sweep asks about the world
// beyond git. `gh issue view <n> --json state,labels` is the same shape the loop already
// classifies an iteration from (templates/ralph.sh reads `state` and `labels` the same
// way), so the sweep's "is this issue done?" verdict cannot drift from the loop's. Returns
// `{ state, labels: [names] }`, or `null` for ANY failure — a non-zero exit (gh not
// authenticated, the issue deleted) or unparseable JSON — because the sweep treats an
// answer it could not get as a reason to KEEP the tree, never to remove it.
function realQueryIssue(n, { cwd } = {}) {
  const res = spawnSync('gh', ['issue', 'view', String(n), '--json', 'state,labels'], {
    cwd,
    encoding: 'utf8',
  })
  ...
  try {
    ...
    return {
      state: parsed.state,
      labels: Array.isArray(parsed.labels) ? parsed.labels.map((l) => l?.name ?? l) : [],
    }
  } catch {
    return null
  }
...
// The handles of every LIVE worktree git lists directly under `<mainRoot>/.ralph/worktrees`
// whose basename is an `issue-<N>` — the only shape the github create path produces (#218).
// Parsed from `git worktree list --porcelain`, whose records are blank-line separated and
// whose first line of each is `worktree <path>`; git prints one canonical spelling per
// worktree (absolute, symlinks already resolved), so the only normalization the compare
// needs is resolve() on both sides. A path one level deeper, one with a `task-<N>` handle
// (folder mode never reaches this arm), or a foreign registration is skipped: the sweep
// only reasons about trees it knows how to map back to a github issue.
function issueWorktreeHandles(list, root) {
  const handles = []
  for (const line of String(list ?? '').split('\n')) {
    if (!line.startsWith('worktree ')) continue
    const path = resolve(line.slice('worktree '.length).trim())
    if (resolve(dirname(path)) !== root) continue
    const handle = basename(path)
    if (/^issue-\d+$/.test(handle)) handles.push(handle)
  }
  return handles
}

/**
 * Sweep stray worktrees once at loop start, before the first iteration (#223). This is
 * teardown's counterpart to the create path: #220 keeps a worktree whenever an iteration
 * does not succeed — which is what makes a failure inspectable — and over weeks that is
 * also what fills `.ralph/worktrees/` with checkouts of issues closed long ago. This mirrors
 * the stale-label sweep lib/orphan-cleanup.js performs: best-effort, before the work starts,
 * and never able to change a run's outcome.
 *
 * BEST-EFFORT THROUGHOUT. It never throws out for an ordinary sweep — a git call that fails,
 * an issue query that cannot answer, a removal git and the filesystem both decline — each
 * becomes a warning on stderr and the sweep carries on to the next tree. The ONE thing it
 * still refuses is a broken invocation (an unsafe root), consistent with the rest of this
 * module: that is a bug in the caller, not an outcome of a run, and it is caught before any
 * git runs.
 *
 * TWO PHASES:
 *   1. PRUNE, for every source. `git worktree prune -v` drops the administrative record of
 *      any worktree whose directory was deleted by hand (`-v` names each one), and what it
 *      names is echoed to stdout so the tmux pane records it. This is the whole of the sweep
 *      under `folder` and `jira`, where there is no issue to consult.
 *   2. REMOVE THE DONE ONES, github only. Enumerate the live `issue-<N>` worktrees and ask
 *      each issue's state: a CLOSED issue, or an OPEN one carrying `pending-merge`, is done
 *      (this mirrors the loop's own terminal-success classification in templates/ralph.sh),
 *      so its tree is removed through removeWorktree — the same escalate-then-sweep-then-prune
 *      dance the teardown uses — and named on stdout. An issue still OPEN is left alone; it is
 *      work a human may be looking at. An issue whose state could not be read is ALSO left
 *      alone and warned about: an answer that never arrived is not permission to delete.
 */
export function sweepWorktrees(
  mainRoot,
  {
    ...
    fs = { existsSync, mkdirSync, rmSync },
    git = realGit,
    queryIssue = realQueryIssue,
    home = homedir(),
    stdout = process.stdout,
    stderr = process.stderr,
  } = {},
) {
  // Derivation first, so an unsafe root is refused before any git runs — the same
  // boundary every other verb in this module puts ahead of a spawn.
  const root = assertSafeRoot(mainRoot, home)
  const parent = worktreesRoot(mainRoot, { home })
  const cwd = root

  // Phase 1 — prune git-prunable records, always. `-v` prints one line per dropped record;
  // git splits that reporting across stdout and stderr depending on version, so both are
  // read and echoed to the sweep's stdout for the pane.
  const pruned = git(['worktree', 'prune', '-v'], { cwd })
  const prunedText = `${pruned.stdout ?? ''}${pruned.stderr ?? ''}`.trim()
  if (prunedText) stdout.write(`${prunedText}\n`)

  // Phase 2 — github only: the other sources have no issue to query, and the prune above
  // is the whole of their sweep.
  if (source !== 'github') return

  const listed = git(['worktree', 'list', '--porcelain'], { cwd })
  if (listed.status !== 0) {
    stderr.write(
      `⚠️  worktree: could not list worktrees to sweep (${firstLine(listed.stderr) || `exit ${listed.status}`})\n`,
    )
    return
  }

  for (const handle of issueWorktreeHandles(listed.stdout, parent)) {
    const number = handle.slice('issue-'.length)

    let issue
    try {
      issue = queryIssue(number, { cwd })
    } catch (e) {
      // A query that threw is indeterminate — keep the tree, keep gh's own words.
      stderr.write(
        `⚠️  worktree: could not check ${handle}'s issue (${firstLine(e?.message)}) — keeping it\n`,
      )
      continue
    }
    if (!issue || typeof issue.state !== 'string') {
      // Indeterminate: never taken as permission to delete. Keep the tree, say why.
      stderr.write(
        `⚠️  worktree: could not determine ${handle}'s issue state — keeping it\n`,
      )
      continue
    }
    ...
    const pending = labels.includes(PENDING_MERGE_LABEL)
    if (!closed && !pending) continue // still open — a human may be looking at it

    const reason = closed ? 'closed' : PENDING_MERGE_LABEL
    try {
      removeWorktree(mainRoot, handle, { fs, git, home, stderr })
      stdout.write(`swept ${handle} (${reason})\n`)
    } catch (e) {
      // removeWorktree already warned about which leg failed; this names the tree the
      // sweep gave up on and moves to the next. A stuck removal never stops the run.
      stderr.write(
        `⚠️  worktree: could not sweep ${handle} (${firstLine(e?.message)}) — leaving it in place\n`,
      )
    }
  }
}
...
const CLI_VERBS = ['path', 'create', 'create-detached', 'advance', 'remove', 'sweep']
function runCli(argv) {
  const [cmd, mainRoot, handle, baseRef] = argv
  const usage = () => {
    process.stderr.write(
      `usage: worktree.js <${CLI_VERBS.join('|')}> <mainRoot> <handle|source> [baseRef|branch]\n`,
    )
    return 2
  }
  ...
  try {
    switch (cmd) {
      ...
      case 'remove':
        ...
        return 0
      case 'sweep':
        // `sweep <mainRoot> <source>` (#223): the third argument is TASK_SOURCE, not a
        // handle — the sweep derives its own handles from `git worktree list`. Best-effort
        // like the loop's `remove`: it writes what it swept to stdout and any trouble to
        // stderr, and returns 0 for an ordinary sweep. Only a broken invocation (an unsafe
        // root) reaches the catch below and exits 1 — teardown gets no vote on the run.
        sweepWorktrees(mainRoot, { source: handle })
        return 0
    }
  } catch (e) {
