...
// an optional `export` prefix, a space-or-tab indent before the name, and padding
// after the value. NOT whitespace between the name and the `=`, and NOT an indent
// outside bash's own blanks — space and tab, not the wider set a JS `\s` would take
// (#147): bash runs either of those lines as a command and assigns nothing, so
// neither does this. Commented lines (leading `#`) are ignored. The LAST uncommented
// assignment wins (bash semantics). Never throws. The grammar itself, and the measurements
// behind it, are in parse-config-var.js.
