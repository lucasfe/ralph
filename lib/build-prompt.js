...
// lands in THIRTEEN places in the rendered prompt (MEASURED: `grep -o '{{RALPH_TASK_KEY}}'
// templates/prompt-team-jira.md | wc -l` → 13; it was 9 before #129 added step 7's completion
// and its failure comment), four of them inside a fenced command a model reads as something to
// run: the step-1 `acli … view --key <KEY> --fields "*all" --json`, and the three
// `node lib/jira-queue.js complete|comment <KEY>` calls step 7 and "Failed" name. So it is
...
//
//   usableJiraKey (lib/jira-key.js) — the same call `queuePick` in lib/jira-queue.js already
//     makes on the pick (named by function, not by line: this citation was already stale once,
//     when #129 split the acli layer out and moved every line number in that file), which is
//     what closes the only reachable divergence: bash's `$task_key` is
...
//     for a terminal box; here the argument is the 13 above — an N-character key is 13N
//     characters of remote-chosen text in the model's context, so the shared bound holds that
//     at 2600 instead of at nothing. Truncation is not silent either: `cap` ends the value with
...
export function buildPrompt({
  ...
  const vars = {
    ...
    TASK_SOURCE: source,
    // #218: github mode resolves each issue in a per-issue git worktree and spawns
    // the AGENT there, while THIS process keeps running with cwd at the main repo
    // root. So the placeholder and the PROMPT.md lookup deliberately come apart:
    // `projectPromptPath` above stays on `projectRoot` (a project's PROMPT.md is
    // typically untracked — `git ls-files PROMPT.md` prints nothing in this
    // repository — so a worktree checkout would not contain it), while the
    // placeholder names the tree the agent actually woke up in, which is what makes
    // the template's "stay inside it for all operations" true by construction.
    // Since #221 FOLDER MODE EXPORTS IT TOO (a detached worktree per task); jira mode
    // and the validation prompt still do not, and both run in the main tree. Empty is
    // treated as absent because bash exports an unset variable as ''.
    // Read unconditionally, unlike RALPH_TASK_KEY above: the github and folder arms of
    // templates/ralph.sh export it, and test/setup/hermetic-env.js neutralizes RALPH_*.
    PROJECT_ROOT: env.RALPH_PROMPT_PROJECT_ROOT?.trim() || projectRoot,
    // #221: the checkout that owns `.ralph/` — tasks, state, and the worktrees
    // themselves — which is where THIS process is running. It exists because the two
    // roots come apart: a folder task's code is in the worktree ({{PROJECT_ROOT}}
    // above) while the task file it must move is in the main root's gitignored
    // `.ralph/tasks/`, so a template that spelled `.ralph/tasks/` relative to the
    // agent's own tree would name an empty directory.
    //
    // NEVER OVERRIDDEN, and that asymmetry is the whole point: this is always the main
    // root, so `{{MAIN_REPO_ROOT}}` and `{{PROJECT_ROOT}}` render the SAME path in
    // every mode that has no worktree override, and differ only where they must.
    MAIN_REPO_ROOT: projectRoot,
