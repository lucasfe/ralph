...
// #215 added a fourth field to a query descriptor: the id the resolved version is FILED under.
// This literal stands in for the descriptor lib/install-target.js builds for a Cellar, so it
// carries the id that descriptor carries — a stand-in missing it would be testing the
// unattributable-channel path under a Homebrew heading. The join between this spelling and the
// real table's is pinned in lib/update-check.cache-channel.test.js, over every layout.
const BREW_QUERY = {
  argv: BREW_ARGV,
  format: 'brew-json-v2',
  unreachable: 'tap unreachable?',
  channel: 'brew',
}
...
// #215: a seeded version comes with the channel that resolved it, because a version with no
// channel is one no copy may read back. The parameter has no default on purpose — a caller that
// seeds a number here has to say whose it is, which is the whole content of the fix.
const openWindows = (latest = null, channel = null) =>
  seeded({
    last_check_at: null,
...
describe('resolveUpdateDecision — a named channel that answers unreadably (#200 QA)', () => {
  ...
  for (const [label, stdout] of [
    ...
    it(`keeps the previously known version when the tap answers ${label}`, async () => {
      ...
      // bad document.
      // #215: the previously known version is the TAP's own last readable answer — this run
      // asks the tap, and a number npm resolved would (correctly) not survive into the decision
      // at all. "Keeps the previously known version" was always a claim about this channel's
      // history; before the key there was no way to say so.
      const fs = openWindows('0.2.0', 'brew')
