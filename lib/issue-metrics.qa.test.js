...
// ===========================================================================
// QA augmentation for #131 — a MIXED log, and the un-numbered event in it.
// ===========================================================================
//
// issue-metrics.test.js owns the well-formed jira rows. The question left over is what
// `ralph cycle`'s exit code does when the log is what a real repo's log is: several runs,
// three sources, a truncated last line from a killed process, and — new since #131 — events
// whose `issue_number` is null because the ticket key carried no readable number.
//
// THE TWO HALVES OF THE ANSWER PULL APART, which is why they are asserted apart: `ok`/`failed`
// are a COUNT of iterations (the exit code is built from them, so an event that happened has to
// be counted whatever it is called), while `okIssues`/`failedIssues` are a list of NAMES which
// a null would render into as `#null`. A single `toEqual` on the whole object would pass for an
// implementation that dropped the un-numbered event from both.
describe('aggregateCycleCounts — a real mixed log, jira included (#131 QA)', () => {
  const line = (fields) => ISSUE_EVENT_TAG + JSON.stringify(fields)

  // One log, in append order: two github iterations, one folder, three jira (one of them with
  // no readable number), a truncated line, a line with no tag, and an event too old for the
  // window. Nothing here is grouped by source — that is the point.
  const MIXED = [
    line({ issue_number: 42, verdict: 'pass', ts: 1_000 }),
    'not a ralph line at all',
    line({ issue_number: 7, verdict: 'fail', ts: 1_100 }),
    line({ issue_number: 123, task_key: 'FOO-123', verdict: 'pass', ts: 1_200 }),
    ISSUE_EVENT_TAG + '{"issue_number":124,"task_key":"FOO-124","verdict":"pa',
    line({ issue_number: null, task_key: 'FOO-BAR-1', verdict: 'pass', ts: 1_300 }),
    line({ issue_number: 125, task_key: 'FOO-125', verdict: 'fail', ts: 1_400 }),
    line({ issue_number: 99, task_key: 'OLD-99', verdict: 'pass', ts: 500 }),
  ].join('\n')

  it('counts every event in the window and lists only the ones with a number', () => {
    const counts = aggregateCycleCounts(MIXED, 1_000)
    // Four passes happened (github 42, jira 123, the un-numbered jira ticket, and NOT the
    // pre-window OLD-99); two failures (github 7, jira 125). The truncated line is not an
    // event at all.
    expect(counts.ok).toBe(3)
    expect(counts.failed).toBe(2)
    expect(counts.processed).toBe(5)
    // File order, and the null is absent from the list without shifting anything after it.
    expect(counts.okIssues).toEqual([42, 123])
    expect(counts.failedIssues).toEqual([7, 125])
  })

  it('the lists are shorter than the counts, and that is the documented split', () => {
    const counts = aggregateCycleCounts(MIXED, 1_000)
    expect(counts.okIssues.length).toBeLessThan(counts.ok)
    expect(counts.okIssues.every((n) => Number.isFinite(n))).toBe(true)
    expect(counts.failedIssues.every((n) => Number.isFinite(n))).toBe(true)
  })

  it('an un-numbered FAILURE still fails the cycle', () => {
    // The direction that matters for an exit code: a jira ticket whose key had no readable
    // number and whose run failed must not vanish into a clean run.
    const counts = aggregateCycleCounts(line({ issue_number: null, task_key: 'WEIRD', verdict: 'fail', ts: 5 }), 0)
    expect(counts).toEqual({ ok: 0, failed: 1, processed: 1, okIssues: [], failedIssues: [] })
  })

  it('a task_key with no issue_number FIELD at all counts the same way', () => {
    // Not a shape lib/issue-event.js writes — it always emits `issue_number`, even as null —
    // but issues.jsonl is untrusted text and an absent key must read as unknown rather than
    // as `undefined` reaching a `#` prefix.
    const counts = aggregateCycleCounts(line({ task_key: 'FOO-9', verdict: 'pass', ts: 5 }), 0)
    expect(counts).toEqual({ ok: 1, failed: 0, processed: 1, okIssues: [], failedIssues: [] })
  })

  it('the derived-number COLLISION is counted twice and named once per event', () => {
    // `AAA-7` and `BBB-7` both derive 7 (lib/jira-key.js's accepted collision). Nothing here
    // de-duplicates, and nothing should: two tickets were worked, so the cycle processed two —
    // the cost is a summary that says `OK: #7, #7`, which is the rendering follow-up the key
    // was recorded to make possible rather than a tally this function gets wrong.
    const counts = aggregateCycleCounts(
      [
        line({ issue_number: 7, task_key: 'AAA-7', verdict: 'pass', ts: 5 }),
        line({ issue_number: 7, task_key: 'BBB-7', verdict: 'pass', ts: 6 }),
      ].join('\n'),
      0,
    )
    expect(counts.ok).toBe(2)
    expect(counts.processed).toBe(2)
    expect(counts.okIssues).toEqual([7, 7])
  })

  it('a task_key that is not a string cannot reach the tally or the lists', () => {
    // The aggregation is on the numeric field alone, so a hostile `task_key` is inert here.
    // Pinned because it is the reason this function needed no change in #131 at all.
    const counts = aggregateCycleCounts(
      [
        line({ issue_number: 1, task_key: { toString: 'x' }, verdict: 'pass', ts: 5 }),
        line({ issue_number: 2, task_key: 12345, verdict: 'fail', ts: 6 }),
      ].join('\n'),
      0,
    )
    ...
    expect(counts.failedIssues).toEqual([2])
  })
})
