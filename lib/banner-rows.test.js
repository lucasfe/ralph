...
describe('bannerRows — the list, and the order a reader reads it in (#122)', () => {
  it('draws every fact it was given, in the order the box has always drawn them', () => {
    ...
    expect(labelsOf(bannerRows(ALL_FACTS))).toEqual([
      ...
      'context',
      'cached',
      // ...#201's channel directly under the verdict it qualifies, because the two channels
      // hold different versions on purpose and `cached` alone cannot be attributed...
      'channel',
      // ...then where...
      'cwd',
      'source',
...
describe('bannerRows — the plain single-fact rows (#75/#69/#76/#201)', () => {
  it('draws each one only when the caller passed the fact behind it', () => {
    for (const [label, value] of [
      ['os', 'mac'],
      ['source', 'github'],
      ['repo', 'lucasfe/ralph'],
      // #201's channel: a third table read by the same gate, so the same argument covers it —
      // and "never asked" matters more here than for any other row in this box. Doctor cannot
      // probe `npm root -g`, so a caller that passed nothing has learned nothing, and a row
