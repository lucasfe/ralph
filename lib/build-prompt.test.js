...
describe('buildPrompt', () => {
  ...
  describe('RALPH_PROMPT_PROJECT_ROOT — the worktree the agent runs in (#218)', () => {
    ...
    // Folder and jira mode get no worktree in this slice, and neither does the
    // validation prompt, so every one of them reaches here with the variable absent.
    it.each([
      ['unset', {}],
  ...
  describe('jira task source selection (#128)', () => {
    ...
    it('commits to DEV_BRANCH, says the commit stays local, and opens no PR', () => {
      ...
      // Nothing pushes a jira-mode commit — MEASURED: `grep -n push templates/ralph.sh`
      // matches nothing. The folder template promises the bash pushes for you; that
      // promise must not have been carried over.
