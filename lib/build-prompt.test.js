...
function setupFs({ projectFiles = {} } = {}) {
  ...
  const jiraTemplate = readFileSync(templatePath('prompt-team-jira.md'), 'utf8')
  ...
  vol.writeFileSync(templatePath('prompt-team-jira.md'), jiraTemplate)
...
describe('buildPrompt', () => {
  it('substitutes the runtime placeholders from env into the orchestrator template', () => {
    ...
    expect(out).toContain('from `origin/dev`')
  ...
  it('falls back to safe defaults when env vars are missing', () => {
    ...
    expect(out).toContain('from `origin/main`')
  })
  ...
  // #218 — github mode resolves each issue in a per-issue git worktree and spawns the
  // AGENT with cwd set to it, while the prompt builder keeps running with cwd at the
  // main repo root. The two roots have to be separable, and this is the seam.
  //
  // WHY NOT SIMPLY RUN THE BUILDER IN THE WORKTREE, which would need no new variable:
  // because {{PROJECT_PROMPT}} is read from `join(projectRoot, 'PROMPT.md')` and a
  // project's PROMPT.md is typically untracked — MEASURED in this repository, where
  // `git ls-files PROMPT.md` prints nothing at all. A fresh worktree checkout contains
  // only tracked files, so building the prompt from inside one would silently drop the
  // project's own instructions on every single github iteration.
  //
  // So the override moves the PLACEHOLDER and nothing else: the agent is told to stay
  // inside the tree it actually woke up in (making that restriction true by
  // construction), and PROMPT.md is still found where the user keeps it.
  describe('RALPH_PROMPT_PROJECT_ROOT — the worktree the agent runs in (#218)', () => {
    const WORKTREE = '/project/.ralph/worktrees/issue-7'

    it('renders {{PROJECT_ROOT}} as the override when the loop exports one', () => {
      const vol = setupFs({ projectFiles: { 'PROMPT.md': '## Stack\nFoo' } })
      const out = buildPrompt({
        projectRoot: PROJECT,
        env: { RALPH_PROMPT_PROJECT_ROOT: WORKTREE },
        fs: vol,
      })
      expect(out).toContain(`Your project root is \`${WORKTREE}\``)
      expect(out).not.toContain('Your project root is `/project`')
    })

    it('still reads PROMPT.md from the MAIN root, which a worktree checkout would not have', () => {
      const vol = setupFs({ projectFiles: { 'PROMPT.md': '## Stack\nReact + Vite' } })
      const out = buildPrompt({
        projectRoot: PROJECT,
        env: { RALPH_PROMPT_PROJECT_ROOT: WORKTREE },
    ...
    // Jira mode gets no worktree yet (#224), and neither does the validation prompt,
    // so both reach here with the variable absent. Since #221 folder mode DOES export
    // it — the describe below is where that half is asserted.
    it.each([
      ['unset', {}],
      ['empty', { RALPH_PROMPT_PROJECT_ROOT: '' }],
      ['nothing but whitespace', { RALPH_PROMPT_PROJECT_ROOT: '   ' }],
    ])('falls back to projectRoot when the override is %s', (_label, env) => {
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env, fs: vol })
      expect(out).toContain('Your project root is `/project`')
    })

    it('leaves no placeholder unresolved and warns about nothing', () => {
      const vol = setupFs({ projectFiles: { 'PROMPT.md': '' } })
      const stderr = makeStderr()
      const out = buildPrompt({
        projectRoot: PROJECT,
  ...
  // #221 — folder mode now runs the agent in a detached worktree, so its two roots come
  // apart the way github's already did: {{PROJECT_ROOT}} is the tree the agent woke in,
  // and {{MAIN_REPO_ROOT}} is the checkout whose gitignored `.ralph/tasks/` holds the
  // queue. A worktree checkout carries only TRACKED files, so the task files are not in
  // it at all and every path the folder template names for them has to be absolute
  // against the main root.
  //
  // No new env var for it: `projectRoot` already IS the main root here (the builder runs
  // in the main tree and reads its PROMPT.md from there), while {{PROJECT_ROOT}} is the
  // one that gets overridden. So the second placeholder is the value this function was
  // called with, rendered under its own name.
  describe('MAIN_REPO_ROOT — the checkout that owns .ralph/tasks (#221)', () => {
    const WORKTREE = '/project/.ralph/worktrees/task-7'

    it('names the MAIN root on every task path while {{PROJECT_ROOT}} is the worktree', () => {
      const vol = setupFs()
      const out = buildPrompt({
        projectRoot: PROJECT,
        env: { TASK_SOURCE: 'folder', RALPH_PROMPT_PROJECT_ROOT: WORKTREE },
        fs: vol,
      })
      expect(out).toContain(`Your project root is \`${WORKTREE}\``)
      expect(out).toContain(`${PROJECT}/.ralph/tasks/afk/todo/`)
      expect(out).toContain(`${PROJECT}/.ralph/tasks/afk/done/`)
      // The bare, worktree-relative spelling is what would have the agent hunting for
      // a queue directory that a fresh checkout does not contain.
      expect(out).not.toMatch(/[^/]`\.ralph\/tasks\//)
    })

    it('leaves no placeholder unresolved and warns about nothing in folder mode', () => {
      // The red this was written for: a template placeholder with no entry in the bag
      // makes interpolate() warn `unknown placeholder {{MAIN_REPO_ROOT}}` and leave the
      // token in the prompt the agent reads.
      const vol = setupFs()
      const stderr = makeStderr()
      const out = buildPrompt({
        projectRoot: PROJECT,
    ...
    it('renders both roots as the same path when the loop exports no override', () => {
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: { TASK_SOURCE: 'folder' }, fs: vol })
      expect(out).toContain(`Your project root is \`${PROJECT}\``)
      expect(out).toContain(`${PROJECT}/.ralph/tasks/afk/todo/`)
    })
  })
  ...
  describe('triage-and-scale — QA hardening', () => {
    ...
    it('places step 3b strictly AFTER "Confirm the worktree" (step 3) and BEFORE the dev dispatch (step 4)', () => {
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: {}, fs: vol })
      const prepareIdx = out.search(/3\.\s+\*\*Confirm the worktree\*\*/)
  ...
  describe('issue selection query', () => {
    ...
    it('keeps the existing in-progress and failed exclusions', () => {
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: {}, fs: vol })
      const query = selectQuery(out)
      expect(query).not.toBeNull()
      expect(query).toContain('-label:in-progress')
      expect(query).toContain('-label:failed')
    })
  })
  ...
  // -------------------------------------------------------------------------
  // Jira orchestrator selection (#128)
  // -------------------------------------------------------------------------
  //
  // Jira is the SECOND task source that overrides the agent's own orchestrator.
  // Like folder, BOTH claude and codex get the one commit-direct template: the
  // delivery shape (commit straight to DEV_BRANCH, no branch, no PR, no
  // auto-merge) is a property of the source, not of the CLI driving it. These
  // tests pin both halves of that — jira beats the agent template, and neither
  // the folder nor the github arm changes because jira now exists.
  describe('jira task source selection (#128)', () => {
    const KEY = 'FOO-123'
    const jiraEnv = (extra = {}) => ({ TASK_SOURCE: 'jira', RALPH_TASK_KEY: KEY, ...extra })
    const JIRA_HEADER = '# Ralph Loop — Team orchestrator (Jira mode)'

    it('selects the jira orchestrator when TASK_SOURCE=jira', () => {
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: jiraEnv(), fs: vol })
      expect(out).toContain(JIRA_HEADER)
    })

    it('selects the jira orchestrator for RALPH_AGENT=codex too — the source wins', () => {
      const vol = setupFs()
      const out = buildPrompt({
        projectRoot: PROJECT,
        env: jiraEnv({ RALPH_AGENT: 'codex' }),
        fs: vol,
      })
      expect(out).toContain(JIRA_HEADER)
    ...
    it('normalizes a mixed-case / padded TASK_SOURCE to jira', () => {
      const vol = setupFs()
      const out = buildPrompt({
        projectRoot: PROJECT,
        env: { TASK_SOURCE: '  JIRA  ', RALPH_TASK_KEY: KEY },
        fs: vol,
      })
      expect(out).toContain(JIRA_HEADER)
    })

    it('leaves the folder arm alone — folder still beats the codex agent template', () => {
      const vol = setupFs()
      const out = buildPrompt({
        projectRoot: PROJECT,
        env: { TASK_SOURCE: 'folder', RALPH_AGENT: 'codex' },
        fs: vol,
      })
      expect(out).toContain('.ralph/tasks/afk')
      expect(out).not.toContain(JIRA_HEADER)
    })

    it.each([{}, { RALPH_AGENT: 'codex' }])(
      'leaves the github arm alone — %j never renders the jira orchestrator',
      (extra) => {
        const vol = setupFs()
        const out = buildPrompt({ projectRoot: PROJECT, env: { ...extra }, fs: vol })
        expect(out).not.toContain(JIRA_HEADER)
        expect(out).toContain('gh issue')
      },
    )

    it('interpolates RALPH_TASK_KEY everywhere the template names the ticket', () => {
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: jiraEnv(), fs: vol })
      expect(out).not.toContain('{{RALPH_TASK_KEY}}')
      // The read, the commit subject, the trailer, and the per-ticket log path all
      // have to resolve to the SAME key — the loop passes the key to exactly one
      // place (the log file name) and the prompt to another, so a template that
      // named the ticket inconsistently would silently split them.
      expect(out).toContain(`acli jira workitem view --key ${KEY}`)
      expect(out).toContain(`fix: <description> (${KEY})`)
      expect(out).toContain(`Resolves ${KEY}`)
      expect(out).toContain(`logs/ralph-issue-${KEY}.log`)
    })

    it('renders an empty ticket key rather than leaking the placeholder, and WARNS, when RALPH_TASK_KEY is unset', () => {
      const vol = setupFs({ projectFiles: { 'PROMPT.md': '' } })
      const stderr = makeStderr()
      const out = buildPrompt({
        projectRoot: PROJECT,
      ...
      expect(out).toContain(JIRA_HEADER)
      expect(out).not.toContain('{{RALPH_TASK_KEY}}')
      // It does not THROW — the loop pipes this straight into the agent, so a hard
      // failure here would turn a nameless ticket into a dead invocation — but a
      // keyless jira prompt is not silent either: every place the template names
      // the ticket renders empty, and interpolate() cannot report that because the
      // var IS in the bag. So buildPrompt says so itself.
      expect(stderr.calls).toHaveLength(1)
      expect(stderr.calls[0]).toContain('TASK_SOURCE=jira')
      expect(stderr.calls[0]).toContain('RALPH_TASK_KEY is unset')
    })

    it('composes all five roles and leaves no unresolved placeholder', () => {
      const vol = setupFs({ projectFiles: { 'PROMPT.md': '## Stack\nGo' } })
      const out = buildPrompt({
        projectRoot: PROJECT,
        env: jiraEnv({
    ...
    it('does not warn on unknown placeholders for the jira orchestrator', () => {
      const vol = setupFs({ projectFiles: { 'PROMPT.md': '' } })
      const stderr = makeStderr()
      buildPrompt({ projectRoot: PROJECT, env: jiraEnv(), fs: vol, stderr })
      expect(stderr.calls).toHaveLength(0)
    })

    it('has the agent read its ticket with acli and never with gh', () => {
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: jiraEnv(), fs: vol })
      expect(out).toContain('acli jira workitem view')
      // There is no GitHub in jira mode: no issue read, no PR, no merge.
      expect(out).not.toMatch(/gh issue/)
      expect(out).not.toMatch(/gh pr create/)
      expect(out).not.toMatch(/gh pr merge/)
    })

    it('tells the agent the ticket is already claimed and never to label it itself', () => {
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: jiraEnv(), fs: vol })
      expect(out).toMatch(/already claimed/i)
      expect(out).toMatch(/in-progress/)
      expect(out).toMatch(/Add NO label here/)
      // The bash claimed it in #127, so no step may re-claim it — and completion (#129)
      // goes through Ralph's own `complete` verb, so no step hand-rolls an acli label
      // write either: an `edit` spelled here would be a second copy of argv the loop
      // already owns, free to drift from it.
      expect(out).not.toContain('**Mark in progress**')
      expect(out).not.toMatch(/workitem edit/)
    })

    it('commits to DEV_BRANCH, says the commit stays local, and opens no PR', () => {
      const vol = setupFs()
      const out = buildPrompt({
        projectRoot: PROJECT,
        env: jiraEnv({ DEV_BRANCH: 'dev' }),
        fs: vol,
      })
      // #224: step 3 no longer checks the branch out and pulls it — the loop cut a detached
      // worktree at the local dev-branch tip and moves the branch afterward, so the branch
      // name lands in the prose describing that tree instead of in a `git checkout`.
      expect(out).not.toMatch(/git checkout/)
      expect(out).toContain('the local `dev`')
      expect(out).toMatch(/commit stays local/i)
      // Nothing pushes a jira-mode commit — MEASURED: `awk '!/^[[:space:]]*#/ && /push/'
      // templates/ralph.sh` prints nothing at all, so no line of that loop's CODE even
      // contains the word (the two lines `grep push` does match are #221's own comments,
      // added on this branch). The folder template used to promise that the bash pushes
      // for you, which is where this guard came from; #221 deleted that sentence, so now
      // neither template may claim it.
      expect(out).not.toMatch(/bash pushes/i)
    })

    it('names do-not-ralph in the restrictions instead of a folder-mode task lane', () => {
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: jiraEnv(), fs: vol })
      const restrictions = out.slice(out.indexOf('## Absolute restrictions'))
      expect(restrictions).toContain('do-not-ralph')
      expect(restrictions).not.toContain('.ralph/tasks/')
      expect(restrictions).toContain('CLAUDE_GIVE_UP')
    })

    // -----------------------------------------------------------------------
    // #129 — completion bookkeeping, asserted on the RENDERED prompt.
    //
    // These are prose acceptance criteria, and prose is the only thing the agent
    // actually receives, so they are pinned the way template content is pinned
    // everywhere else in this repo (template-parity.test.js, init.test.js): read the
    // rendered text and assert the claim. The failure they guard is specific — with no
    // PR and no pushed branch, the Jira comment is the ONLY audit trail of what Ralph
    // changed, so an instruction that comments before committing, or comments without
    // the SHA, produces a board that points at work nobody can find.
    // -----------------------------------------------------------------------
    it('orders the board writes AFTER the commit — commit, then complete, then comment (#129)', () => {
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: jiraEnv({ DEV_BRANCH: 'dev' }), fs: vol })
      const commit = out.indexOf('git commit -m')
      const complete = out.indexOf('jira-queue.js" complete')
      const comment = out.indexOf('jira-queue.js" comment')
      for (const [label, at] of Object.entries({ commit, complete, comment })) {
        expect(at, label).toBeGreaterThan(-1)
      }
      // The SHA cannot be commented before it exists, and a ticket marked done for a
      // commit that never happened is worse than one left in flight.
      expect(commit).toBeLessThan(complete)
      expect(complete).toBeLessThan(comment)
      // ...and the paragraph that said there was no step 7 is gone.
      expect(out).not.toMatch(/There is no step 7/)
      expect(out).toMatch(/^7\. /m)
    })

    it('marks completion through Ralph’s own verb rather than a hand-rolled acli write', () => {
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: jiraEnv(), fs: vol })
      // One module owns every acli argv Ralph sends. The prompt names the verb; it does
      // not re-spell `transition`, `--status`, `--labels` or `--remove-labels`, because a
      // second spelling is a second thing to correct when a transcribed flag is wrong.
      // THE KEY IS QUOTED at every call site, which is not cosmetic: the CLI reads one
      // argument for the key, so an unquoted key containing a space would complete a
      // ticket named by its first word and silently drop the rest.
      expect(out).toContain(`node "$RALPH_PKG_DIR/lib/jira-queue.js" complete "${KEY}"`)
      expect(out).toContain(`node "$RALPH_PKG_DIR/lib/jira-queue.js" comment "${KEY}"`)
      expect(out).not.toMatch(/workitem transition/)
      expect(out).not.toMatch(/--remove-labels/)
      expect(out).not.toMatch(/acli jira workitem comment/)
      // The prompt also says what a refused transition costs, so an agent that sees the
      // warning does not treat its own successful run as a failure.
      expect(out).toMatch(/refus/i)
    })

    it('describes the completion the SHIPPED DEFAULT performs, not the fully-configured one', () => {
      // Review finding, #129 round 1. An empty JIRA_DONE_STATUS makes `complete` skip the
      // transition, so the command makes TWO writes and prints a warning naming that knob.
      // When this test was written the template shipped that knob empty, so the empty case
      // WAS the fresh-install case; since #133 a jira `ralph init` writes a value, so the
      // empty case is a blanked knob or a config predating that prompt. The prompt still
      // has to describe it, because the two-write run is what those configs produce.
      // The prompt used to promise three writes and account for one warning ("Jira refused
      // the move"), i.e. it described the configured case and left the default case as an
      // unexplained line of output on the stream it tells the agent to read.
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: jiraEnv(), fs: vol })
      const step7 = out.slice(out.indexOf('jira-queue.js" complete'), out.indexOf('## Failed'))
      expect(step7).toMatch(/up to.{0,10}three/i)
      // Both warnings are named, and the default one is named as the likelier.
      expect(step7).toContain('JIRA_DONE_STATUS is not set')
      expect(step7).toMatch(/refused to transition/i)
      // ...and neither is a failure.
      expect(step7).toMatch(/not a failure/i)
      // The exit-code claim is the other half of the finding: `complete` also exits 1 for a
      // key it cannot use, before any label is attempted, so "exactly one reason" would send
      // the agent to "Failed" reporting a label write that never happened.
      expect(step7).not.toMatch(/exactly one reason/)
      expect(step7).toMatch(/two reasons/i)
      expect(step7).toMatch(/work item key/i)
    })

    it('the success comment carries the SHA, the branch and the test/lint result', () => {
      const vol = setupFs()
      const out = buildPrompt({
        projectRoot: PROJECT,
        env: jiraEnv({ DEV_BRANCH: 'dev', TEST_CMD: 'go test ./...' }),
        fs: vol,
      })
      const step7 = out.slice(out.indexOf('jira-queue.js" complete'), out.indexOf('## Failed'))
      // The SHA is the whole point: it is the only pointer from the board to the work,
      // and it is spelled as the command that produces it so the agent cannot invent one.
      expect(step7).toContain('git rev-parse')
      expect(step7.toLowerCase()).toContain('sha')
      // The branch, because the commit is local and unpushed — "which branch on which
      // machine" is the question a reader of that comment has.
      expect(step7).toContain('dev')
      // ...and the validation result from step 5, so the comment says whether the work
      // was checked rather than merely done.
      expect(step7).toMatch(/test/i)
      expect(step7).toMatch(/lint/i)
    })

    it('never says a LATER slice will write the SHA back — this slice does', () => {
      // Review finding, #129 round 1. Step 6 justified naming the ticket in the commit
      // subject with "until the next slice writes the SHA back to the board, that key is
      // the ONLY link" — a sentence step 7, fifty lines below, falsifies. Nothing pinned
      // it, which is how it survived the slice that made it false.
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: jiraEnv(), fs: vol })
      expect(out).not.toMatch(/next slice/i)
      // The positive, so this cannot pass on a step 6 that stopped explaining itself: the
      // two links are named as two, and the commit's one is placed against the comment's.
      const step6 = out.slice(out.indexOf('6. **Commit to'), out.indexOf('7. **Record'))
      expect(step6).toMatch(/two links/i)
      expect(step6).toMatch(/step.7/i)
    })

    it('classifies the two transition warnings by their ENDING, not their prefix', () => {
      // Review finding, #129 round 2, and a defect round 1 INTRODUCED at the seam between
      // its own two fixes: the held warning gained a second ending ("...and the done label
      // could not be written either, so it is still in Ralph's queue and this ticket is NOT
      // complete") while step 7 was rewritten to enumerate warnings by PREFIX and rule on
      // them unconditionally — "neither one is a failed ticket ... the ticket is still
      // complete ... carry on to the comment". Both listed prefixes are also the prefixes of
      // the NOT-complete ending, so the paragraph told the agent in bold that a warning whose
      // own text ends `NOT complete` is not a failed ticket.
      //
      // Not an edge: any config with JIRA_DONE_STATUS empty prints the held sentence on
      // every completion, and ANY label-write failure produces the pairing. (Since #133 a
      // jira init writes a value, so that is a blanked knob or an older config rather than
      // a fresh install — the sentence is no less reachable for it.)
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: jiraEnv(), fs: vol })
      const step7 = out.slice(out.indexOf('jira-queue.js" complete'), out.indexOf('## Failed'))
      // BOTH endings are quoted, because the ending is the only part of that line that says
      // which run this was. These strings are the ones `completeTask` actually emits.
      expect(step7).toContain("out of Ralph's queue")
      expect(step7).toContain('NOT complete')
      // The verdict is no longer shared. The old sentences said the opposite of the second
      // ending and must not come back.
      expect(step7).not.toMatch(/neither one is a failed ticket/i)
      expect(step7).not.toMatch(/same conclusion for both/i)
      expect(step7).not.toMatch(/the ticket is still complete/i)
      // ...and the NOT-complete ending routes to "Failed", where the exit code sends it too.
      const notComplete = step7.slice(step7.indexOf('NOT complete'))
      expect(notComplete).toMatch(/"Failed"/)
    })

    it('the failure comment carries the reason, and still adds NO label', () => {
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: jiraEnv(), fs: vol })
      const failed = out.slice(out.indexOf('## Failed'), out.indexOf('## Absolute restrictions'))
      expect(failed).toContain(`node "$RALPH_PKG_DIR/lib/jira-queue.js" comment "${KEY}"`)
      expect(failed.toLowerCase()).toContain('reason')
      // A failed invocation must NOT complete the ticket: `done` would take a ticket
      // nobody resolved out of the queue for good.
      expect(failed).not.toContain('jira-queue.js" complete')
      // Sweeping a ticket this invocation could not finish is the outer bash's job
      // (#130), so the agent still invents no label of its own.
      expect(failed).toMatch(/not.*label/is)
      expect(failed).toContain('failed')
    })

    it('the restrictions permit completion by the documented path, and only that path', () => {
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: jiraEnv(), fs: vol })
      const restrictions = out.slice(out.indexOf('## Absolute restrictions'))
      // The #128 rule was a flat prohibition, and step 7 now does exactly what it
      // forbade. A restriction that contradicts a step is worse than no restriction:
      // the agent has to guess which half of its own prompt to believe.
      expect(restrictions).not.toMatch(/NEVER transition/)
      expect(restrictions).toMatch(/step 7/)
      // Still fenced, though — the permission is for Ralph's verb, not for acli writes
      // of the agent's own devising.
      expect(restrictions).toMatch(/jira-queue\.js/)
    })
  })
