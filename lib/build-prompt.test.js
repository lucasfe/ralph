...
describe('buildPrompt', () => {
  ...
  describe('jira task source selection (#128)', () => {
    ...
    it('describes the completion the SHIPPED DEFAULT performs, not the fully-configured one', () => {
      // Review finding, #129 round 1. `templates/ralph.config.sh` ships JIRA_DONE_STATUS
      // EMPTY, and an empty status makes `complete` skip the transition — so on every
      // fresh install the command makes TWO writes and prints a warning naming that knob.
    ...
    it('classifies the two transition warnings by their ENDING, not their prefix', () => {
      ...
      //
      // Not an edge: JIRA_DONE_STATUS ships empty, so the held sentence is printed on
      // essentially every install, and ANY label-write failure produces the pairing.
      const vol = setupFs()
      const out = buildPrompt({ projectRoot: PROJECT, env: jiraEnv(), fs: vol })
      const step7 = out.slice(out.indexOf('jira-queue.js" complete'), out.indexOf('## Failed'))
