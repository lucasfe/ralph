...
describe('readVersionCache — total, never throws', () => {
  ...
  // #215 added a FOURTH field, `latest_version_channel` — which channel resolved
  // `latest_version` — so every assertion in this file about "the shape" grew one entry. The
  // subject of these tests has not moved (what is read back, what is dropped, what is
  // normalized); the shape they enumerate has, and a list that enumerates a shape has to.
  it('reads the four cache fields back', () => {
    const fs = vol({
  ...
  it('nulls out hand-mangled non-string fields instead of trusting them', () => {
    const fs = vol({
      [CACHE_PATH]: JSON.stringify({
        ...
        latest_version: ['0.17.0'],
        // #215: the new field is mangled the same way and nulled by the same rule — which is
        // what makes a hand-edited channel unreadable rather than a channel named `[object
        // Object]`, and therefore a number served to nobody.
        latest_version_channel: { npm: true },
      }),
    })
    expect(readVersionCache({ fs, home: HOME, processEnv: {} })).toEqual(
      EMPTY_VERSION_CACHE,
...
describe('writeVersionCache', () => {
  ...
  it('writes the fixed four-field shape even from a partial cache', () => {
    const fs = vol()
    writeVersionCache({ cache: { latest_version: '0.17.0' }, fs, home: HOME, processEnv: {} })
    // #215: an unattributed version is written unattributed — the write does not invent a
    // channel for a caller that named none, because a wrong stamp is the whole bug.
    expect(JSON.parse(fs.readFileSync(CACHE_PATH, 'utf8'))).toEqual({
      last_check_at: null,
      last_prompted_at: null,
      latest_version: '0.17.0',
