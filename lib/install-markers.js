...
// #215: the CHANNELS a published version can come from — the ids the update-check cache
// files a resolved version under, so a copy of Ralph only ever reads back the number its
// own channel produced.
//
// TWO IDS, WHICH IS COARSER THAN THE STORE ROWS BELOW AND EXACTLY RIGHT. pnpm, yarn and
// bun each have a directory of their own and an update command of their own, but all three
// install the npm TARBALL — so all three ask `npm view`, all three get the same answer, and
// a number one of them resolved is a true answer for the other two. An npx cache and a dev
// checkout are npm's too. Only Homebrew installs from somewhere else: its own tap, which
// #196 added precisely so a refused `npm publish` cannot stop a release from being
// installable, and which therefore holds a different version on purpose. The partition is
// "who published the number", not "who installed the copy".
//
// These ids go IN A CACHE FILE, so they are a storage format and not a wording: they are
// short, lowercase and stable, and renaming one would orphan every stamp on every machine
// (readable, unattributed — see `cachedVersionFor` in lib/version-cache.js — but a lost
// version notice for one weekly window). The user-facing sentences stay the `channel`
// fields on the rows below; nothing renders these.
//
// SPELLED ONCE HERE, and read by both sides of the cache: lib/install-target.js decorates
// the Homebrew row's version query with the row's own `versionChannel` (so the WRITER files
// the answer under the channel it asked), and `versionChannelFor` below derives the same id
// from an install path (so the READER can ask for its own). `lib/update-check.js` cannot
// import this module — lib/update-check.channel.qa.test.js pins that it takes no dependency
// on install-layout recognition at all — so NPM_VERSION_QUERY spells `'npm'` for itself,
// and lib/update-check.cache-channel.test.js joins the two spellings from the outside.
export const VERSION_CHANNEL = Object.freeze({
  NPM: 'npm',
  BREW: 'brew',
})
...
export const INSTALL_MARKERS = deepFreeze([
  ...
  },
  {
    ...
    // #215: ...and the id the same fact is STORED under, in the update-check cache. On the
    // row for the reason `channel` is: a marker, the words it is reported with and the
    // channel whose versions it is compared against are one fact, and the day this marker
    // is widened is the day a second table would start attributing it wrongly. The only row
    // that carries one, because it is the only row that does not install from npm —
    // `versionChannelFor` below defaults every other layout, which is what keeps the three
    // npm-shaped stores sharing NPM_VERSION_QUERY by identity over in lib/install-target.js.
    versionChannel: VERSION_CHANNEL.BREW,
...
/**
 * Which CHANNEL's published versions this copy of Ralph should be compared against.
 *
 * #215: the READ side of the channel-keyed update-check cache. The writer knows its channel
 * because it just spawned the query — the id travels on the query descriptor
 * lib/install-target.js attaches to every classification — but the reader has no descriptor:
 * `ralph doctor` draws its `cached` row from the cache alone, and it is architecturally
 * forbidden from importing lib/install-target.js (that module imports a process runner at
 * module scope; doctor's import-graph spec is in lib/commands/doctor.version-line.qa.test.js).
 * So the reader derives the id from its own install path. PATH MATCHING AND NOTHING ELSE — no
 * subprocess, no socket, no environment, and, unlike `describeInstallChannel` above, NO
 * FILESYSTEM PROBE either.
 *
 * WHY NO PROBE, since the row two functions up takes two. Ask what a probe could change here:
 * `linkSignal` and the npx marker both answer "npm" (a working tree and a `npm link` are npm's,
 * and an npx run is npm's by definition), and so does every store row except Homebrew's. So the
 * ONLY path whose id a probe could move is a Cellar that is also a symlink to a working tree —
 * a contributor who linked their checkout into `<prefix>/Cellar/ralph/<version>/libexec/...`,
 * which is not a layout that exists. Against that: the probes are not free of consequence.
 * `ralph doctor` would pay them TWICE for one path (its `channel` row takes the same two), and
 * two suites count doctor's syscalls exactly because "the row costs path matching" is a promise
 * (lib/commands/doctor.install-channel.qa.test.js), and `ralph start` would grow a filesystem
 * seam it has no other use for, before its first preflight line. A version comparison is not
 * worth a syscall the answer cannot depend on.
 *
 * THE ONE DIVERGENCE THAT BUYS, stated rather than hidden: for that impossible linked Cellar,
 * `classifyInstall` says "checkout" and asks npm, while this says brew. They disagree, so the
 * reader is served NOTHING and the row reads "nobody has checked yet" — the fail-safe direction,
 * and the same one every other unattributable number takes. A wrong number is the bug; a missing
 * one is a week of a late notice.
 *
 * TOTAL, AND IT ALWAYS NAMES A CHANNEL — the one way this differs from
 * `describeInstallChannel`, which answers null for a path it was not given. That function
 * feeds a ROW, and a row can be silent; this feeds a COMPARISON, and there is no silent
 * version to compare against. So every layout no marker claims answers npm, which is the
 * same fallback lib/install-target.js makes for an unrecognized path (`latest:
 * NPM_VERSION_QUERY`) and the same one #201's row hedges as `npm or other (not probed)`. That
 * is what keeps the two derivations joined: a reader and a writer looking at one install
 * agree, including when neither can place it.
 *
 * A GUESS OF npm IS NOT THE #215 BUG, and the distinction is the whole safety argument. The
 * bug was reading ANOTHER channel's number under this channel's name; npm here is a reading
 * of npm's own number by a copy that is, on the evidence available without a spawn, an
 * npm-shaped install. The one layout that is NOT is the one layout a path can prove — a
 * Cellar — so a Homebrew copy is never mistaken for an npm one. The reverse mistake stays
 * possible in principle (a plain `npm install -g` cannot be positively identified without
 * `npm root -g`) and is harmless: it reads npm's number, which is its own.
 *
 * @param {object} [options]
 * @param {string} [options.ralphHome] the package root to place. Blank, absent or not a string
 *   answers npm, per the paragraph above — never null, because a caller with no channel has
 *   nothing to read a version for.
 * @returns {string} a VERSION_CHANNEL id, always
 */
export function versionChannelFor({ ralphHome } = {}) {
  // THE `typeof` GATE LIVES HERE, not in the caller — the opposite of `describeInstallChannel`,
  // which lib/commands/doctor.js gates from the outside. The reason is the return type: a caller
  // can gate a function that may answer null, because refusing to ask and getting no answer are
  // the same thing; this one always names a channel, so a caller that gated it would have to
  // invent the fallback itself, in every caller, and the two would drift. What the gate refuses
  // is `normalizePath`'s coercion, which is a real trap and not a formality: `String(0)` is
  // `'0'`, which RESOLVES AGAINST THE PROCESS CWD, and a bag whose `toString` throws would take
  // `ralph doctor` down with it (lib/commands/doctor.install-channel.qa.test.js pins both).
  if (typeof ralphHome !== 'string') return VERSION_CHANNEL.NPM
  const home = normalizePath(ralphHome)
  if (!home) return VERSION_CHANNEL.NPM
  const stores = matchingStores(pathSegments(home))
  // An AMBIGUOUS path answers npm rather than picking a manager, exactly as the two
  // consumers of `matchingStores` fail closed on it: `classifyInstall` refuses to run
  // anything and doctor's row names the collision. Here "fail closed" is npm, because npm is
  // what every colliding store but brew installs from — and a Cellar that also matched a
  // pnpm store is not a path this can attribute a tap's version to.
  if (stores.length !== 1) return VERSION_CHANNEL.NPM
  return stores[0].versionChannel ?? VERSION_CHANNEL.NPM
}
