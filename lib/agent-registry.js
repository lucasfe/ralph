...
//     template, jq output-stream filter program, and the environment the loop
//     must set before invoking that CLI).
...
export function agentSpec(agent) {
  ...
  // corrupt the registry for every later caller in the same process.
  return { ...spec, argv: [...spec.argv], env: { ...spec.env } }
}
