...
//     template, jq output-stream filter program, and the one-shot DIGEST
//     invocation spec).
...
export function agentSpec(agent) {
  ...
  // and the nested digest block with ITS two arrays (#61), which a top-level
  // spread would otherwise hand out by reference.
  return {
    ...spec,
    argv: [...spec.argv],
    digest: {
