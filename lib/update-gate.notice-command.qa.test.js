...
// #215: a channel id of its own, so the tripwire cache below is READABLE by the poison
// classification. Without one, a throttled run over it would report nothing because the number
// could not be attributed — and a test about spending would be passing for a reason that has
// nothing to do with spending.
const POISON_QUERY = { argv: ['poison-query', 'latest'], format: 'semver-line', channel: 'poison' }
const poison = () =>
  recording(async () => ({
    kind: 'poison',
...
//
// #215 keyed the number by the channel that resolved it, so the seed names one — `brew` by
// default, because the runs that read a version out of this cache are the Homebrew ones below.
// An unstamped number is readable by nobody, which is the fix and would silently gut every
// "throttled run still prints the notice" assertion here.
const throttledCache = (latest = LATEST, channel = 'brew') =>
  Volume.fromJSON(
    {
...
describe('runUpdateGate — what the cheap paths must not spend (#200 QA)', () => {
  ...
  it('consults it ONCE on a throttled run with nothing newer to say, and spends nothing else', async () => {
    // #215 CHANGED THIS EXPECTATION, and here is why the old one was wrong.
    //
    // It said a throttled run with nothing to report asks the layout zero times. That was true
    // and it was true for a bad reason: the run served a cached version WITHOUT KNOWING WHOSE IT
    // WAS. On a two-install machine that is #215 — the brew copy's 0.27.0 read by the npm copy,
    // which then advertises it with `npm i -g`. Comparing a number to the running version is
    // exactly the point at which the number has to have an owner, so the channel is resolved
    // above the throttle branch now, and this path pays for one classification.
    //
    // What criterion 7 actually protects is intact, and the last two assertions are it: no
    // subprocess, and no output. `runUpdateGate` hands `classify` a NULL exec precisely so a
    // classification costs no spawn (update-gate.js:93) — it is path matching over RALPH_HOME
    // plus at most two stats, on a run that already read a file. That is the budget #200 sized
    // this seam for, and one more read of a memoized promise is inside it.
    const g = gate({
      classify: poison(),
      currentVersion: LATEST,
      cacheFs: throttledCache(LATEST, 'poison'),
    })
    await runUpdateGate(g)
    expect(g.stdout.output()).toBe('')
    expect(g.exec.calls).toEqual([])
    // The classification happened; nothing it CARRIES was acted on — no notice, no query.
    expect(g.classify.calls).toEqual([{ exec: null }])
    expect(g.stdout.output()).not.toContain(POISON_LABEL)
    expect(g.exec.keys()).not.toContain('poison-query latest')
  })
...
describe('runUpdateGate — the cache carries one channel to every reader (#200 QA, fixed by #215)', () => {
  it('a throttled run reads its OWN channel’s number, and nothing when there is none', async () => {
    // #215 CHANGED THIS EXPECTATION. It used to assert the bug — the test was called
    // "DOCUMENTED: a throttled run reads whichever channel last wrote, not its own" and it
    // required the npm-layout run to print the TAP's version beside `npm i -g`. That was an
    // honest pin of a known defect, and it was the wrong verdict to hold a suite to: #215 was
    // filed off exactly this scenario, from a real machine, and the number the npm copy would
    // have been sent to install (`npm i -g @lucasfe/ralph@latest`) is 0.23.0 — the version npm
    // has been stuck on since the 403 of #196, three releases behind what the notice promised.
    //
    // The old comment named its own fix: "the fix is a field in the cache, which is `ralph
    // doctor`'s shape too". That field now exists, so the pin flips to the behaviour instead of
    // the defect. Same two runs, same one cache file, same week.
    const brew = gate({
      classify: recording(layoutAt(BREW_RALPH)),
    ...
    // The throttle is unchanged — the npm run still asks nobody, which is what made the bug
    // reachable and is also #24's whole point. What changed is that silence is now the answer
    // for a number it cannot claim, rather than the other channel's number under its own advice.
    expect(npm.exec.calls).toEqual([])
    expect(noticeOf(npm)).toBeUndefined()
    expect(npm.stdout.output()).not.toContain(LATEST)
    // ...and the tap's pair is still on disk, untouched, for the copy it belongs to.
    const cache = readVersionCache({ fs: brew.cacheFs, home: HOME, processEnv: {} })
    expect(cache).toMatchObject({ latest_version: LATEST, latest_version_channel: 'brew' })
  })
})
