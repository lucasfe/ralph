...
// box beside — or, on a terminal too narrow for that, under — the sprite that says which
// Ralph this is, and it knows nothing about pixels. #68 gives it its first two facts — the
// installed version and the working directory — plus the one piece of advice the box
// carries, an update hint. WHERE the box goes is #161's rung in `bannerLayout` below and
// lib/banner-beside.js's join; this file draws the same box either way.
//
...
// at all: the whole ladder — beside the sprite at 72 columns and up, unbox under 44, no
// sprite under 26 — is `bannerLayout` below, decided from the injected `width` alone, which
// is what makes every rung of it testable without a terminal. lib/sprite-banner.js asks the
// same function rather than keeping its own copy of 26, and #161's `ralph start` asks it
// rather than keeping its own copy of 72.
//
 ...
 * #69's model ids, narrow enough that the eye does not lose the rule when the box sits
 * under a 26-column sprite. It is a TARGET rather than a minimum — a 200-column terminal
 * gets the same box, in either arrangement, because a 200-wide rule is a line nobody can
 * follow, and #161's `besideWidth` is capped here too — and it is
 ...
 *
 * 26 is the sprite's own cell width, and it is the lower rung because the two blocks are
 * STACKED whenever the screen is not wide enough to hold them side by side (see `beside`
 * in `bannerLayout`): stacked, each block gets the whole width to itself, so the sprite is
 * the narrow element and the box the wide one. The box therefore gives way first and the
 * sprite is dropped last — at 26 it fits exactly, and below that it cannot be drawn at all.
 * A clipped sprite is not a smaller sprite, it is half a face with a torn edge, so it goes
 * whole.
 *
 ...
 * art is redrawn wider the suite says so rather than a terminal finding out.
 *
 * WHAT FINDING OUT WOULD COST IS WORSE SINCE #161 than the torn 26-column edge this note used
 * to name, which is the reason that test matters more than it did. The beside arrangement is
 * budgeted here, from this constant, while the box is offset at the call site by the REAL
 * `spriteWidth` out of lib/sprite-data.js: a sprite redrawn to 30 cells would therefore push
 * every joined line four columns past the right edge of a terminal this function had declared
 * wide enough — and a line the terminal folds in two is a row lib/sprite-player.js never
 * counted, so the animation would walk back up through it.
 */
export const SPRITE_MIN_WIDTH = 26

/**
 * The air between the sprite and the box when they sit side by side (#161).
 *
 * Two columns: enough that the box's left border reads as its own edge rather than as part
 * of the picture, and not so much that a 72-column terminal loses a rung over whitespace.
 *
 * It lives HERE, on this side of #122's seam, for the same reason `LABEL_WIDTH` does — a
 * gutter is a number of COLUMNS, so the half that knows how wide the terminal is owns it.
 * `bannerLayout` subtracts it to decide whether the arrangement fits at all, and
 * lib/banner-beside.js spends it as literal spaces; exporting it is what keeps those two
 * from disagreeing about how far to the right the box starts.
 */
export const BESIDE_GAP = 2
...
/**
 * The degradation ladder, as one decision — #72's whole story, plus #161's arrangement.
 *
 * Pure, total and the ONLY place any rung is read: lib/sprite-banner.js asks this
 * function whether it may draw rather than holding a 26 of its own, because two copies
 * of a threshold are two thresholds the day one of them moves. #161 added the third rung
 * here for exactly that reason — `ralph start` decides WHERE to put the box, and it asks.
 *
 ...
 *   beside: boolean, besideWidth: number}}
 ...
 *   the rows are LAID OUT at wherever the box is NOT beside the sprite, which is smaller on a
 *   narrow terminal and capped at the 60-column target on a wide one. `boxed` is whether
 *   the frame is drawn, `sprite` whether the sprite may be. `beside` is whether the box
 *   may sit to the RIGHT of the sprite instead of under it, and `besideWidth` the width to
 *   lay it out at there — the columns left over, capped at the same target, and never
 *   negative even though it is meaningless wherever `beside` is false. Never throws — a
 *   banner is not worth losing a run over.
 */
export function bannerLayout(width) {
  const limit = usableWidth(width)
  // What is left of the terminal once the sprite and the air between them are paid for.
  // Floored at zero rather than left negative: it reaches a `repeat` in lib/banner-beside.js.
  const leftover = Math.max(0, limit - SPRITE_MIN_WIDTH - BESIDE_GAP)
  return {
    width: limit,
    boxWidth: Math.min(limit, BANNER_WIDTH),
    boxed: limit >= BOX_MIN_WIDTH,
    sprite: limit >= SPRITE_MIN_WIDTH,
    beside: leftover >= BOX_MIN_WIDTH,
    besideWidth: Math.min(leftover, BANNER_WIDTH),
  }
}
