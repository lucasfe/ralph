...
// The value column. `update` and `cwd` were this slice's labels; #69's `context` is the
// longest one this box draws, which is why the gutter is eight rather than seven — one space
// of air after the longest label there will ever be here, so adding a row never re-flows the
// ones above it.
const LABEL_WIDTH = 8
...
// still gated on a fact `ralph start` does not pass: `os` and `cachedLatest` are DOCTOR's
// questions, so its box is what they draw on and start's is unchanged by them, which
// banner-compose.test.js pins as a literal. (Until #69 that gate was also the whole reason
...
// pushes at the call site: `source` and `repo` are plain facts, exactly the shape
...
//
// SPELLED HERE, NOT IMPORTED, and the same argument SPRITE_MIN_WIDTH makes above: this
// module's import list is one line long and the reason is written at the top of the file.
// banner-compose.test.js holds the two together instead — it enumerates MODEL_PROVENANCE and
// demands a DISTINCT sentence for every tag in it, so a fourth tag added there with no wording
// here fails a test rather than printing a row nobody wrote.
//
...
// this file, because `repo` comes out of GH_REPO — an ambient environment variable — and a
// `.git/config` nobody reads as bytes, and `os` out of a platform detector: `textOr` refusing
// non-strings is what stops a hostile bag's `toString` running just because someone put an
// object in the facts. The value pushed
...
// ONE GATE, TWO TABLES (#69): the rows it draws are named by the `table` it is handed rather
// than by a constant it reads, so `source` and `repo` — which sit in a different PLACE in the
// box — cost a table entry rather than a second copy of the gate above.
...
// the row gate exists to prevent. `rowLine` gates the result again, which is idempotent and
// left in place deliberately — the rule is that every row is gated by the builder that makes
// it, not that every value is gated exactly once.
...
function windowTokens(value) {
  ...
  // function's own arithmetic. The table below pins `0` as no row; a value that BECOMES zero
  // here is that same number and gets that same answer.
  //
...
// and #70 adds bullets; if the sanitisation sat at the call sites,
// `{ label: 'repo', value: facts.repo }` would be a plain-looking line that re-opens all
// three of the hostile-fact defects at once — a `\n` out of GH_REPO or a hand-edited
// `.git/config` forging an unframed line outside the width guarantee, an ESC leaking into
// a run that promised none, and `gutter + value` coercing a hostile object's `toString` on
// a value that came from a JSON cache.
//
