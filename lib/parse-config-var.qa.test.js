...
describe('parseConfigVar — the widened comment strip, measured against bash (#62 QA)', () => {
  ...
  it.each([
    ...
    async (_label, line, bashSays, parserSays) => {
      ...
      // shape lib/commands/start.js:277 uses — the same 626 shapes leave 211 divergences, so 144 of
...
describe('parseConfigVar — whitespace around the `=`, and a bare `=` (#147 QA)', () => {
  ...
  // it into one closure, `sourcedValue` at lib/commands/start.js:277, and pointed EVERY knob of
  // `ralph start`'s box at it — RALPH_AGENT (:347), GH_REPO (:1311), RALPH_CODEX_MODEL (:538),
  // RALPH_CONTEXT_WINDOW (:544) and TASK_SOURCE (:510).
  //
  ...
  // What `sourcedValue('TASK_SOURCE')` at start.js:510 evaluates to — deliberately spelled through
  ...
  it('`TASK_SOURCE = folder` resolves to the same source in the loop and in `ralph start`', async () => {
    ...
    // `parseConfigSource` at all: `resolveSource` there (lib/commands/start.js:490) is handed
    // `sourcedValue('TASK_SOURCE')` (:510), the closure at :277 that asks
