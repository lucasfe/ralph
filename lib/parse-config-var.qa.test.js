...
describe('parseConfigVar — the widened comment strip, measured against bash (#62 QA)', () => {
  ...
  it.each([
    ...
    async (_label, line, bashSays, parserSays) => {
      ...
      // shape lib/commands/start.js:298 uses — the same 626 shapes leave 211 divergences, so 144 of
...
describe('parseConfigVar — whitespace around the `=`, and a bare `=` (#147 QA)', () => {
  ...
  // it into one closure, `sourcedValue` at lib/commands/start.js:298, and pointed EVERY knob of
  // `ralph start`'s box at it — RALPH_AGENT (:368), GH_REPO (:1355), RALPH_CODEX_MODEL (:559),
  // RALPH_CONTEXT_WINDOW (:565) and TASK_SOURCE (:531).
  //
  ...
  // used to be spelled here is still live in three other commands (cycle.js:194, status.js:384,
  // doctor.js:208), and `||` reaches past a BLANK config value where the presence test keeps it, so
  ...
  // What `sourcedValue('TASK_SOURCE')` at start.js:531 evaluates to — deliberately spelled through
  ...
  it('`TASK_SOURCE = folder` resolves to the same source in the loop and in `ralph start`', async () => {
    ...
    // `parseConfigSource` at all: `resolveSource` there (lib/commands/start.js:511) is handed
    // `sourcedValue('TASK_SOURCE')` (:531), the closure at :298 that asks
