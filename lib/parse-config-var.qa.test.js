...
describe('parseConfigVar — the widened comment strip, measured against bash (#62 QA)', () => {
  ...
  it.each([
    ...
    // Whitespace AFTER the `=`, which is what is LEFT of the pair #147 was filed for. Its
    // sibling — whitespace BEFORE the `=` — used to sit on this row and is now refused by the
    // grammar (see "refuses whitespace BEFORE the `=`" above, and the #147 block at the foot
    // of this file for both halves measured side by side). This half stays, because the rule
    // it belongs to is bash's WORD rule rather than its assignment rule: `X= folder` and
    // `X=a b` are one family, `X= folder` is not more wrong than `X=a b`, and the parser has
    // never modelled either. Fixing one and not the other would be a new inconsistency.
    ['whitespace after the =', 'X= folder', '«unset»', 'folder'],
    ['a space inside an unquoted value', 'X=a b', '«unset»', 'a b'],
  ])(
    ...
    async (_label, line, bashSays, parserSays) => {
      ...
      // parse-config-var.test.js pins the concatenation shapes, and the two `whitespace
      // after the =` / `a space inside an unquoted value` rows above mark the other. The
      // sweep itself is not in this repo: the 1554 follows from
...
describe('parseConfigVar — whitespace around the `=`, and a bare `=` (#147 QA)', () => {
  ...
  // `??` onto the environment. Written out here so the comparison below is against a shape a
  // caller uses rather than against `parseConfigVar` alone. TWO of the seven knobs are read
  // this way, not all of them: RALPH_AGENT (lib/commands/start.js:280) and GH_REPO (:1157).
  //
  // TASK_SOURCE — which is the knob the rows below are written on — has a reader of its own:
  // `parseConfigSource(configText) || processEnv.TASK_SOURCE` (lib/commands/start.js:446, and
  // the same `||` at cycle.js:190, status.js:379 and doctor.js:138). `||` reaches past a BLANK
  // config value where `??` keeps it, so the two shapes are not interchangeable in general —
  // that divergence is documented at lib/commands/start.js:406-408 / :416-429 and pinned at the
  // command in lib/commands/start.precedence.qa.test.js ("reaches past a blanked TASK_SOURCE
  // too, on the same `||` — pinned, not endorsed"), with the blank row measured through both
  // shapes in parse-config-var.boundary.qa.test.js ("a blanked TASK_SOURCE parts the two
  // shapes"). No row below carries a blank config value, so on these rows the two shapes agree
  // — and rather than assert that in prose, the test asserts BOTH, which is what lets its title
  // say `ralph start`.
  const jsResolve = (config, name, ambient) =>
    (configAssignsVar(config, name) ? parseConfigVar(config, name) : null) ?? ambient ?? UNSET
  // TASK_SOURCE's real reader, spelled exactly as lib/commands/start.js:446 spells it.
  const startResolve = (config, ambient) => parseConfigSource(config) || ambient
  ...
  it('`TASK_SOURCE = folder` resolves to the same source in the loop and in `ralph start`', async () => {
    ...
    // lifted above; the JS half is `resolveSource` over `parseConfigSource`, which is the
    // pair lib/commands/start.js line 445 calls.
  ...
  it('leaves whitespace AFTER the `=` where it was, and says what bash does with it', async () => {
    // THE SCOPE LINE, measured rather than argued. A space after the `=` is a DIFFERENT
    // bash rule from a space before it: `X= folder` parses fine, assigns nothing, and runs
    // `folder` as a command with `X` empty in its environment only — so the sourcing shell
    // is left with whatever it already held, exactly as for `X = folder`.
    ...
    // The parser still reads `folder`, and that is deliberate: this spelling belongs to
    // bash's WORD rule, not its assignment rule, and the same rule is what makes
    // `TASK_SOURCE=fol der` a non-assignment too. The parser has never modelled either, and
    // half of a rule is worse than a documented whole — so both stay, both are pinned as
    // divergences in the table above, and lib/parse-config-var.js records this transcript.
