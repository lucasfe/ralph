...
// #215 keyed the cached version by the channel that resolved it, so a seed carries the stamp —
// defaulted to `npm`, which is the channel every run in this file asks: none passes a
// `latestSource`, and OMITTING it takes the parameter default, which is the npm query itself
// (lib/update-check.js:251). So these runs NAME npm — both as the channel that would have
// written what the seeds pretend was written and as the channel reading it. It is a named
// channel and not a fallback, which is the distinction the slice turns on: a run that TRIES to
// name its channel and fails reads nothing at all (`readingChannelOf`, lib/update-check.js
// :447-449). Without the stamp the throttled path
// serves nothing, and the subjects here (the prompt window's verdict over a cached version, the
// fields a stamp carries through, what a decision write drops) would be asserted against a
// decision with no version in it. The spread is last so a test whose subject IS the stamp can
// spell its own.
const seeded = (cache) =>
  Volume.fromJSON(
  ...
  for (const [label, raw] of hostile) {
    it(`${label} → both windows due, and the run repairs the file`, async () => {
      ...
      expect(cacheOf(vol)).toEqual({
        ...
        latest_version: '0.2.0',
        // #215: the repair writes the channel it just asked. That is the same claim the rest of
        // this assertion makes — a run over a garbage file leaves a FULL, valid cache behind —
        // and the field is part of what "valid" now means: a number nobody could read would be
        // a repair in name only.
...
describe('QA #26 there is no declined_version, anywhere', () => {
  it('is dropped by the DECISION write, not only by the stamp', async () => {
    ...
    expect(Object.keys(JSON.parse(raw)).sort()).toEqual([
      ...
      'last_prompted_at',
      'latest_version',
      // #215: a fourth key, and this assertion is the reason to name it rather than loosen the
      // list — it is a whitelist proving `declined_version` never reaches disk, so it must say
      // exactly which keys DO.
  ...
  it('the empty cache shape has exactly the three #26 fields, plus #215’s key', () => {
    // The name says "three #26 fields" because that is still what #26 owns; the fourth belongs
    // to #215 and is named as such. The claim — a FIXED shape, with no room for a
    // `declined_version` to appear in it — is unchanged, and it is what forces this list to
    // grow with the shape instead of being relaxed.
...
describe('QA #26 the write is the fixed field shape', () => {
  it('carries last_check_at, latest_version and its channel through, adding only the stamp', () => {
    ...
    expect(JSON.parse(rawOf(vol))).toEqual({
      ...
      last_prompted_at: iso(T0),
      latest_version: '0.2.0',
      // #215: the channel travels with the version it attributes, and this assertion is where
      // that matters most — a stamp that carried the version through and DROPPED its channel
      // would turn a readable cached version into one no copy can read, so being prompted would
      // silently cost the notice next week.
      latest_version_channel: 'npm',
    })
  })
...
describe('QA #26 the two windows crossing over one file', () => {
  ...
  it('ten stamps in the same millisecond leave one valid file and one closed window', async () => {
    ...
    expect(JSON.parse(rawOf(vol))).toEqual({
      ...
      last_prompted_at: iso(T0),
      latest_version: null,
      // #215: no version was ever resolved here, so no channel is claimed either — the pair is
      // null together, which is what "nobody has checked yet" looks like to every reader.
