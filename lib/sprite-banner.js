 ...
 * signal source to be interrupt-safe on. #74's `RALPH_BANNER=static` may use either and get
 * the same bytes — a `cycles` of 1 writes exactly these rows and nothing else, with no hide
 * and no restore, because the player only hides a cursor it is going to redraw over — so
 * that mode is a choice about plumbing, not about output, and neither choice deletes this.
 *
 * @param {object} [options]
 ...
 * once. How many frames get played is the player's `cycles`, because that is the thing
 * #74's `full`/`static` modes will want to vary, and it is not a fact about the art.
 *
 * @param {object} [options]
 * @param {boolean} [options.isTTY] whether stdout is a terminal
 * @param {boolean} [options.color] whether ANSI colour may be emitted
