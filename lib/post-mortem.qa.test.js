...
// A JIRA RUN'S CARD, NEWLY POSSIBLE (#131 QA). Everything above this line was written when
// only `github` and `folder` appended events, so `tallyVerdicts` had never been handed a
// number DERIVED from something — and the fallback it feeds is the whole reason this module
// exists: an interrupted run never reached endRun, so its record carries no counts and its
// events are the only account of what it finished. #131 makes that path reachable for a Jira
// run, which is the state a hard-killed overnight Jira run now leaves behind.
describe('buildPostMortem — an interrupted JIRA run tallied off its own events (#131 QA)', () => {
  const JIRA_RUN = 'ralph-repo-9f2c1a'
  const jiraEvent = ({ number, key, verdict, cost = 30 }) =>
    'RALPH_ISSUE_EVENT ' +
    JSON.stringify({
      issue_number: number,
      ...(key === undefined ? {} : { task_key: key }),
      run_id: JIRA_RUN,
      verdict,
      total_cost_usd: cost,
    })

  // FOO-123 failed, FOO/9 failed with no number the grammar could read out of it, FOO-124
  // passed. The record is `killed()`'s shape with a jira `current`, so both count sources are
  // the ones a kill really leaves: null on the record, and these three lines on disk.
  const JIRA_METRICS = [
    jiraEvent({ number: 123, key: 'FOO-123', verdict: 'fail' }),
    jiraEvent({ number: null, key: 'FOO/9', verdict: 'fail' }),
    jiraEvent({ number: 124, key: 'FOO-124', verdict: 'pass' }),
  ].join('\n')
  ...
  const jiraRecord = () =>
    killed({
      ...
      current: {
        number: 123,
        task_key: 'FOO-123',
        started_at: new Date(2026, 7, 26, 5, 30, 0).toISOString(),
        iteration: 9,
      },
    })
  ...
  const jiraCard = () =>
    renderPostMortem(
      buildPostMortem({ metricsText: JIRA_METRICS, record: jiraRecord(), queue: 2, now: NOW }),
    )

  it('counts every event and lists only the ones it can name', () => {
    const snapshot = buildPostMortem({
      metricsText: JIRA_METRICS,
      record: jiraRecord(),
      queue: 2,
      now: NOW,
    })
    expect({ ok: snapshot.ok, failed: snapshot.failed }).toEqual({ ok: 1, failed: 2 })
    // The list is SHORTER than the count, and that is the correct shape rather than a
    // dropped failure: an unreadable key still costs the run an iteration, it just has no
    // `#number` to print. The same split lib/issue-metrics.js's aggregator makes.
    expect(snapshot.failedNumbers).toEqual([123])
    expectNoInventedNumbers(snapshot, 'jira tally')
    expectReadableCard(jiraCard(), 'jira card')
  })

  it('prints the ticket TWO WAYS on one card — `#123` and `FOO-123`', () => {
    // PINNED, NOT ARGUED, and it is a rendering gap rather than a wrong number: the two
    // spellings name the same ticket, since the number was derived from the key
    // (lib/jira-key.js). But `last task` was taught the key by #127 and `outcome` was not,
    // so a reader of one card is shown both — and only one of them can be pasted into Jira.
    //
    // WHY IT LANDS HERE AND NOT ON THE LIST OF KNOWN SURFACES: lib/jira-key.js's header
    // enumerates the places that still publish the derived number — `ralph status --json`,
    // `ralph digest`'s transcript path and `ralph cycle`'s `OK:`/`FAIL:` lists — and this
    // row is not among them, because until #131 a jira run appended no events for
    // `tallyVerdicts` to read. Recorded here so the enumeration has somewhere to be
    // corrected FROM, and so that teaching `outcomePhrase` the key reddens one test.
    const lines = jiraCard()
    expect(lines[1]).toBe('  outcome    1 ok · 2 failed  — #123')
    expect(lines.find((l) => l.startsWith('  last task'))).toBe('  last task  FOO-123')
  })

  it('reads the number and never the key, so a key alone names nothing', () => {
    // `tallyVerdicts` reads `issue_number` and nothing else, which is what keeps this module
    // out of the key grammar entirely: an event carrying ONLY a key counts as a failure and
    // contributes no name. The alternative — reading `task_key` here — would put a second,
    // divergent opinion of what a task is called in a module that has no I/O and no imports
    // from the Jira side.
    const keyOnly = jiraEvent({ number: undefined, key: 'BAR-7', verdict: 'fail' })
    const snapshot = buildPostMortem({
      metricsText: keyOnly,
      record: jiraRecord(),
      queue: 2,
    ...
    expect({ ok: snapshot.ok, failed: snapshot.failed }).toEqual({ ok: 0, failed: 1 })
    expect(snapshot.failedNumbers).toEqual([])
    // No dangling `—`: the count stands alone when there is nothing to annotate it with.
    expect(renderPostMortem(snapshot)[1]).toBe('  outcome    0 ok · 1 failed')
  })

  it('is unmoved by a task_key that is not a string', () => {
    // The log is untrusted text (see this file's header), and `task_key` is the newest field
    // in it — so the shapes an older release or a half-written line can put there have to be
    // inert to a module that never reads the field at all.
    for (const key of [42, true, null, ['FOO-1'], { key: 'FOO-1' }]) {
      const text = 'RALPH_ISSUE_EVENT ' + JSON.stringify({
        issue_number: 123,
        task_key: key,
        run_id: JIRA_RUN,
        verdict: 'fail',
      ...
      const snapshot = buildPostMortem({ metricsText: text, record: jiraRecord(), queue: 2, now: NOW })
      expect({ ok: snapshot.ok, failed: snapshot.failed }, JSON.stringify(key)).toEqual({ ok: 0, failed: 1 })
      expect(snapshot.failedNumbers, JSON.stringify(key)).toEqual([123])
      expectReadableCard(renderPostMortem(snapshot), `task_key ${JSON.stringify(key)}`)
    }
  })
})
