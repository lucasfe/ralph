...
describe('assembleDigestContext — the whole of what the model will know (#61)', () => {
  ...
  it('names a Jira task by its KEY, so the model narrates the ticket the board has (#127)', () => {
    // The third surface the derived number reaches, found by tracing review's
    // post-mortem finding through the rest of `current.number`'s readers. `TASK` is
    // interpolated into the prompt a model narrates FROM, so `#123` for `FOO-123` does
    // not merely misname the ticket in one row — it teaches the narration a task
    // identifier nobody can look up, and the sentence it produces is what a reader gets
    // on WhatsApp. Scrubbed and preferred exactly as in the status views.
    const keyed = (current) => assembleDigestContext({ record: { ...runningRecord(), current }, now: NOW }).TASK
    expect(keyed({ number: 123, task_key: 'FOO-123' })).toBe('FOO-123')
    expect(keyed({ number: null, task_key: 'FOO-BAR-9' })).toBe('FOO-BAR-9')
    expect(keyed({ number: 123, task_key: `FOO-123${String.fromCharCode(0x1b)}[31m` })).toBe('FOO-123')
    // A github run is untouched: no key on the record, so the padded number stands.
    expect(keyed({ number: 31 })).toBe('#031')
    expect(keyed({ number: 31, task_key: '  ' })).toBe('#031')
    expect(keyed({ task_key: null })).toBe('none')
  })
