...
// it alive?". Two commands still ask it inline (status.js:318 and cycle.js:152), each with
// its own spelling of the root, and `ralph start` still keys its launch on the CWD
// (start.js:684) while `ralph cycle` keys on the git toplevel — two different sessions for
// one repo, depending on which directory you are standing in. `ralph live` is this module's
// first consumer (#167) and `ralph stop` its second (#168, which is where the CWD keying
// stopped being a bug in a command that kills things); status.js and cycle.js still hold
// their own copies.
//
