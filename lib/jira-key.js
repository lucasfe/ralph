...
// from the KEY (templates/ralph.sh:570 derives a filesystem-safe handle from it, which is the
// key itself for anything the grammar above recognises), while digest.js:483 still builds its
// tail path from `record.current.number` alone and `inFlightLogPath` (digest.js:167) formats
...
// than a regression — it was equally wrong and entirely harmless the day it was written — and
// #131 (per-ticket telemetry carries the Jira key) is its home. The README carries the same
// argument, and lib/digest.qa.test.js pins the current behaviour so the day it changes, a test
// says so. Telemetry consumes this properly in that slice, and if a cross-project reading is
// ever needed it needs the key, not this.
//
