...
// harmless: `ralph status --json`, whose key set is frozen — a machine surface, and a
// documented follow-up rather than a reader being shown `#123`; `ralph digest`, which
// derives the transcript path it quotes (`logs/ralph-issue-123.log`) from the number rather
// than the key, so the same prompt that says `FOO-123` carries a GitHub-shaped path two
// lines later; and TWO summary lines built out of the EVENTS, which #131 made reachable
// under this source by giving the jira arm telemetry at all — before it, a jira run appended
// no events, so both lines below had nothing to name and printed nothing.
//
...
//   because both production callers of this trio call the same member of it and both are on
//   paths that must not fail over a name they could not parse: `numberFromKey`, in
//   run-state.js's `beginTask` and in capture-issue-event.js's
//   `resolveTaskNumber` (#131). One is a RECORD WRITE and the
//   other a TELEMETRY append; the loop runs each with `|| true`, and neither may turn into a
//   failed iteration over an unparseable key.
//
//   `isJiraKey` AND `normalizeJiraKey` STILL HAVE NO PRODUCTION CALLER — said plainly rather
//   than implied by "every caller", which review caught this comment asserting, and worth
//   saying again now that the slice this paragraph expected to need them has landed: #131's
//   telemetry records the key with `usableJiraKey`, for the same reason every other consumer
//   does. They are the grammar's own vocabulary: `normalizeJiraKey` is what `usableJiraKey`
//   is built out of, and both are exercised directly by this module's tests (a predicate is
//   the honest way to pin what the regex accepts). Kept for that, not deleted, and not
