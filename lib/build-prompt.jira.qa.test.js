...
describe('QA #128 the jira orchestrator says nothing it must not say', () => {
  ...
  it('promises no push, and says the commit stays local instead', () => {
    // The loop pushes on NO arm, so neither sibling may claim it does. This row used to
    // pin the folder render as still CARRYING that false promise ("The bash pushes … for
    // you"), which is what the jira template must not copy; #221 deleted the sentence, so
    // the folder side is asserted the same way as the jira side now. The sibling
    // comparison is kept rather than dropped: a copy that reintroduces the claim in
    // either template fails here.
    const out = jira()
    expect(out).toMatch(/commit stays local/i)
    expect(out).not.toMatch(/bash pushes/i)
    expect(out).not.toMatch(/git push (?!--force|-f)/)
    expect(render({ TASK_SOURCE: 'folder' })).not.toMatch(/bash pushes/i)
  })
  ...
  it('leaves no placeholder of ANY case behind under a fully populated adversarial env', () => {
    ...
    // #224 removed step 3's `git checkout {{DEV_BRANCH}}`; the branch now lands in step 3's
    // prose describing the detached worktree the loop cut at the local dev-branch tip.
    expect(out).toContain('pytest -q')
    expect(out).toContain('the local `integration`')
