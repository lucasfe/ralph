...
//      (lib/one-line.js) — the same pair lib/jira-queue.js:273 and lib/agent-registry.js:237
...
describe('QA #128 RALPH_TASK_KEY absent or empty — what the agent is actually told', () => {
  ...
  it('tells the agent to view a work item with NO KEY, and now WARNS about it (#128 fix)', () => {
    ...
    // parenthetical. Six of the nine {{RALPH_TASK_KEY}} sites (MEASURED: `grep -o` counts 9 in
    // templates/prompt-team-jira.md).
    const stderr = makeStderr()
    ...
    expect(out).toContain('NEVER transition `` to another status')
  ...
  it('normalizes a padded key, so the prompt and the loop`s own log path agree (#128 fix)', () => {
    ...
    // `$task_key` is post-`usableJiraKey` (lib/jira-queue.js:273 runs it on the pick), so the
...
describe('QA #128 RALPH_TASK_KEY carrying markdown that means something', () => {
  ...
  it('collapses a NEWLINE in the key so step 1 stays ONE command in the fenced block (#128 fix)', () => {
    ...
    // with the key's first half, so none of the other eight sites split either.
    expect(fenceLines.some((l) => l.endsWith('FOO-1'))).toBe(false)
    expect((out.match(/rm -rf \//g) || []).length).toBe(9)
  })
  ...
  it('does NOT re-interpolate a placeholder embedded in the key (single pass), and warns about none of it', () => {
    ...
    // MEASURED count: nine, one per `{{RALPH_TASK_KEY}}` site in templates/prompt-team-jira.md
    // (`grep -o '{{RALPH_TASK_KEY}}' | wc -l` → 9).
    const stderr = makeStderr()
    const out = jiraRender('{{RALPH_HEAVY_TIER}}', { projectFiles: { 'PROMPT.md': '' }, stderr })
    expect((out.match(/\{\{RALPH_HEAVY_TIER\}\}/g) || []).length).toBe(9)
  ...
  it('does not let a key impersonate a role composition slot', () => {
    ...
    expect((out.match(/\{\{ROLE_DEV\}\}/g) || []).length).toBe(9)
  })
  ...
  it('caps a very long key at the same bound as the RALPH_AGENT echo (#108), visibly (#128 fix)', () => {
    ...
    // lands in NINE sites (MEASURED: `grep -o '{{RALPH_TASK_KEY}}' templates/prompt-team-jira.md
    // | wc -l` → 9), so an N-character key is 9N characters of remote-chosen text in the model's
    ...
    // All nine sites carry the capped value — `split` rather than a regex, because the needle
    // is 200 characters of literal text and one of them is a metacharacter-free ellipsis.
    expect(out.split(capped).length - 1).toBe(9)
  })
  ...
  it('does not re-interpolate a literal {{RALPH_TASK_KEY}} written in the project PROMPT.md', () => {
    ...
    // documents the token keeps it verbatim and the orchestrator's own nine sites still
...
describe('QA #128 the jira orchestrator says nothing it must not say', () => {
  ...
  it('asks for no acli WRITE except the failure-path comment', () => {
    // #129 owns the board bookkeeping (transition, done label, SHA comment) and #130 the
    // `failed` sweep. The template names those in PROHIBITIONS, which is why this cannot be a
    // keyword search: what is asserted is that no `acli jira workitem <write>` COMMAND is
    // spelled out. MEASURED: the only two `acli jira workitem` subcommands in the template are
    // `view` (step 1) and `comment` (the Failed section).
    const out = jira()
    const subcommands = [...out.matchAll(/acli jira workitem ([a-z]+)/g)].map((m) => m[1])
    expect([...new Set(subcommands)].sort()).toEqual(['comment', 'view'])
  })
