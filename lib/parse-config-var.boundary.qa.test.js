...
// lib/commands/start.js:298, and EVERY knob of the box reads through it — RALPH_AGENT (:368),
// GH_REPO (:1355), RALPH_CODEX_MODEL (:559), RALPH_CONTEXT_WINDOW (:565) and TASK_SOURCE (:531).
// Compared against
...
// still live in three other commands (cycle.js:194, status.js:384, doctor.js:208), and it reaches
...
// rather than left to be inferred. lib/commands/start.js:249-299 states the one rule and carries the
...
describe('parseConfigVar — every spelling of a blank tail agrees (#147 QA)', () => {
  ...
  it('a blanked TASK_SOURCE parts the two shapes — presence keeps it, a `||` reaches past', async () => {
    ...
    // with `jsResolve`. The `||` row below is still shipped code — cycle.js:194, status.js:384 and
    // doctor.js:208 spell it — and doctor.js's own note is where that follow-up is recorded.
    //
