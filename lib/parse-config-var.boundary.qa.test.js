...
// The presence-then-`??` shape — `configAssignsVar(...) ? parseConfigVar(...) : null`, then
// `??` onto the environment. TWO knobs are read this way and not all seven: GH_REPO
// (lib/commands/start.js:1157) and RALPH_AGENT (:280). Compared against `sourceFile` this is
// the property that matters for those two: one config, one ambient value, two programs
// holding the same string.
//
// IT IS NOT TASK_SOURCE'S OWN READER, which matters because most rows below are written on
// TASK_SOURCE (it is the knob whose two answers are two different QUEUES). That knob goes
// through `parseConfigSource(configText) || processEnv.TASK_SOURCE` — lib/commands/start.js:446,
// and the same `||` at cycle.js:190, status.js:379 and doctor.js:138 — and `||` reaches PAST a
// blank where `??` keeps it. On every row here the config value is non-blank, so the two shapes
// agree and this one models them both; the one shape where they part is a blanked assignment,
// and that is asserted through the real reader below ("a blanked TASK_SOURCE parts the two
// shapes") rather than left to be inferred. lib/commands/start.js:406-408 and :416-429 document the `||` as a deliberate
// divergence and lib/commands/start.precedence.qa.test.js pins it at the command ("reaches past
// a blanked TASK_SOURCE too, on the same `||` — pinned, not endorsed").
...
describe('parseConfigVar — every spelling of a blank tail agrees (#147 QA)', () => {
  ...
  it.each([
  ...
  ])('%s: reads as blank here and as NO assignment to bash — pinned divergence', async (_label, line) => {
    ...
    // family that #147 deliberately left alone, so bash reads `X=` as an environment prefix
    // and `""` as the COMMAND to run it on. The command is the empty word, the shell says so,
    // and X is never assigned in the sourcing shell at all.
    const alone = `${line}\n`
    ...
    // This parser reads it as the blank the author plainly meant, and — the part worth
    // pinning — `configAssignsVar` calls it PRESENT, so the pair does not merely read a
    // different value from bash, it reads a different ANSWER to "did the file set this":
    // present-and-blank is the combination that masks an exported value.
    expect(parseConfigVar(alone, 'X'), line).toBe('')
    expect(configAssignsVar(alone, 'X'), line).toBe(true)
    expect(jsResolve(alone, 'X', 'ambient'), line).toBe('')
    // So it also BLANKS a live line above it here, where bash keeps that line standing.
    // This is the sharpest instance of the divergence the slice left open — the pinned
    // `X= folder` row in parse-config-var.qa.test.js reads a value bash does not assign,
    // and this one DESTROYS a value bash does assign.
    const after = `X=live\n${line}\n`
    expect((await sourceFile(after, 'X', { ambient: 'ambient' })).value, line).toBe('live')
    expect(parseConfigVar(after, 'X'), line).toBe('')
  })
  ...
  it('a blanked TASK_SOURCE parts the two shapes — `??` keeps it, the real reader reaches past', async () => {
    // WHERE `jsResolve` STOPS MODELLING THE COMMAND, asserted rather than left to the reader of
    // the note at the top of this file. Every other row here carries a non-blank config value, on
    // which `??` and `||` cannot disagree; a blanked assignment is exactly the shape on which they
    // do, and it lands on TASK_SOURCE, which is the knob most of this file is written on.
    //
    ...
    // GITHUB; `jsResolve` keeps the blank through `??` and agrees; but `ralph start`'s own reader
    // is `parseConfigSource(configText) || processEnv.TASK_SOURCE` (lib/commands/start.js:446),
    // and `||` reaches past the blank to the exported `folder` — so the box announces FOLDER for
    // a loop reading GitHub issues. That is the criterion-2 shape of #147 at the knob #147 is
    // about, in the reader `jsResolve` does not model.
    //
    // NOT this grammar's defect and not fixed here: `parseConfigVar` and `parseConfigSource` both
    // report the blank correctly, the divergence is one module later in the `||`, and it is
    // documented at lib/commands/start.js:406-408 / :416-429 and pinned at the command in
    // lib/commands/start.precedence.qa.test.js ("reaches past a blanked TASK_SOURCE too, on the
    // same `||` — pinned, not endorsed"). Recorded here so this file's own model is honest about
    // its edge.
    ...
    // The shape `ralph start` actually uses: does not.
...
// carries the reason this is the #147 defect rather than the `X= folder` scope exclusion (bash
// ASSIGNS these lines, so there is a right answer to inherit; no shipped configuration depends
// on a no-break space being deleted; and the deletion manufactured a VALID enum value out of an
// invalid one, which is exactly how #147 masked a run). That is this repo's convention for a
