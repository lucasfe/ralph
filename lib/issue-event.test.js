...
describe('buildIssueEvent — verdict precedence', () => {
  it("failed label => 'fail' even when state is CLOSED", () => {
    const e = buildIssueEvent(
      baseInput({ labels: ['failed'], state: 'CLOSED' }),
    )
    expect(e.verdict).toBe('fail')
  })

  it("failed label => 'fail' even with pending-merge label", () => {
    const e = buildIssueEvent(
      baseInput({ labels: ['failed', 'pending-merge'], state: 'OPEN' }),
    )
    expect(e.verdict).toBe('fail')
  })
  ...
  const VERDICT_CASES = [
    { labels: ['failed'], state: 'CLOSED', verdict: 'fail' },
    { labels: ['failed', 'pending-merge'], state: 'OPEN', verdict: 'fail' },
  ...
  it('verdictOverride wins over label/state precedence when provided', () => {
    const e = buildIssueEvent(
      baseInput({ labels: ['failed'], state: 'CLOSED', verdictOverride: 'pass' }),
    )
    expect(e.verdict).toBe('pass')
  })
...
// ---------------------------------------------------------------------------
// #131: the optional `task_key` field. A jira iteration records the ticket key
// beside the number derived from it, and every OTHER source records no such key
// at all — the field is ABSENT rather than null-padded, so the on-disk format of
// a github or folder event is byte-for-byte what it was.
// ---------------------------------------------------------------------------
describe('buildIssueEvent — task_key (#131)', () => {
  it('emits NO task_key key at all when none is injected (github/folder shape)', () => {
    const e = buildIssueEvent(baseInput())
    expect(Object.keys(e)).not.toContain('task_key')
    expect('task_key' in e).toBe(false)
  })

  it('records an injected task_key beside the number', () => {
    const e = buildIssueEvent(baseInput({ issueNumber: 123, taskKey: 'FOO-123' }))
    expect(e.task_key).toBe('FOO-123')
    expect(e.issue_number).toBe(123)
  })

  it('adds task_key and moves nothing else, whatever order it lands in', () => {
    // The key SET is the contract; the position is not. Nothing reads insertion order —
    // every consumer looks fields up by name — and no README row promises one, so pinning
    // "immediately after `issue_number`" would only make a future reshuffle of this literal
    // fail a test for no reader's benefit. What must hold is that a keyed event is the
    // unkeyed one PLUS this field: no dropped column, no renamed one.
    const github = Object.keys(buildIssueEvent(baseInput()))
    const jira = Object.keys(buildIssueEvent(baseInput({ taskKey: 'FOO-123' })))
    expect(jira).toContain('task_key')
    expect(jira.filter((k) => k !== 'task_key')).toEqual(github)
  })

  it('trims a padded key rather than recording the padding', () => {
    const e = buildIssueEvent(baseInput({ taskKey: '  FOO-123\n' }))
    expect(e.task_key).toBe('FOO-123')
  })

  it('omits the key for null, empty, whitespace-only and non-string input', () => {
    for (const taskKey of [null, undefined, '', '   ', 42, {}, []]) {
      const e = buildIssueEvent(baseInput({ taskKey }))
      expect('task_key' in e, JSON.stringify(taskKey)).toBe(false)
    }
  })

  it('a key the Jira grammar does not recognise is still recorded verbatim', () => {
    // buildIssueEvent is a RECORDER, not a validator: lib/jira-key.js decides what
    // a usable key is, and it deliberately passes an unrecognised one through
    // because Jira names its own tickets.
    const e = buildIssueEvent(baseInput({ issueNumber: null, taskKey: 'not a key' }))
    expect(e.task_key).toBe('not a key')
    expect(e.issue_number).toBeNull()
  })

  it('leaves every other field identical to the same event without a key', () => {
    const withKey = buildIssueEvent(baseInput({ taskKey: 'FOO-123' }))
    const without = buildIssueEvent(baseInput())
    const { task_key: _ignored, ...rest } = withKey
