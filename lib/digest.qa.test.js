...
describe('runDigest — the record at its edges (#61 QA)', () => {
  ...
  it('tails the NUMBER-named log for a jira run, not the key-named one the loop writes (HAZARD, pinned)', async () => {
    ...
    //   • templates/ralph.sh:571 dispatches `run_agent_for_issue "$task_log_handle"`, and the
    //     handle is derived from the key one line earlier (ralph.sh:570 replaces every
    ...
    //     is deliberately NOT the key (ralph.sh:563-565 says so), but for the ordinary key
