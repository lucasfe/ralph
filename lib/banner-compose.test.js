...
// lib/banner-rows.js for why they are spelled out there rather than imported.
const YELLOW = '\u001B[33m'
...
describe('composeBanner — the degradation ladder, by width alone (#72)', () => {
  ...
  //
  // Why the box gives way FIRST, at the wider of the two rungs: the sprite is 26 columns
  // and is dropped whole, so it is the narrow element, while the box is the wide one (a
  // 60-column target) and can be shrunk and then unframed. The frame is the cheapest thing
  // to drop — four columns of `│ ` and ` │` handed back to the fact — and the sprite is the
  // last, because half a Ralph is not a smaller Ralph.
  //
  // #161 ADDS A RUNG AT THE TOP, and it is the only one that is about an ARRANGEMENT rather
  // than about what survives: `beside` is whether the box may sit to the RIGHT of the sprite
  // instead of under it, and `besideWidth` is the width it would be laid out at there. Two
  // more columns per row, in the same spirit as the three above them:
  //
  //   `beside`      whether the columns left after the sprite and its two spaces of air
  //                 still hold a FRAMED box: 26 + 2 + 44 = 72.
  //   `besideWidth` those leftover columns, capped at the 60-column target — meaningful
  //                 only where `beside` is true, and stated at every width anyway so that
  //                 this table pins the whole return value rather than most of it.
  const LADDER = [
    [200, 'boxed', 60, true, true, 60],
    [80, 'boxed', 60, true, true, 52],
    [72, 'boxed', 60, true, true, 44],
    [71, 'boxed', 60, true, false, 43],
    [60, 'boxed', 60, true, false, 32],
    [59, 'boxed', 59, true, false, 31],
    [50, 'boxed', 50, true, false, 22],
    [44, 'boxed', 44, true, false, 16],
    [43, 'bare', 43, true, false, 15],
    [30, 'bare', 30, true, false, 2],
    [26, 'bare', 26, true, false, 0],
    [25, 'bare', 25, false, false, 0],
    [12, 'bare', 12, false, false, 0],
    [5, 'bare', 5, false, false, 0],
    [1, 'bare', 1, false, false, 0],
  ]
  ...
  it('reports one decision per width, and it is only ever about the width', () => {
    for (const [width, form, drawn, sprite, beside, besideWidth] of LADDER) {
    ...
    // ...and the rungs are where the issues put them, named rather than inlined so a caller
    // (lib/sprite-banner.js for the sprite, lib/banner-beside.js for the air) can ask the
    // same question without re-deriving it.
  ...
  it('falls back to the full box for an absent or nonsensical column count', () => {
    ...
    //
    // #161's rung falls the OTHER way on the fallback, and that is deliberate rather than
    // incidental: 60 columns is not 72, so an unknown terminal keeps the stacked layout it
    // has always had. A pipe, a launchd log and a CI runner all land here, which is what
    // makes the "unchanged unless the width is known to be wide" promise a property of the
    // ladder rather than a promise the caller has to keep.
    ...
    for (const width of [...unusable, '80', {}, [], true]) {
      expect(bannerLayout(width), JSON.stringify(width) ?? String(width)).toEqual({
        ...
        sprite: true,
        beside: false,
        besideWidth: BANNER_WIDTH - SPRITE_MIN_WIDTH - BESIDE_GAP,
      })
    }
...
describe('bannerLayout — the rung that puts the box beside the sprite (#161)', () => {
  // THE ARITHMETIC OF THE ARRANGEMENT, on its own, at the column either side of it. The
  // ladder table above already states the whole return value at fifteen widths; what this
  // block adds is the WHY of the one number in it, and the boundary asserted twice — once
  // as an arithmetic identity and once as the literal 72 — so that a change to any of the
  // three constants has to be a deliberate change to a stated total.
  //
  // Side by side needs three things across the terminal at once: the sprite's own 26 cells,
  // the air between the two blocks, and a box wide enough to still be a box. Under that the
  // box would either overhang the right edge or arrive unframed beside a picture, and the
  // stacked layout the banner has always had is better than both.
  const FLOOR = SPRITE_MIN_WIDTH + BESIDE_GAP + BOX_MIN_WIDTH

  it('needs the sprite, the air and a framed box across the terminal at once', () => {
    expect(FLOOR).toBe(72)
    expect(bannerLayout(FLOOR).beside).toBe(true)
    expect(bannerLayout(FLOOR - 1).beside).toBe(false)
    // ...and the boundary is the ONLY thing the rung is about: it is monotonic, so there is
    // no width above 72 that goes back to stacking and none below it that starts.
    for (const width of [72, 73, 80, 120, 200, 1000, 1e6]) {
      expect(bannerLayout(width).beside, String(width)).toBe(true)
    }
    for (const width of [71, 70, 60, 44, 43, 26, 25, 1]) {
      expect(bannerLayout(width).beside, String(width)).toBe(false)
    }
  })

  it('lays the box out in the columns the sprite and the air leave behind', () => {
    // The width is a LEFTOVER, not a target: whatever the terminal has after the picture
    // and the gap. Which is why it is reported here rather than computed by the caller —
    // the caller would need the sprite's cell width and the gap to do it, and the point of
    // this function is that the widths live in one place.
    for (const width of [72, 80, 86, 87]) {
      expect(bannerLayout(width).besideWidth, String(width)).toBe(width - SPRITE_MIN_WIDTH - BESIDE_GAP)
    }
    // ...capped at the target, because the box is a fixed-width object and a 400-column
    // terminal should not stretch `cwd` across the whole screen. Same cap the stacked
    // `boxWidth` uses, so the two arrangements draw the same box on a wide terminal and
    // only its POSITION changes — which is all #161 asked for.
    for (const width of [88, 120, 200, 1e6]) {
      expect(bannerLayout(width).besideWidth, String(width)).toBe(BANNER_WIDTH)
      expect(bannerLayout(width).besideWidth, String(width)).toBe(bannerLayout(width).boxWidth)
    }
    expect(BANNER_WIDTH + SPRITE_MIN_WIDTH + BESIDE_GAP).toBe(88)
  })

  it('never reports a negative width, however narrow or nonsensical the terminal', () => {
    // TOTAL, on the same argument as every other rung: this number reaches a `repeat` in
    // lib/banner-beside.js, and a negative one would throw a RangeError from inside a
    // banner. Meaningless where `beside` is false, and a floor of zero rather than a
    // negative is what makes it meaningless rather than dangerous.
    for (const width of [undefined, null, 0, -1, -1000, Number.NaN, 1, 5, 26, 27, 44, 60, 71]) {
      const { besideWidth } = bannerLayout(width)
      expect(besideWidth, JSON.stringify(width) ?? String(width)).toBeGreaterThanOrEqual(0)
      expect(Number.isInteger(besideWidth), JSON.stringify(width) ?? String(width)).toBe(true)
    }
    expect(bannerLayout(1).besideWidth).toBe(0)
    expect(bannerLayout(28).besideWidth).toBe(0)
    expect(bannerLayout(29).besideWidth).toBe(1)
  })
})
...
describe('composeBanner — the row gate', () => {
  ...
  it('keeps the gate inside the line builders, not at the call sites', () => {
    ...
    // #122 moved the ROW builders to lib/banner-rows.js and left the two LINE builders here.
    // The rule they are swept for did not move with them — a gate in the builder is a rule the
    // next row inherits — so the sweep reads both halves of the seam and says which half each
    // claim is about.
    const rowsCode = codeWithoutComments(new URL('./banner-rows.js', import.meta.url))
    const bodyOf = (name, source = code) => functionBody(source, name)
    ...
    expect(bodyOf('newerVersion', rowsCode)).toMatch(/textOr\(/)
    ...
    expect(bodyOf('whatsNewRows', rowsCode)).toMatch(/textOr\(/)
  })
})
...
describe('composeBanner — the diagnostic rows `ralph doctor` folds in (#75)', () => {
  ...
  it('gates the new facts in the builders, like every other row', () => {
    const code = codeWithoutComments(new URL('./banner-compose.js', import.meta.url))
    // #122: the two row builders live in lib/banner-rows.js now; the composer that must NOT
    // gate is still here, which is why this sweep reads both files.
    const rowsCode = codeWithoutComments(new URL('./banner-rows.js', import.meta.url))
    const bodyOf = (name, source = code) => functionBody(source, name)
    ...
    expect(bodyOf('factRows', rowsCode)).toMatch(/textOr\(/)
    expect(bodyOf('updateCheckRows', rowsCode)).toMatch(/textOr\(/)
    // ...and the composer still hands over raw facts.
    expect(bodyOf('composeBanner')).not.toMatch(/textOr\(/)
  })

  it('leaves room for the value: every label fits the gutter with air after it', () => {
    // `padEnd` does not grow: a nine-character label would print `platformmac` with no space
    // at all. Asserted over the labels this module actually DRAWS, so a future row cannot
    // collide silently. #122 moved the labels themselves to lib/banner-rows.js; the gutter they
    // have to fit is still this module's number, and it is imported rather than typed again —
    // an `8` written into the pattern below would be a second copy of the same decision.
    const lines = composeBanner({
    ...
    // Either a continuation row (the whole gutter blank) or a label of at most LABEL_WIDTH - 1
    // columns, then blank, then the value.
    const withAir = new RegExp(`^(\\s{${LABEL_WIDTH}}|\\S.{0,${LABEL_WIDTH - 2}}\\s+)\\S`)
    for (const line of lines.slice(1, -1)) {
      const content = stripAnsi(line).slice(2, -2)
      expect(content, line).toMatch(withAir)
    }
  })
})
...
describe('composeBanner — the agent, its model and the run’s two locations (#69)', () => {
  ...
  it('gates the new facts in the builders, like every other row', () => {
    const code = codeWithoutComments(new URL('./banner-compose.js', import.meta.url))
    // #122: all four builders below crossed the seam into lib/banner-rows.js; `composeBanner`,
    // which must gate nothing, stayed. The claim is unchanged — only which file answers it.
    const rowsCode = codeWithoutComments(new URL('./banner-rows.js', import.meta.url))
    const bodyOf = (name, source = code) => functionBody(source, name)
    ...
    expect(bodyOf('agentRows', rowsCode)).toMatch(/textOr\(/)
    ...
    expect(bodyOf('contextRows', rowsCode)).toMatch(/windowTokens\(/)
    expect(bodyOf('windowTokens', rowsCode)).toMatch(/typeof \w+ !== 'number'/)
...
describe('banner-compose — purity', () => {
  it('reads no clock, no environment and no filesystem', () => {
    ...
    // Its ONE import, and #122 changed which one: the ROW half. The semver rule the update
    // machinery owns (#21/#24) went with the two rows that ask it, so what this module borrows
    // now is the row list, the word for a fact nobody gave us, the gate that decides what
    // counts as a fact, and the reset that closes a painted span. Still one edge, still no
    // capability behind it — and banner-rows.test.js makes the same claim about the other half,
    // which is where `./update-check.js` is now pinned.
