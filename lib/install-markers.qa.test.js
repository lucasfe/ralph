...
//    `probe` now takes the whole QUESTION as a thunk (lib/install-markers.js:449-455, called at
//    :404, with the argument for that shape at :392-403 and :434-448), so the guard covers
...
describe('QA #201 the extraction is byte-faithful — every layout, every field', () => {
  ...
  // #215 gave the Homebrew row a `versionChannel`, and the join now decorates the ONE row that
  // does not resolve versions through npm with the id its answers are cached under. It belongs on
  // the golden rather than being matched loosely: this describe's whole promise is the WHOLE
  // object, and the field that says "this number is the tap's" is exactly the field a silent drop
  // would resurrect #215 through.
  const BREW_LATEST = {
...
describe('QA #201 the probe seam must be TOTAL — a diagnostic never crashes over its own fs', () => {
  ...
  // (lib/install-markers.js:449-455), so every case below passes; they stay because a future
  ...
  // :392-403 and :434-448, and these cases are what it points at. They STAY because the guard
