...
// rather than the rule: what an unnamed channel spawns and (#215) what its answer
// is filed under — while a channel a caller TRIED and failed to resolve reads
// nothing back. Frozen: shared by identity, so a mutation changes every query.
export const NPM_VERSION_QUERY = Object.freeze({
  ...
  // #215: the id an answer from this query is CACHED under, so the number npm resolved is
  // read back only by copies installed from npm. It rides on the descriptor beside the argv
  // that produces it, which is what makes "the channel that answered" a fact this module can
  // record rather than one it has to infer.
  //
  // A BARE LITERAL, and it is the one duplication this file is allowed. lib/install-markers.js
  // declares VERSION_CHANNEL, and this module may not reach for it: #199's architecture is that
  // the module which QUERIES a channel is never the module which decides which layout a copy is,
  // and lib/update-check.channel.qa.test.js:514 holds that line by sweeping this file's
  // comment-free source for `install-target`, `classifyInstall` and even the string 'brew'.
  // So the id is spelled here and derived there, and
  // lib/update-check.cache-channel.test.js joins the two spellings from the outside, exactly
  // as test/homebrew-formula.test.js joins the two spellings of the formula name that cannot
...
//
// #200: `latestSource` is WHICH CHANNEL this copy came from — a query descriptor as
...
// names a channel at all, and it passes a thunk, because a thunk defers the work to the
// one place that knows whether it is needed — the opt-out returns before resolving it at
// all. The descriptor arm is there for a caller that has a
// classification in hand and no reason to wrap it; nothing in `lib/` is that caller yet,
// so it is exercised only by tests — lib/update-check.decision-channel.qa.test.js:242
// passes a hostile descriptor with no thunk around it, and its four-row argv table at
// :268-289 hands descriptors in directly. Both arms land in the same guard one line
// later, so anything unusable — a thunk that throws, a descriptor with no argv — falls
// back to npm rather than skipping the check.
//
// #215: IT IS ALSO WHICH CHANNEL'S NUMBER MAY BE READ BACK, which is why the throttled path
// now resolves it too — the cache is one global file shared by every copy of Ralph on the
// machine, and a version is only ever served to the channel that produced it. A caller that
// names no channel therefore reads and writes NPM's number and nobody else's, rather than
// whatever the last copy to run happened to leave behind.
//
// Returns:
//   latestVersion — the newest version THIS RUN'S CHANNEL has published, or null.
//                   #215: never another channel's, on either path — the cache is one
//                   global file and two copies of Ralph installed from two channels
//                   share it, so a number is only ever read back by the channel that
//                   resolved it. A copy whose channel has nothing cached gets null,
//                   which reads as "nobody has checked" rather than as "up to date
//                   with a version this channel cannot install".
...
export async function resolveUpdateDecision({
  ...
  home,
  timeoutMs = 5000,
  latestSource = NPM_VERSION_QUERY,
} = {}) {
  ...
  // #200 resolved the caller's channel BELOW this point, on the argument that a run which
  // asks nothing needs no channel — and #215 is that argument's counterexample. The
  // throttled path does not query, but it SERVES a version, and a version cannot be served
  // without naming the channel it belongs to: with one un-keyed cache field, a Homebrew copy
  // inside the weekly window was serving whatever the npm copy had written, which is the bug.
  // So the channel is resolved once, here, for BOTH paths.
  //
  // WHAT #200'S GUARANTEE ACTUALLY WAS is unchanged, and it is the half that matters: the
  // throttled path still SPAWNS NOTHING. The gate's thunk classifies from the install path
  // alone (`exec: null`, lib/update-gate.js:93) and memoizes the promise, so a throttled run
  // that HAD a notice to print pays exactly what it always did. The run with NOTHING to report
  // is the one that changed: zero classifications before, one now — path matching over
  // RALPH_HOME plus at most two stats, a real cost and not a free read, just not a spawn or a
  // socket. Both counts are asserted and this slice flipped them, at
  // lib/update-gate.notice-command.test.js:306 and at
  // lib/update-gate.notice-command.qa.test.js:568. The thunk is also AWAITED here, which the
  // throttled path never did, so a classification that never answers now hangs `ralph start`
  // — there is no timeout between the gate and it. Characterized, not fixed, at
  // lib/update-gate.cache-channel.qa.test.js:294. The opt-out still resolves nothing at all:
  // it returned above, before this line, and it reads no cache either, so it has no version
  // to attribute.
  const source = await resolveLatestSource(latestSource)
  const readerChannel = readingChannelOf(source)

  if (checkThrottled) {
    // Throttled: the prompt is still served, from the cached version — but only if THIS
    // channel is the one that resolved it. Another channel's number, a number no channel is
    // recorded for, and a caller that could not say which channel it is all read as "nothing
    // cached" — `cachedVersionFor`'s rule 3 is the refusal: the caller's own 'up to date' is
    // a better answer than a nag pointing at a release the caller's channel cannot install.
    return buildDecision({
      latestVersion: cachedVersionFor(cache, readerChannel),
      currentVersion,
      source: 'cache',
      updatedCache: cache,
      promptDue,
  ...
  // #200: the ONLY path that asks a channel anything. #199 left this call on the npm
  ...
  // an npm copy against the registry.
  const fetched = await fetchLatestVersion(exec, timeoutMs, source)
  ...
  //
  // #215: and the version/channel pair is written ONLY when this run actually resolved a
  // number, through the one function that keeps the two fields in step. A failed query
  // therefore leaves the pair exactly as it was — which is what makes "the previously known
  // latest_version survives" a promise about THIS channel's number rather than a licence to
  // re-stamp another channel's under this one's name.
  //
  // Stamped with the ANSWERING channel, not the reading one (see the two functions below):
  // `fetched` came out of `fetchLatestVersion`, which substitutes NPM_VERSION_QUERY for a
  // source it could not use — so for an unresolvable source this number really is npm's, and
  // 'npm' is the honest label for it.
  const updatedCache = {
    ...(fetched
      ? withLatestVersion(cache, { version: fetched, channel: answeringChannelOf(source) })
      : cache),
  ...
  return buildDecision({
    // What this run KNOWS, which is what it just resolved or — when the query failed — what
    // this channel had already resolved. #24's rule that a flaky night must not hide a pending
    // notice, kept; #215's rule that the notice must be about the reader's own channel, added.
    //
    // The two halves are keyed differently on purpose. `fetched` is reported as-is, whatever
    // the source was: THIS run asked and got an answer, so even an unattributable one is a
    // fact about the channel it asked — while `cachedVersionFor` would refuse it, because no
    // LATER run can tell whose it was. The FALLBACK is a read of an older run's number, so it
    // goes through the reading channel and fails closed exactly as the throttled path does.
    // Off `cache` rather than `updatedCache` only so the two expressions read the same object;
    // the pair is identical in either, since a failed query wrote neither field.
    latestVersion: fetched ?? cachedVersionFor(cache, readerChannel),
    currentVersion,
    source: 'network',
    updatedCache,
    promptDue,
...
// #215: the CHANNEL ID A SOURCE SPELLS, trimmed, or null for a source that spells none.
//
// The one spelling rule, shared by the two questions below so a blank-string `channel` and a
// missing one cannot diverge. Reads `source.channel` off a plain object this module owns:
// `resolveLatestSource` has already copied whatever the caller handed over, inside its own
// try, so there is no getter left here to throw and no second read for a hostile one to
// answer differently.
//
// Only ever called with a DEFINED source — `undefined` is answered by each caller below.
function namedChannelOf(source) {
  const named = source.channel
  return typeof named === 'string' && named.trim() ? named.trim() : null
}

// #215: TWO QUESTIONS, TWO VALUES. The first cut of this slice asked one — a single
// `versionChannelOf` that answered npm for a source it could not resolve — and used that answer
// both to LABEL a number being written and to DECIDE which number may be read. `source ===
// undefined` is the input where their honest answers differ:
//
//   WHO ANSWERED (`answeringChannelOf`) is a fact about a query this run already made.
//   `fetchLatestVersion` substitutes NPM_VERSION_QUERY for a source it cannot use (see its
//   `isChannel` line), so when nothing usable was named the registry really is what replied and
//   'npm' records that. It is wrong only if it names a channel that did NOT answer — which is
//   why a descriptor with a runnable argv and no id still answers null here: that descriptor is
//   spawned as ITSELF, so npm never speaks for it. That substitution is the policy of the pair,
//   and it belongs to the WRITE alone.
//
//   WHO IS READING (`readingChannelOf`) is a claim about the identity of THIS COPY of Ralph, and
//   an unresolvable source is no evidence for it whatsoever. It ENFORCES nothing, and should not
//   be read as if it did: it trims `source.channel` to a string or null, which is what
//   `cachedVersionFor` does to any key handed to it (lib/version-cache.js:198), and rule 3 there
//   (:180-185) refuses an unnamed reader whether this function is called or not. It is a NAME
//   for the question at the two read sites, which read one value that is pointedly not the one
//   filed beside the write. Nothing pins the call: a source-text pin on a wrapper that computes
//   nothing only forecloses deleting the wrapper, and what protects the read is behavioural, at
//   lib/update-check.cache-channel.qa.test.js:135 and :159.
//
// DROPPING THE NPM FALLBACK FROM THE READ, and from the read only, is the whole fix: 'npm' is
// simultaneously true of the query and unfounded about the reader. Keep the fallback for the
// reader and a brew copy reads npm's number — :135 and :159 measure that on the decision,
// lib/update-gate.cache-channel.qa.test.js:334 and :360 on the printed line, where npm's number
// is offered beside `brew upgrade ralph`, a command that cannot fetch it. Drop it for the writer
// too and a number npm demonstrably produced is filed unattributed, so no copy may ever read it
// and every npm run re-queries for a version already on disk
// (lib/update-check.cache-channel.qa.test.js:193 pins the stamp through the argv spawned). The
// contrast — a caller that names a channel id of nothing, always null and still null — is at
// lib/update-check.cache-channel.qa.test.js:169 and lib/update-gate.cache-channel.qa.test.js:444.
//
// AND `undefined` MEANS ONE THING ONLY, because the parameter default now carries the other:
// `resolveUpdateDecision`'s `latestSource` defaults to NPM_VERSION_QUERY, so a caller that never
// mentions a channel NAMES npm — the documented default this module has had since #199
// (`fetchLatestVersion` carries the same one). What reaches `resolveLatestSource` as `undefined`
// is therefore never "nobody asked"; it is only ever a caller that TRIED to name a channel and
// could not — a thunk that threw, one that answered undefined or null, a descriptor with no
// runnable argv — which is the case that must fail closed. Nothing shipped takes the default:
// lib/commands/start.js and lib/commands/cycle.js both reach this function through
// runUpdateGate, whose thunk always passes a source. So the default is the SEAM's convenience
// and fail-closed is production's behaviour, and lib/update-check.cache-channel.test.js pins the
// two apart.
function answeringChannelOf(source) {
  return source === undefined ? NPM_VERSION_QUERY.channel : namedChannelOf(source)
}

function readingChannelOf(source) {
  return source === undefined ? null : namedChannelOf(source)
}
...
//
// #215: `undefined` is therefore npm's query, and — because that query is what answers —
// npm's stamp; it is NOT npm's cache read. `cachedVersionFor` refuses an unnamed reader.
...
// declined_version and never will be — normalizeCache writes a fixed field
// shape, and a declined release is re-offered at the next window
// rather than being forgotten forever.
//
// #215: it stamps ONE field and carries the rest through by spread, so the version and the
// channel that resolved it move together and neither is re-attributed by the act of showing a
// prompt. Nothing here needs to know which channel is being prompted — the decision it is
// recording already answered that.
//
