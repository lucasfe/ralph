...
// #21: `fetchLatestVersion` is the registry query on its own, so `ralph update`
// can ask "what is the latest version?" without any throttle or notice policy.
// #24: that policy now lives in `resolveUpdateDecision` (which also delegates
// here); the old `checkForUpdate` wrapper is gone. This file stays scoped to the
// query itself — the policy is pinned in `update-check.decision.test.js`.
