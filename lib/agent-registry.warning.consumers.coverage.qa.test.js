 ...
 * repo's configured agent into the digest window it opens: lib/commands/start.js:834
 * (`openDigestWindow`'s `envPrefix`) builds `RALPH_AGENT=${shellQuote(agent)} ` into the `tmux
 * new-window` command line, out of `parseConfigVar(configText, 'RALPH_AGENT').trim()` — the RAW
 ...
 * templates/ralph.config.sh:70 documents that forwarding as a promise to the user. `shellQuote`
...
const SILENT = {
  ...
  'ralph cycle preflight': async (value, { stdout, stderr }) => {
    ...
    return { agent, extra: `${JSON.stringify(result)}${LF}${execCalls.join(LF)}` }
  },
}
