...
// direction. Swept rather than argued: 3,744 config lines (4 indents x {bare, `export`} x 18 value
// shapes x 26 tails), each one sourced by a real bash exactly the way templates/ralph.sh sources
// it, and its result compared with this module's — 720 of the 3,744 are lines bash assigns nothing
// on, 432 of those are refused here, and ZERO refusals landed on a line bash assigned. The 288-row
// remainder is two named groups and nothing else: 144 rows carrying an `export` (deliberately
// outside the refusal, see the table above) and 144 whose tail is `| cat` or `&`, where bash makes
// the assignment in a subshell. Both are still read rather than refused, exactly as before #149.
//
...
// `X=v | cat` and `X=v &` are in that last group and
// are the two rows in it where bash assigns NOTHING (the assignment happens in a subshell), so they
// stay as they were: read, not refused.
//
...
// a quote, `$( )`, `${ }` or backtick whose close it does not find, or an unquoted OPERATOR — so the
// caller can decline to answer instead of guessing. -1 is not a claim that the line is malformed:
