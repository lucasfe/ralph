...
describe('progress.js — PURE, with no exceptions (#57 QA)', () => {
  ...
  it('imports exactly one module, and it is one with nothing to lend (#121)', () => {
    // THIS USED TO READ "imports nothing at all", and the claim it was making was about
    // CAPABILITY: no fs, no path, no execa, no shell, nothing this pure module could reach a
    // machine through. #121 gave it one edge — `issueEvents`, the walk that turns a
    // `RALPH_ISSUE_EVENT` line into an event — because the alternative was keeping the third
    // hand-maintained copy of that walk, and `ralph status` disagreeing with the launch box
    // about which lines of issues.jsonl are events is the bug the sharing prevents.
    //
    // So the row is not weakened, it is re-aimed. The specifier is pinned as an exact list, so
    // a SECOND edge — to the module that holds `node:fs`, say — still fails here; and the far
    // end is read, because "one import" only preserves the capability claim while the thing
    // imported has no imports of its own. That is where the promise now lives, and it is
    // asserted at length in issue-event-lines.test.js as well.
    ...
    // Read with the shared stripper rather than this file's own: the far end is mostly JSDoc,
    // and the local one-liner above only takes `//` prose out — which is precisely the false
    // positive test/helpers/source-code.js exists for.
    ...
    expect(shared).not.toMatch(/require\(|node:|\bprocess\b|\bDate\b/)
  })

  it('imports exactly one module by STATEMENT COUNT too, not only by specifier (#121 QA)', () => {
    // THE HALF THE RE-AIM ABOVE DROPPED. The retired row was `not.toMatch(/^\s*import\s/m)` — no
    // import statement of ANY shape — and its replacement pins the `from` specifiers instead.
    // That is stronger about named imports and blind to the one shape that has no `from`: a
    // side-effect import, `import './evil.js'`, which runs a whole module for its effects at
    // load time and matches nothing the row above looks at. Since running someone else's module
    // at load is precisely the capability the purity claim is about, the STATEMENT count is
    // asserted here beside the specifier list, and the two together say what the single blanket
    // match used to say on its own.
    //
    // (A bare `import 'node:fs'` is caught today, but only in passing, and only by
    // progress.table.qa.test.js's separate `not.toMatch(/node:/)` — nothing in this file sees it.)
    expect([...CODE.matchAll(/^\s*import\b/gm)]).toHaveLength(1)
    // Non-vacuity, and proof of the gap being closed: the planted line moves this count and
    // leaves the specifier list above untouched.
    const planted = `import './evil.js'\n${CODE}`
    expect([...planted.matchAll(/from\s*['"]([^'"]+)['"]/g)].map((m) => m[1])).toEqual([
      './issue-event-lines.js',
    ])
    expect([...planted.matchAll(/^\s*import\b/gm)]).toHaveLength(2)
  })
