...
//      it is one of the files allowed to spell a retired name and the only one whose retyped
//      literal would be EXECUTED. The others, in the roles they earn their exemptions for: a
//      spec pinning the mapping's contents, a changelog entry describing what a past version
//      really did, and — since #142 — the README's upgrade note, which cannot tell a user how to
//      rename a label without naming it. That last one SHIPS as well (README.md is in the npm
//      tarball), but as a line a human pastes rather than one the program reads, and it is held
//      byte-for-byte against what `ralph start` prints by lib/labels.parity.test.js rather than
//      by any sweep. Which of them spell BOTH retired names, measured rather than recalled: the
//      module, the spec and the README — while the changelog names just the retired in-progress
//      one, once, in the single release entry that describes stamping it.
//
...
import { claimText, prose } from '../test/helpers/doc-guard.js'
import { RALPH_HOME } from './paths.js'
import { IN_PROGRESS_LABEL, findLegacyLabels } from './labels.js'
...
describe('QA #140 — a retired spelling cannot survive a capital letter or a line break', () => {
  ...
  it('no tracked file outside the exemption list spells one, however cased or wrapped', () => {
    expect(
  ...
  it('and no tracked file states the exemption list’s SIZE, which is what actually drifted', () => {
    // test/helpers/legacy-label-sweep.js's own header makes this argument: consolidating the
    // exemption list into one definition did nothing for the prose COUNTS of it, because a number
    // in a comment is not a thing an import can keep honest. #142 added an entry to the list and
    // rewrote the counts it found — but its first pass did not find them all, and one comment was
    // left describing the list by a length it no longer had. That comment is corrected in the
    // landed tree, so do not go looking for it: the REGRESSION fixture below is the only
    // surviving copy of the stale claim, which is why it is quoted verbatim rather than
    // paraphrased.
    //
    // So the rule is pinned as a RULE rather than re-counted at every rename: a comment may name
    // the LIST, and may not state its LENGTH. That is enforceable in a way "remember to update the
    // counts" is not, and it is the only version of this that a fifth exemption cannot falsify.
    //
    // Read through `claimText()` rather than `prose()`, and that is measured rather than tidy:
    // every one of these claims lives in a wrapped `//` comment block, so the collapsed text of a
    // claim split at the line break reads "four-file // exemption list" — which `prose()` leaves
    // and every pattern below walks straight past. `claimText()` strips the repeated comment
    // marker first, which is the difference between catching the wrapped half of these and
    // catching only the ones that happened to fit on one line.
    //
    // The phrasings are the ones that really occurred plus the numerals, which makes this a drift
    // TRAP and not a grammar of English: it cannot catch every way a count could be worded, and it
    // does catch every way this repository has actually worded one.
    const NUMBER = String.raw`\d+|one|two|three|four|five|six|seven|eight|nine|ten`
    const SIZE_CLAIMS = [
      new RegExp(`(?:${NUMBER})[- ]file exemption`, 'i'),
      new RegExp(`(?:${NUMBER}) files allowed to spell`, 'i'),
      new RegExp(`exemption list(?:'s)? (?:${NUMBER}) (?:files|entries)`, 'i'),
    ]
    // Fail closed on the patterns themselves: a typo that made one of them match nothing would
    // make this sweep pass forever. Demonstrated on the exact string #142 left behind, and on the
    // same claim wrapped at a comment line break, since that variant really did escape the first
    // draft of this test.
    const REGRESSION = "legacy-label-sweep.js's three-file exemption list for a retired spelling"
    const WRAPPED = "legacy-label-sweep.js's three-file\n  // exemption list for a spelling"
    for (const fixture of [REGRESSION, WRAPPED]) {
      expect(
        SIZE_CLAIMS.some((pattern) => pattern.test(claimText(fixture))),
    ...
    for (const path of trackedFiles()) {
      ...
      if (!/\.(js|md)$/.test(file)) continue
      // This file has to quote the regression above in order to demonstrate the matcher on it,
      // the same way section 5's matcher test builds its own needles — so it is its own haystack's
      // one exemption, and only for the line that is a fixture rather than a claim.
      if (file === 'lib/labels.rename.qa.test.js') continue
      const text = claimText(readFileSync(path, 'utf8'))
      for (const pattern of SIZE_CLAIMS) {
...
// ---------------------------------------------------------------------------
// 7. #142's exemption — the 250KB the README's upgrade note stopped defending
// ---------------------------------------------------------------------------
//
// #142 put README.md on the exemption list in section 5, and the reason is sound: a note telling a
// user how to finish #140's rename cannot avoid naming what #140 retired. What it COSTS is
// specific, and easy to miss because the exemption reads like a two-line concession.
//
// AN EXEMPTION IS WHOLESALE. `legacyOffenders` skips the FILE, not the paragraph. Until #142 the
// sweep in section 5 was the thing holding README's other quarter-megabyte to the current names —
// a doc that, MEASURED AT #142, writes one of the four current label words as a bare code span 86
// times, against 155 raw occurrences of those same four strings. The BACKTICKED count is the one
// worth quoting: `failed` supplies 49 of the 69-hit gap, and only a handful of those are a label
// mention at all — the rest is ordinary English about an install or a probe failing, the folder
// lane's `afk/failed/` directory, an `N ok, N failed` run summary, and the retired spelling
// inside the upgrade note itself. The 86 include the troubleshooting entry IMMEDIATELY above the
// upgrade note, whose whole subject is a label left behind by a crash. From #142 on, a retired
// spelling reintroduced ANYWHERE in that file reads as green.
//
// WHAT PAYS FOR THE EXEMPTION COVERS TWO LINES. lib/labels.parity.test.js pins the migration
// commands to what a real findLegacyLabels composes — verbatim and once — and that is genuinely
// stronger than a substring sweep AT THOSE TWO LINES, because it fails on a drifted flag and not
// only on a drifted name. It is also no guard at all anywhere else in the file: it asks "is the
// documented command the printed command" and never "is this the only place the old word appears".
//
// MEASURED RATHER THAN REASONED ABOUT, which is the only reason this section exists. Two edits to
// README.md were made and the WHOLE suite run serially against each: the orphan-label entry above
// the note put back on the retired spelling, and a fresh sentence claiming Ralph applies a
// retired-prefix label as current behaviour. 230 files and 11339 tests AS THE SUITE STOOD FOR THAT
// EXPERIMENT, which is not today's total and is not meant to be read as one: what the run proves
// is that both edits were INVISIBLE to it, green both times.
//
// So this restores the property the exemption removed, scoped the way the exemption should have
// been: every retired spelling in README.md must sit inside the one troubleshooting entry that
// carries the migration commands. That is #142's criterion 5 — every README reference to the two
// retired names outside the upgrade section uses the new names — and criterion 8, that no doc
// describes Ralph as applying a retired-prefix label as current behaviour, asked of the file
// instead of left to whoever edits it next.
//
// THE ENTRY IS LOCATED BY THE COMMANDS, NEVER BY ITS WORDING. The parity spec anchors on the bold
// lead as a string literal, which it can: it is asserting that the lead is present. Copying that
// literal here would be a second place the anchor is known, and it would go on agreeing with a
// README that had reworded its own heading. The commands come out of a real findLegacyLabels call,
// so the anchor is the same bytes the runtime prints, and this file still types no needle.

// One `gh label edit` line per name #140 retired, off a board still carrying every one of them.
// The same seam `ralph start` hands over — one `exec`, one listing, no filesystem — so these are
// the runtime's own words rather than a second opinion about them.
const migrationCommands = async () =>
  (
    await findLegacyLabels({
      exec: async () => ({
        exitCode: 0,
        stdout: JSON.stringify(RETIRED_SPELLINGS.map((name) => ({ name }))),
        stderr: '',
      }),
    })
  ).map(({ command }) => command)

// README's troubleshooting entries are BOLD-LEAD paragraphs rather than headings — `##
// Troubleshooting` is built that way throughout, and #142 kept to it deliberately, since a `###`
// would have adopted the seven entries below the note as subsections of it. So an entry boundary
// is a blank line followed by bold emphasis, and splitting there needs no heading-depth arithmetic
// (which is why this does not reach for doc-guard's `section`, whose subject is `#` headings).
// Emphasis MID-PARAGRAPH does not split: all four of the note's own emphasised spans —
// `**clean break**`, `**ID**`, `**Portuguese**` and `**current**` — sit inside sentences, and the
// lookahead requires the blank line in front.
const readmeEntries = () => read('README.md').split(/\n\n(?=\*\*)/)

// The shared matcher plus the one flag it does not carry, so occurrences can be COUNTED and not
// merely detected. Rebuilt from RETIRED_SPELLING's own source rather than re-derived from the
// mapping: there is still exactly one pattern in the suite, and this file still spells no part of
// a retired name.
const everyRetiredSpelling = () =>
  new RegExp(RETIRED_SPELLING.source, `${RETIRED_SPELLING.flags}g`)

const countRetired = (text) => (text.match(everyRetiredSpelling()) ?? []).length

// A chunk named by its lead line, so a failure says WHICH entry reintroduced the word instead of
// dumping a paragraph of Markdown into the report.
const leadOf = (entry) => entry.split('\n')[0].slice(0, 90)

// The entry carrying both migration commands. Returned as `undefined` when there is not exactly
// one, so every test below anchors on it before measuring rather than asserting about ''.
const migrationNote = async () => {
  const commands = await migrationCommands()
  const holding = readmeEntries().filter((entry) =>
    commands.every((command) => entry.includes(command)),
  )
  return holding.length === 1 ? holding[0] : undefined
}

describe('QA #142 — README’s exemption is one entry wide, and the file is a quarter of a megabyte', () => {
  it('locates the upgrade note by the commands the runtime composes, not by its wording', async () => {
    // The premise every assertion below rests on, failing closed on each way it could go vacuous:
    // the check found a command per retired name, the split really found entries, and both
    // commands live in exactly ONE of them. A note whose commands had been split across two
    // entries would make "inside the note" an answer about half a migration.
    const commands = await migrationCommands()
    expect(commands).toHaveLength(RETIRED_SPELLINGS.length)
    expect(commands.length, 'the mapping is empty — nothing to anchor on').toBeGreaterThan(0)

    const entries = readmeEntries()
    expect(entries.length, 'no bold-lead entries — the entry split is wrong').toBeGreaterThan(20)

    const holding = entries.filter((entry) => commands.every((command) => entry.includes(command)))
    expect(holding.map(leadOf), 'the migration commands are not in exactly one entry').toHaveLength(
      1,
    )
  })

  it('spells a retired name ONLY in that entry — nowhere else in README (#142 criteria 5 and 8)', async () => {
    // THE PROPERTY THE EXEMPTION REMOVED, restored at the scope the exemption was argued at.
    const note = await migrationNote()
    expect(note, 'no single entry carries both migration commands').toBeTruthy()

    // Non-vacuity FIRST: the note really does spell what #140 retired, so the negative below is
    // measuring a live matcher rather than agreeing with a broken one. This is also the half that
    // catches the note being deleted once the migration feels old — section 5's exemption check
    // would then still pass on the CHANGELOG's copy alone.
    expect(countRetired(note), 'the note names nothing retired — is it still a migration note?')
      .toBeGreaterThan(0)

    // Reported as the LEAD LINES of the offending entries, because "README.md carries one" is not
    // an actionable failure in a file this size — the sweep in section 5 could only ever say that,
    // and it is exactly what the exemption silenced.
    expect(
      readmeEntries()
        .filter((entry) => entry !== note && RETIRED_SPELLING.test(entry))
        .map(leadOf),
    ).toEqual([])

    // The same fact as arithmetic over the WHOLE file, which is not a restatement: the filter
    // above can only see what the split produced, so a future change to the entry separator that
    // silently dropped a chunk would make it vacuous while this stays true.
    expect(countRetired(read('README.md'))).toBe(countRetired(note))
  })

  it('puts both commands in ONE fenced bash block, as the only lines in it', async () => {
    // A command a reader has to PASTE is not documented by being present in the text — it is
    // documented by being selectable as a line. Unfenced, the two lines reflow into one rendered
    // paragraph and the `'…'` around each description becomes prose a renderer is free to
    // smart-quote. MEASURED: with the fence markers deleted and the commands left as a paragraph,
    // the whole suite stays green, because `toContain` over raw bytes cannot see a fence.
    //
    // The info string is asserted too — a bare ``` block loses the shell highlighting every other
    // command block in this section has — and the body is pinned to the commands and NOTHING else,
    // so a `gh auth login` a later edit tucked in beside them is a line the runtime never printed
    // and the reader cannot tell apart from one it did.
    const commands = await migrationCommands()
    const note = await migrationNote()
    expect(note).toBeTruthy()
    const fences = [...note.matchAll(/```(\w*)\n([\s\S]*?)```/g)]
    expect(fences.map(([, info]) => info)).toEqual(['bash'])
    expect(fences[0][2].trimEnd().split('\n')).toEqual(commands)
  })

  it('and no character in that block would break the paste it exists to be', async () => {
    // The meanest failure available in this issue: a command that looks right on the page and dies
    // in the terminal, in the one paragraph a stuck upgrader is reading. Every character below is
    // a substitution an editor, a word processor or a copy out of a rendered page performs on its
    // own — and each is spelled as a CODE POINT rather than typed, so this test cannot be the
    // thing that introduces one, and so a failure names the character by number instead of
    // printing a glyph indistinguishable from the right one.
    const HOSTILE = [
    ...
    const note = await migrationNote()
    expect(note).toBeTruthy()
    const body = [...note.matchAll(/```\w*\n([\s\S]*?)```/g)][0][1]
    for (const [name, code] of HOSTILE) {
      const label = `U+${code.toString(16).toUpperCase().padStart(4, '0')} ${name}`
      expect(body, label).not.toContain(String.fromCharCode(code))
    }
    // ...and the ASCII apostrophe the shell actually needs IS there, two per command, quoting each
    // description — so the absences above rule out a substitution rather than agreeing with a
    // block that quotes nothing at all.
    const commands = await migrationCommands()
    expect(body.split(String.fromCharCode(0x27))).toHaveLength(2 * commands.length + 1)
    // No trailing whitespace on any line: invisible on the page, and it rides into the shell.
    expect(body).not.toMatch(/[ \t]+$/m)
  })

  it('documents no `gh label edit` line the check does not compose — the reverse drift', async () => {
    // lib/labels.parity.test.js iterates the commands the CHECK composes and looks each one up in
    // the README. That direction cannot see a command the README carries and the check does NOT,
    // which is the shape a retirement REVERSED leaves behind: drop an entry from LEGACY_LABELS and
    // the note goes on telling users to rename a label nothing has retired, with every existing
    // assertion green because each only iterates what is left. Asked as set equality, so it fails
    // in both directions at once.
    const commands = await migrationCommands()
    const documented = (read('README.md').match(/^gh label edit .*$/gm) ?? []).sort()
    expect(documented).toEqual([...commands].sort())
  })
})

// #142's criterion 6 asks that the label-lifecycle prose still makes its ORIGINAL argument in the
// new names. Nothing was asking it. The parity table only requires README to CONTAIN each of the
// four names, and README spells `in-progress` in 57 places, so every sentence of the
// argument could be deleted with the table still green — MEASURED, twice: with the sticky-label
// sentence removed, and again with the sweep's span narrowed from "both open and closed" to
// "open", the whole suite stays green.
//
// The two claims pinned below are the two the criterion names, and they are the two a reader
// cannot recover from the code. The sweep's SPAN is visible in lib/orphan-cleanup.js's argv, so a
// doc that lost it is merely incomplete; but WHY a zero-progress iteration keeps the label is a
// DECISION, and without the sentence the sweep two paragraphs later reads as contradicting it.
//
// Scoped to the entry that argues it rather than to the file, for the reason doc-guard slices
// before it asserts: a claim three sections from its subject is a coincidence, not documentation.
// Read through `prose()` because README hard-wraps at ~78 columns and both sentences are split
// mid-phrase in the file — an assertion on raw text would be an assertion about where a wrap fell.
describe('QA #142 — the label-lifecycle argument survived the rename (criterion 6)', () => {
  // Located by the word the argument turns on rather than by the entry's lead, which is about
  // something else: the lifecycle paragraphs sit inside the `No issues are picked up.` entry,
  // because they explain why that filter behaves as it does.
  const lifecycleClaims = () => {
    const entries = readmeEntries().filter((entry) => entry.includes('sticky label'))
    return entries.length === 1 ? prose(entries[0]) : undefined
  }

  it('argues why the label is STICKY on a zero-progress iteration, in the CURRENT name', () => {
    const claims = lifecycleClaims()
    expect(claims, 'the lifecycle argument is gone, or now split across two entries').toBeTruthy()
    // The name the argument is about, then the argument: the label is kept ON PURPOSE while the
    // issue is open and unadvanced, which is the one case that looks exactly like the leftover the
    // orphan sweep exists to clear.
    expect(claims).toContain(IN_PROGRESS_LABEL)
    expect(claims).toMatch(/still \*\*open\*\* after an iteration made no progress/)
    expect(claims).toContain('keeps a stuck issue from being re-selected')
  })

  it('argues why the SWEEP spans CLOSED issues too, in the CURRENT name', () => {
    const claims = lifecycleClaims()
    expect(claims).toBeTruthy()
    // The surprising half, and so the one a later "tidy" would drop: a label on a CLOSED issue
    // costs a reader nothing they can see, so narrowing the sweep to open issues would read as a
    // simplification rather than as a regression. The name is COMPOSED into the pattern, so this
    // sentence cannot be the one left behind on a retired spelling.
    expect(claims).toMatch(
      new RegExp(`clears \`${IN_PROGRESS_LABEL}\` from \\*\\*both open and closed\\*\\* issues`),
    )
  })

  it('and makes both of them without naming anything #140 retired', () => {
    // Criterion 6's "with the new names" half, asked of the entry the criterion is about. Implied
    // by the file-wide containment test above and asked anyway, because that one would still pass
    // if this argument were folded into the migration note — where it would be exempt, and where a
    // reader looking up why an issue is stuck would never find it.
    const claims = lifecycleClaims()
    expect(claims).toBeTruthy()
    expect(RETIRED_SPELLING.test(claims)).toBe(false)
  })
})
