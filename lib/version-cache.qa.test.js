...
describe('QA readVersionCache — every malformed file resolves to defaults, never throws (#24)', () => {
    ...
    const cache = readVersionCache({
      fs: vol({
        [CACHE_PATH]: JSON.stringify({
          ...
          last_prompted_at: '\t\n ',
          latest_version: ' 0.17.0 ',
          // #215: the channel field is trimmed by the same rule, which is what lets
          // `cachedVersionFor` compare a hand-edited '  npm ' against a reader's 'npm'.
  ...
  it('drops unknown keys even when they look like something worth keeping', () => {
    ...
    expect(cache).toEqual({
      ...
      last_prompted_at: null,
      latest_version: '0.17.0',
      // #215: the shape this file enumerates grew a fourth field — which channel resolved
      // `latest_version`. The claim is unchanged (only known keys survive, and a
      // `__proto__` in the file cannot reach the returned object); the enumeration has to
      // list what the shape is now, or it stops being an enumeration of it.
  ...
  it('still finds the four fields in a bloated file full of junk keys', () => {
    const junk = {}
...
describe('QA writeVersionCache — failure surface and on-disk shape (#24)', () => {
  ...
  it('never persists unknown keys, however sensitive-looking', () => {
    ...
    expect(Object.keys(JSON.parse(raw)).sort()).toEqual([
      ...
      'last_prompted_at',
      'latest_version',
      // #215: the written shape grew a fourth field. The claim under test is the OPPOSITE of a
      // growing shape — that nothing a caller smuggles in reaches the file — so it has to list
      // the whitelist as it now stands, or an added key would read as a leak.
      'latest_version_channel',
    ])
  })
  ...
  it('writes the all-null shape for a missing or non-object cache instead of throwing', () => {
    for (const cache of [undefined, null, 'a string', 42, [], true]) {
      ...
      expect(JSON.parse(fs.readFileSync(CACHE_PATH, 'utf8'))).toEqual({
        ...
        last_prompted_at: null,
        latest_version: null,
        // #215: a non-object cache names no channel either, so the fourth field is null with the
        // other three — a file that says "nobody has checked", to every channel that reads it.
        latest_version_channel: null,
      })
    }
  })
  ...
  it('overwrites rather than appends, and is byte-identical for identical input', () => {
    const fs = vol()
    // #215: the literal is the full four-field shape so the round-trip below compares like with
    // like; the subject (idempotent overwrite, one JSON document) is untouched by the new field.
    const cache = {
  ...
  it('round-trips a written cache through read unchanged, repeatedly', () => {
    ...
    const cache = {
      ...
      latest_version: '0.17.0',
      // #215: the version and its channel round-trip TOGETHER or the pair is worthless — a
      // rewrite that dropped the stamp would silently unattribute every cached number.
      latest_version_channel: 'npm',
    }
    writeVersionCache({ cache, fs, home: HOME, processEnv: {} })
    const first = readVersionCache({ fs, home: HOME, processEnv: {} })
