...
// The statuses `ralph cycle` emits for a run that decided NOT to drain. They are
// counted separately and contribute no duration, because a cycle that never
// attempted work is not a fast cycle — averaging a 0 into `durations` would report
// the schedule as quicker than it is.
//
// #52: 'updated' joins them. It is the status of a cycle that installed a newer
// Ralph and stopped rather than draining through a half-swapped install (see
// lib/commands/cycle.js): no issue was touched and no time was spent, so its 0 must
// stay out of `durations`. Membership here changes nothing else — `cycles` below is
// events.length, so an aborted event still counts toward the total the daily line
// prints, and rightly: the schedule did tick. What it did not do is work, which is
// what `abortedCycles` is for.
const ABORTED_STATUSES = new Set(['preflight-failed', 'lock-held', 'tmux-active', 'updated'])
