...
//     pollute a prototype.
//   - the ONE caller #199 deliberately left on npm — `resolveUpdateDecision`, the
//     weekly background check — asserted by its spawn list and by its source, so a
//     later change cannot quietly make `ralph start` shell out to `brew` on every
//     run.
//
...
describe('resolveUpdateDecision — the weekly check stays on npm (#199 QA)', () => {
  // The tradeoff #199 wrote down: the background check has no classification in
  // hand, so it asks npm on every install layout. That is a DELIBERATE choice, and
  // an untested deliberate choice is one a later change can reverse silently —
  // which here would mean `ralph start` spawning `brew` before every loop.
  const HOME = '/home/me'

  it('spawns exactly the npm query — no brew, and no `npm root -g` probe', async () => {
  ...
  it('does not classify the install, even for a home that is a Homebrew Cellar', async () => {
    // The gate takes no `ralphHome` and has no business finding one: whatever the
    // layout, the notice tracks the registry. Driven with a Cellar-shaped home so a
    // future implementation that "helpfully" classified would be visible as a
    // second spawn.
    const exec = makeExec(okOut('0.17.0\n'))
  ...
  it('takes no dependency on the classification, by construction', async () => {
    ...
    // the same class of claim: threading a classification into this module is the
    // change #199 declined to make, and it must not arrive by accident. Comments are
    // stripped first, because #199's own prose argues about install layouts at
    // length and a guard a paragraph can trip is one people learn to route around.
