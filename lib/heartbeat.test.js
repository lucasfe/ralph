...
describe('summarizeLast24h', () => {
  ...
  it('counts an update-and-stop cycle as aborted, not as a zero-minute cycle (#52)', () => {
    // #52: `ralph cycle` emits status 'updated' when it installed a new version and
    // stopped INSTEAD of draining — no queue touched, no work attempted, no time
    // spent. Left out of ABORTED_STATUSES it would read as a normal cycle that
    // happened to take 0 minutes, pushing a 0 into `durations` that drags the
    // reported average down. It belongs with the other three aborts
    // (preflight-failed, lock-held, tmux-active) for the same reason they do: the
    // cycle decided not to run. Both still count toward `cycles` below — that total
    // is every event in the window, aborted or not.
    const lines = [
      eventLine({
        ts: '2026-04-29T01:00:00Z',
        status: 'updated',
    ...
    expect(result.abortedCycles).toBe(1)
    expect(result.durations).toEqual([9])
    expect(result.ok).toBe(2)
    expect(result.failed).toBe(0)
  })
