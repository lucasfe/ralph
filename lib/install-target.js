...
//
// #215: the join is also where a row's version query acquires the CHANNEL ID its answer is
// cached under — the marker half owns that id (`versionChannel`, beside the wording, because a
// marker and the channel whose versions it is compared against are one fact), and this half
// owns the descriptor, so neither spells the other's half twice. Decorating here rather than in
// STORE_UPDATES above is what keeps the two guarantees this table already makes:
//
//   - a row with no `latest` of its own falls through to NPM_VERSION_QUERY BY IDENTITY, which
//     lib/install-target.channel.test.js pins with `toBe` — so the three npm-shaped stores
//     cannot acquire three near-identical descriptors, and a caller cannot mutate one channel's
//     query into another's;
//   - the two tables above stay UNWRITTEN. This builds a new descriptor rather than assigning
//     a `channel` onto STORE_UPDATES' `latest` literal — that literal is the very object every
//     Cellar classification hands its caller back, so a write here would be a write to state
//     shared with everyone downstream. The npm rows are spared by the `!update?.latest` guard
//     below: their descriptor IS the frozen NPM_VERSION_QUERY, and assigning to it would throw
//     at import time in a module that has no way to recover — which matters more than it looks.
const GLOBAL_STORES = INSTALL_MARKERS.map((row) => {
  const update = STORE_UPDATES.get(row.store)
  if (!update?.latest) return { ...row, ...update }
  return { ...row, ...update, latest: { ...update.latest, channel: row.versionChannel } }
})
...
//
// #215: which is also how a row inherits the npm CACHE CHANNEL id. NPM_VERSION_QUERY carries
// `channel: 'npm'`, so every layout that falls through to it files its answer under npm and
// reads npm's back; the Homebrew row's descriptor got its own id at the GLOBAL_STORES join.
// Nothing here branches on the channel — it travels on the descriptor, like the argv and the
// format it is spawned and parsed with.
//
