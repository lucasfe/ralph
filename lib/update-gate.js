...
export async function runUpdateGate({
  ...
  // lib/commands/start.js:153 and lib/commands/cycle.js:138 declare it with no default
  ...
  // exactly once (lib/commands/cycle.js:284, under no loop) and bin/ralph.js:208 invokes
  ...
  try {
    const decision = await update({
      ...
      home,
      fs: cacheFs,
      // #200: a THUNK, not a descriptor — resolveUpdateDecision resolves it only where the
      // answer is used, and the opted-out run returns before it ever asks. #215 moved the
      // THROTTLED run inside that set (it has to know whose cached number it is reading), so
      // `exec: null` above is what keeps the new cost a path match rather than a spawn.
