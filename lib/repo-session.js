...
// place. Three commands ask it today, each with its own spelling: lib/commands/status.js
// (the `attach` hint and the `running` mode), lib/commands/cycle.js (the guard that skips
// a scheduled tick while an interactive loop is up) and lib/commands/stop.js (the session
// it kills). `ralph live` is the fourth and this module's first consumer; #168 is where
// `ralph stop` moves onto it.
//
...
// exactly the state `ralph stop` is in at HEAD (stop.js:22 passes its `cwd` straight
// through), while `ralph cycle` and `ralph status` resolve the toplevel first. Anchoring
// here is what lets `ralph live` be typed from `lib/` or `test/`.
//
...
//                           (status.js:312) because its whole job is to describe a run
...
// Byte-identical to the private `resolveRoot` at lib/commands/status.js:695-699 — diffed,
...
// end at the same cwd fallback, while `resolveRepoRoot` at lib/commands/cycle.js:426
...
// liveness TOGETHER. A caller only drops its copy by wanting all three — which is
// `ralph stop` (#168) and, of the rest, nobody yet: cycle.js:426 aborts instead of
// degrading, schedule.js and init.js want a root and no session at all, and status.js
// prefers the RECORDED session name over a derived one (status.js:312, see above). Kept as
// a named function rather than folded into the caller because the name is what says the
