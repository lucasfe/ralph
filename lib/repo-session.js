...
// is in (start.js:679 passes its `cwd` straight through), while `ralph cycle` and
...
// cycle.js:426 aborts instead of degrading, schedule.js and init.js want a root and no
// session at all, and status.js prefers the RECORDED session name over a derived one
// (status.js:312, see above). Kept as a named function rather than folded into the caller
