...
// gets spawned, HOW MANY times the caller's channel is resolved, and the ONE path
// that must resolve it zero times.
//
// #215 took the second of those paths away. The throttled path used to resolve nothing; it now
// resolves the channel it is about to serve a cached version FOR, because a version served
// without an owner is what #215 was filed about. The opt-out path still resolves nothing — it
// serves no version either — and no path gained a subprocess. The argument is at the test.
...
// #215: a throttled cache now names the channel that resolved its number, and this one names
// Homebrew because the one test that uses it hands `resolveUpdateDecision` a Cellar descriptor.
// An unstamped 0.2.0 would be served to nobody, and the test below would pass for the wrong
// reason — no version reported because none was readable, rather than because none was fetched.
const throttled = () =>
  seeded({
...
describe('resolveUpdateDecision — a channel resolved lazily, or not at all (#200)', () => {
  ...
  it('DOES resolve it on the throttled path — and still spawns nothing', async () => {
    // #215 CHANGED THIS EXPECTATION, and the old one was wrong from the day #200 wrote it.
    //
    // It said "inside the weekly window there is no channel to ask, so the caller is never asked
    // which one". The premise is true and the conclusion does not follow: the throttled path
    // asks no channel, but it SERVES a version — and #215 is the bug report proving a version
    // cannot be served without knowing whose it is. A Homebrew copy inside the window was handed
    // npm's `latest` and told a reader it was the tap's, because this path read a number it had
    // no way to attribute. So the channel is now resolved above the throttle branch.
    //
    // What #200 was actually protecting is untouched, and is what the last two assertions pin:
    // resolving a source costs no subprocess. `latestSource` is a function precisely so that
    // `ralph start` need not run `npm root -g` before its loop, and lib/update-gate.js hands
    // over a classification that is already in hand. One extra call of a function that returns a
    // frozen table row is not the cost #200 was avoiding — a spawn is, and there is still none.
    const exec = makeExec()
    const latestSource = makeSource(await brewSource())
