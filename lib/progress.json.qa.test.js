...
//
// `tasks.current.task_key` (#132) is the seventh, and it is the second leaf here that is a raw
// record field ON PURPOSE — `run_id` was the first. Both are NAMES: a name has no numeric
// reading to derive, and re-spelling a Jira key would publish an identity that resolves to no
// ticket. Typed, though, exactly like `run_id`: a string or null, never a number and never the
// empty string, which is what the leaf sweep below is checking.
...
function expectLeafContract(doc, label) {
  ...
  if (doc.tasks.current !== null) {
    // `task_key` (#132) is the third key, APPENDED rather than inserted, because a
    // consumer reading the document by position sees the two it already knew about
    // in the two places it already read them. A `null` here is the github and folder
    // reading of the same record, not a missing field.
...
describe('toJsonSnapshot — a projection, provably (#58 QA)', () => {
  ...
  it('reads the record for identity only — never for a number it could derive', () => {
    ...
    // `current.task_key` (#132) joined the list as a fourth NAME, which is exactly the
    // kind of read this test permits: it is transcribed, not derived. The rule it
    // enforces is unchanged — nothing here reads a duration, a cost or a queue depth.
