...
//
// `tasks.current.task_key` (#132) is the seventh and `tasks.current.worktree` (#222) the
// eighth, and they are the second and third leaves here that are raw record fields ON
// PURPOSE — `run_id` was the first. All three are IDENTITIES: an identity has no numeric
// reading to derive, and re-spelling a Jira key, or tidying a path, would publish something
// that resolves to no ticket and no directory. Typed, though, exactly like `run_id`: a string
// or null, never a number and never the empty string, which is what the leaf sweep below is
// checking.
...
function expectLeafContract(doc, label) {
  ...
  if (doc.tasks.current !== null) {
    // `task_key` (#132) is the third key and `worktree` (#222) the fourth, both APPENDED
    // rather than inserted, because a consumer reading the document by position sees the
    // keys it already knew about in the places it already read them. A `null` in either is
    // another reading of the same record — no Jira key, no worktree — not a missing field.
...
describe('toJsonSnapshot — a projection, provably (#58 QA)', () => {
  ...
  it('reads the record for identity only — never for a number it could derive', () => {
    ...
    // `current.task_key` (#132) and `current.worktree` (#222) joined the list as further
    // IDENTITIES, which is exactly the kind of read this test permits: they are transcribed,
    // not derived. The rule it enforces is unchanged — nothing here reads a duration, a cost
    // or a queue depth.
