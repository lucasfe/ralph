// #69 — the spec for the two facts the banner cannot simply be handed.
//
// Every other row of the identity box is a value some caller already holds: a version out
// of package.json, a cwd out of argv, a cached version out of a JSON file. The two facts
// this module resolves are different in kind, and both of them are questions about the
// PAST or about the ENVIRONMENT rather than lookups:
//
//   1. WHICH MODEL. The Claude model cannot be known at launch — CLAUDE_ARGV carries no
//      `--model`, Ralph has no model setting, and Claude Code exposes no way to ask — so
//      the only honest answer available before the first turn is what the LAST run used,
//      read out of the metrics log the loop appends to. For Codex the stream carries no
//      model id at all, so the configured value is the answer instead. Two different
//      kinds of evidence, and therefore two different sentences: the whole point of the
//      `provenance` tag this module returns is that the box must never state a model with
//      more confidence than its source warrants.
//   2. WHICH REPO. In github mode the box names the repository the loop will read issues
//      from, and it does so without a `gh repo view` — that is a GraphQL round trip, and
//      this row is printed before the first preflight line.
//
// PURE, and asserted so by a static read at the bottom: no clock, no environment, no
// filesystem. The text of the metrics log and the text of .git/config arrive as strings,
// which is what makes the whole table below testable without a .ralph directory, a git
// remote, or a run that has ever happened (#41).
//
// TABLE-DRIVEN wherever the input is a shape rather than a value, because the interesting
// half of this module is what it refuses: a truncated trailing line, a model belonging to
// another agent, a remote that is not a GitHub slug. Each of those is one row here.
...
describe('resolveBannerModel — the agent it was handed, and never a throw (#69)', () => {
  ...
  it('answers with one of exactly three provenance tags, and names them once', () => {
    // The tags are this module's vocabulary and lib/banner-compose.js's wording is keyed on
    // them. They are exported so the box's spec can enumerate them rather than restate them
    // — see the drift guard in banner-compose.test.js.
...
describe('resolveBannerRepo — the repo the box names, resolved locally (#69)', () => {
  ...
  it('answers null rather than guessing, for everything that is not a GitHub slug', () => {
    ...
    // row above answers to. gh resolves its base repo from more than origin, so a missing
...
describe('banner-model — purity', () => {
  it('reads no clock, no environment and no filesystem', () => {
    ...
    // handed the text of two files it must never open itself, which is the whole reason
    // every case above is a string literal rather than a fixture on disk (#41).
    ...
    // Its ONE import is the context-window map the telemetry writer already owns, for the
    // reason banner-compose.js states about its own single import: the alternative is two
    // copies of a table, and the day one of them moves the box and the log disagree about
    // the same model. lib/issue-event.js is pure too (it imports only agent-stream.js), so
    // this costs the banner no capability it did not already have.
