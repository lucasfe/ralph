...
describe('runUpdateGate — classifying costs no subprocess (#200)', () => {
  ...
  it('spawns NOTHING on a throttled run, and still names the layout’s command', async () => {
    ...
    //
    // #215: the seed names the channel that resolved its number — the tap, since this is the
    // Homebrew layout reading back its own last answer. Cheapness is measured in spawns and this
    // still spends none; what the stamp buys is that the number the notice prints is the one THIS
    // copy's `brew upgrade ralph` will actually install.
    const g = gate({
      layout: 'brew',
  ...
  it('classifies ONCE when a throttled run has nothing to say, and spawns nothing', async () => {
    // #215 CHANGED THIS EXPECTATION. The old one — zero classifications — held only because the
    // throttled path served a cached version without knowing whose channel produced it, which is
    // the defect #215 was filed for: on a two-install machine the npm copy read the tap's number
    // and advertised it with `npm i -g`, where the newest thing available is 0.23.0.
    //
    // Deciding whether a cached number is newer than the running one requires knowing it is even
    // THIS copy's number, so the channel is resolved above the throttle branch and this path
    // classifies once. The cost is a classification with `exec: null` — path matching plus at
    // most two stats, which is what that null argument exists to make affordable
    // (update-gate.js:93) — and the two assertions below are the ones criterion 7 was about: no
