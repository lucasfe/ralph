...
export function buildAgentInvocation(env = {}) {
  ...
  if (agent === 'codex') {
    // #221 — ONE EXTRA WRITABLE ROOT: the main repo root's `.ralph/`. Since the loop
    // runs a task in a worktree under `<mainRoot>/.ralph/worktrees/`, the agent's cwd
    // is that worktree and codex's workspace-write sandbox is rooted there — but the
    // task files it has to move live in the MAIN root's gitignored `.ralph/tasks/`,
    // outside it. Naming `<mainRoot>/.ralph` covers the task lanes and the worktrees
    // directory in one entry, and nothing else in the user's checkout.
    //
    // MEASURED on codex 0.154.0.474: `codex debug models -c
    // 'sandbox_workspace_write.writable_roots=["/private/tmp/x"]'` exits 0, while the
    // same key with a scalar value (`=5`) exits 1 with `Error: invalid type: integer 5,
    // expected a sequence` naming `sandbox_workspace_write.writable_roots` — which is
    // how we know the key is READ and not silently dropped (an unknown sibling key
    // exits 0 in silence). `codex exec --help`: "-c, --config <key=value> … The `value`
    // portion is parsed as TOML. If it fails to parse as TOML, the raw string is used
    // as a literal", with `-c 'sandbox_permissions=["disk-full-read-access"]'` as its
    // own example — so the list spelling below is the documented shape.
    //
    // JSON.stringify does the quoting because its escape set is a SUBSET of TOML's
    // basic-string one, so a path holding a `"` or a `\` still parses as the list codex
    // expects instead of degrading to that literal-string fallback.
    //
    // NOT GATED ON TASK_SOURCE: this bridge is resolved once per run, before any task
    // is selected, and a writable `.ralph` is correct for every source (github mode's
    // worktrees live in the same directory). A blank or absent PROJECT_ROOT means the
    // loop is not the caller — `ralph doctor` and the tests reach this too — and the
    // flag is omitted entirely rather than pointed at `undefined/.ralph`.
    //
    // CLAUDE IS UNTOUCHED, deliberately: it already runs with
    // `--dangerously-skip-permissions`, so it needs no path at all.
    const mainRoot = (env.PROJECT_ROOT || '').trim()
    if (mainRoot) {
      const writableRoots = JSON.stringify([join(mainRoot, '.ralph')])
