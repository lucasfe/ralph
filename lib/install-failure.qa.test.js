...
//   3. `Object.hasOwn(PREFIX_FIX, argv[0])` with a hostile argv[0]
...
describe('installFailureDetails — the per-manager fix, keyed on argv[0] (#23 QA)', () => {
  ...
  it('stays generic for a manager Ralph has no data for, and still offers sudo', async () => {
    const argv = ['brew', 'upgrade', 'ralph']
    const lines = installFailureDetails({ stderr: 'EACCES' }, { argv, label: argv.join(' ') })
    expect(text(lines)).toMatch(/see brew's global-prefix setting/)
    expect(text(lines)).toContain('sudo brew upgrade ralph')
  })
})
