...
// #215 keyed the cached version by the channel that resolved it. Every run in this file goes
// through `runUpdateGate`, whose `latestSource` resolves to NPM_VERSION_QUERY unless a test hands
// over a Cellar classification, so `npm` is the channel that would have written these seeds — and
// without the stamp the throttled path serves nothing, which would quietly turn every
// "notice from the cache" assertion here into a test of an empty decision. The spread is last so
// a test whose subject is the channel can spell its own.
const seededCache = (cache, path = CACHE_PATH) =>
  Volume.fromJSON({ [path]: JSON.stringify({ latest_version_channel: 'npm', ...cache }) }, '/')
