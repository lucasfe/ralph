...
describe('classifyInstall — one return shape for all seven kinds (#22 QA)', () => {
  ...
  it('returns exactly the same six keys for every kind', async () => {
    // #199 added the sixth, `latest` — the version query for the channel this
    // layout installs from. It is on EVERY kind for the same reason the other five
    // are: a caller must never have to ask which kinds carry which fields.
...
describe('classifyInstall — the Homebrew kind keeps the one return shape (#198 QA)', () => {
  it('returns the same six keys, with an argv and no advice', async () => {
    // Six since #199, `latest` included — the Homebrew row is the one whose query
    // differs, so the shape it must NOT differ in is the set of keys.
