...
// ---------------------------------------------------------------------------
// #221 — the loop now runs a folder task in a git worktree under the main root's
// `.ralph/worktrees/`, while the task FILES it has to move stay in the main root's
// gitignored `.ralph/tasks/`. Codex's workspace-write sandbox is rooted at the cwd it
// is spawned in — the worktree — so that move is outside it and the agent cannot make
// it. The fix is one extra writable root, composed here beside the model flag rather
// than spelled in bash, and only for codex: claude already runs
// `--dangerously-skip-permissions` and needs nothing.
//
// MEASURED on codex 0.154.0.474: `codex debug models -c
// 'sandbox_workspace_write.writable_roots=["/private/tmp/x"]'` exits 0, while the same
// key given a scalar (`=5`) exits 1 with `Error: invalid type: integer 5, expected a
// sequence` naming `sandbox_workspace_write.writable_roots` — which is what proves the
// key is READ rather than silently ignored (an unknown sibling key exits 0 in silence).
// `codex exec --help` documents `-c, --config <key=value>` as "The `value` portion is
// parsed as TOML. If it fails to parse as TOML, the raw string is used as a literal",
// with `-c 'sandbox_permissions=["disk-full-read-access"]'` as its own example.
// ---------------------------------------------------------------------------
describe('buildAgentInvocation — the main root stays writable for codex (#221)', () => {
  const ROOT = '/Users/dev/repo'
  const codex = (env = {}) => buildAgentInvocation({ RALPH_AGENT: 'codex', ...env })

  it('adds the main root .ralph as a codex writable sandbox root', () => {
    const args = codex({ PROJECT_ROOT: ROOT }).args
    const i = args.indexOf(`sandbox_workspace_write.writable_roots=["${ROOT}/.ralph"]`)
    expect(i).toBeGreaterThan(0)
    expect(args[i - 1]).toBe('-c')
  })

  it('keeps the stdin marker last, after the new -c pair and the model', () => {
    const args = codex({ PROJECT_ROOT: ROOT, RALPH_CODEX_MODEL: 'gpt-5-codex' }).args
    expect(args[args.length - 1]).toBe('-')
    const writable = args.findIndex((a) => a.startsWith('sandbox_workspace_write.writable_roots='))
    // Present FIRST: a missing element indexes -1, which is "before the last" for free —
    // so without this line the ordering check below would also pass on an argv that never
    // got the flag at all.
    expect(writable).toBeGreaterThan(-1)
    expect(writable).toBeLessThan(args.length - 1)
    expect(args.indexOf('-m')).toBeLessThan(args.length - 1)
  })

  it('composes on TOP of the static registry base, which is unchanged', () => {
    const base = agentSpec('codex').argv
    expect(codex({ PROJECT_ROOT: ROOT }).args.slice(0, base.length)).toEqual(base)
    expect(base.join(' ')).not.toContain('writable_roots')
  })

  it('omits the flag entirely when PROJECT_ROOT is absent or blank', () => {
    // Never `["undefined/.ralph"]`: a missing project root means the loop is not the
    // caller, and a bogus path would either widen the sandbox somewhere random or
    // make codex fail on a directory that does not exist.
    for (const env of [{}, { PROJECT_ROOT: '' }, { PROJECT_ROOT: '   ' }]) {
      expect(codex(env).args.join(' ')).not.toContain('writable_roots')
    }
  })

  it('spells the path as a TOML string, escaped, even when it contains a quote', () => {
    // JSON.stringify's escape set is a subset of TOML's basic-string one, so the
    // value stays parseable as the LIST codex expects rather than degrading to the
    // literal-string fallback its --help documents.
    const args = codex({ PROJECT_ROOT: '/Users/dev/my "repo"' }).args
    expect(args).toContain(
      'sandbox_workspace_write.writable_roots=["/Users/dev/my \\"repo\\"/.ralph"]',
    )
  })

  it('leaves the CLAUDE argv byte-for-byte unchanged, with or without a project root', () => {
    expect(buildAgentInvocation({ PROJECT_ROOT: ROOT }).args).toEqual(agentSpec('claude').argv)
    expect(buildAgentInvocation({ PROJECT_ROOT: ROOT }).args.join(' ')).not.toContain('-c')
  })
})
